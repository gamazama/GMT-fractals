...
export class DeepZoomController {
    ...
    /** Period of a minibrot-nucleus reference (0 = non-periodic). When > 0 the
     *  orbit is exactly one period and the kernel wraps the reference index
     *  modulo this instead of rebasing past a non-periodic end. @see ADR-0066 */
    private refOrbitPeriod = 0;
    ...
    bindUniforms(
        ...
        // Periodic-reference wrap: > 0 only for a minibrot-nucleus orbit, where
        // orbit[period] == orbit[0] == 0 so the kernel can wrap ref %= period
        // exactly. 0 keeps the non-periodic rebase-to-0 fallback. @see ADR-0066
        gl.uniform1i(prog.uniforms['uRefPeriod'], deepActive ? this.refOrbitPeriod : 0);
