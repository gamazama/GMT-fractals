...
import { MaterialState } from '../../features/materials';
import { MAX_LIGHTS } from '../../data/constants';
import { EngineRenderState, engine } from '../FractalEngine';
...
export class UniformManager {
    ...
    public syncFrame(
        ...
        if (renderer && !runtimeState.isExporting && !runtimeState.isBucketRendering) {
            ...
            // Ensure integer pixel dimensions for perfect 1:1 mapping
            ...
            if (uRes.x !== w || uRes.y !== h) {
                ...
                // CRITICAL: Sync Post-Process Material Resolution immediately
                // This prevents 1-frame distortion lags during dynamic scaling
                if (engine.materials) {
                    engine.materials.displayMaterial.uniforms.uResolution.value.set(w, h);
                    engine.materials.exportMaterial.uniforms.uResolution.value.set(w, h);
                }
            }
        ...
        // --- CAMERA BASIS UPDATE (WITH ROTATION MODULATION) ---
        // Instead of calling virtualSpace.updateCameraBasis(cam), we implement it here to inject the rotation offset.
        
        const rotModX = engine.modulations['camera.rotation.x'] || 0;
        const rotModY = engine.modulations['camera.rotation.y'] || 0;
        const rotModZ = engine.modulations['camera.rotation.z'] || 0;
        ...
        if (rotModX !== 0 || rotModY !== 0 || rotModZ !== 0) {
            ...
            // Multiply local rotation: camera * modulation
        ...
        // --- APPLY CAMERA POSITION MODULATION ---
        const camModX = engine.modulations['camera.unified.x'] || 0;
        const camModY = engine.modulations['camera.unified.y'] || 0;
        const camModZ = engine.modulations['camera.unified.z'] || 0;

        // Apply modulation offset to the position sent to shader
        ...
        // --- ENVIRONMENT ROTATION OPTIMIZATION ---
        // Calculate 2x2 rotation matrix for skybox
        const mats = (useFractalStore.getState() as any).materials as MaterialState;
        if (mats) {
            const rot = mats.envRotation;
            const c = Math.cos(rot);
            const s = Math.sin(rot);
            // mat2(c, -s, s, c) -> Column Major: [col1, col2] -> [c, s, -s, c]
        ...
        if (lighting && Array.isArray(lighting.lights)) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                // Get modulation deltas from Engine
                const dIntensity = engine.modulations[`lighting.light${i}_intensity`] || 0;
                const dFalloff = engine.modulations[`lighting.light${i}_falloff`] || 0;
                const dX = engine.modulations[`lighting.light${i}_posX`] || 0;
                const dY = engine.modulations[`lighting.light${i}_posY`] || 0;
                const dZ = engine.modulations[`lighting.light${i}_posZ`] || 0;
