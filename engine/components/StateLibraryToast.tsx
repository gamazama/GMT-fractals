...
export const StateLibraryToast: React.FC<StateLibraryToastProps> = ({ arrayKey }) => {
    ...
    const borderClass = isWarning ? 'border-warn/50' : 'border-accent-500/40';
    const dotClass = isWarning ? 'bg-warn' : 'bg-accent-400';
    const textClass = isWarning ? 'text-warn' : 'text-accent-300';

    return (
        <div className="relative pointer-events-none">
            <div className="absolute right-0 top-full mt-2 z-[600] whitespace-nowrap">
                <div className={`flex items-center gap-1.5 px-2.5 py-1.5 bg-surface-sunken/95 border ${borderClass} rounded-lg shadow-lg`}>
                    <div className={`w-1.5 h-1.5 rounded-full ${dotClass} shrink-0`} />
                    <span className={`text-[10px] font-bold ${textClass}`}>{toast.message}</span>
                    <kbd className="text-[8px] text-fg-dim bg-surface-header px-1 rounded ml-0.5">{toast.slot}</kbd>
                </div>
            </div>
        </div>
    );
