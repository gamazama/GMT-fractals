...
export const ViewportQuality: React.FC = () => {
    ...
    const applyScalabilityPreset = useFractalStore(s => s.applyScalabilityPreset);
    ...
    const renderMode = useFractalStore(s => (s as any).lighting?.renderMode ?? 0);
    const isPT = ptEnabled && renderMode === 1.0;

    // PT runtime params — editable inline when PT is active
    const ptBounces = useFractalStore(s => (s as any).lighting?.ptBounces ?? 3);
    const ptGIStrength = useFractalStore(s => (s as any).lighting?.ptGIStrength ?? 1.0);
    // PT compile-time params — batched with Apply
    const ptNEEAllLights = useFractalStore(s => (s as any).lighting?.ptNEEAllLights ?? false);
