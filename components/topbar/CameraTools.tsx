...
import { CameraIcon, UndoIcon, RedoIcon, ResetIcon } from '../Icons';
...
export const CameraTools: React.FC<{ isMobileMode: boolean, vibrate: (ms: number) => void, btnBase: string, btnActive: string, btnInactive: string }> = ({ isMobileMode, vibrate, btnBase, btnActive, btnInactive }) => {
    ...
    return (
        ...
        >
            ...
            {showCameraMenu && (
                <div className="absolute top-full right-0 mt-2 w-40 bg-black border border-white/20 rounded-xl p-2 shadow-2xl z-[70] animate-fade-in origin-top-right">
                    <div className="absolute -top-1.5 right-4 w-3 h-3 bg-black border-t border-l border-white/20 transform rotate-45" />
                    <div className="px-2 py-1 text-[10px] font-bold text-gray-500 uppercase tracking-wider border-b border-white/10 mb-1">Camera</div>
