 ...
 *
 * - **Fade** (`fade`): the soft IN-FEATURE divider — a flat `surface-raised`
 *   rectangle, no gradients, no cap, no lip. It separates sibling blocks that
 *   belong to the same feature run (e.g. Fog ↔ Volumetric Scatter) without
 *   stamping a card edge between them.
 *   Manifest: `{ type: 'separator', variant: 'fade' }`.
 */
export const SectionDivider: React.FC<{ nested?: boolean; capFrom?: string; fade?: boolean }> = ({ nested = false, capFrom, fade = false }) => {
    if (fade) {
        return <div className="h-2.5 bg-surface-raised" />;
    }
    if (nested) {
        return (
