 ...
 *
 * @assumption One slot, last-writer-wins; a host that never registers leaves the
 * editor header clean. Neither half is exercised today, which is why this is an
 * assumption and not an invariant: grep for `setGradientEditorEntrance` and it has
 * exactly ONE caller (`palette/registerPaletteUI.ts`), which calls it
 * unconditionally — so every in-tree host registers, no second writer ever
 * overwrites, and the empty-slot path is dead. No guard reaches this file either
 * (`grep -rl createSingleSlot debug/` is empty), so no command would go red if
 * either half broke. The previous text named fluid-toy as a host with no Favients
 * shelf; fluid-toy calls `registerPaletteUI()` and mounts the shelf.
 */
