 ...
 * auth, and feedback surfaces: `rounded border border-red-500/30 bg-red-500/10`.
...
export const ErrorNote: React.FC<{ className?: string; children: React.ReactNode }> = ({ className = '', children }) => (
    <div className={`rounded border border-red-500/30 bg-red-500/10 ${className}`.trim()}>{children}</div>
);
