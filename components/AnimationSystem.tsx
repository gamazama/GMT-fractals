...
export const AnimationSystem: React.FC = () => {
    ...
    useFrame((state, delta) => {
        ...
        const shouldRecord = isRecordingModulation && currentFrame > lastFrameRecorded.current;
        ...
        if (isRecordingModulation && allTargetsToProcess.size > 0) {
            ...
            if (targetKey.startsWith('coloring.')) {
                if (targetKey === 'coloring.repeats') {
                    ...
                    if (c && Math.abs(c.repeats) > 0.001) {
                        // Use resolvedBase to allow feedback loop prevention logic above to flow down if needed
                        // But for specialized params, logic is complex. 
                        // Simplified: If recording, use clean base.
