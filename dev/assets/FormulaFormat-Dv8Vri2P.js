...
`,r+=`    float ${c}_d = ${T}_d;
`,r+=`    float ${c}_dr = ${T}_dr;
`,s.enabled){const F=u.get(s.type);if(F){const q=s.condition&&s.condition.active;let $="    ";if(q){const D=l<I?`uModularParams[${l++}]`:"2.0",xe=l<I?`uModularParams[${l++}]`:"0.0";r+=`    { int ${c}_cmod = max(1, int(${D})); int ${c}_crem = int(${xe});
`,r+=`    if ( (i - (i/${c}_cmod)*${c}_cmod) == ${c}_crem ) {
`,$="        "}const ve=D=>s.bindings&&s.bindings[D]?`u${s.bindings[D]}`:l<I?`uModularParams[${l++}]`:"0.0";r+=F.glsl({varName:c,in1:T,in2:be,getParam:ve,indent:$}),q&&(r+=`    }}
`)}}r+=`
