var ve=Object.defineProperty;var xe=(e,t,a)=>t in e?ve(e,t,{enumerable:!0,configurable:!0,writable:!0,value:a}):e[t]=a;var q=(e,t,a)=>xe(e,typeof t!="symbol"?t+"":t,a);import{H as x,K as A,b5 as I,U as y,a0 as m,f,b6 as ze,b7 as Se,b8 as we,b9 as _e,ba as Fe,bb as Me}from"./FractalRegistry-BUHzgo28.js";import{n as w,d as L,p as Ie,q as Ce,r as $,s as Z,c as g,l as V}from"./three-DQWx7qFd.js";function Pe(e,t,a){var o,i;return!!((o=e.shader.capabilities)!=null&&o.has(a)||(i=t==null?void 0:t.shader.capabilities)!=null&&i.has(a))}const X=`
...
`});u.register({id:"Custom",label:"Custom (Legacy)",category:"Utils",description:"Legacy node.",inputs:[],glsl:e=>""});const vt=e=>{const t=new Map;return e.forEach(a=>{t.has(a.target)||t.set(a.target,[]),t.get(a.target).push(a)}),t},xt=e=>{const t=new Set,a=["root-end"],o=new Set;for(;a.length>0;){const i=a.pop();o.has(i)||(o.add(i),i!=="root-end"&&i!=="root-start"&&t.add(i),(e.get(i)??[]).forEach(r=>a.push(r.source)))}return t},zt=(e,t)=>{const a=vt(t),o=xt(a),i=e.filter(l=>o.has(l.id));if(!i||i.length===0)return`
    ...
    `;const n=new Map;n.set("root-start","v_start");let d=0;i.forEach((l,b)=>{const c=`v_${l.id.replace(/[^a-zA-Z0-9]/g,"")}`;n.set(l.id,c);const v=a.get(l.id)??[],M=v.find(F=>!F.targetHandle||F.targetHandle==="a"),E=v.find(F=>F.targetHandle==="b"),D=M&&n.get(M.source)||"v_start",ge=E&&n.get(E.source)||"v_start";if(r+=`    // Node: ${l.type} (${l.id})
`,r+=`    vec3 ${c}_p = ${D}_p;
`,r+=`    float ${c}_d = ${D}_d;
`,r+=`    float ${c}_dr = ${D}_dr;
`,l.enabled){const F=u.get(l.type);if(F){const G=l.condition&&l.condition.active;let H="    ";if(G){const T=d<I?`uModularParams[${d++}]`:"2.0",be=d<I?`uModularParams[${d++}]`:"0.0";r+=`    { int ${c}_cmod = max(1, int(${T})); int ${c}_crem = int(${be});
`,r+=`    if ( (i - (i/${c}_cmod)*${c}_cmod) == ${c}_crem ) {
`,H="        "}const ye=T=>l.bindings&&l.bindings[T]?`u${l.bindings[T]}`:d<I?`uModularParams[${d++}]`:"0.0";r+=F.glsl({varName:c,in1:D,in2:ge,getParam:ye,indent:H}),G&&(r+=`    }}
`)}}r+=`
