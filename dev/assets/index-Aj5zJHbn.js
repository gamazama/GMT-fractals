...
`,extraParams:{hybridFoldingValue:{type:"vec3",default:new U(2,2,2),label:"Folding Value",shortId:"hfv",uniform:"uHybridFoldingValue",min:.1,max:5,step:.01,group:"hybrid"}},defaults:{hybridScale:2,hybridFoldLimitVec:{x:1,y:1,z:1},hybridMinR:.5,hybridFixedR:1,hybridIter:2,hybridFoldingValue:{x:2,y:2,z:2}}},Tn={id:"kali",label:"Kali",glsl:`
void foldOperation(inout vec3 z, inout float dr, vec3 foldLimit) {
    z = uHybridKaliConstant - abs(z);
}
`,rotMode:"post",extraParams:{hybridKaliConstant:{type:"vec3",default:new U(1,1,1),label:"Kali Constant",shortId:"hkc",uniform:"uHybridKaliConstant",min:-3,max:3,step:.01,group:"hybrid"}},defaults:{hybridScale:1.5,hybridFoldLimitVec:{x:1,y:1,z:1},hybridMinR:.6,hybridFixedR:1,hybridIter:3,hybridKaliConstant:{x:.5,y:.5,z:.5}}},Rn={id:"tetra",label:"Tetrahedral (KIFS)",glsl:`
void foldOperation(inout vec3 z, inout float dr, vec3 foldLimit) {
    if (z.x + z.y < 0.0) z.xy = -z.yx;
    if (z.x + z.z < 0.0) z.xz = -z.zx;
    if (z.y + z.z < 0.0) z.yz = -z.zy;
}
`,defaults:{hybridScale:2,hybridFoldLimitVec:{x:1,y:1,z:1},hybridMinR:.25,hybridFixedR:1,hybridIter:3}},In={id:"octa",label:"Octahedral (KIFS)",glsl:`
void foldOperation(inout vec3 z, inout float dr, vec3 foldLimit) {
...
`,defaults:{hybridScale:2,hybridFoldLimitVec:{x:1,y:1,z:1},hybridMinR:.5,hybridFixedR:1,hybridIter:3}},Fn={id:"icosa",label:"Icosahedral (KIFS)",glsl:`
void foldOperation(inout vec3 z, inout float dr, vec3 foldLimit) {
...
`,defaults:{hybridScale:2,hybridFoldLimitVec:{x:1,y:1,z:1},hybridMinR:.3,hybridFixedR:1,hybridIter:4}},_n={id:"menger",label:"Menger (Cubic)",glsl:`
void foldOperation(inout vec3 z, inout float dr, vec3 foldLimit) {
...
`,defaults:{hybridScale:3,hybridFoldLimitVec:{x:1,y:1,z:1},hybridMinR:.5,hybridFixedR:1,hybridIter:3}},aa=[Cn,kn,jn,Pn,Tn,Rn,In,Fn,_n],Dn=aa.map((e,o)=>({label:e.label,value:o}));function Ln(e){return aa[e]??aa[0]}const En=["xyz","xzy","yxz","yzx","zxy","zyx"];function An(e){const o=En[e]??"xyz";return o==="xyz"?"vec3 c_perm = c.xyz;":`vec3 c_perm = c.${o};`}function Nn(e,o){return`
mat3 hybridRotMat;
bool hybridHasRot;
...
void initHybridTransform() {}
void formula_Hybrid(inout vec4 z, inout float dr, inout float trap, vec4 c) {}`);else{const d=(a==null?void 0:a.hybridFoldType)??0,f=Ln(d);e.addFunction(f.glsl);const p=(a==null?void 0:a.hybridPermute)??0,h=An(p);e.addFunction(Nn(h,f.rotMode??"wrap"))}let i="",l="";if(o.formula!=="MandelTerrain"&&(l+="z.xyz = mix(z.xyz, abs(z.xyz), step(0.5, uBurningEnabled));"),s)if(!(a&&a.hybridComplex))i+=`
                if (uHybrid > 0.5) {
                    initHybridTransform();
...
`});ze.register({id:"Custom",label:"Custom (Legacy)",category:"Utils",description:"Legacy node.",inputs:[],glsl:e=>""});const Gn=(e,o)=>{const a=new Set,r=["root-end"],n=new Set;for(;r.length>0;){const p=r.pop();if(n.has(p))continue;n.add(p),p!=="root-end"&&p!=="root-start"&&a.add(p),o.filter(u=>u.target===p).forEach(u=>r.push(u.source))}const s=e.filter(p=>a.has(p.id));if(!s||s.length===0)return`
    ...
    `;const l=new Map;l.set("root-start","v_start");let c=0;s.forEach((p,h)=>{const g=`v_${p.id.replace(/[^a-zA-Z0-9]/g,"")}`;l.set(p.id,g);const S=o.filter(C=>C.target===p.id),y=S.find(C=>!C.targetHandle||C.targetHandle==="a"),b=S.find(C=>C.targetHandle==="b"),m=y&&l.get(y.source)||"v_start",x=b&&l.get(b.source)||"v_start";if(i+=`    // Node: ${p.type} (${p.id})
`,i+=`    vec3 ${g}_p = ${m}_p;
`,i+=`    float ${g}_d = ${m}_d;
`,i+=`    float ${g}_dr = ${m}_dr;
`,p.enabled){const C=ze.get(p.type);if(C){const z=p.condition&&p.condition.active;let w="    ";if(z){const P=Math.round(Math.max(1,p.condition.mod)),k=Math.round(p.condition.rem);i+=`    if ( (i - (i/${P})*${P}) == ${k} ) {
`,w="        "}const M=P=>p.bindings&&p.bindings[P]?`u${p.bindings[P]}`:c<La?`uModularParams[${c++}]`:"0.0";i+=C.glsl({varName:g,in1:m,in2:x,getParam:M,indent:w}),z&&(i+=`    }
`)}}i+=`
...
`},Vn=e=>{let o="d = 0.5 * log(max(r, 1.0e-5)) * r / dr_safe;";return e<.5?o=`
        float logR2 = log2(m2);
...
`,i=n.shader.loopBody,l=n.shader.loopInit||"",n.shader.preamble&&e.addPreamble(n.shader.preamble),n.shader.getDist&&(d=`vec2 getDist(float r, float dr, float iter, vec4 z) { ${n.shader.getDist} }`));e.addFunction(s),e.setFormula(i,l,d)}};let qn=0;function zt(){return`l${++qn}`}const Wn=(e,o)=>{if(!e)return`
...
#endif
`,bs={id:"volumetric",shortId:"vol",name:"Volumetric Scatter",category:"Rendering",engineConfig:{toggleParam:"ptVolumetric",mode:"compile",label:"Volumetric Scattering",description:"Henyey-Greenstein single scatter. Enables god rays, colored haze, and directional fog.",groupFilter:"engine_settings"},groups:{density:{label:"Density & Shadow Rays",collapsible:!0},emissive:{label:"Color Scatter",collapsible:!0},height:{label:"Height Fog",collapsible:!0}},params:{ptVolumetric:{type:"boolean",default:!1,label:"Volume Scatter",shortId:"pvs",group:"main",noReset:!0,onUpdate:"compile",estCompileMs:5500},volDensity:{type:"float",default:.01,label:"Density",shortId:"vd",uniform:"uVolDensity",min:.001,max:5,step:.01,scale:"log",group:"density",condition:{param:"ptVolumetric",bool:!0}},volAnisotropy:{type:"float",default:.3,label:"Anisotropy (g)",shortId:"va",uniform:"uVolAnisotropy",min:-.99,max:.99,step:.01,group:"density",parentId:"volDensity",condition:{gt:0},description:"0=isotropic, +0.9=forward (god rays), -0.9=back scatter."},volMaxLights:{type:"float",default:1,label:"Light Sources",shortId:"vml",uniform:"uVolMaxLights",min:1,max:3,step:1,group:"density",parentId:"volDensity",condition:{gt:0},isAdvanced:!0,description:"Max lights for shadow rays. More = more expensive."},volScatterTint:{type:"color",default:new $e(1,1,1),label:"Scatter Tint",shortId:"vst",uniform:"uVolScatterTint",group:"density",parentId:"volDensity",condition:{gt:0}},volEmissive:{type:"float",default:0,label:"Color Scatter",shortId:"ves",uniform:"uVolEmissive",min:0,max:100,step:.1,scale:"log",group:"emissive",condition:{param:"ptVolumetric",bool:!0},description:"Orbit trap color field scattered through the volume. No shadow rays needed."},volEmissiveFalloff:{type:"float",default:0,label:"Surface Falloff",shortId:"vef",uniform:"uVolEmissiveFalloff",min:0,max:5,step:.01,scale:"log",group:"emissive",parentId:"volEmissive",condition:{gt:0},description:"Concentrate color near fractal surface."},volHeightFalloff:{type:"float",default:0,label:"Height Falloff",shortId:"vhf",uniform:"uVolHeightFalloff",min:0,max:5,step:.01,scale:"log",group:"height",condition:{param:"ptVolumetric",bool:!0},description:"Density varies with Y. Creates ground fog, rising mist."},volHeightOrigin:{type:"float",default:0,label:"Height Origin",shortId:"vho",uniform:"uVolHeightOrigin",min:-5,max:5,step:.01,group:"height",parentId:"volHeightFalloff",condition:{gt:0}}},inject:(e,o,a)=>{if(a!=="Main")return;const r=o.volumetric;r!=null&&r.ptVolumetric&&(e.addDefine("PT_VOLUMETRIC","1"),e.addVolumeLogic(ys,""))}},vs=()=>{ne.register(Un),ne.register(On),ne.register(es),ne.register(us),ne.register(ps),ne.register(pn),ne.register(bs),ne.register(vn),ne.register(gs),ne.register(zn),ne.register(Sn),ne.register($n),ne.register(mn),ne.register(gn),ne.register(wn),ne.register(ts),ne.register(as),ne.register(xs),ne.register(os),ne.register(rs),ne.register(is),ne.register(ns),ne.register(ss),ne.register(cs)},lt=e=>{const o=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(e);return o?{r:parseInt(o[1],16),g:parseInt(o[2],16),b:parseInt(o[3],16)}:null},Nt=(e,o,a)=>(typeof e=="object"&&(o=e.g,a=e.b,e=e.r),"#"+((1<<24)+(Math.round(e)<<16)+(Math.round(o)<<8)+Math.round(a)).toString(16).slice(1).toUpperCase()),Vt=({r:e,g:o,b:a})=>{e/=255,o/=255,a/=255;const r=Math.max(e,o,a),n=Math.min(e,o,a);let s=0,i=0,l=r;const c=r-n;if(i=r===0?0:c/r,r!==n){switch(r){case e:s=(o-a)/c+(o<a?6:0);break;case o:s=(a-e)/c+2;break;case a:s=(e-o)/c+4;break}s/=6}return{h:s*360,s:i*100,v:l*100}},Ut=(e,o,a)=>{e/=360,o/=100,a/=100;let r=0,n=0,s=0;const i=Math.floor(e*6),l=e*6-i,c=a*(1-o),d=a*(1-l*o),f=a*(1-(1-l)*o);switch(i%6){case 0:r=a,n=f,s=c;break;case 1:r=d,n=a,s=c;break;case 2:r=c,n=a,s=f;break;case 3:r=c,n=d,s=a;break;case 4:r=f,n=c,s=a;break;case 5:r=a,n=c,s=d;break}return{r:r*255,g:n*255,b:s*255}},ws=(e,o,a)=>({r:e.r+(o.r-e.r)*a,g:e.g+(o.g-e.g)*a,b:e.b+(o.b-e.b)*a}),Ss=(e,o)=>{if(Math.abs(o-.5)<.001)return e;const a=Math.max(.001,Math.min(.999,o)),r=Math.log(.5)/Math.log(a);return Math.pow(e,r)},Aa=(e,o=1)=>{let a;if(!e)return"linear-gradient(90deg, #000 0%, #fff 100%)";if(Array.isArray(e))a=e;else if(e&&Array.isArray(e.stops))a=e.stops;else return"linear-gradient(90deg, #000 0%, #fff 100%)";if(!a||a.length===0)return"linear-gradient(90deg, #000 0%, #fff 100%)";const r=[...a].sort((s,i)=>s.position-i.position),n=[];for(let s=0;s<r.length;s++){const i=r[s];let l=Math.pow(i.position,1/o);if(l=Math.max(0,Math.min(1,l))*100,n.push(`${i.color} ${l.toFixed(2)}%`),s<r.length-1){const c=r[s+1],d=i.bias??.5;if((i.interpolation||"linear")==="step"){let p=Math.pow(c.position,1/o);p=Math.max(0,Math.min(1,p))*100,n.push(`${i.color} ${p.toFixed(2)}%`),n.push(`${c.color} ${p.toFixed(2)}%`)}else if(Math.abs(d-.5)>.001){const p=i.position+(c.position-i.position)*d;let h=Math.pow(p,1/o)*100;h=Math.max(0,Math.min(100,h)),n.push(`${h.toFixed(2)}%`)}}}return`linear-gradient(90deg, ${n.join(", ")})`},wa=e=>Math.pow(e/255,2.2)*255,Sa=e=>{const o=e/255;if(o>=.99)return 255;const a=(Math.sqrt(-10127*o*o+13702*o+9)+59*o-3)/(502-486*o);return Math.max(0,a)*255},zo=e=>{const a=new Uint8Array(1024);let r,n="srgb";if(Array.isArray(e))r=e;else if(e&&Array.isArray(e.stops))r=e.stops,n=e.colorSpace||"srgb";else return a;if(r.length===0){for(let l=0;l<256;l++){const c=Math.floor(l/255*255);a[l*4]=c,a[l*4+1]=c,a[l*4+2]=c,a[l*4+3]=255}return a}const s=[...r].sort((l,c)=>l.position-c.position),i=l=>{let c={r:0,g:0,b:0};if(l<=s[0].position)c=lt(s[0].color)||{r:0,g:0,b:0};else if(l>=s[s.length-1].position)c=lt(s[s.length-1].color)||{r:0,g:0,b:0};else for(let d=0;d<s.length-1;d++)if(l>=s[d].position&&l<=s[d+1].position){const f=s[d],p=s[d+1];let h=(l-f.position)/(p.position-f.position);const u=f.bias??.5;Math.abs(u-.5)>.001&&(h=Ss(h,u));const g=f.interpolation||"linear";g==="step"?h=0:(g==="smooth"||g==="cubic")&&(h=h*h*(3-2*h));const S=lt(f.color)||{r:0,g:0,b:0},y=lt(p.color)||{r:0,g:0,b:0};c=ws(S,y,h);break}return n==="linear"?{r:wa(c.r),g:wa(c.g),b:wa(c.b)}:n==="aces_inverse"?{r:Sa(c.r),g:Sa(c.g),b:Sa(c.b)}:c};for(let l=0;l<256;l++){const c=l/255,d=i(c);a[l*4]=d.r,a[l*4+1]=d.g,a[l*4+2]=d.b,a[l*4+3]=255}return a},Ms=e=>{const o=Math.max(1e3,Math.min(4e4,e))/100;let a,r,n;return o<=66?a=255:(a=o-60,a=329.698727446*Math.pow(a,-.1332047592),a=Math.max(0,Math.min(255,a))),o<=66?(r=o,r=99.4708025861*Math.log(r)-161.1195681661,r=Math.max(0,Math.min(255,r))):(r=o-60,r=288.1221695283*Math.pow(r,-.0755148492),r=Math.max(0,Math.min(255,r))),o>=66?n=255:o<=19?n=0:(n=o-10,n=138.5177312231*Math.log(n)-305.0447927307,n=Math.max(0,Math.min(255,n))),{r:Math.round(a),g:Math.round(r),b:Math.round(n)}},Co=e=>{const{r:o,g:a,b:r}=Ms(e);return Nt(o,a,r)},zs=(e,o)=>{const a={};return vs(),ne.getAll().forEach(n=>{const s={},i={};n.state&&Object.assign(s,n.state),Object.entries(n.params).forEach(([c,d])=>{d.composeFrom?d.composeFrom.forEach(f=>{i[f]=c}):s[c]===void 0&&(s[c]=d.default)}),a[n.id]=s;const l=`set${n.id.charAt(0).toUpperCase()+n.id.slice(1)}`;a[l]=c=>{let d=!1;const f={};e(p=>{const h=p[n.id],u={...c};Object.keys(c).forEach(y=>{const b=n.params[y];if(b){const m=c[y];if(m==null)return;b.type==="vec2"&&!(m instanceof je)&&(u[y]=new je(m.x,m.y)),b.type==="vec3"&&!(m instanceof U)&&(u[y]=new U(m.x,m.y,m.z)),b.type==="color"&&!(m instanceof $e)&&(typeof m=="string"?u[y]=new $e(m):typeof m=="number"?u[y]=new $e(m):typeof m=="object"&&"r"in m&&(u[y]=new $e(m.r,m.g,m.b)))}});const g={...h,...u},S=new Set;return Object.keys(u).forEach(y=>{const b=n.params[y];if(i[y]&&S.add(i[y]),b&&(b.noReset||(d=!0),b.type!=="image"&&(f[n.id]||(f[n.id]={}),f[n.id][y]=g[y]),b.uniform)){const m=g[y];if(b.type==="image"){const x=b.uniform.toLowerCase().includes("env")?"env":"color";m&&typeof m=="string"?(Z.emit("texture",{textureType:x,dataUrl:m}),y==="envMapData"&&g.useEnvMap===!1&&(g.useEnvMap=!0,Z.emit("uniform",{key:"uUseEnvMap",value:1,noReset:!1})),y==="layer1Data"&&g.active===!1&&(g.active=!0,Z.emit("uniform",{key:"uUseTexture",value:1,noReset:!1}))):(Z.emit("texture",{textureType:x,dataUrl:null}),y==="envMapData"&&g.useEnvMap===!0&&(g.useEnvMap=!1,Z.emit("uniform",{key:"uUseEnvMap",value:0,noReset:!1})),y==="layer1Data"&&g.active===!0&&(g.active=!1,Z.emit("uniform",{key:"uUseTexture",value:0,noReset:!1})))}else if(b.type==="gradient"){const x=zo(m);Z.emit("uniform",{key:b.uniform,value:{isGradientBuffer:!0,buffer:x},noReset:!!b.noReset})}else{let x=m;b.type==="boolean"&&(x=m?1:0),b.type==="color"&&!(x instanceof $e)&&(x=new $e(x)),Z.emit("uniform",{key:b.uniform,value:x,noReset:!!b.noReset})}}}),S.forEach(y=>{const b=n.params[y];if(b&&b.composeFrom&&b.uniform){const m=b.composeFrom.map(x=>g[x]);if(b.type==="gradient"){const x=g[y];if(x){const C=zo(x);Z.emit("uniform",{key:b.uniform,value:{isGradientBuffer:!0,buffer:C},noReset:!!b.noReset}),b.noReset||(d=!0)}}else if(b.type==="vec2"){const x=new je(m[0],m[1]);Z.emit("uniform",{key:b.uniform,value:x,noReset:!!b.noReset}),b.noReset||(d=!0)}else if(b.type==="vec3"){const x=new U(m[0],m[1],m[2]);Z.emit("uniform",{key:b.uniform,value:x,noReset:!!b.noReset}),b.noReset||(d=!0)}}}),{[n.id]:g}}),Object.keys(f).length>0&&Z.emit("config",f),d&&Z.emit("reset_accum",void 0)},n.actions&&Object.entries(n.actions).forEach(([c,d])=>{a[c]=f=>{const h=o()[n.id],u=d(h,f);u&&Object.keys(u).length>0&&(e({[n.id]:{...h,...u}}),Z.emit("reset_accum",void 0))}})}),a};class Na{constructor(o,a=null){J(this,"defaultState");J(this,"dictionary");J(this,"reverseDictCache",new Map);this.defaultState=o,this.dictionary=a}encode(o,a){try{const r=this.getDiff(o,this.defaultState);if(!r||Object.keys(r).length===0)return"";let n=this.quantize(r);if(!n||Object.keys(n).length===0)return"";this.dictionary&&(n=this.applyDictionary(n,this.dictionary,!0));const s=JSON.stringify(n),i=uo.deflate(s),l=Array.from(i).map(d=>String.fromCharCode(d)).join("");return btoa(l).replace(/\+/g,"-").replace(/\//g,"_").replace(/=+$/,"")}catch(r){return console.error("UrlStateEncoder: Error encoding",r),""}}decode(o){try{if(!o)return null;let a=o.replace(/-/g,"+").replace(/_/g,"/");for(;a.length%4;)a+="=";const r=atob(a),n=new Uint8Array(r.length);for(let l=0;l<r.length;l++)n[l]=r.charCodeAt(l);const s=uo.inflate(n,{to:"string"});let i=JSON.parse(s);return this.dictionary&&(i=this.applyDictionary(i,this.dictionary,!1)),this.deepMerge({...this.defaultState},i)}catch(a){return console.error("UrlStateEncoder: Error decoding",a),null}}getReverseDict(o){if(this.reverseDictCache.has(o))return this.reverseDictCache.get(o);const a={};return Object.keys(o).forEach(r=>{const n=o[r];typeof n=="string"?a[n]=r:a[n._alias]=r}),this.reverseDictCache.set(o,a),a}applyDictionary(o,a,r){if(!o||typeof o!="object"||Array.isArray(o))return o;const n={};if(r)Object.keys(o).forEach(s=>{let i=s,l=null;const c=a[s];c&&(typeof c=="string"?i=c:(i=c._alias,l=c.children));const d=o[s];l&&d&&typeof d=="object"&&!Array.isArray(d)?n[i]=this.applyDictionary(d,l,!0):n[i]=d});else{const s=this.getReverseDict(a);Object.keys(o).forEach(i=>{const l=s[i]||i,c=o[i],d=a[l],f=d&&typeof d=="object"?d.children:null;f&&c&&typeof c=="object"&&!Array.isArray(c)?n[l]=this.applyDictionary(c,f,!1):n[l]=c})}return n}isEqual(o,a){if(o===a)return!0;if(o==null||a==null)return o===a;if(typeof o=="number"&&typeof a=="number")return Math.abs(o-a)<1e-4;if(Array.isArray(o)&&Array.isArray(a))return o.length!==a.length?!1:o.every((r,n)=>this.isEqual(r,a[n]));if(typeof o=="object"&&typeof a=="object"){const r=o,n=a,s=Object.keys(r).filter(l=>!l.startsWith("is")),i=Object.keys(n).filter(l=>!l.startsWith("is"));return s.length!==i.length?!1:s.every(l=>this.isEqual(r[l],n[l]))}return!1}quantize(o){if(typeof o=="string")return o.startsWith("data:image")?void 0:o;if(typeof o=="number")return o===0||Math.abs(o)<1e-9?0:parseFloat(o.toFixed(5));if(Array.isArray(o))return o.map(a=>this.quantize(a));if(o!==null&&typeof o=="object"){const a={};let r=!1;const n=Object.keys(o).filter(s=>!s.startsWith("is"));for(const s of n){const i=this.quantize(o[s]);i!==void 0&&(a[s]=i,r=!0)}return r?a:void 0}return o}getDiff(o,a){if(this.isEqual(o,a))return;if(typeof o!="object"||o===null||typeof a!="object"||a===null||Array.isArray(o))return o;const r={};let n=!1;const s=o,i=a;return Object.keys(s).forEach(l=>{if(l.startsWith("is")||l==="histogramData"||l==="interactionSnapshot"||l==="liveModulations"||l.endsWith("Stack"))return;const c=this.getDiff(s[l],i[l]);c!==void 0&&(r[l]=c,n=!0)}),n?r:void 0}deepMerge(o,a){if(typeof a!="object"||a===null)return a;const r={...o};return Object.keys(a).forEach(n=>{typeof a[n]=="object"&&a[n]!==null&&!Array.isArray(a[n])?r[n]=this.deepMerge(o[n]||{},a[n]):r[n]=a[n]}),r}}const Cs=(e,o)=>({isPlaying:!1,isRecording:!1,isScrubbing:!1,recordCamera:!0,isCameraInteracting:!1,currentFrame:0,fps:30,durationFrames:300,zoomLevel:1,loopMode:"Loop",isArmingModulation:!1,isRecordingModulation:!1,recordingSnapshot:null,play:()=>{const a=o();if(a.currentFrame>=a.durationFrames-.1&&e({currentFrame:0}),a.isArmingModulation){a.snapshot();const r=JSON.parse(JSON.stringify(a.sequence));e({isRecordingModulation:!0,isArmingModulation:!1,recordingSnapshot:r,isPlaying:!0,currentFrame:0})}else e({isPlaying:!0})},pause:()=>e({isPlaying:!1,isRecordingModulation:!1,recordingSnapshot:null}),stop:()=>e({isPlaying:!1,currentFrame:0,isRecordingModulation:!1,recordingSnapshot:null}),toggleRecording:()=>e(a=>({isRecording:!a.isRecording})),toggleRecordCamera:()=>e(a=>({recordCamera:!a.recordCamera})),toggleArmModulation:()=>e(a=>({isArmingModulation:!a.isArmingModulation,isRecording:!1})),stopModulationRecording:()=>e({isRecordingModulation:!1,isPlaying:!1,recordingSnapshot:null}),setLoopMode:a=>e({loopMode:a}),setIsScrubbing:a=>e({isScrubbing:a}),setIsCameraInteracting:a=>e({isCameraInteracting:a}),seek:a=>e({currentFrame:Math.max(0,Math.min(o().durationFrames,a))}),setDuration:a=>{e({durationFrames:a})},setFps:a=>{e({fps:a})}}),ks=(e,o)=>({selectedTrackIds:[],selectedKeyframeIds:[],softSelectionRadius:0,softSelectionEnabled:!1,softSelectionType:"S-Curve",bounceTension:.5,bounceFriction:.6,selectTrack:(a,r)=>e(n=>({selectedTrackIds:r?n.selectedTrackIds.includes(a)?n.selectedTrackIds.filter(s=>s!==a):[...n.selectedTrackIds,a]:[a]})),selectTracks:(a,r)=>e(n=>{const s=new Set(n.selectedTrackIds);return r?a.forEach(i=>s.add(i)):a.forEach(i=>s.delete(i)),{selectedTrackIds:Array.from(s)}}),selectKeyframe:(a,r,n)=>e(s=>{const i=`${a}::${r}`;return{selectedKeyframeIds:n?s.selectedKeyframeIds.includes(i)?s.selectedKeyframeIds.filter(l=>l!==i):[...s.selectedKeyframeIds,i]:[i]}}),selectKeyframes:(a,r)=>e(n=>({selectedKeyframeIds:r?Array.from(new Set([...n.selectedKeyframeIds,...a])):a})),deselectAll:()=>e({selectedTrackIds:[],selectedKeyframeIds:[]}),deselectAllKeys:()=>e({selectedKeyframeIds:[]}),setSoftSelection:(a,r)=>e({softSelectionRadius:a,softSelectionEnabled:r}),setSoftSelectionType:a=>e({softSelectionType:a}),setBouncePhysics:(a,r)=>e({bounceTension:a,bounceFriction:r})});function wr(e,o,a,r,n){const s=1-e,i=e*e,l=s*s,c=l*s,d=i*e;return c*o+3*l*e*a+3*s*i*r+d*n}function ko(e,o){let a=o[0],r=o[o.length-1];for(let f=0;f<o.length-1;f++)if(e>=o[f].frame&&e<o[f+1].frame){a=o[f],r=o[f+1];break}if(e>=r.frame)return r.value;if(e<=a.frame)return a.value;const n=r.frame-a.frame,s=(e-a.frame)/n;if(a.interpolation==="Step")return a.value;if(a.interpolation==="Linear")return a.value+(r.value-a.value)*s;const i=a.value,l=a.value+(a.rightTangent?a.rightTangent.y:0),c=r.value+(r.leftTangent?r.leftTangent.y:0),d=r.value;return wr(s,i,l,c,d)}function js(e,o=1){const a=[],r=e[0].frame,n=e[e.length-1].frame,s=Math.max(o,(n-r)/50);for(let i=r;i<=n;i+=s)a.push({t:i,val:ko(i,e)});return a.length>0&&a[a.length-1].t<n&&a.push({t:n,val:ko(n,e)}),a}function Ps(e,o,a){let r=0,n=0,s=0,i=0,l=0,c=0,d=0;for(let y=0;y<e.length;y++){const b=e[y].t,m=1-b,x=e[y].val;c+=x,d+=x*x;const C=3*m*m*b,z=3*m*b*b,w=m*m*m*o+b*b*b*a,M=x-w;r+=C*C,n+=C*z,s+=z*z,i+=M*C,l+=M*z}const f=e.length,p=c/f;if(d/f-p*p<1e-9)return null;const u=r*s-n*n;if(Math.abs(u)<1e-9)return null;const g=(s*i-n*l)/u,S=(r*l-n*i)/u;return{h1:g,h2:S}}function Ts(e,o){const a=e.length;if(a<2){const u=e[0].val;return{leftY:u,rightY:u}}const r=e[0].val,n=e[a-1].val,s=n-r,i=r+s*.333,l=r+s*.666,c=Ps(e,r,n);let d=i,f=l;c&&(d=c.h1,f=c.h2);const p=i+(d-i)*o,h=l+(f-l)*o;return{leftY:p,rightY:h}}function Ba(e,o,a,r){if(e.length<2)return;const n=e[0],s=e[e.length-1],i=s.t-n.t,l=e.map(h=>({t:(h.t-n.t)/i,val:h.val})),{leftY:c,rightY:d}=Ts(l,r);let f=0,p=-1;if(i<1)f=0;else for(let h=1;h<l.length-1;h++){const u=l[h].t,g=wr(u,n.val,c,d,s.val),S=Math.abs(g-l[h].val);S>f&&(f=S,p=h)}if(f<=a||e.length<=2){const h=o[o.length-1];h&&(h.rightTangent={x:i*.333,y:c-n.val});const u={id:Ze(),frame:s.t,value:s.val,interpolation:"Bezier",brokenTangents:!1,autoTangent:!1,leftTangent:{x:-i*.333,y:d-s.val},rightTangent:{x:1,y:0}};o.push(u)}else{const h=e.slice(0,p+1),u=e.slice(p);Ba(h,o,a,r),Ba(u,o,a,r)}}const Rs=(e,o,a=1)=>{if(e.length<2)return e;a=Math.max(0,Math.min(1,a));const r=[...e].sort((l,c)=>l.frame-c.frame),n=js(r,1),s=[],i=n[0];return s.push({id:Ze(),frame:i.t,value:i.val,interpolation:"Bezier",brokenTangents:!1,autoTangent:!1,leftTangent:{x:-1,y:0},rightTangent:{x:1,y:0}}),Ba(n,s,o,a),s.length>0&&(s[0].leftTangent={x:-1,y:0},s[s.length-1].rightTangent={x:1,y:0}),s},Is=4;function Sr(e,o,a,r,n){const s=1-e,i=e*e,l=s*s,c=l*s,d=i*e;return c*o+3*l*e*a+3*s*i*r+d*n}function Fs(e,o,a,r,n){const s=1-e;return 3*s*s*(a-o)+6*s*e*(r-a)+3*e*e*(n-r)}function _s(e,o,a,r,n){const s=n-o;if(s<=1e-9)return 0;let i=(e-o)/s;for(let l=0;l<Is;++l){const c=Sr(i,o,a,r,n),d=Fs(i,o,a,r,n);if(Math.abs(d)<1e-9)break;const f=c-e;i-=f/d}return Math.max(0,Math.min(1,i))}function Ds(e,o,a,r,n,s,i,l,c){const d=o,f=a,p=o+r,h=a+n,u=s+l,g=i+c,S=s,y=i,b=_s(e,d,p,u,S);return Sr(b,f,h,g,y)}const Ke=.333,Qe={interpolate:(e,o,a,r=!1)=>{if(o.interpolation==="Step")return o.value;let n=o.value,s=a.value;if(r){const c=Math.PI*2,d=s-n;d>Math.PI?s-=c:d<-Math.PI&&(s+=c)}if(o.interpolation==="Bezier"){const c=o.rightTangent?o.rightTangent.x:(a.frame-o.frame)*Ke,d=o.rightTangent?o.rightTangent.y:0,f=a.leftTangent?a.leftTangent.x:-(a.frame-o.frame)*Ke,p=a.leftTangent?a.leftTangent.y:0;return Ds(e,o.frame,n,c,d,a.frame,s,f,p)}const i=a.frame-o.frame;if(i<1e-9)return n;const l=(e-o.frame)/i;return n+(s-n)*l},scaleHandles:(e,o,a,r,n)=>{const s={};if(e.interpolation!=="Bezier")return s;if(o&&e.leftTangent){const i=r-o.frame,l=n-o.frame;if(Math.abs(i)>1e-5&&Math.abs(l)>1e-5){const c=l/i;s.leftTangent={x:e.leftTangent.x*c,y:e.leftTangent.y*c}}}if(a&&e.rightTangent){const i=a.frame-r,l=a.frame-n;if(Math.abs(i)>1e-5&&Math.abs(l)>1e-5){const c=l/i;s.rightTangent={x:e.rightTangent.x*c,y:e.rightTangent.y*c}}}return s},calculateTangents:(e,o,a,r)=>{if(r==="Ease"){const y=o?(e.frame-o.frame)*Ke:10,b=a?(a.frame-e.frame)*Ke:10;return{l:{x:-y,y:0},r:{x:b,y:0}}}if(!o&&!a)return{l:{x:-10,y:0},r:{x:10,y:0}};if(!o){const y=(a.value-e.value)/(a.frame-e.frame),b=(a.frame-e.frame)*Ke;return{l:{x:-10,y:0},r:{x:b,y:b*y}}}if(!a){const y=(e.value-o.value)/(e.frame-o.frame),b=(e.frame-o.frame)*Ke;return{l:{x:-b,y:-b*y},r:{x:10,y:0}}}const n=e.frame-o.frame,s=e.value-o.value,i=n===0?0:s/n,l=a.frame-e.frame,c=a.value-e.value,d=l===0?0:c/l;if(i*d<=0){const y=n*Ke,b=l*Ke;return{l:{x:-y,y:0},r:{x:b,y:0}}}const f=a.frame-o.frame,p=a.value-o.value;let h=f===0?0:p/f;const u=3*Math.min(Math.abs(i),Math.abs(d));Math.abs(h)>u&&(h=Math.sign(h)*u);const g=n*Ke,S=l*Ke;return{l:{x:-g,y:-g*h},r:{x:S,y:S*h}}},constrainHandles:(e,o,a)=>{var n,s;const r={};if(e.leftTangent&&o){const i=e.frame-o.frame;if(i>.001){const l=i*Ke;if(Math.abs(e.leftTangent.x)>l){const c=l/Math.abs(e.leftTangent.x);r.leftTangent={x:e.leftTangent.x*c,y:e.leftTangent.y*c}}e.leftTangent.x>0&&(r.leftTangent={x:0,y:((n=r.leftTangent)==null?void 0:n.y)??e.leftTangent.y})}}if(e.rightTangent&&a){const i=a.frame-e.frame;if(i>.001){const l=i*Ke;if(Math.abs(e.rightTangent.x)>l){const c=l/Math.abs(e.rightTangent.x);r.rightTangent={x:e.rightTangent.x*c,y:e.rightTangent.y*c}}e.rightTangent.x<0&&(r.rightTangent={x:0,y:((s=r.rightTangent)==null?void 0:s.y)??e.rightTangent.y})}}return r},calculateSoftFalloff:(e,o,a)=>{if(e>=o)return 0;const r=e/o;switch(a){case"Linear":return 1-r;case"Dome":return Math.sqrt(1-r*r);case"Pinpoint":return Math.pow(1-r,4);case"S-Curve":return .5*(1+Math.cos(r*Math.PI));default:return 1-r}}},Ma={updateNeighbors:(e,o)=>{const a=e[o],r=o===e.length-1,n=o-1;if(n>=0){const i={...e[n]};if(e[n]=i,i.interpolation==="Bezier"){const l=a.frame-i.frame;if(i.autoTangent){const c=e[n-1],{l:d,r:f}=Qe.calculateTangents(i,c,a,"Auto");i.leftTangent=d,i.rightTangent=f}else{const c=Qe.constrainHandles(i,e[n-1],a);Object.assign(i,c)}if(r&&l>1e-4){const c=l*.3,d=i.rightTangent||{x:10,y:0};if(d.x<c){const f=c/Math.max(1e-4,Math.abs(d.x));i.rightTangent={x:c,y:d.y*f}}}}}const s=o+1;if(s<e.length){const i={...e[s]};if(e[s]=i,i.interpolation==="Bezier")if(i.autoTangent){const l=e[s+1],{l:c,r:d}=Qe.calculateTangents(i,a,l,"Auto");i.leftTangent=c,i.rightTangent=d}else{const l=Qe.constrainHandles(i,a,e[s+1]);Object.assign(i,l)}}},inferInterpolation:(e,o)=>{const a=e.filter(r=>r.frame<o).sort((r,n)=>n.frame-r.frame);return a.length===0||a[0].interpolation==="Linear"?"Linear":a[0].interpolation==="Step"?"Step":"Bezier"}},Ls=xe(),Es={durationFrames:300,fps:30,tracks:{}},As=(e,o)=>({sequence:Es,clipboard:null,undoStack:[],redoStack:[],snapshot:()=>{const a=o().sequence,r=JSON.parse(JSON.stringify(a));e(n=>{const s=[...n.undoStack,{type:"SEQUENCE",data:r}];return{undoStack:s.length>50?s.slice(1):s,redoStack:[]}})},undo:()=>{const{undoStack:a,redoStack:r,sequence:n}=o();if(a.length===0)return!1;const s=a[a.length-1],i=a.slice(0,-1),c={type:"SEQUENCE",data:JSON.parse(JSON.stringify(n))};return e({sequence:s.data,undoStack:i,redoStack:[c,...r]}),!0},redo:()=>{const{undoStack:a,redoStack:r,sequence:n}=o();if(r.length===0)return!1;const s=r[0],i=r.slice(1),c={type:"SEQUENCE",data:JSON.parse(JSON.stringify(n))};return e({sequence:s.data,undoStack:[...a,c],redoStack:i}),!0},setSequence:a=>{o().snapshot(),e({sequence:a})},addTrack:(a,r)=>{o().snapshot(),e(n=>n.sequence.tracks[a]?n:{sequence:{...n.sequence,tracks:{...n.sequence.tracks,[a]:{id:a,type:"float",label:r,keyframes:[]}}}})},removeTrack:a=>{o().snapshot(),e(r=>{const n={...r.sequence.tracks};return delete n[a],{sequence:{...r.sequence,tracks:n},selectedTrackIds:r.selectedTrackIds.filter(s=>s!==a)}})},setTrackBehavior:(a,r)=>{o().snapshot(),e(n=>{const s=n.sequence.tracks[a];return s?{sequence:{...n.sequence,tracks:{...n.sequence.tracks,[a]:{...s,postBehavior:r}}}}:n})},addKeyframe:(a,r,n,s)=>{e(i=>{const l=i.sequence.tracks[a];if(!l)return i;let c=s||"Bezier";s||(c=Ma.inferInterpolation(l.keyframes,r));const d=c==="Bezier",f={id:Ze(),frame:r,value:n,interpolation:c,autoTangent:d,brokenTangents:!1},h=[...l.keyframes.filter(g=>Math.abs(g.frame-r)>.001),f].sort((g,S)=>g.frame-S.frame),u=h.findIndex(g=>g.id===f.id);if(c==="Bezier"){const g=u>0?h[u-1]:void 0,S=u<h.length-1?h[u+1]:void 0,{l:y,r:b}=Qe.calculateTangents(f,g,S,"Auto");f.leftTangent=y,f.rightTangent=b}return Ma.updateNeighbors(h,u),{sequence:{...i.sequence,tracks:{...i.sequence.tracks,[a]:{...l,keyframes:h}}}}})},batchAddKeyframes:(a,r,n)=>{e(s=>{const i={...s.sequence.tracks};let l=!1;return r.forEach(({trackId:c,value:d})=>{i[c]||(i[c]={id:c,type:"float",label:c,keyframes:[]},l=!0);const f=i[c],p=[...f.keyframes],h=p.length>0?p[p.length-1]:null,u={id:Ze(),frame:a,value:d,interpolation:n||"Linear",autoTangent:n==="Bezier",brokenTangents:!1};if(h)if(a>h.frame)p.push(u);else if(Math.abs(a-h.frame)<.001)u.id=h.id,p[p.length-1]=u;else{const g=p.filter(S=>Math.abs(S.frame-a)>.001);g.push(u),g.sort((S,y)=>S.frame-y.frame),f.keyframes=g,l=!0;return}else p.push(u);f.keyframes=p,l=!0}),l?{sequence:{...s.sequence,tracks:i}}:s})},removeKeyframe:(a,r)=>{o().snapshot(),e(n=>{const s=n.sequence.tracks[a];return s?{sequence:{...n.sequence,tracks:{...n.sequence.tracks,[a]:{...s,keyframes:s.keyframes.filter(i=>i.id!==r)}}}}:n})},updateKeyframe:(a,r,n)=>{e(s=>{const i=s.sequence.tracks[a];if(!i)return s;const l=i.keyframes.map(c=>c.id===r?{...c,...n}:c).sort((c,d)=>c.frame-d.frame);return{sequence:{...s.sequence,tracks:{...s.sequence.tracks,[a]:{...i,keyframes:l}}}}})},updateKeyframes:a=>{e(r=>{const n={...r.sequence.tracks};return a.forEach(({trackId:s,keyId:i,patch:l})=>{const c=n[s];if(c){const d=c.keyframes.findIndex(f=>f.id===i);if(d!==-1){const f=c.keyframes[d];l.interpolation==="Bezier"&&f.interpolation!=="Bezier"&&(l.autoTangent=!0),c.keyframes[d]={...f,...l}}}}),Object.keys(n).forEach(s=>{n[s].keyframes.sort((i,l)=>i.frame-l.frame)}),{sequence:{...r.sequence,tracks:n}}})},deleteSelectedKeyframes:()=>{o().snapshot(),e(a=>{const r={...a.sequence.tracks},n=new Set(a.selectedKeyframeIds);return Object.keys(r).forEach(s=>{r[s]={...r[s],keyframes:r[s].keyframes.filter(i=>!n.has(`${s}::${i.id}`))}}),{sequence:{...a.sequence,tracks:r},selectedKeyframeIds:[]}})},deleteAllKeys:()=>{o().snapshot(),e(a=>{const r={...a.sequence.tracks};return Object.keys(r).forEach(n=>{r[n]={...r[n],keyframes:[]}}),{sequence:{...a.sequence,tracks:r},selectedKeyframeIds:[]}})},deleteAllTracks:()=>{o().snapshot(),e({sequence:{...o().sequence,tracks:{}},selectedTrackIds:[],selectedKeyframeIds:[]})},setTangents:a=>{o().snapshot(),e(r=>{const n={...r.sequence.tracks};return r.selectedKeyframeIds.forEach(s=>{const[i,l]=s.split("::"),c=n[i];if(c){const d=c.keyframes.findIndex(p=>p.id===l);if(d===-1)return;const f=c.keyframes[d];if(a==="Split")c.keyframes[d]={...f,brokenTangents:!0,autoTangent:!1};else if(a==="Unified"){let p=f.rightTangent,h=f.leftTangent;if(p&&h){const u=Math.sqrt(p.x*p.x+p.y*p.y),g=Math.sqrt(h.x*h.x+h.y*h.y);p={x:-h.x*(u/Math.max(.001,g)),y:-h.y*(u/Math.max(.001,g))}}c.keyframes[d]={...f,rightTangent:p,brokenTangents:!1,autoTangent:!1}}else if(a==="Auto"||a==="Ease"){const p=c.keyframes[d-1],h=c.keyframes[d+1],{l:u,r:g}=Qe.calculateTangents(f,p,h,a);c.keyframes[d]={...f,autoTangent:a==="Auto",brokenTangents:!1,leftTangent:u,rightTangent:g}}}}),{sequence:{...r.sequence,tracks:n}}})},setGlobalInterpolation:(a,r)=>{o().snapshot(),e(n=>{const s={...n.sequence.tracks};return Object.keys(s).forEach(i=>{const l=s[i];l.keyframes.length!==0&&l.keyframes.forEach((c,d)=>{if(c.interpolation=a,a==="Bezier"&&r){const f=l.keyframes[d-1],p=l.keyframes[d+1],{l:h,r:u}=Qe.calculateTangents(c,f,p,r);c.leftTangent=h,c.rightTangent=u,c.autoTangent=r==="Auto",c.brokenTangents=!1}})}),{sequence:{...n.sequence,tracks:s}}})},copySelectedKeyframes:()=>{const{sequence:a,selectedKeyframeIds:r}=o();if(r.length===0)return;let n=1/0;r.forEach(i=>{var f,p;const[l,c]=i.split("::"),d=(p=(f=a.tracks[l])==null?void 0:f.keyframes.find(h=>h.id===c))==null?void 0:p.frame;d!==void 0&&d<n&&(n=d)});const s=[];r.forEach(i=>{var f;const[l,c]=i.split("::"),d=(f=a.tracks[l])==null?void 0:f.keyframes.find(p=>p.id===c);d&&s.push({relativeFrame:d.frame-n,value:d.value,interpolation:d.interpolation,leftTangent:d.leftTangent,rightTangent:d.rightTangent,originalTrackId:l})}),s.length>0&&e({clipboard:s})},pasteKeyframes:a=>{const{clipboard:r,currentFrame:n}=o();r&&(o().snapshot(),e(s=>{const i={...s.sequence.tracks},l=a!==void 0?a:n;return r.forEach(c=>{const d=i[c.originalTrackId];if(d){const f=l+c.relativeFrame,p={id:Ze(),frame:f,value:c.value,interpolation:c.interpolation,leftTangent:c.leftTangent,rightTangent:c.rightTangent,autoTangent:!1,brokenTangents:!1};d.keyframes=[...d.keyframes.filter(h=>Math.abs(h.frame-f)>.001),p].sort((h,u)=>h.frame-u.frame)}}),{sequence:{...s.sequence,tracks:i}}}))},duplicateSelection:()=>{o().copySelectedKeyframes(),o().pasteKeyframes(o().currentFrame)},loopSelection:a=>{const r=o();if(r.selectedKeyframeIds.length<1)return;r.snapshot();let n=1/0,s=-1/0;if(r.selectedKeyframeIds.forEach(l=>{const[c,d]=l.split("::"),f=r.sequence.tracks[c],p=f==null?void 0:f.keyframes.find(h=>h.id===d);p&&(p.frame<n&&(n=p.frame),p.frame>s&&(s=p.frame))}),n===1/0||s===-1/0)return;const i=Math.max(1,s-n);e(l=>{const c={...l.sequence.tracks};for(let d=1;d<=a;d++){const f=i*d;l.selectedKeyframeIds.forEach(p=>{const[h,u]=p.split("::"),g=c[h];if(!g)return;const S=g.keyframes.find(y=>y.id===u);if(S){const y=S.frame+f,b={...S,id:Ze(),frame:y};g.keyframes=[...g.keyframes.filter(m=>Math.abs(m.frame-y)>.001),b]}})}return Object.values(c).forEach(d=>d.keyframes.sort((f,p)=>f.frame-p.frame)),{sequence:{...l.sequence,tracks:c}}})},captureCameraFrame:(a,r=!1,n)=>{const s=We()||Ls.activeCamera;if(!s)return;r||o().snapshot();const i=He.getUnifiedFromEngine(),l=s.quaternion,c=new Fe().setFromQuaternion(l),d=[{id:"camera.unified.x",val:i.x,label:"Position X"},{id:"camera.unified.y",val:i.y,label:"Position Y"},{id:"camera.unified.z",val:i.z,label:"Position Z"},{id:"camera.rotation.x",val:c.x,label:"Rotation X"},{id:"camera.rotation.y",val:c.y,label:"Rotation Y"},{id:"camera.rotation.z",val:c.z,label:"Rotation Z"}];e(f=>{const p={...f.sequence.tracks},h=p["camera.unified.x"],u=!h||h.keyframes.length===0,g=n||(u?"Linear":"Bezier");return d.forEach(S=>{let y=p[S.id];y||(y={id:S.id,type:"float",label:S.label,keyframes:[],hidden:!1},p[S.id]=y);const b={id:Ze(),frame:a,value:S.val,interpolation:g,autoTangent:g==="Bezier",brokenTangents:!1},x=[...y.keyframes.filter(z=>Math.abs(z.frame-a)>.001),b].sort((z,w)=>z.frame-w.frame),C=x.findIndex(z=>z.id===b.id);if(g==="Bezier"){const z=C>0?x[C-1]:void 0,w=C<x.length-1?x[C+1]:void 0,{l:M,r:P}=Qe.calculateTangents(b,z,w,"Auto");b.leftTangent=M,b.rightTangent=P}Ma.updateNeighbors(x,C),y.keyframes=x}),{sequence:{...f.sequence,tracks:p}}})},simplifySelectedKeys:(a=.01)=>{o().snapshot(),e(r=>{const n=r,s={...n.sequence.tracks},i=new Set(n.selectedKeyframeIds),l={};n.selectedKeyframeIds.forEach(d=>{const[f,p]=d.split("::");l[f]||(l[f]=[]);const h=n.sequence.tracks[f],u=h==null?void 0:h.keyframes.find(g=>g.id===p);u&&l[f].push(u)});const c=[];return Object.entries(l).forEach(([d,f])=>{const p=s[d];if(!p)return;const h={...p};if(s[d]=h,f.length<3)return;const u=f.sort((S,y)=>S.frame-y.frame);h.keyframes=h.keyframes.filter(S=>!i.has(`${d}::${S.id}`));const g=Rs(u,a);h.keyframes=[...h.keyframes,...g].sort((S,y)=>S.frame-y.frame),g.forEach(S=>c.push(`${d}::${S.id}`))}),{sequence:{...n.sequence,tracks:s},selectedKeyframeIds:c}})}}),fe=cr()(ur((e,o,a)=>({...Cs(e,o),...ks(e),...As(e,o)})));typeof window<"u"&&(window.useAnimationStore=fe);const qt=xe(),Jt=e=>{const o={};return e&&Object.keys(e).forEach(a=>{if(a.startsWith("is"))return;const r=e[a];if(r&&typeof r=="object"&&"isColor"in r)o[a]="#"+r.getHexString();else if(r&&typeof r=="object"&&("isVector2"in r||"isVector3"in r)){const n={...r};delete n.isVector2,delete n.isVector3,o[a]=n}else o[a]=r}),o},Mr=e=>{const o=Me.get(e),a=o&&o.defaultPreset?o.defaultPreset:{},r={version:5,name:e,formula:e,features:{}};return ne.getAll().forEach(n=>{const s={};Object.entries(n.params).forEach(([i,l])=>{l.composeFrom||(s[i]=l.default)}),r.features[n.id]=Jt(s)}),a.features&&Object.entries(a.features).forEach(([n,s])=>{r.features[n]?r.features[n]={...r.features[n],...Jt(s)}:r.features[n]=Jt(s)}),a.lights&&(r.features.lighting||(r.features.lighting={}),r.features.lighting.lights=a.lights),a.renderMode&&(r.features.lighting||(r.features.lighting={}),r.features.lighting.renderMode=a.renderMode==="PathTracing"?1:0),r.cameraMode=a.cameraMode||"Orbit",r.quality={aaMode:"Always",aaLevel:1,msaa:1,accumulation:!0,...a.quality||{}},r.lights=[],r.animations=a.animations||[],r.navigation={flySpeed:.5,autoSlow:!0,...a.navigation||{}},r.sceneOffset=a.sceneOffset||{x:0,y:0,z:0,xL:0,yL:0,zL:0},r.cameraPos=a.cameraPos||{x:0,y:0,z:3.5},r.cameraRot=a.cameraRot||{x:0,y:0,z:0,w:1},r.targetDistance=a.targetDistance||3.5,r.duration=a.duration||300,r.sequence=a.sequence||{durationFrames:300,fps:30,tracks:{}},r},Ns=(e,o,a)=>{const r=a(),n=e.features||{};if(e.renderMode&&(n.lighting||(n.lighting={}),n.lighting.renderMode===void 0&&(n.lighting.renderMode=e.renderMode==="PathTracing"?1:0)),n.atmosphere&&!n.ao){const y={};n.atmosphere.aoIntensity!==void 0&&(y.aoIntensity=n.atmosphere.aoIntensity),n.atmosphere.aoSpread!==void 0&&(y.aoSpread=n.atmosphere.aoSpread),n.atmosphere.aoMode!==void 0&&(y.aoMode=n.atmosphere.aoMode),n.atmosphere.aoEnabled!==void 0&&(y.aoEnabled=n.atmosphere.aoEnabled),Object.keys(y).length>0&&(n.ao=y)}if(ne.getAll().forEach(y=>{const b=`set${y.id.charAt(0).toUpperCase()+y.id.slice(1)}`,m=r[b];if(typeof m=="function"){const x=n[y.id],C={};if(y.state&&Object.assign(C,y.state),Object.entries(y.params).forEach(([z,w])=>{if(x&&x[z]!==void 0){let M=x[z];w.type==="vec2"&&M&&!(M instanceof je)?M=new je(M.x,M.y):w.type==="vec3"&&M&&!(M instanceof U)?M=new U(M.x,M.y,M.z):w.type==="color"&&M&&!(M instanceof $e)&&(M=new $e(M)),C[z]=M}else if(C[z]===void 0){let M=w.default;M&&typeof M=="object"&&(typeof M.clone=="function"?M=M.clone():Array.isArray(M)?M=[...M]:M={...M}),C[z]=M}}),y.id==="lighting"&&x){if(x.lights)C.lights=Ea(x.lights.map(z=>({...z,type:z.type||"Point",rotation:z.rotation||{x:0,y:0,z:0}})));else if(x.light0_posX!==void 0){const z=[];for(let w=0;w<3;w++)if(x[`light${w}_posX`]!==void 0){let M=x[`light${w}_color`]||"#ffffff";M.getHexString&&(M="#"+M.getHexString()),z.push({type:"Point",position:{x:x[`light${w}_posX`],y:x[`light${w}_posY`],z:x[`light${w}_posZ`]},rotation:{x:0,y:0,z:0},color:M,intensity:x[`light${w}_intensity`]??1,falloff:x[`light${w}_falloff`]??0,falloffType:x[`light${w}_type`]?"Linear":"Quadratic",fixed:x[`light${w}_fixed`]??!1,visible:x[`light${w}_visible`]??w===0,castShadow:x[`light${w}_castShadow`]??!0})}z.length>0&&(C.lights=z)}}y.id==="materials"&&x&&x.envMapVisible!==void 0&&x.envBackgroundStrength===void 0&&(C.envBackgroundStrength=x.envMapVisible?1:0),m(C)}}),e.lights&&e.lights.length>0){const y=r.setLighting;if(typeof y=="function"){const b=Ea(e.lights.map(m=>({...m,type:m.type||"Point",rotation:m.rotation||{x:0,y:0,z:0}})));y({lights:b})}}e.sequence&&fe.getState().setSequence(e.sequence),r.setAnimations(e.animations||[]),e.savedCameras&&Array.isArray(e.savedCameras)&&e.savedCameras.length>0&&o({savedCameras:e.savedCameras,activeCameraId:e.savedCameras[0].id||null});const s=e.cameraPos||{x:0,y:0,z:3.5},i=e.sceneOffset||{x:0,y:0,z:0,xL:0,yL:0,zL:0},l=e.targetDistance||3.5,c=e.cameraRot||{x:0,y:0,z:0,w:1},d=i.x+i.xL+s.x,f=i.y+i.yL+s.y,p=i.z+i.zL+s.z,h=ye.split(d),u=ye.split(f),g=ye.split(p),S={x:h.high,y:u.high,z:g.high,xL:h.low,yL:u.low,zL:g.low};o({cameraRot:c,targetDistance:l,sceneOffset:S,cameraMode:e.cameraMode||a().cameraMode}),qt.activeCamera&&qt.virtualSpace&&qt.virtualSpace.applyCameraState(qt.activeCamera,{position:{x:0,y:0,z:0},rotation:c,sceneOffset:S,targetDistance:l}),Z.emit("camera_teleport",{position:{x:0,y:0,z:0},rotation:c,sceneOffset:S,targetDistance:l}),e.duration&&fe.getState().setDuration(e.duration),e.formula==="Modular"&&r.refreshPipeline(),r.refreshHistogram(),Z.emit("reset_accum",void 0)},Bs={formula:"Mandelbulb",cameraPos:{x:0,y:0,z:0},cameraRot:{x:0,y:0,z:0,w:1},sceneOffset:{x:0,y:0,z:0,xL:0,yL:0,zL:0},targetDistance:3.5,cameraMode:"Orbit"},Os=(e,o,a={includeAnimations:!0})=>{try{e.quality&&(delete e.quality.aaLevel,delete e.quality.aaMode,delete e.quality.msaa),e.features&&e.features.quality&&delete e.features.quality.resolutionMode,a.includeAnimations===!1&&(delete e.sequence,delete e.animations);const r=Mr(e.formula);r.formula="";const n=ne.getDictionary();return new Na(r,n).encode(e,o)}catch(r){return console.error("Sharing: Failed to generate share string",r),""}},zr=e=>{if(!e)return null;try{const o=ne.getDictionary(),r=new Na(Bs,o).decode(e);if(r&&r.formula){const n=Mr(r.formula);return new Na(n,o).decode(e)}}catch(o){console.error("Sharing: Failed to load share string",o)}return null},za=xe();class $s{constructor(){J(this,"pendingCam");J(this,"binders",new Map);J(this,"overriddenTracks",new Set);J(this,"lastCameraIndex",-1);J(this,"animStore",null);J(this,"fractalStore",null);this.pendingCam={rot:new Fe,unified:new U,rotDirty:!1,unifiedDirty:!1}}connect(o,a){this.animStore=o,this.fractalStore=a}setOverriddenTracks(o){this.overriddenTracks=o}getBinder(o){if(this.binders.has(o))return this.binders.get(o);let a=()=>{};if(o==="camera.active_index")a=r=>{const n=Math.round(r);if(n!==this.lastCameraIndex){const s=this.fractalStore.getState(),i=s.savedCameras;i&&i[n]&&(s.selectCamera(i[n].id),this.lastCameraIndex=n)}};else if(o.startsWith("camera.")){const r=o.split("."),n=r[1],s=r[2];n==="unified"?a=i=>{this.pendingCam.unified[s]=i,this.pendingCam.unifiedDirty=!0}:n==="rotation"&&(a=i=>{this.pendingCam.rot[s]=i,this.pendingCam.rotDirty=!0})}else if(o.startsWith("lights.")){const r=o.split("."),n=parseInt(r[1]),s=r[2];let i="";s==="position"?i=`pos${r[3].toUpperCase()}`:s==="color"?i="color":i=s;const l=`lighting.light${n}_${i}`;return this.getBinder(l)}else if(o.startsWith("lighting.light")){const r=o.match(/lighting\.light(\d+)_(\w+)/);if(r){const n=parseInt(r[1]),s=r[2],i=this.fractalStore.getState();if(s==="intensity")a=l=>i.updateLight({index:n,params:{intensity:l}});else if(s==="falloff")a=l=>i.updateLight({index:n,params:{falloff:l}});else if(s.startsWith("pos")){const l=s.replace("pos","").toLowerCase();a=c=>{var p;const f=(p=this.fractalStore.getState().lighting)==null?void 0:p.lights[n];if(f){const h={...f.position,[l]:c};i.updateLight({index:n,params:{position:h}})}}}else if(s.startsWith("rot")){const l=s.replace("rot","").toLowerCase();a=c=>{var p;const f=(p=this.fractalStore.getState().lighting)==null?void 0:p.lights[n];if(f){const h={...f.rotation,[l]:c};i.updateLight({index:n,params:{rotation:h}})}}}}}else if(o.includes(".")){const r=o.split("."),n=r[0],s=r[1];if(ne.get(n)){const l=this.fractalStore.getState(),c=`set${n.charAt(0).toUpperCase()+n.slice(1)}`,d=l[c];if(d&&typeof d=="function"){const f=s.match(/^(vec[23][ABC])_(x|y|z)$/);if(f){const p=f[1],h=f[2];a=u=>{var y;const S=(y=this.fractalStore.getState()[n])==null?void 0:y[p];if(S){const b=S.clone();b[h]=u,d({[p]:b})}}}else a=p=>d({[s]:p})}else console.warn(`AnimationEngine: Setter ${c} not found for feature ${n}`)}}else{const r=this.fractalStore.getState(),n="set"+o.charAt(0).toUpperCase()+o.slice(1);typeof r[n]=="function"&&(a=s=>r[n](s))}return this.binders.set(o,a),a}tick(o){if(!this.animStore)return;const a=this.animStore.getState();if(!a.isPlaying)return;const r=a.fps,n=a.currentFrame,s=a.durationFrames,i=a.loopMode,l=o*r;let c=n+l;if(c>=s)if(i==="Once"||a.isRecordingModulation){c=s,this.scrub(s),this.animStore.setState({isPlaying:!1,currentFrame:s}),a.isRecordingModulation&&a.stopModulationRecording();return}else c=0;this.animStore.setState({currentFrame:c}),this.scrub(c)}scrub(o){if(!this.animStore)return;const{sequence:a,isPlaying:r,isRecording:n,recordCamera:s}=this.animStore.getState(),i=Object.values(a.tracks);this.syncBuffersFromEngine();const l=r&&n&&s;for(let c=0;c<i.length;c++){const d=i[c];if(this.overriddenTracks.has(d.id)||d.keyframes.length===0||d.type!=="float"||d.id.includes("camera.position")||d.id.includes("camera.offset")||l&&d.id.startsWith("camera."))continue;const f=this.interpolate(d,o);this.getBinder(d.id)(f)}this.commitState()}syncBuffersFromEngine(){const o=We()||za.activeCamera;if(o){this.pendingCam.rot.setFromQuaternion(o.quaternion);const a=za.sceneOffset;this.pendingCam.unified.set(a.x+a.xL+o.position.x,a.y+a.yL+o.position.y,a.z+a.zL+o.position.z),this.pendingCam.rotDirty=!1,this.pendingCam.unifiedDirty=!1}}interpolate(o,a){const r=o.keyframes;if(r.length===0)return 0;const n=r[0],s=r[r.length-1],i=o.id.startsWith("camera.rotation")||o.id.includes("rot")||o.id.includes("phase")||o.id.includes("twist");if(a>s.frame){const l=o.postBehavior||"Hold";if(l==="Hold")return s.value;if(l==="Continue"){let u=0;if(r.length>1){const g=r[r.length-2];s.interpolation==="Linear"?u=(s.value-g.value)/(s.frame-g.frame):s.interpolation==="Bezier"&&(s.leftTangent&&Math.abs(s.leftTangent.x)>.001?u=s.leftTangent.y/s.leftTangent.x:u=(s.value-g.value)/(s.frame-g.frame))}return s.value+u*(a-s.frame)}const c=s.frame-n.frame;if(c<=.001)return s.value;const d=a-n.frame,f=Math.floor(d/c),p=n.frame+d%c,h=this.evaluateCurveInternal(r,p,i);if(l==="Loop")return h;if(l==="PingPong"){if(f%2===1){const g=s.frame-d%c;return this.evaluateCurveInternal(r,g,i)}return h}if(l==="OffsetLoop"){const u=s.value-n.value;return h+u*f}}return a<n.frame?n.value:this.evaluateCurveInternal(r,a,i)}evaluateCurveInternal(o,a,r){for(let n=0;n<o.length-1;n++){const s=o[n],i=o[n+1];if(a>=s.frame&&a<=i.frame)return Qe.interpolate(a,s,i,r)}return o[o.length-1].value}commitState(){if(this.pendingCam.unifiedDirty||this.pendingCam.rotDirty){za.shouldSnapCamera=!0;const o=new De().setFromEuler(this.pendingCam.rot),a={x:o.x,y:o.y,z:o.z,w:o.w},r=ye.split(this.pendingCam.unified.x),n=ye.split(this.pendingCam.unified.y),s=ye.split(this.pendingCam.unified.z);Z.emit(we.CAMERA_TELEPORT,{position:{x:0,y:0,z:0},rotation:a,sceneOffset:{x:r.high,y:n.high,z:s.high,xL:r.low,yL:n.low,zL:s.low}}),this.fractalStore.setState({cameraRot:a})}}}const oa=new $s,ju=(e,o)=>{const a={};e.forEach(i=>a[i.id]=[]),o.forEach(i=>{a[i.source]&&a[i.source].push(i.target)});const r=new Set,n=new Set,s=i=>{if(!r.has(i)){r.add(i),n.add(i);const l=a[i]||[];for(const c of l)if(!r.has(c)&&s(c)||n.has(c))return!0}return n.delete(i),!1};for(const i of e)if(s(i.id))return!0;return!1},jo=(e,o)=>{const a={},r={};e.forEach(i=>{a[i.id]=[],r[i.id]=0}),o.forEach(i=>{a[i.source]&&(a[i.source].push(i.target),r[i.target]=(r[i.target]||0)+1)});const n=[];e.forEach(i=>{r[i.id]===0&&n.push(i.id)});const s=[];for(;n.length>0;){n.sort();const i=n.shift(),l=e.find(c=>c.id===i);if(l){const{position:c,...d}=l;s.push(d)}if(a[i])for(const c of a[i])r[c]--,r[c]===0&&n.push(c)}return s},Po=e=>{const o=e.map((r,n)=>({...r,position:{x:250,y:150+n*200}})),a=[];if(o.length>0){a.push({id:`e-root-start-${o[0].id}`,source:"root-start",target:o[0].id});for(let r=0;r<o.length-1;r++)a.push({id:`e-${o[r].id}-${o[r+1].id}`,source:o[r].id,target:o[r+1].id});a.push({id:`e-${o[o.length-1].id}-root-end`,source:o[o.length-1].id,target:"root-end"})}return{nodes:o,edges:a}},Hs=(e,o)=>{if(e.length!==o.length)return!1;for(let a=0;a<e.length;a++){const r=e[a],n=o[a];if(r.id!==n.id||r.type!==n.type||r.enabled!==n.enabled)return!1;const s=r.bindings||{},i=n.bindings||{},l=Object.keys(s).filter(p=>s[p]!==void 0),c=Object.keys(i).filter(p=>i[p]!==void 0);if(l.length!==c.length)return!1;for(const p of l)if(s[p]!==i[p])return!1;const d=r.condition||{active:!1,mod:0,rem:0},f=n.condition||{active:!1,mod:0,rem:0};if(d.active!==f.active||d.active&&(d.mod!==f.mod||d.rem!==f.rem))return!1}return!0},Gs=(e,o)=>e.length!==o.length?!1:JSON.stringify(e)===JSON.stringify(o),Oa=[{id:"note-1",type:"Note",enabled:!0,params:{},text:`Infinite Repetition
...
This rotation is bound to 'ParamC' (Slider below). Try dragging it!`},{id:"rot-1",type:"Rotate",enabled:!0,params:{x:0,y:0,z:0},bindings:{z:"ParamC"}},{id:"bulb-1",type:"Mandelbulb",enabled:!0,params:{power:8}},{id:"add-c",type:"AddConstant",enabled:!0,params:{scale:1}}],Je=xe(),N=cr()(ur((e,o,a)=>({...qi(e,o),...Xi(e,o),...nn(e,o),...un(e,o),...zs(e,o),formula:"Mandelbulb",pipeline:Oa,pipelineRevision:1,graph:Po(Oa),projectSettings:{name:"Mandelbulb",version:0},lastSavedHash:null,animations:[],liveModulations:{},setFormula:(r,n={})=>{const s=o(),i=s.formula;if(i===r&&r!=="Modular")return;n.skipDefaultPreset||(o().resetParamHistory(),e({undoStack:[],redoStack:[]}));const l=s.projectSettings.name;let c=l;if((l===i||l==="Untitled"||l==="Custom Preset")&&(c=r),e({formula:r,projectSettings:{...s.projectSettings,name:c}}),Z.emit(we.CONFIG,{formula:r,pipeline:s.pipeline,graph:s.graph}),r!=="Modular"&&!n.skipDefaultPreset){const d=Me.get(r),f=d&&d.defaultPreset?JSON.parse(JSON.stringify(d.defaultPreset)):{formula:r};f.features||(f.features={});const p=o();if(ne.getEngineFeatures().forEach(u=>{const g=p[u.id];if(!g)return;const S=f.features[u.id]||{},y={},b=u.engineConfig.toggleParam;g[b]!==void 0&&S[b]===void 0&&(y[b]=g[b]),Object.entries(u.params).forEach(([m,x])=>{x.onUpdate==="compile"&&g[m]!==void 0&&S[m]===void 0&&(y[m]=g[m])}),f.features[u.id]||(f.features[u.id]={}),Object.assign(f.features[u.id],y)}),o().lockSceneOnSwitch){const u=o().getPreset(),g={...u.features||{}},S=f.features||{};S.coreMath&&(g.coreMath=S.coreMath),S.geometry&&(g.geometry=S.geometry);const y={...u,formula:r,features:g};o().loadPreset(y)}else o().loadPreset(f)}o().handleInteractionEnd()},setProjectSettings:r=>e(n=>{const s={...n.projectSettings,...r};return r.name&&r.name!==n.projectSettings.name?(s.version=0,{projectSettings:s,lastSavedHash:null}):{projectSettings:s}}),prepareExport:()=>{const r=o(),n=r.getPreset({includeScene:!0}),{version:s,name:i,...l}=n,c=JSON.stringify(l);if(r.lastSavedHash===null||r.projectSettings.version===0){const d=Math.max(1,r.projectSettings.version+1);return e({projectSettings:{...r.projectSettings,version:d},lastSavedHash:c}),d}if(r.lastSavedHash!==c){const d=r.projectSettings.version+1;return e({projectSettings:{...r.projectSettings,version:d},lastSavedHash:c}),d}return r.projectSettings.version},setAnimations:r=>{const n=o().animations,s=r.map(i=>{const l=n.find(c=>c.id===i.id);if(!l)return i;if(i.period!==l.period&&i.period>0){const c=performance.now()/1e3,d=(c/l.period+l.phase-c/i.period)%1;return{...i,phase:(d+1)%1}}return i});e({animations:s})},setLiveModulations:r=>e({liveModulations:r}),setGraph:r=>{const n=jo(r.nodes,r.edges),s=o();if(Hs(s.pipeline,n))Gs(s.pipeline,n)?e({graph:r}):(e({graph:r,pipeline:n}),Z.emit(we.CONFIG,{pipeline:n}));else if(s.autoCompile){const i=s.pipelineRevision+1;e({graph:r,pipeline:n,pipelineRevision:i}),Z.emit(we.CONFIG,{pipeline:n,graph:r,pipelineRevision:i})}else e({graph:r})},setPipeline:r=>{const n=o().pipelineRevision+1,s=Po(r);e({pipeline:r,graph:s,pipelineRevision:n}),Z.emit(we.CONFIG,{pipeline:r,graph:s,pipelineRevision:n})},refreshPipeline:()=>{const r=o(),n=jo(r.graph.nodes,r.graph.edges),s=r.pipelineRevision+1;e({pipeline:n,pipelineRevision:s}),Z.emit(we.CONFIG,{pipeline:n,graph:r.graph,pipelineRevision:s})},loadPreset:r=>{o().resetParamHistory();const n=Me.get(r.formula),s=n?n.id:r.formula;e({formula:s}),Z.emit(we.CONFIG,{formula:s});let i=r.name;(!i||i==="Untitled"||i==="Custom Preset")&&(i=s),e({projectSettings:{name:i,version:0},lastSavedHash:null}),Ns(r,e,o),setTimeout(()=>{const l=o().getPreset({includeScene:!0}),{version:c,name:d,...f}=l;e({lastSavedHash:JSON.stringify(f)})},50)},getPreset:r=>{var l,c;const n=o(),s={version:n.projectSettings.version,name:n.projectSettings.name,formula:n.formula,features:{}};if((r==null?void 0:r.includeScene)!==!1){if(s.cameraPos={x:0,y:0,z:0},Je.activeCamera&&Je.virtualSpace){const d=Je.virtualSpace.getUnifiedCameraState(Je.activeCamera,n.targetDistance);s.cameraRot=d.rotation,s.sceneOffset=d.sceneOffset,s.targetDistance=d.targetDistance}else s.cameraRot=n.cameraRot,s.sceneOffset=n.sceneOffset,s.targetDistance=n.targetDistance;s.cameraMode=n.cameraMode,s.lights=[],s.renderMode=n.renderMode,s.quality={aaMode:n.aaMode,aaLevel:n.aaLevel,msaa:n.msaaSamples,accumulation:n.accumulation}}ne.getAll().forEach(d=>{const f=n[d.id];f&&(s.features||(s.features={}),s.features[d.id]=Jt(f))}),s.animations=n.animations,n.savedCameras.length>0&&(s.savedCameras=n.savedCameras.map(d=>({id:d.id,label:d.label,position:d.position,rotation:d.rotation,sceneOffset:d.sceneOffset,targetDistance:d.targetDistance,optics:d.optics}))),n.formula==="Modular"&&(s.graph=n.graph,s.pipeline=n.pipeline);try{const d=(c=(l=window.useAnimationStore)==null?void 0:l.getState)==null?void 0:c.call(l);d&&(s.sequence=d.sequence,s.duration=d.durationFrames)}catch(d){console.warn("Failed to save animation sequence:",d)}return s},getShareString:r=>{const n=o().getPreset({includeScene:!0}),s=o().advancedMode;return Os(n,s,r)},loadShareString:r=>{const n=zr(r);return n?(o().loadPreset(n),!0):!1}}))),Cr=e=>e.isUserInteracting||e.interactionMode!=="none",$a=e=>{var a;if(e.isGizmoDragging||e.interactionMode!=="none"||e.isExporting)return!0;const o=ne.getAll();for(const r of o)if((a=r.interactionConfig)!=null&&a.blockCamera&&r.interactionConfig.activeParam){const n=e[r.id];if(n&&n[r.interactionConfig.activeParam])return!0}return!1},kr=e=>{const o={formula:e.formula,pipeline:e.pipeline,pipelineRevision:e.pipelineRevision,graph:e.graph,msaaSamples:e.msaaSamples,previewMode:e.previewMode,renderMode:e.renderMode,compilerHardCap:e.compilerHardCap,shadows:!0,quality:e.quality};return ne.getAll().forEach(r=>{const n=e[r.id];n&&(o[r.id]=n)}),o},Vs=()=>{const e=N.getState();oa.connect(window.useAnimationStore,N),Je.isPaused=e.isPaused,Je.setPreviewSampleCap(e.sampleCap),Je.onBooted=()=>{const r=N.getState().sceneOffset;if(r){const n={x:r.x,y:r.y,z:r.z,xL:r.xL??0,yL:r.yL??0,zL:r.zL??0};Je.setShadowOffset(n),Je.post({type:"OFFSET_SET",offset:n})}},N.subscribe(a=>a.isPaused,a=>{Je.isPaused=a}),N.subscribe(a=>a.sampleCap,a=>{Je.setPreviewSampleCap(a)}),N.subscribe(a=>{var r;return(r=a.lighting)==null?void 0:r.renderMode},a=>{const r=a===1?"PathTracing":"Direct";N.getState().renderMode!==r&&N.setState({renderMode:r})});let o;N.subscribe(a=>{var r;return(r=a.optics)==null?void 0:r.camType},a=>{var s;if(a===void 0)return;const r=o!==void 0&&o<.5,n=a>.5&&a<1.5;if(r&&n){const i=N.getState();if(!i.activeCameraId){const l=((s=i.optics)==null?void 0:s.camFov)||60;let c=Je.lastMeasuredDistance;(!c||c>=1e3||c<=0)&&(c=i.targetDistance||3.5);const d=c*Math.tan(l*Math.PI/360),f=i.setOptics;typeof f=="function"&&f({orthoScale:d})}}o=a}),Z.on(we.BUCKET_STATUS,({isRendering:a})=>{N.getState().setIsBucketRendering(a)})};typeof window<"u"&&(window.__store=N);const Tt=()=>t.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",children:t.jsx("polyline",{points:"6 9 12 15 18 9"})}),Us=()=>t.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",children:t.jsx("polyline",{points:"18 15 12 9 6 15"})}),To=()=>t.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",children:t.jsx("polyline",{points:"15 18 9 12 15 6"})}),ra=()=>t.jsx("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"3",strokeLinecap:"round",strokeLinejoin:"round",children:t.jsx("polyline",{points:"9 18 15 12 9 6"})}),qs=()=>t.jsx("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:t.jsx("polyline",{points:"9 18 15 12 9 6"})}),Ws=()=>t.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2.5",strokeLinecap:"round",strokeLinejoin:"round",children:t.jsx("path",{d:"M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3"})}),Xs=()=>t.jsx("svg",{width:"100%",height:"100%",viewBox:"0 0 10 10",children:t.jsx("path",{d:"M 6 10 L 10 6 L 10 10 Z",fill:"currentColor",opacity:"0.5"})}),Ja=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("line",{x1:"4",y1:"9",x2:"20",y2:"9"}),t.jsx("line",{x1:"4",y1:"15",x2:"20",y2:"15"})]}),wt=()=>t.jsx("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",children:t.jsx("path",{d:"M3 6h18M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"})}),eo=()=>t.jsxs("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"3",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("line",{x1:"18",y1:"6",x2:"6",y2:"18"}),t.jsx("line",{x1:"6",y1:"6",x2:"18",y2:"18"})]}),Ys=()=>t.jsxs("svg",{width:"16",height:"16",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("line",{x1:"3",y1:"12",x2:"21",y2:"12"}),t.jsx("line",{x1:"3",y1:"6",x2:"21",y2:"6"}),t.jsx("line",{x1:"3",y1:"18",x2:"21",y2:"18"})]}),Ha=()=>t.jsxs("svg",{width:"16",height:"16",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"}),t.jsx("path",{d:"M17 21v-8H7v8"}),t.jsx("path",{d:"M7 3v5h8"})]}),Ga=()=>t.jsxs("svg",{width:"16",height:"16",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"}),t.jsx("polyline",{points:"7 10 12 15 17 10"}),t.jsx("line",{x1:"12",y1:"15",x2:"12",y2:"3"})]}),jr=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("circle",{cx:"12",cy:"12",r:"10"}),t.jsx("line",{x1:"12",cy:"16",x2:"12",y2:"12"}),t.jsx("line",{x1:"12",cy:"8",x2:"12.01",y2:"8"})]}),Pr=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("circle",{cx:"12",cy:"12",r:"10"}),t.jsx("path",{d:"M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"}),t.jsx("line",{x1:"12",y1:"17",x2:"12.01",y2:"17"})]}),Zs=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",children:[t.jsx("path",{d:"M3 10h10a5 5 0 0 1 5 5v2"}),t.jsx("path",{d:"M7 6l-4 4 4 4"})]}),Qs=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",children:[t.jsx("path",{d:"M21 10h-10a5 5 0 0 0 -5 5v2"}),t.jsx("path",{d:"M17 6l4 4 -4 4"})]}),Ks=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",children:[t.jsx("path",{d:"M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"}),t.jsx("path",{d:"M3 3v5h5"})]}),ft=()=>t.jsx("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"3",strokeLinecap:"round",strokeLinejoin:"round",children:t.jsx("polyline",{points:"20 6 9 17 4 12"})}),Tr=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2.5",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"}),t.jsx("polyline",{points:"17 8 12 3 7 8"}),t.jsx("line",{x1:"12",y1:"3",x2:"12",y2:"15"})]}),Rr=()=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"}),t.jsx("polyline",{points:"7 10 12 15 17 10"}),t.jsx("line",{x1:"12",y1:"15",x2:"12",y2:"3"})]}),ha=()=>t.jsxs("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"4",children:[t.jsx("line",{x1:"12",y1:"5",x2:"12",y2:"19"}),t.jsx("line",{x1:"5",y1:"12",x2:"19",y2:"12"})]}),Js=()=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("rect",{x:"9",y:"9",width:"13",height:"13",rx:"2",ry:"2"}),t.jsx("path",{d:"M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"})]}),Rt=()=>t.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",children:t.jsx("path",{d:"M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"})}),el=()=>t.jsxs("svg",{width:"64",height:"64",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"1.5",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("rect",{x:"5",y:"2",width:"14",height:"20",rx:"2",ry:"2"}),t.jsx("path",{d:"M12 18h.01"})]}),Ir=({className:e})=>t.jsxs("svg",{className:e,xmlns:"http://www.w3.org/2000/svg",fill:"none",viewBox:"0 0 24 24",children:[t.jsx("circle",{className:"opacity-25",cx:"12",cy:"12",r:"10",stroke:"currentColor",strokeWidth:"4"}),t.jsx("path",{className:"opacity-75",fill:"currentColor",d:"M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"})]}),ia=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("polyline",{points:"16 18 22 12 16 6"}),t.jsx("polyline",{points:"8 6 2 12 8 18"})]}),tl=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("circle",{cx:"12",cy:"12",r:"10"}),t.jsx("path",{d:"M8 14s1.5 2 4 2 4-2 4-2"}),t.jsx("line",{x1:"9",y1:"9",x2:"9.01",y2:"9"}),t.jsx("line",{x1:"15",y1:"9",x2:"15.01",y2:"9"})]}),ma=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"}),t.jsx("polyline",{points:"3.27 6.96 12 12.01 20.73 6.96"}),t.jsx("line",{x1:"12",y1:"22.08",x2:"12",y2:"12"})]}),Pu=()=>t.jsxs("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2.5",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("rect",{x:"3",y:"3",width:"18",height:"18",rx:"2",strokeDasharray:"4 4"}),t.jsx("path",{d:"M9 12l2 2 4-4"})]}),al=()=>t.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:t.jsx("rect",{x:"3",y:"3",width:"18",height:"18",rx:"2",ry:"2"})}),ol=()=>t.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:t.jsx("circle",{cx:"12",cy:"12",r:"10"})}),Ro=()=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"}),t.jsx("circle",{cx:"12",cy:"13",r:"4"})]}),rl=()=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M12 3v18"}),t.jsx("path",{d:"M3 12h18"}),t.jsx("circle",{cx:"12",cy:"12",r:"3"})]}),il=()=>t.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:t.jsx("path",{d:"M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"})}),to=()=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("polygon",{points:"12 2 2 7 12 12 22 7 12 2"}),t.jsx("polyline",{points:"2 17 12 22 22 17"}),t.jsx("polyline",{points:"2 12 12 17 22 12"})]}),nl=()=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M12 2l-5 9h10l-5 9"}),t.jsx("path",{d:"M12 2v20"})]}),Fr=()=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M6 2v14a2 2 0 0 0 2 2h14"}),t.jsx("path",{d:"M18 22V8a2 2 0 0 0-2-2H2"})]}),sl=()=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("rect",{x:"3",y:"3",width:"18",height:"18",rx:"2",ry:"2"}),t.jsx("line",{x1:"9",y1:"9",x2:"15",y2:"15"}),t.jsx("line",{x1:"15",y1:"9",x2:"9",y2:"15"})]}),ll=()=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("rect",{x:"3",y:"3",width:"7",height:"7"}),t.jsx("rect",{x:"14",y:"3",width:"7",height:"7"}),t.jsx("rect",{x:"14",y:"14",width:"7",height:"7"}),t.jsx("path",{d:"M3 14h7v7H3z",fill:"currentColor",stroke:"none"})]}),cl=()=>t.jsx("svg",{width:"16",height:"16",viewBox:"0 0 24 24",fill:"currentColor",children:t.jsx("path",{d:"M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V5h14v14zM7 10h2v7H7zm4-3h2v10h-2zm4 6h2v4h-2z"})}),_r=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2.5",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("rect",{x:"3",y:"3",width:"7",height:"7"}),t.jsx("rect",{x:"14",y:"3",width:"7",height:"7"}),t.jsx("rect",{x:"14",y:"14",width:"7",height:"7"}),t.jsx("path",{d:"M10 7h4"}),t.jsx("path",{d:"M17 10v4"})]}),dl=()=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2.5",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M12 22V8"}),t.jsx("path",{d:"M5 12H2a10 10 0 0 0 20 0h-3"}),t.jsx("circle",{cx:"12",cy:"5",r:"3"})]}),ul=()=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2.5",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M12 18V8"}),t.jsx("path",{d:"M5 12H2a10 10 0 0 0 20 0h-3"}),t.jsx("circle",{cx:"12",cy:"5",r:"3"}),t.jsx("line",{x1:"3",y1:"21",x2:"21",y2:"3",stroke:"currentColor",opacity:"0.9"})]}),fl=({status:e})=>{let o="currentColor";e==="keyed"||e==="partial"?o="#f59e0b":(e==="dirty"||e==="keyed-dirty")&&(o="#ef4444");const a=e==="keyed"||e==="keyed-dirty"?o:"none";return t.jsx("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:o,strokeWidth:"2.5",children:t.jsx("path",{d:"M12 2L2 12l10 10 10-10L12 2z",fill:a})})},Va=({active:e})=>t.jsxs("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:e?"#22d3ee":"#666",strokeWidth:"3",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"}),t.jsx("path",{d:"M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"})]}),Tu=()=>t.jsxs("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",children:[t.jsx("path",{d:"M10 13l-4 4"}),t.jsx("path",{d:"M14 11l4 -4"})]}),Ru=({active:e})=>t.jsxs("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:e?"#f59e0b":"#666",strokeWidth:"3",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("polyline",{points:"16 18 22 12 16 6"}),t.jsx("polyline",{points:"8 6 2 12 8 18"})]}),Iu=({open:e})=>t.jsx("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",className:`transition-transform ${e?"rotate-90":""}`,children:t.jsx("path",{d:"M9 18l6-6-6-6"})}),pl=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("rect",{x:"2",y:"2",width:"20",height:"20",rx:"5",ry:"5"}),t.jsx("path",{d:"M16 8h.01"}),t.jsx("path",{d:"M8 8h.01"}),t.jsx("path",{d:"M8 16h.01"}),t.jsx("path",{d:"M16 16h.01"}),t.jsx("path",{d:"M12 12h.01"})]}),hl=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("polyline",{points:"16 3 21 3 21 8"}),t.jsx("line",{x1:"4",y1:"20",x2:"21",y2:"3"}),t.jsx("polyline",{points:"21 16 21 21 16 21"}),t.jsx("line",{x1:"15",y1:"15",x2:"21",y2:"21"}),t.jsx("line",{x1:"4",y1:"4",x2:"9",y2:"9"})]}),ml=()=>t.jsx("svg",{width:"12",height:"12",fill:"currentColor",viewBox:"0 0 24 24",children:t.jsx("path",{d:"M8 5v14l11-7z"})}),gl=()=>t.jsx("svg",{width:"12",height:"12",fill:"currentColor",viewBox:"0 0 24 24",children:t.jsx("path",{d:"M6 19h4V5H6v14zm8-14v14h4V5h-4z"})}),Fu=()=>t.jsx("svg",{width:"12",height:"12",fill:"currentColor",viewBox:"0 0 24 24",children:t.jsx("path",{d:"M6 6h12v12H6z"})}),_u=({active:e})=>t.jsx("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:e?"currentColor":"none",stroke:"currentColor",strokeWidth:"2",children:t.jsx("circle",{cx:"12",cy:"12",r:"10",stroke:e?"none":"currentColor",fill:e?"#ef4444":"none"})}),Du=({active:e})=>t.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:e?"#22d3ee":"currentColor",strokeWidth:"2",children:t.jsx("path",{d:"M3 18C3 18 6 5 12 12C18 19 21 5 21 5"})}),Lu=({active:e})=>t.jsxs("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:e?"#22d3ee":"currentColor",strokeWidth:"2",children:[t.jsx("rect",{x:"3",y:"8",width:"6",height:"8"}),t.jsx("rect",{x:"15",y:"8",width:"6",height:"8"})]}),Eu=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"}),t.jsx("path",{d:"M4 22v-7"}),t.jsx("path",{d:"M8 4v10"}),t.jsx("path",{d:"M12 5v10"}),t.jsx("path",{d:"M16 4v10"}),t.jsx("path",{d:"M4 8h16"}),t.jsx("path",{d:"M4 12h16"})]}),Au=({active:e})=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("rect",{x:"3",y:"3",width:"18",height:"18",rx:"2",ry:"2"}),t.jsx("line",{x1:"3",y1:"9",x2:"21",y2:"9"}),t.jsx("line",{x1:"3",y1:"15",x2:"21",y2:"15"}),t.jsx("line",{x1:"9",y1:"3",x2:"9",y2:"21"}),t.jsx("line",{x1:"15",y1:"3",x2:"15",y2:"21"})]}),Nu=({mode:e})=>t.jsx("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:e==="Loop"||e==="PingPong"?t.jsx("path",{d:"M17 2l4 4-4 4 M3 11v-1a4 4 0 0 1 4-4h14 M7 22l-4-4 4-4 M21 13v1a4 4 0 0 1-4 4H3"}):t.jsx("path",{d:"M5 12h14 M12 5l7 7-7 7"})}),Bu=({active:e,arming:o})=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M3 12h18",strokeOpacity:e||o?.3:.2}),t.jsx("path",{d:"M3 12 Q 6 2, 9 12 T 15 12 T 21 12",stroke:e?"#ef4444":o?"#fca5a5":"currentColor"}),o&&!e&&t.jsx("circle",{cx:"12",cy:"12",r:"3",fill:"#fca5a5",stroke:"none"})]}),Ou=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("polyline",{points:"4 14 10 14 10 20"}),t.jsx("polyline",{points:"20 10 14 10 14 4"}),t.jsx("line",{x1:"14",y1:"10",x2:"21",y2:"3"}),t.jsx("line",{x1:"3",y1:"21",x2:"10",y2:"14"})]}),$u=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3"}),t.jsx("circle",{cx:"12",cy:"12",r:"3",fill:"currentColor",stroke:"none"})]}),Hu=({active:e})=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:e?"#22d3ee":"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("line",{x1:"4",y1:"20",x2:"20",y2:"20"}),t.jsx("line",{x1:"4",y1:"4",x2:"20",y2:"4"}),t.jsx("polyline",{points:"4 14 8 10 12 14 16 10 20 14"})]}),Gu=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"}),t.jsx("path",{d:"M4 12h16"}),t.jsx("path",{d:"M12 4v16"}),t.jsx("path",{d:"M16 16l-4 4-4-4"})]}),Vu=({active:e})=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:e?"#22d3ee":"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M19 13l-7-7-7 7"}),t.jsx("path",{d:"M5 19l7-7 7 7"}),t.jsx("path",{d:"M12 5l2-2 2 2-2 2-2-2z",fill:e?"#22d3ee":"none",stroke:"none"}),t.jsx("path",{d:"M12 5l-2-2-2 2 2 2 2-2z",fill:e?"#22d3ee":"none",stroke:"none"})]}),Uu=({active:e})=>t.jsx("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:e?"#22d3ee":"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:t.jsx("path",{d:"M2 12s3-7 7-7 7 7 7 7 3-7 7-7"})}),qu=({active:e})=>t.jsxs("svg",{width:"10",height:"10",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2.5",strokeLinecap:"round",strokeLinejoin:"round",className:e?"text-gray-200":"text-gray-600",children:[t.jsx("path",{d:"M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"}),t.jsx("circle",{cx:"12",cy:"12",r:"3"})]}),et={frameCount:0,lastTime:performance.now(),ref:null,workerFrameCount:0};Ki(()=>{et.workerFrameCount++});const xl=()=>{const e=performance.now();et.frameCount++,e-et.lastTime>=1e3&&(et.ref&&(et.ref.innerText=`${et.workerFrameCount} FPS`),et.frameCount=0,et.workerFrameCount=0,et.lastTime=e)},yl=()=>{const e=N(i=>i.isPaused),o=N(Cr),a=fe(i=>i.isCameraInteracting),r=fe(i=>i.isScrubbing),n=e&&!o&&!a&&!r,s=v.useRef(null);return v.useEffect(()=>(et.ref=s.current,()=>{et.ref===s.current&&(et.ref=null)}),[]),t.jsx("span",{ref:s,className:`text-[10px] font-mono w-12 text-right transition-colors duration-300 ${n?"text-gray-600":"text-cyan-500/80"}`,title:e?"Rendering Paused (Battery Saver)":"Frames Per Second",children:"-- FPS"})},Ge=e=>{var r;const o=new Set;let a=e;for(;a&&a!==document.body;)(r=a.dataset)!=null&&r.helpId&&a.dataset.helpId.split(/\s+/).forEach(s=>{s&&o.add(s)}),a=a.parentElement;return Array.from(o)},Io=xe(),Ca=(e,o,a,r)=>{if(o)return 0;const n=N.getState();n.geometry;const s=n.lighting;if(e.startsWith("camera.unified")){const l=We()||Io.activeCamera,c=l?l.position:{x:0,y:0,z:0},d=He.getUnifiedPosition(c,n.sceneOffset);if(e==="camera.unified.x")return d.x;if(e==="camera.unified.y")return d.y;if(e==="camera.unified.z")return d.z}if(e.startsWith("camera.rotation")){const l=We()||Io.activeCamera;if(l){const c=new Fe().setFromQuaternion(l.quaternion);if(e==="camera.rotation.x")return c.x;if(e==="camera.rotation.y")return c.y;if(e==="camera.rotation.z")return c.z}return 0}if(e.startsWith("lights.")||e.startsWith("lighting.")){const l=e.match(/lighting\.light(\d+)_(\w+)/);if(l){const c=parseInt(l[1]),d=l[2],f=bt(s,c);if(f){if(d==="intensity")return f.intensity;if(d==="falloff")return f.falloff;if(d==="posX")return f.position.x;if(d==="posY")return f.position.y;if(d==="posZ")return f.position.z}}}if(e.includes(".")){const l=e.split("."),c=l[0],d=l[1];if(ne.get(c)){const f=n[c];if(f){if(f[d]!==void 0){const h=f[d];if(typeof h=="boolean")return h?1:0;if(typeof h=="number")return h}const p=d.match(/^(.+)_(x|y|z)$/);if(p){const h=p[1],u=p[2],g=f[h];if(g&&typeof g=="object"&&u in g)return g[u]}}}}const i=n.coreMath;if(i){if(e==="paramA")return i.paramA;if(e==="paramB")return i.paramB;if(e==="paramC")return i.paramC;if(e==="paramD")return i.paramD;if(e==="paramE")return i.paramE;if(e==="paramF")return i.paramF;if(e==="iterations")return i.iterations}return 0},ot=(e,o,a)=>{if(e.length===0)return 0;if(o<=e[0].frame)return e[0].value;if(o>=e[e.length-1].frame)return e[e.length-1].value;for(let r=0;r<e.length-1;r++){const n=e[r],s=e[r+1];if(o>=n.frame&&o<s.frame)return Qe.interpolate(o,n,s,a)}return e[0].value},Wu=(e,o,a)=>{if(!e)return{angle:0,length:0};const r=a?-e.x:e.x,n=a?-e.y:e.y,s=Math.atan2(n,r)*(180/Math.PI);let i=0;return o&&Math.abs(o)>1e-9?i=Math.abs(e.x)/Math.abs(o)*100:i=Math.abs(e.x)*10,{angle:s,length:i}},Xu=(e,o,a,r)=>{const s=Math.max(-89.9,Math.min(89.9,o))*(Math.PI/180),i=Math.abs(r)<1e-4?10:Math.abs(r),l=(e?-1:1)*(a/100)*i,c=Math.abs(l)*Math.tan(s)*(e?-1:1);return{x:l,y:c}},Yu=Qe.constrainHandles,Zu=Qe.calculateSoftFalloff,Qu=Qe.scaleHandles,Ku=(e,o)=>{const a=[],r=Math.PI*2;return e.forEach(n=>{const s=o.tracks[n],i=/rotation|rot|phase|twist/i.test(n)||/param[C-F]/i.test(n);if(s&&s.keyframes.length>1&&i){const l=[...s.keyframes].sort((f,p)=>f.frame-p.frame),d=[...l.map(f=>f.value)];for(let f=1;f<d.length;f++){let p=d[f]-d[f-1];p-=Math.round(p/r)*r,d[f]=d[f-1]+p}l.forEach((f,p)=>{Math.abs(f.value-d[p])>1e-4&&a.push({trackId:n,keyId:f.id,patch:{value:d[p]}})})}}),a},Ju=(e,o,a,r=1,n,s=.5,i=.6)=>{const l=[],c=a.length>0,d=new Set(a),f=r<0,p=Math.abs(r);if(p===0)return n&&e.forEach(y=>{const b=n.tracks[y],m=o.tracks[y];!b||!m||b.keyframes.forEach(x=>{const C=m.keyframes.find(z=>z.id===x.id);C&&(!c||d.has(`${y}::${x.id}`))&&Math.abs(C.value-x.value)>1e-9&&l.push({trackId:y,keyId:x.id,patch:{value:x.value}})})}),l;const h=n||o,u=Math.max(.1,p/2.5),g=2*u*u,S=Math.ceil(p);return e.forEach(y=>{const b=h.tracks[y];if(!b||b.keyframes.length<2)return;const m=/rotation|rot|phase|twist/i.test(y)||/param[C-F]/i.test(y),x=[...b.keyframes].sort((z,w)=>z.frame-w.frame);if(f){const z=Math.min(p,5)/5,w=s*(1-z*.9),M=i*(1-z*.8);let P=x[0].value,k=0;const T=x[0].frame,_=x[x.length-1].frame;let F=ot(x,T+1,m)-P;m&&(F>Math.PI?F-=Math.PI*2:F<-Math.PI&&(F+=Math.PI*2)),k=F;for(let O=T;O<=_;O++){let $=ot(x,O,m);if(m){const X=$-P;X>Math.PI?$-=Math.PI*2:X<-Math.PI&&($+=Math.PI*2)}const R=($-P)*w,Y=-k*M,L=R+Y;k+=L,P+=k;const G=x.find(X=>Math.abs(X.frame-O)<.1);if(G)if(!c||d.has(`${y}::${G.id}`))l.push({trackId:y,keyId:G.id,patch:{value:P}});else{P=G.value;let D=ot(x,O+1,m)-P;m&&(D>Math.PI?D-=Math.PI*2:D<-Math.PI&&(D+=Math.PI*2)),k=D}}return}let C=[];if(c){let z=[];x.forEach((w,M)=>{d.has(`${y}::${w.id}`)?z.push(M):z.length>0&&(C.push(z),z=[])}),z.length>0&&C.push(z)}else C.push(x.map((z,w)=>w));C.forEach(z=>{const w=M=>M<0?x[0].value:M>=x.length?x[x.length-1].value:x[M].value;for(let M=0;M<z.length;M++){const P=z[M],k=x[P];let T=0,_=0;for(let E=-S;E<=S;E++){const F=P+E,O=Math.exp(-(E*E)/g);let $=w(F);_+=$*O,T+=O}if(T>0){let E=_/T;const F=o.tracks[y],O=F?F.keyframes.find($=>$.id===k.id):null;O&&Math.abs(E-O.value)>1e-9&&l.push({trackId:y,keyId:k.id,patch:{value:E}})}}})}),l},ef=(e,o,a=1)=>{const r=[];return e.forEach(n=>{const s=o.tracks[n];if(!s||s.keyframes.length===0)return;const i=[...s.keyframes].sort((p,h)=>p.frame-h.frame),l=Math.ceil(i[0].frame),c=Math.floor(i[i.length-1].frame),d=n.startsWith("camera.rotation"),f=[];for(let p=l;p<=c;p+=a){const h=ot(i,p,d);f.push({id:Ze(),frame:p,value:h,interpolation:"Linear",autoTangent:!1,brokenTangents:!1})}f.length>0&&r.push({trackId:n,newKeys:f})}),r},bl=(e,o,a)=>{const{sequence:r,currentFrame:n,addTrack:s,addKeyframe:i,removeKeyframe:l,isRecording:c,snapshot:d}=fe(),f=(()=>{if(!e||!r.tracks[e])return"none";const g=r.tracks[e],S=g.keyframes.find(y=>Math.abs(y.frame-n)<.1);if(S)return Math.abs(S.value-o)>1e-4?"keyed-dirty":"keyed";{const y=/rotation|rot|phase|twist/i.test(e)||/param[C-F]/i.test(e),b=ot(g.keyframes,n,y);return Math.abs(b-o)>.001?"dirty":"partial"}})();return{status:f,toggleKey:()=>{if(e){if(d(),f==="keyed"){const g=r.tracks[e].keyframes.find(S=>Math.abs(S.frame-n)<.1);g&&l(e,g.id)}else r.tracks[e]||s(e,a),i(e,n,o);Z.emit(we.TRACK_FOCUS,e)}},autoKeyOnChange:g=>{e&&c&&(r.tracks[e]||s(e,a),i(e,n,g))},autoKeyOnDragStart:()=>{e&&c&&(d(),Z.emit(we.TRACK_FOCUS,e))}}},Ht=({status:e,onClick:o,className:a=""})=>t.jsx("button",{onClick:r=>{r.stopPropagation(),o()},tabIndex:-1,className:`p-0.5 rounded hover:bg-white/10 transition-colors shrink-0 ${e==="keyed"?"text-amber-400":e==="keyed-dirty"||e==="dirty"?"text-red-500":e==="partial"?"text-orange-500 hover:text-amber-300":"text-gray-600 hover:text-amber-200"} ${a}`,title:e==="none"?"Add Keyframe":e==="dirty"?"Add Key (Value mismatch)":e==="keyed-dirty"?"Update Key (Value changed)":e==="partial"?"Add Key (Track exists)":"Remove Key",children:t.jsx(fl,{status:e})}),vl=e=>{const{value:o,onChange:a,onDragStart:r,onDragEnd:n,step:s=.01,sensitivity:i=1,hardMin:l,hardMax:c,mapping:d,disabled:f,dragThreshold:p=2}=e,[h,u]=v.useState(!1),g=v.useRef(0),S=v.useRef(0),y=v.useRef(!1),b=v.useRef(!1),m=v.useRef(!1),x=v.useRef(null),C=v.useCallback((k,T)=>{let _=s*.5*i;return k&&(_*=10),T&&(_*=.1),_},[s,i]),z=v.useCallback(k=>{if(f||k.button!==0)return;k.preventDefault(),k.stopPropagation(),k.currentTarget.setPointerCapture(k.pointerId),x.current=k.pointerId,g.current=k.clientX;const T=d?d.toDisplay(o):o;S.current=isNaN(T)?0:T,y.current=!1,b.current=k.shiftKey,m.current=k.altKey,u(!0),r==null||r()},[o,d,f,r]),w=v.useCallback(k=>{if(f||!h||!k.currentTarget.hasPointerCapture(k.pointerId))return;const T=k.clientX-g.current;if(Math.abs(T)>p&&(y.current=!0),!y.current)return;k.preventDefault(),k.stopPropagation();const _=b.current!==k.shiftKey,E=m.current!==k.altKey;if(_||E){const R=C(b.current,m.current),Y=S.current+T*R;S.current=Y,g.current=k.clientX,b.current=k.shiftKey,m.current=k.altKey}const F=C(k.shiftKey,k.altKey);let O=S.current+T*F;l!==void 0&&(O=Math.max(l,O)),c!==void 0&&(O=Math.min(c,O));const $=d?d.fromDisplay(O):O;isNaN($)||a($)},[h,f,s,l,c,d,a,C,p]),M=v.useCallback(k=>{f||(k.currentTarget.releasePointerCapture(k.pointerId),x.current=null,u(!1),n==null||n())},[f,n]),P=v.useCallback(()=>{const k=!y.current;return y.current=!1,k},[]);return{isDragging:h,handlePointerDown:z,handlePointerMove:w,handlePointerUp:M,handleClick:P}},wl=e=>{const{value:o,mapping:a,onChange:r,onDragStart:n,onDragEnd:s,disabled:i,mapTextInput:l=!1}=e,[c,d]=v.useState(!1),[f,p]=v.useState(""),h=v.useRef(null),u=v.useCallback(()=>{if(i)return;d(!0);const C=l&&a?a.toDisplay(o):o,z=typeof C=="number"?parseFloat(C.toFixed(6)):C??0;p(String(z)),setTimeout(()=>{h.current&&(h.current.focus(),h.current.select())},10)},[o,a,i,l]),g=v.useCallback(()=>{let C;if(a!=null&&a.parseInput&&l?C=a.parseInput(f):(C=parseFloat(f),isNaN(C)&&(C=null)),C!==null){const z=l&&a?a.fromDisplay(C):C;n==null||n(),r(z),s==null||s()}d(!1)},[f,a,r,n,s,l]),S=v.useCallback(()=>{d(!1)},[]),y=v.useCallback(C=>{p(C)},[]),b=v.useCallback(C=>{C.key==="Enter"?(C.preventDefault(),g()):C.key==="Escape"&&(C.preventDefault(),S()),C.key!=="Tab"&&C.stopPropagation()},[g,S]),m=v.useCallback(()=>{c||u()},[c,u]),x=v.useCallback(()=>{c&&g()},[c,g]);return{isEditing:c,inputValue:f,inputRef:h,startEditing:u,commitEdit:g,cancelEdit:S,handleInputChange:y,handleKeyDown:b,handleFocus:m,handleBlur:x}},Dr=e=>e===0||Math.abs(e)<1e-9?"0":parseFloat(e.toFixed(8)).toString(),Fo={toDisplay:e=>e/Math.PI,fromDisplay:e=>e*Math.PI,format:e=>{const o=e/Math.PI,a=Math.abs(o),r=o<0?"-":"";if(a<.001)return"0";if(Math.abs(a-1)<.001)return`${r}π`;if(Math.abs(a-.5)<.001)return`${r}π/2`;if(Math.abs(a-.25)<.001)return`${r}π/4`;if(Math.abs(a-.75)<.001)return`${r}3π/4`;if(Math.abs(a-2)<.001)return`${r}2π`;const n=Math.round(a*3);if(Math.abs(a-n/3)<.001&&n!==0){if(n===1)return`${r}π/3`;if(n===2)return`${r}2π/3`;if(n===3)return`${r}π`;if(n===4)return`${r}4π/3`;if(n===5)return`${r}5π/3`}return`${r}${a.toFixed(2)}π`},parseInput:e=>{const o=e.trim().toLowerCase().replace(/\s/g,"");if(o==="π"||o==="pi")return Math.PI;if(o==="-π"||o==="-pi")return-Math.PI;if(o.includes("π")||o.includes("pi")){const r=o.replace(/[πpi]/g,"");if(r.includes("/")){const[i,l]=r.split("/").map(d=>parseFloat(d)||1);return(o.startsWith("-")?-1:1)*(Math.abs(i)/l)*Math.PI}const n=r?parseFloat(r):1;return isNaN(n)?null:(o.startsWith("-")?-1:1)*Math.abs(n)*Math.PI}const a=parseFloat(o);return isNaN(a)?null:a}},Dt={toDisplay:e=>e*(180/Math.PI),fromDisplay:e=>e*(Math.PI/180),format:e=>`${(e*(180/Math.PI)).toFixed(1)}°`,parseInput:e=>{const o=e.trim().replace(/°/g,""),a=parseFloat(o);return isNaN(a)?null:a}},ka=({value:e,onChange:o,onDragStart:a,onDragEnd:r,step:n=.01,sensitivity:s=1,min:i,max:l,hardMin:c,hardMax:d,mapping:f,format:p,mapTextInput:h,disabled:u=!1,highlight:g=!1,liveValue:S,defaultValue:y})=>{const{isDragging:b,handlePointerDown:m,handlePointerMove:x,handlePointerUp:C,handleClick:z}=vl({value:e,onChange:o,onDragStart:a,onDragEnd:r,step:n,sensitivity:s,hardMin:c,hardMax:d,mapping:f,disabled:u}),{isEditing:w,inputValue:M,inputRef:P,startEditing:k,handleInputChange:T,handleKeyDown:_,handleBlur:E}=wl({value:e,mapping:f,onChange:o,onDragStart:a,onDragEnd:r,disabled:u,mapTextInput:h}),F=ke.useMemo(()=>p?p(e):f!=null&&f.format?f.format(e):Dr(e),[e,p,f]),O=ke.useCallback(()=>{!u&&!w&&k()},[u,w,k]),$=ke.useCallback(L=>{if(u)return;z()&&k()},[u,z,k]),Y=`
        ...
        ${b?"bg-cyan-500/20 text-cyan-300":(b||g||S!==void 0&&!u)&&!u?"text-cyan-400":u?"":"text-gray-300 hover:text-white"}
    `;return w?t.jsx("input",{ref:P,type:"text",value:M,onChange:L=>T(L.target.value),onBlur:E,onKeyDown:_,className:"w-full h-full bg-gray-900 text-white text-xs border-none outline-none font-mono text-center px-1",onClick:L=>L.stopPropagation(),autoFocus:!0}):t.jsx("div",{tabIndex:u?-1:0,onPointerDown:m,onPointerMove:x,onPointerUp:C,onClick:$,onFocus:O,className:Y,title:u?"Disabled":"Click to edit, Drag to adjust (Shift=Fast, Alt=Slow)",children:F})},ao=({value:e,onChange:o,onDragStart:a,onDragEnd:r,step:n=.01,min:s,max:i,hardMin:l,hardMax:c,mapping:d,format:f,overrideText:p,mapTextInput:h,label:u,labelSuffix:g,headerRight:S,showTrack:y=!0,trackPosition:b="below",trackHeight:m=20,variant:x="full",className:C="",defaultValue:z,onReset:w,liveValue:M,showLiveIndicator:P=!0,onContextMenu:k,dataHelpId:T,disabled:_=!1,highlight:E=!1})=>{const F=s!==void 0&&i!==void 0&&s!==i,O=ke.useMemo(()=>{if(!F)return 0;const A=d?d.toDisplay(e):e,j=d?d.toDisplay(s):s,B=d?d.toDisplay(i):i;return Math.max(0,Math.min(100,(A-j)/(B-j)*100))},[e,s,i,d,F]),$=ke.useMemo(()=>{if(!F||M===void 0)return 0;const A=d?d.toDisplay(M):M,j=d?d.toDisplay(s):s,B=d?d.toDisplay(i):i;return Math.max(0,Math.min(100,(A-j)/(B-j)*100))},[M,s,i,d,F]),R=ke.useMemo(()=>{if(!F||z===void 0)return null;const A=d?d.toDisplay(z):z,j=d?d.toDisplay(s):s,B=d?d.toDisplay(i):i;return(A-j)/(B-j)*100},[z,s,i,d,F]),Y=ke.useCallback(A=>{if(_)return;const j=parseFloat(A.target.value),B=d?d.fromDisplay(j):j;o(B)},[_,d,o]),L=ke.useCallback(()=>{z!==void 0&&!_&&(a==null||a(),o(z),r==null||r(),w==null||w())},[z,_,o,a,r,w]),G=E||M!==void 0,X=x==="compact";return x==="minimal"?t.jsx("div",{className:C,children:t.jsx(ka,{value:e,onChange:o,onDragStart:a,onDragEnd:r,step:n,hardMin:l,hardMax:c,mapping:d,format:p?()=>p:f,mapTextInput:h,defaultValue:z,disabled:_,highlight:G})}):X?t.jsxs("div",{className:`relative flex-1 h-9 md:h-[26px] overflow-hidden group transition-all duration-150 ${_?"opacity-70 pointer-events-none":""} ${C}`,onContextMenu:k,"data-help-id":T,children:[t.jsx("div",{className:"absolute inset-0 bg-white/[0.12]",style:_?{}:{backgroundImage:"repeating-linear-gradient(45deg, transparent, transparent 5px, rgba(255,255,255,0.03) 5px, rgba(255,255,255,0.03) 10px)"}}),y&&F&&t.jsx("div",{className:`absolute top-0 bottom-0 left-0 pointer-events-none ${_?"bg-gray-500/20":G?"bg-cyan-500/30":"bg-cyan-500/20"}`,style:{width:`${O}%`}}),P&&M!==void 0&&!_&&F&&t.jsx("div",{className:"absolute top-0 bottom-0 w-1.5 bg-purple-500 blur-[1px] transition-all duration-75 ease-out z-0",style:{left:`calc(${$}% - 0.75px)`}}),t.jsx("div",{className:"absolute inset-0",children:t.jsx(ka,{value:e,onChange:o,onDragStart:a,onDragEnd:r,step:n,hardMin:l,hardMax:c,mapping:d,format:p?()=>p:f,mapTextInput:h,defaultValue:z,disabled:_,highlight:G})}),G&&!_&&t.jsx("div",{className:"absolute inset-0 bg-cyan-500/10 pointer-events-none"})]}):t.jsxs("div",{className:`mb-px animate-slider-entry ${_?"opacity-70 pointer-events-none":""} ${C}`,"data-help-id":T,onContextMenu:k,children:[u&&t.jsxs("div",{className:"flex items-stretch bg-white/[0.12] rounded-t-sm h-9 md:h-[26px] overflow-hidden border-b border-white/5",children:[t.jsxs("div",{className:"flex-1 flex items-center gap-2 px-2 min-w-0",children:[S,t.jsxs("label",{className:`text-[10px] font-medium tracking-tight select-none flex items-center gap-2 truncate pointer-events-none ${_?"text-gray-600":"text-gray-400"}`,children:[u,g,M!==void 0&&!_&&t.jsx("span",{className:"w-1.5 h-1.5 rounded-full bg-purple-500 animate-pulse shadow-[0_0_4px_#a855f7]"})]})]}),t.jsx("div",{className:"w-1/2 relative bg-white/[0.02] border-l border-white/10 group/num-area touch-none",style:_?{}:{backgroundImage:"repeating-linear-gradient(45deg, transparent, transparent 5px, rgba(255,255,255,0.03) 5px, rgba(255,255,255,0.03) 10px)"},children:t.jsx(ka,{value:e,onChange:o,onDragStart:a,onDragEnd:r,step:n,hardMin:l,hardMax:c,mapping:d,format:p?()=>p:f,mapTextInput:h,defaultValue:z,disabled:_,highlight:G})})]}),y&&F&&t.jsxs("div",{className:"relative flex items-center touch-none overflow-hidden",style:{touchAction:"none",height:m},children:[t.jsx("input",{type:"range",min:d?d.toDisplay(s):s,max:d?d.toDisplay(i):i,step:n,value:d?d.toDisplay(e):e,onChange:Y,disabled:_,onPointerDown:A=>{_||(A.stopPropagation(),a==null||a())},onPointerUp:()=>{_||r==null||r()},className:`precision-slider w-full h-full appearance-none cursor-pointer focus:outline-none z-10 ${G&&!_?"accent-cyan-500":"accent-gray-400"}`,style:{background:"transparent",touchAction:"none"},tabIndex:-1}),t.jsxs("div",{className:"absolute inset-0 bg-white/10 pointer-events-none",children:[t.jsx("div",{className:`absolute top-0 bottom-0 left-0 ${_?"bg-gray-400/20":"bg-cyan-500/30"}`,style:{width:`${O}%`}}),P&&M!==void 0&&!_&&t.jsx("div",{className:"absolute top-0 bottom-0 w-1.5 bg-purple-500 blur-[1px] transition-all duration-75 ease-out z-0",style:{left:`calc(${$}% - 0.75px)`}})]}),R!==null&&t.jsxs(t.Fragment,{children:[t.jsx("div",{className:"absolute w-0.5 h-full bg-white/40 pointer-events-none z-0 transform -translate-x-1/2",style:{left:`${R}%`}}),t.jsx("button",{onClick:A=>{A.preventDefault(),A.stopPropagation(),L()},className:"absolute top-0 bottom-0 right-0 w-2 bg-gray-500/20 hover:bg-gray-400/50 cursor-pointer z-20 transition-colors border-l border-black/10",title:`Reset to ${z}`,"aria-label":"Reset to default",tabIndex:-1})]})]})]})},Sl=[{label:"X",color:"bg-red-500",text:"text-red-400",border:"group-focus-within:border-red-500/50",hoverBg:"hover:bg-red-500/20",accent:"#ef4444"},{label:"Y",color:"bg-green-500",text:"text-green-400",border:"group-focus-within:border-green-500/50",hoverBg:"hover:bg-green-500/20",accent:"#22c55e"},{label:"Z",color:"bg-blue-500",text:"text-blue-400",border:"group-focus-within:border-blue-500/50",hoverBg:"hover:bg-blue-500/20",accent:"#3b82f6"}],Lr=({value:e,onChange:o,step:a,min:r,max:n,hardMin:s,hardMax:i,highlight:l,overrideText:c,onDragStart:d,onDragEnd:f,sensitivity:p=1,disabled:h=!1})=>t.jsx(ao,{value:e,onChange:o,onDragStart:d,onDragEnd:f,step:a,min:r,max:n,hardMin:s,hardMax:i,variant:"minimal",disabled:h,highlight:l,overrideText:c,showTrack:!1}),ct=e=>{const{handleInteractionStart:o,handleInteractionEnd:a}=N();return t.jsx(Lr,{...e,onDragStart:()=>{o("param"),e.onDragStart&&e.onDragStart()},onDragEnd:()=>{a(),e.onDragEnd&&e.onDragEnd()}})},me=({trackId:e,onKeyToggle:o,defaultValue:a,overrideInputText:r,dataHelpId:n,onChange:s,...i})=>{var w,M,P;const{openContextMenu:l,handleInteractionStart:c,handleInteractionEnd:d}=N(),{status:f,toggleKey:p,autoKeyOnChange:h,autoKeyOnDragStart:u}=bl(e,i.value??0,i.label),g=[];e&&g.push(e),n&&g.push(n),g.push("ui.slider");const S=g.join(" "),y=k=>{if(i.disabled)return;k.preventDefault(),k.stopPropagation();const T=[];a!==void 0&&T.push({label:"Reset to Default",action:()=>{c("param"),e&&u(),s(a),h(a),d()}});const _=Ge(k.currentTarget);l(k.clientX,k.clientY,T,_)},b=k=>{s(k),h(k)},m=()=>{c("param"),u(),i.onDragStart&&i.onDragStart()},x=()=>{d(),i.onDragEnd&&i.onDragEnd()},C=e&&!i.disabled?t.jsx(Ht,{status:f,onClick:()=>{p(),o&&o()}}):void 0;a!==void 0&&!i.disabled&&(t.Fragment,((i.customMapping?i.customMapping.toSlider(a):a)-(((w=i.customMapping)==null?void 0:w.min)??i.min??0))/((((M=i.customMapping)==null?void 0:M.max)??i.max??1)-(((P=i.customMapping)==null?void 0:P.min)??i.min??0))*100,`${a}`);const z=ke.useMemo(()=>{if(i.customMapping)return{toDisplay:i.customMapping.toSlider,fromDisplay:i.customMapping.fromSlider,format:Dr,parseInput:k=>{const T=parseFloat(k);return isNaN(T)?null:T}}},[i.customMapping]);return t.jsx(ao,{label:i.label,value:i.value,onChange:b,onDragStart:m,onDragEnd:x,step:i.step??.01,min:i.min,max:i.max,hardMin:i.hardMin,hardMax:i.hardMax,mapping:z,format:r?()=>r:void 0,mapTextInput:i.mapTextInput,variant:"full",showTrack:!0,trackPosition:"below",disabled:i.disabled,highlight:i.highlight||f!=="none",liveValue:i.liveValue,showLiveIndicator:!0,headerRight:C,labelSuffix:i.labelSuffix,onContextMenu:y,dataHelpId:S,className:i.className,defaultValue:a,onReset:()=>{c("param"),e&&u(),s(a),h(a),d()}})},_o=e=>e.includes("red")?{on:"bg-red-500/30 text-red-300 border-red-500/40",off:"bg-white/[0.04] text-gray-600 border-white/5"}:e.includes("green")?{on:"bg-green-500/30 text-green-300 border-green-500/40",off:"bg-white/[0.04] text-gray-600 border-white/5"}:e.includes("amber")||e.includes("yellow")?{on:"bg-amber-500/30 text-amber-300 border-amber-500/40",off:"bg-white/[0.04] text-gray-600 border-white/5"}:e.includes("purple")?{on:"bg-purple-500/30 text-purple-300 border-purple-500/40",off:"bg-white/[0.04] text-gray-600 border-white/5"}:{on:"bg-cyan-500/30 text-cyan-300 border-cyan-500/40",off:"bg-white/[0.04] text-gray-600 border-white/5"};function Ae({label:e,value:o,onChange:a,options:r,helpId:n,color:s="bg-cyan-600",onLfoToggle:i,isLfoActive:l,icon:c,disabled:d=!1,variant:f="default",labelSuffix:p}){const{openContextMenu:h,handleInteractionStart:u,handleInteractionEnd:g}=N(),S=x=>{if(d)return;const C=Ge(x.currentTarget);C.length>0&&(x.preventDefault(),x.stopPropagation(),h(x.clientX,x.clientY,[],C))};if(f==="dense"&&!r&&typeof o=="boolean"){const x=_o(s);return t.jsxs("div",{className:`flex items-center justify-between px-3 py-1 border-b border-white/5 hover:bg-white/5 transition-colors ${d?"opacity-50 pointer-events-none":""}`,"data-help-id":n,onContextMenu:S,children:[t.jsxs("div",{className:"flex items-center gap-2",children:[c,t.jsx("span",{className:"text-[10px] text-gray-400 font-medium tracking-tight truncate select-none",children:e})]}),t.jsx("button",{onClick:()=>{u("param"),a(!o),g()},className:`px-2 py-0.5 text-[8px] font-bold rounded-sm transition-all border cursor-pointer ${o?x.on:x.off} ${d?"":"hover:brightness-125"}`,children:o?"ON":"OFF"})]})}const y=x=>{d||(u("param"),a(x),g())},b=()=>{d||(u("param"),a(!o),g())},m=_o(s);return r?t.jsxs("div",{className:`mb-px animate-slider-entry ${d?"opacity-50 pointer-events-none":""}`,"data-help-id":n,onContextMenu:S,children:[e&&t.jsxs("div",{className:"flex items-center bg-white/[0.12] rounded-t-sm h-9 md:h-[26px] overflow-hidden border-b border-white/5 px-2 gap-2",children:[c,t.jsx("span",{className:"text-[10px] text-gray-400 font-medium tracking-tight truncate select-none pointer-events-none",children:e})]}),t.jsx("div",{className:`flex h-9 md:h-[26px] overflow-hidden ${e?"rounded-b-sm":"rounded-sm"}`,children:r.map(x=>t.jsx("button",{onClick:()=>y(x.value),disabled:d,className:`
                            ...
                            `,title:x.tooltip||x.label,children:x.label},String(x.value)))})]}):t.jsx("div",{className:`mb-px animate-slider-entry ${d?"opacity-50 pointer-events-none":""}`,"data-help-id":n,onContextMenu:S,children:t.jsxs("div",{className:`flex items-stretch h-9 md:h-[26px] overflow-hidden rounded-sm ${e?"bg-white/[0.12]":""}`,children:[e&&t.jsxs("div",{className:"flex-1 flex items-center gap-2 px-2 min-w-0 select-none",children:[c,t.jsx("span",{className:"text-[10px] text-gray-400 font-medium tracking-tight truncate pointer-events-none",children:e}),p]}),t.jsxs("div",{className:`flex ${e?"border-l border-white/5":"flex-1"}`,children:[t.jsx("button",{onClick:b,disabled:d,className:`
                            flex items-center justify-center gap-1 px-3 text-[10px] font-bold transition-all border-0 ${o?m.on:m.off} ${d?"opacity-40":"cursor-pointer hover:brightness-125"}
                            ${e?"":"flex-1"}
                        `,children:t.jsx("span",{className:`text-[8px] ${o?"opacity-90":"opacity-50"}`,children:o?"ON":"OFF"})}),i&&t.jsx("button",{onClick:x=>{x.stopPropagation(),d||i()},disabled:d,className:`
                            ...
                            `,title:"LFO",children:t.jsx("span",{className:`text-[8px] ${l?"opacity-90":"opacity-50"}`,children:"LFO"})})]})]})})}const Ml={center:{container:"left-1/2 -translate-x-1/2",arrow:"left-1/2 -translate-x-1/2"},start:{container:"left-0",arrow:"left-4"},end:{container:"right-0",arrow:"right-4"}},vt=({children:e,align:o="center",width:a="w-52",className:r="",onClose:n,arrow:s=!0})=>{const i=v.useRef(null);v.useEffect(()=>{if(!n)return;const c=f=>{i.current&&!i.current.contains(f.target)&&n()},d=setTimeout(()=>document.addEventListener("mousedown",c),0);return()=>{clearTimeout(d),document.removeEventListener("mousedown",c)}},[n]);const l=Ml[o];return t.jsxs("div",{ref:i,className:`absolute top-full mt-3 ${l.container} ${a} bg-black border border-white/20 rounded-xl p-3 shadow-2xl z-[70] animate-fade-in ${r}`,onClick:c=>c.stopPropagation(),children:[s&&t.jsx("div",{className:`absolute -top-1.5 ${l.arrow} w-3 h-3 bg-black border-t border-l border-white/20 transform rotate-45`}),e]})},Lt=xe(),zl=()=>{const e=N(),[o,a]=v.useState(0);v.useEffect(()=>Z.on(we.BUCKET_STATUS,s=>{a(s.progress)}),[]);const r=()=>{e.handleInteractionStart("param"),e.isBucketRendering?Lt.stopBucketRender():(e.setBucketUpscale(1),Lt.startBucketRender(!1,{bucketSize:e.bucketSize,bucketUpscale:1,convergenceThreshold:e.convergenceThreshold,accumulation:e.accumulation,samplesPerBucket:e.samplesPerBucket})),e.handleInteractionEnd()},n=()=>{if(e.handleInteractionStart("param"),e.isBucketRendering)Lt.stopBucketRender();else{const s=e.getPreset({includeScene:!0}),i=e.prepareExport();Lt.startBucketRender(!0,{bucketSize:e.bucketSize,bucketUpscale:e.bucketUpscale,convergenceThreshold:e.convergenceThreshold,accumulation:e.accumulation,samplesPerBucket:e.samplesPerBucket},{preset:s,name:e.projectSettings.name,version:i})}e.handleInteractionEnd()};return t.jsx(vt,{width:"w-72",children:t.jsxs("div",{className:"relative space-y-3",children:[t.jsxs("div",{className:"flex items-center justify-between border-b border-white/10 pb-2",children:[t.jsx("span",{className:"text-[10px] font-bold text-gray-400",children:"High Quality Render"}),e.isBucketRendering&&t.jsx("button",{onClick:()=>Lt.stopBucketRender(),className:"text-[9px] font-bold px-2 py-0.5 rounded border border-red-500/50 bg-red-500/20 text-red-300 animate-pulse",children:"Stop"})]}),e.isBucketRendering?t.jsxs("div",{className:"bg-white/5 rounded p-2 mb-2",children:[t.jsxs("div",{className:"flex justify-between text-[9px] text-gray-400 mb-1",children:[t.jsx("span",{children:"Progress"}),t.jsxs("span",{children:[o.toFixed(1),"%"]})]}),t.jsx("div",{className:"w-full h-1.5 bg-black rounded-full overflow-hidden",children:t.jsx("div",{className:"h-full bg-cyan-500 transition-all duration-300 ease-linear",style:{width:`${o}%`}})})]}):t.jsxs("div",{className:"flex gap-2 mb-2",children:[t.jsxs("button",{onClick:r,className:"flex-1 py-2 rounded bg-gray-800 hover:bg-white/10 border border-white/10 text-[9px] font-bold text-gray-300 transition-all hover:border-cyan-500/50 hover:text-cyan-400 flex flex-col items-center gap-1",title:"Refine Viewport (1x)",children:[t.jsx(ft,{}),t.jsx("span",{children:"Refine View"})]}),t.jsxs("button",{onClick:n,className:"flex-1 py-2 rounded bg-cyan-900/30 hover:bg-cyan-800/50 border border-cyan-500/30 text-[9px] font-bold text-cyan-300 transition-all hover:border-cyan-400 flex flex-col items-center gap-1",title:"Render & Save Image",children:[t.jsx(Rr,{}),t.jsx("span",{children:"Export Image"})]})]}),t.jsxs("div",{className:`space-y-1 transition-opacity ${e.isBucketRendering?"opacity-50 pointer-events-none":"opacity-100"}`,children:[t.jsx(me,{label:"Convergence Threshold",value:e.convergenceThreshold,min:.01,max:1,step:.01,onChange:e.setConvergenceThreshold,customMapping:{min:0,max:100,toSlider:s=>(Math.log10(s)+2)/2*100,fromSlider:s=>Math.pow(10,s/100*2-2)},overrideInputText:`${e.convergenceThreshold.toFixed(2)}%`}),t.jsx("p",{className:"text-[8px] text-gray-500 -mt-1 px-1 mb-2",children:"Lower = more samples, higher quality. 0.1%=production, 1%=fast"}),t.jsx(me,{label:"Max Samples Per Bucket",value:e.samplesPerBucket,min:16,max:1024,step:16,onChange:e.setSamplesPerBucket,overrideInputText:`${e.samplesPerBucket} max`,highlight:e.samplesPerBucket>=256}),t.jsx("p",{className:"text-[8px] text-gray-500 -mt-1 px-1 mb-2",children:"Safety limit. Tiles stop early if converged."}),t.jsxs("div",{className:"pt-2 border-t border-white/5",children:[t.jsx(me,{label:"Export Scale",value:e.bucketUpscale,min:1,max:8,step:.5,onChange:e.setBucketUpscale,overrideInputText:`${e.bucketUpscale}x`,highlight:e.bucketUpscale>1}),t.jsx("p",{className:"text-[8px] text-gray-500 -mt-1 px-1 mb-2",children:"Resolution multiplier. 2x = 4K from 1080p, 4x = 8K, 8x = 10K+"}),t.jsx("label",{className:"text-[9px] font-bold text-gray-400 block mb-1",children:"Bucket Size"}),t.jsx(Ae,{value:e.bucketSize,onChange:e.setBucketSize,options:[{label:"64",value:64},{label:"128",value:128},{label:"256",value:256},{label:"512",value:512}]}),t.jsx("p",{className:"text-[8px] text-gray-500 mt-1 px-1",children:"Smaller = less memory, larger = faster"})]})]})]})})},Cl=xe(),kl=({isMobileMode:e,vibrate:o})=>{const a=N(),r=fe(L=>L.isPlaying),{handleInteractionStart:n,handleInteractionEnd:s,openContextMenu:i}=N(),l=N(Cr),c=fe(L=>L.isCameraInteracting),d=fe(L=>L.isScrubbing),f=a.isPaused&&!c&&!l&&!d,[p,h]=v.useState(!1),[u,g]=v.useState(!1),[S,y]=v.useState(!1),b=v.useRef(null),m=v.useRef(null),x=v.useRef(null),C=v.useRef(null),[z,w]=v.useState(a.projectSettings.name),[M,P]=v.useState(a.projectSettings.version);v.useEffect(()=>{u&&(w(a.projectSettings.name),P(a.projectSettings.version))},[u,a.projectSettings]),v.useEffect(()=>{const L=G=>{b.current&&!b.current.contains(G.target)&&h(!1),x.current&&!x.current.contains(G.target)&&g(!1)};return(p||u)&&window.addEventListener("mousedown",L),()=>window.removeEventListener("mousedown",L)},[p,u]);const k=(L,G)=>{L.preventDefault(),L.stopPropagation(),i(L.clientX,L.clientY,[],G)},T=()=>{if(o(5),a.renderRegion){a.setRenderRegion(null);return}a.interactionMode==="selecting_region"?a.setInteractionMode("none"):a.setInteractionMode("selecting_region")},_=a.lighting,E=(_==null?void 0:_.ptEnabled)!==!1,F=async()=>{E&&(o(5),n("param"),Z.emit("is_compiling","Loading Material..."),await new Promise(L=>setTimeout(L,50)),a.setRenderMode(a.renderMode==="PathTracing"?"Direct":"PathTracing"),s())},O=()=>{a.setProjectSettings({name:z,version:M}),g(!1)},$=()=>{o(5);const L=N.getState().isPaused;a.setIsPaused(!L),Cl.markInteraction()},R=()=>{C.current&&clearTimeout(C.current),y(!0)},Y=()=>{C.current=window.setTimeout(()=>{y(!1)},300)};return t.jsxs("div",{className:"flex items-center gap-6",children:[t.jsxs("div",{className:"flex flex-col leading-none relative",children:[t.jsxs("span",{className:"text-xl font-bold tracking-tighter text-white",children:["G",t.jsx("span",{className:"text-cyan-400",children:"M"}),"T"]}),t.jsx("button",{onClick:()=>g(!0),className:"text-[8px] font-mono text-gray-500 hover:text-cyan-300 transition-colors text-left truncate max-w-[120px]",title:"Click to Rename Project",children:a.projectSettings.name}),u&&t.jsx(vt,{width:"w-48",align:"start",arrow:!1,onClose:()=>g(!1),children:t.jsxs("div",{className:"space-y-3",children:[t.jsxs("div",{children:[t.jsx("label",{className:"text-[9px] text-gray-500 font-bold block mb-1",children:"Project Name"}),t.jsx("input",{type:"text",value:z,onChange:L=>w(L.target.value),className:"w-full bg-gray-900 border border-white/10 rounded px-2 py-1 text-xs text-white outline-none focus:border-cyan-500",placeholder:"Enter name...",autoFocus:!0})]}),t.jsxs("div",{className:"flex gap-2",children:[t.jsxs("div",{className:"flex-1",children:[t.jsx("label",{className:"text-[9px] text-gray-500 font-bold block mb-1",children:"Ver"}),t.jsx("div",{className:"h-6 bg-gray-900 border border-white/10 rounded overflow-hidden",children:t.jsx(ct,{value:M,onChange:L=>P(Math.max(1,Math.round(L))),step:1,min:1,max:99})})]}),t.jsx("button",{onClick:O,className:"flex-1 bg-cyan-900/40 hover:bg-cyan-800 text-cyan-300 border border-cyan-500/30 rounded flex items-center justify-center mt-3.5",title:"Save Settings",children:t.jsx(ft,{})})]})]})})]}),t.jsx("div",{className:"h-6 w-px bg-white/10"}),t.jsxs("div",{className:"flex items-center gap-2",children:[t.jsx(yl,{}),r&&t.jsxs("div",{className:"flex items-center gap-1.5 px-2 py-0.5 bg-green-900/30 border border-green-500/30 rounded text-[9px] font-bold text-green-400 animate-pulse",children:[t.jsx("svg",{width:"8",height:"8",viewBox:"0 0 24 24",fill:"currentColor",children:t.jsx("path",{d:"M8 5v14l11-7z"})}),t.jsx("span",{children:"Playing"})]}),!e&&t.jsxs("div",{className:"relative",onMouseEnter:R,onMouseLeave:Y,ref:m,children:[t.jsx("button",{onClick:$,className:`p-0.5 rounded transition-colors ${f?"text-amber-400 bg-amber-900/30 border border-amber-500/30":"text-gray-600 hover:text-gray-400"}`,title:a.isPaused?"Resume Rendering":"Pause Rendering (Battery Saver)",children:f?t.jsx(ml,{}):t.jsx(gl,{})}),S&&t.jsx(vt,{width:"w-40",children:t.jsxs("div",{className:"mb-1",children:[t.jsx(me,{label:"Auto-Stop (Samples)",value:a.sampleCap,min:0,max:4096,step:32,onChange:a.setSampleCap,overrideInputText:a.sampleCap===0?"Infinite":a.sampleCap.toFixed(0)}),t.jsx("div",{className:"text-[8px] text-gray-500 text-center mt-1",children:"0 = Never Stop"})]})})]}),t.jsx("div",{className:"w-px h-3 bg-white/10 mx-1"}),t.jsx("button",{onClick:()=>{o(5),n("param"),a.setAccumulation(!a.accumulation),s()},onContextMenu:L=>k(L,["quality.tss"]),className:`p-0.5 rounded transition-colors ${a.accumulation?"text-green-400 bg-green-900/30":"text-gray-600 hover:text-gray-400"}`,title:"TSS (Temporal Anti-Aliasing)",children:t.jsx(to,{})}),t.jsx("div",{className:"w-px h-3 bg-white/10 mx-1"}),t.jsx("button",{onClick:F,disabled:!E,className:`p-0.5 rounded transition-colors ${E?a.renderMode==="PathTracing"?"text-purple-400 bg-purple-900/30":"text-gray-600 hover:text-gray-400":"text-gray-700 cursor-not-allowed opacity-50"}`,title:E?e?"Enable Path Tracer (Experimental)":"Path Tracer (Global Illumination)":"Path Tracing disabled in Engine Panel",children:t.jsx(nl,{})}),!e&&t.jsxs(t.Fragment,{children:[t.jsx("button",{onClick:T,className:`p-0.5 rounded transition-colors ${a.interactionMode==="selecting_region"?"text-cyan-400 bg-cyan-900/30 border border-cyan-500/30":a.renderRegion?"text-green-400 bg-green-900/30 border border-green-500/30":"text-gray-600 hover:text-gray-400"}`,title:a.renderRegion?"Clear Region":a.interactionMode==="selecting_region"?"Cancel Selection":"Select Region",children:a.renderRegion?t.jsx(sl,{}):t.jsx(Fr,{})}),t.jsxs("div",{className:"relative",ref:b,children:[t.jsx("button",{onClick:L=>{L.stopPropagation(),o(5),h(!p)},className:`bucket-toggle-btn p-0.5 rounded transition-colors ${a.isBucketRendering?"text-cyan-400 bg-cyan-900/30 border border-cyan-500/30 animate-pulse":"text-gray-600 hover:text-gray-400"}`,title:"Render!",children:t.jsx(ll,{})}),p&&t.jsx(zl,{})]})]})]})]})};let na=[];const Et=e=>{const o=e.toUpperCase();na=[o,...na.filter(a=>a!==o)].slice(0,3)},ga=({color:e,onColorChange:o})=>{const[a,r]=v.useState(()=>{const b=lt(e);return b?Vt(b):{h:0,s:0,v:100}}),n=v.useRef(e.toUpperCase()),{openContextMenu:s,handleInteractionStart:i,handleInteractionEnd:l}=N();v.useEffect(()=>{if(e.toUpperCase()!==n.current){const b=lt(e);if(b){const m=Vt(b);r(m),n.current=e.toUpperCase()}}},[e]);const c=b=>{const m={...a,...b};r(m);const x=Nt(Ut(m.h,m.s,m.v));n.current=x,o(x)},d=()=>{Et(n.current)},f="linear-gradient(to right, #f00, #ff0, #0f0, #0ff, #00f, #f0f, #f00)",p=v.useMemo(()=>`linear-gradient(to right, ${Nt(Ut(a.h,0,a.v))}, ${Nt(Ut(a.h,100,a.v))})`,[a.h,a.v]),h=v.useMemo(()=>`linear-gradient(to right, #000, ${Nt(Ut(a.h,a.s,100))})`,[a.h,a.s]),u=b=>{if(b.button!==0&&b.button!==2)return;b.preventDefault(),b.stopPropagation();const m=[{label:"Actions",action:()=>{},isHeader:!0},{label:`Copy Hex (${e})`,action:()=>navigator.clipboard.writeText(e.toUpperCase())},{label:"Paste Hex",action:async()=>{try{let x=await navigator.clipboard.readText();if(x=x.trim(),x.startsWith("#")||(x="#"+x),/^#[0-9A-F]{6}$/i.test(x)||/^#[0-9A-F]{3}$/i.test(x)){if(x.length===4){const w=x[1],M=x[2],P=x[3];x=`#${w}${w}${M}${M}${P}${P}`}const C=x.toUpperCase(),z=lt(C);z&&(i("param"),r(Vt(z)),n.current=C,o(C),Et(C),l())}}catch(x){console.warn("Paste failed",x)}}},{label:"Quick Picks",action:()=>{},isHeader:!0},{label:"White (#FFFFFF)",icon:t.jsx("div",{className:"w-3 h-3 rounded-full bg-white border border-gray-600"}),action:()=>{i("param");const x="#FFFFFF";r({h:0,s:0,v:100}),n.current=x,o(x),Et(x),l()}},{label:"Black (#000000)",icon:t.jsx("div",{className:"w-3 h-3 rounded-full bg-black border border-gray-600"}),action:()=>{i("param");const x="#000000";r({h:0,s:0,v:0}),n.current=x,o(x),Et(x),l()}}];na.length>0&&(m.push({label:"History",action:()=>{},isHeader:!0}),na.forEach(x=>{m.push({label:x,icon:t.jsx("div",{className:"w-3 h-3 rounded-full border border-gray-600",style:{backgroundColor:x}}),action:()=>{i("param");const C=lt(x);C&&(r(Vt(C)),n.current=x,o(x),Et(x)),l()}})})),s(b.clientX,b.clientY,m,["ui.colorpicker"])},g=b=>{if(b.target.closest(".hsv-stack")){const m=Ge(b.currentTarget);m.length>0&&(b.preventDefault(),b.stopPropagation(),s(b.clientX,b.clientY,[],m))}},S=()=>i("param"),y=()=>l();return t.jsxs("div",{className:"flex flex-row h-[66px] bg-black/40 border border-white/5 overflow-hidden group/picker relative gradient-interactive-element",onMouseUp:d,"data-help-id":"ui.colorpicker",onContextMenu:g,children:[t.jsx("div",{className:"w-8 shrink-0 relative cursor-pointer border-r border-white/10 hover:brightness-110 active:brightness-125 transition-all bg-gray-800",style:{backgroundColor:e},onMouseDown:u,onContextMenu:u,title:"Color Actions & History (Right Click)",children:t.jsx("div",{className:"absolute inset-0 flex items-center justify-center -rotate-90 whitespace-nowrap text-[10px] font-mono font-bold mix-blend-difference text-white opacity-80 group-hover/picker:opacity-100 transition-opacity",children:e})}),t.jsxs("div",{className:"flex-1 flex flex-col gap-[1px] hsv-stack",children:[t.jsx("div",{className:"relative h-[21.3px]",style:{background:f},children:t.jsx("input",{type:"range",min:"0",max:"360",step:"0.1",value:a.h,onChange:b=>c({h:Number(b.target.value)}),onPointerDown:S,onPointerUp:y,className:"precision-slider absolute inset-0 w-full h-full cursor-crosshair"})}),t.jsx("div",{className:"relative h-[21.3px]",style:{background:p},children:t.jsx("input",{type:"range",min:"0",max:"100",step:"0.1",value:a.s,onChange:b=>c({s:Number(b.target.value)}),onPointerDown:S,onPointerUp:y,className:"precision-slider absolute inset-0 w-full h-full cursor-crosshair"})}),t.jsx("div",{className:"relative h-[21.3px]",style:{background:h},children:t.jsx("input",{type:"range",min:"0",max:"100",step:"0.1",value:a.v,onChange:b=>c({v:Number(b.target.value)}),onPointerDown:S,onPointerUp:y,className:"precision-slider absolute inset-0 w-full h-full cursor-crosshair"})})]})]})},Er=({index:e,value:o,onChange:a,isFixed:r=!1,size:n=140,width:s,height:i})=>{const l=s||n,c=i||n,d=l/2,f=c/2,p=l*.35,h=c*.35,u=v.useRef(null),[g,S]=v.useState(!1),{handleInteractionStart:y,handleInteractionEnd:b}=N(),m=()=>{const D=He.getRotationFromEngine();return new De(D.x,D.y,D.z,D.w)},{sequence:x,currentFrame:C,isPlaying:z,addTrack:w,addKeyframe:M,removeKeyframe:P,snapshot:k,isRecording:T}=fe(),_=()=>{const D=new De().setFromEuler(new Fe(o.x,o.y,o.z,"YXZ"));return new U(0,0,-1).applyQuaternion(D)},E=()=>{let H=_().clone();r||H.applyQuaternion(m().invert());const A=new U(0,0,-1),j=H.angleTo(A),B=j/(Math.PI/2),I=Math.atan2(H.y,H.x),V=-Math.cos(I)*B*p,W=Math.sin(I)*B*h;return{x:d+V,y:f+W,isBacklit:j>Math.PI/2}},F=(D,H)=>{if(!u.current)return;const A=u.current.getBoundingClientRect(),j=A.left+l/2,B=A.top+c/2,I=D-j,V=H-B,W=I/p,q=V/h,Q=Math.sqrt(W*W+q*q),K=Math.atan2(q,W),ie=Q*(Math.PI/2),ee=Math.min(ie,Math.PI-.001),oe=Math.sin(ee);let ce=new U(-oe*Math.cos(K),oe*Math.sin(K),-Math.cos(ee));r||ce.applyQuaternion(m());const pe=new De().setFromUnitVectors(new U(0,0,-1),ce),he=new Fe().setFromQuaternion(pe,"YXZ");a({x:he.x,y:he.y,z:he.z})},O=E(),$=D=>{D.preventDefault(),D.stopPropagation(),y("param"),S(!0),D.target.setPointerCapture(D.pointerId),F(D.clientX,D.clientY)},R=D=>{g&&F(D.clientX,D.clientY)},Y=D=>{if(g){if(S(!1),D.target.releasePointerCapture(D.pointerId),T){const H=`lighting.light${e}_rotX`,A=`lighting.light${e}_rotY`,j=`lighting.light${e}_rotZ`;x.tracks[H]||w(H,`Light ${e+1} Pitch`),x.tracks[A]||w(A,`Light ${e+1} Yaw`),x.tracks[j]||w(j,`Light ${e+1} Roll`),M(H,C,o.x),M(A,C,o.y),M(j,C,o.z)}b()}},L=[`lighting.light${e}_rotX`,`lighting.light${e}_rotY`],G=(()=>{let D=!1,H=!1,A=!1;return L.forEach((j,B)=>{const I=x.tracks[j];if(I){D=!0;const V=I.keyframes.find(W=>Math.abs(W.frame-C)<.1);if(V&&(H=!0),!z){const W=B===0?o.x:o.y,q=V?V.value:ot(I.keyframes,C,B===0||B===1);Math.abs(W-q)>.001&&(A=!0)}}}),D?H?A?"keyed-dirty":"keyed":A?"dirty":"partial":"none"})(),X=()=>{k(),G==="keyed"?L.forEach(D=>{const H=x.tracks[D],A=H==null?void 0:H.keyframes.find(j=>Math.abs(j.frame-C)<.1);A&&P(D,A.id)}):L.forEach((D,H)=>{x.tracks[D]||w(D,H===0?`Light ${e+1} Pitch`:`Light ${e+1} Yaw`),M(D,C,H===0?o.x:o.y)})};return t.jsxs("div",{className:"flex flex-col items-center mb-2",children:[t.jsxs("div",{className:"w-full flex justify-between items-center mb-1 px-1",children:[t.jsx("label",{className:"text-[9px] font-bold text-gray-500",children:"Heliotrope"}),t.jsx(Ht,{status:G,onClick:X})]}),t.jsxs("div",{ref:u,className:"relative cursor-crosshair touch-none rounded-[30px] border border-white/10 shadow-inner group overflow-hidden",style:{width:l,height:c,background:"radial-gradient(circle at center, #0f172a 0%, #020617 80%)"},onPointerDown:$,onPointerMove:R,onPointerUp:Y,title:r?"Headlamp Mode: Light is attached to Camera. Center = Camera Forward.":"World Mode: Light is fixed in space. Center = Direction you are looking.",children:[t.jsxs("div",{className:"absolute inset-0 flex items-center justify-center pointer-events-none opacity-20",children:[t.jsx("div",{className:"border border-cyan-500 rounded-full",style:{width:p*2,height:h*2}}),t.jsx("div",{className:"absolute w-full h-px bg-white/20"}),t.jsx("div",{className:"absolute h-full w-px bg-white/20"})]}),t.jsx("div",{className:"absolute top-1 left-1/2 -translate-x-1/2 text-[7px] text-gray-600 font-bold pointer-events-none",children:"TOP"}),t.jsx("div",{className:"absolute bottom-1 left-1/2 -translate-x-1/2 text-[7px] text-gray-600 font-bold pointer-events-none",children:"BTM"}),t.jsx("div",{className:"absolute left-1 top-1/2 -translate-y-1/2 text-[7px] text-gray-600 font-bold pointer-events-none",children:"L"}),t.jsx("div",{className:"absolute right-1 top-1/2 -translate-y-1/2 text-[7px] text-gray-600 font-bold pointer-events-none",children:"R"}),t.jsx("div",{className:`absolute inset-0 rounded-[30px] border-2 border-red-500/30 pointer-events-none transition-opacity duration-300 ${O.isBacklit?"opacity-100 animate-pulse":"opacity-0"}`}),t.jsx("div",{className:`absolute w-3 h-3 -ml-1.5 -mt-1.5 rounded-full shadow-[0_0_10px_white] pointer-events-none transition-transform duration-75 ${g?"scale-125 bg-white":"bg-yellow-400"} ${O.isBacklit?"border-2 border-red-500":""}`,style:{left:O.x,top:O.y}})]}),t.jsxs("div",{className:"flex gap-2 w-full mt-2 px-1",children:[t.jsxs("div",{className:"flex-1 bg-black/40 rounded border border-white/10 flex items-center px-2 py-1",children:[t.jsx("span",{className:"text-[8px] text-gray-500 font-bold mr-2",children:"Pitch"}),t.jsx(ct,{value:o.x*180/Math.PI,onChange:D=>a({...o,x:D*Math.PI/180}),step:1,min:-180,max:180,overrideText:(o.x*180/Math.PI).toFixed(1)+"°",onDragStart:()=>y("param"),onDragEnd:()=>b()})]}),t.jsxs("div",{className:"flex-1 bg-black/40 rounded border border-white/10 flex items-center px-2 py-1",children:[t.jsx("span",{className:"text-[8px] text-gray-500 font-bold mr-2",children:"Yaw"}),t.jsx(ct,{value:o.y*180/Math.PI,onChange:D=>a({...o,y:D*Math.PI/180}),step:1,min:-180,max:180,overrideText:(o.y*180/Math.PI).toFixed(1)+"°",onDragStart:()=>y("param"),onDragEnd:()=>b()})]})]})]})},jl={primary:"text-[10px] font-bold text-gray-400",secondary:"text-[9px] font-bold text-gray-500",tiny:"text-[8px] text-gray-600"},_e=({children:e,variant:o="primary",className:a="",color:r})=>{const n=jl[o],s=r||"";return t.jsx("span",{className:`${n} ${s} select-none ${a}`,children:e})},qe=()=>t.jsxs(t.Fragment,{children:[t.jsx("div",{className:"h-1.5 bg-neutral-800 rounded-b-lg"}),t.jsx("div",{className:"h-2",style:{background:"linear-gradient(to bottom, transparent, rgba(255,255,255,0.08))"}})]}),Pl=xe(),Tl=({index:e,color:o,active:a,type:r,rotation:n,onClick:s,onDragStart:i})=>{const c=(()=>{if(!n)return{x:50,y:50};const f=n.y;return{x:50+Math.sin(f)*35,y:50-Math.cos(f)*35}})(),d=Array.from({length:12}).map((f,p)=>p*30);return t.jsxs("div",{className:`group relative flex flex-col items-center justify-center cursor-grab active:cursor-grabbing transition-all duration-300 touch-none ${a?"opacity-100 scale-100":"opacity-50 hover:opacity-100 scale-90 hover:scale-100"}`,onPointerDown:f=>{f.button===0&&(f.stopPropagation(),i())},onClick:f=>{f.stopPropagation(),s()},children:[!a&&t.jsxs("div",{className:"absolute top-full mt-2 left-1/2 -translate-x-1/2 bg-black border border-white/20 text-[9px] font-bold text-gray-300 px-2 py-1 rounded shadow-xl opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50",children:["Drag to Screen",t.jsx("div",{className:"absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-black border-t border-l border-white/20 transform rotate-45"})]}),t.jsxs("div",{className:"w-8 h-8 relative",children:[a&&t.jsx("div",{className:"absolute inset-0 rounded-full transition-all duration-300",style:{boxShadow:`0 0 ${r==="Directional"?"12px":"20px"} ${o}`,opacity:r==="Directional"?.6:1,backgroundColor:r==="Directional"?"transparent":o}}),a&&r==="Directional"&&t.jsx("div",{className:"absolute inset-0 pointer-events-none",children:d.map(f=>t.jsx("div",{className:"absolute w-px h-[3px] rounded-full",style:{backgroundColor:o,top:"50%",left:"50%",marginTop:"-1.5px",marginLeft:"-0.5px",transform:`rotate(${f}deg) translateY(-17px)`,boxShadow:`0 0 2px ${o}`}},f))}),t.jsx("div",{className:"absolute inset-0 rounded-full border border-white overflow-hidden z-10 shadow-[inset_0_0_6px_rgba(255,255,255,0.4)] isolate",style:{backgroundColor:r==="Directional"?"#000000":o},children:a&&r==="Directional"&&t.jsx("div",{className:"absolute inset-0 w-full h-full",style:{background:`radial-gradient(circle at ${c.x}% ${c.y}%, ${o} 0%, transparent 65%)`,opacity:1}})}),a&&r!=="Directional"&&t.jsx("div",{className:"absolute inset-0 rounded-full border border-white/50 animate-ping opacity-20 pointer-events-none"})]}),t.jsxs(_e,{variant:"tiny",className:"absolute -bottom-4 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap",children:["L",e+1]})]})},Rl=({index:e,onClose:o})=>{const a=N(k=>bt(k.lighting,e)),r=N(k=>k.updateLight),n=N(k=>k.removeLight),s=N(k=>k.duplicateLight),{handleInteractionStart:i,handleInteractionEnd:l}=N(),{addTrack:c,addKeyframe:d,currentFrame:f,sequence:p,isPlaying:h}=fe(),[u,g]=v.useState(a.useTemperature??!1),[S,y]=v.useState(a.temperature??6500);if(!a.visible)return null;const b=()=>{const k=a.fixed;let T=a.position,_=a.rotation;const E=We();if(E)if(a.type==="Point"){const F=Pl.sceneOffset;if(k){const O=new U(T.x,T.y,T.z);O.applyQuaternion(E.quaternion),O.add(E.position),T={x:O.x+F.x+(F.xL??0),y:O.y+F.y+(F.yL??0),z:O.z+F.z+(F.zL??0)}}else{const O=new U(T.x-F.x-(F.xL??0),T.y-F.y-(F.yL??0),T.z-F.z-(F.zL??0));O.sub(E.position),O.applyQuaternion(E.quaternion.clone().invert()),T={x:O.x,y:O.y,z:O.z}}}else{const F=new U(0,0,-1).applyEuler(new Fe(_.x,_.y,_.z,"YXZ"));F.applyQuaternion(k?E.quaternion:E.quaternion.clone().invert());const O=new De().setFromUnitVectors(new U(0,0,-1),F),$=new Fe().setFromQuaternion(O,"YXZ");_={x:$.x,y:$.y,z:$.z}}r({index:e,params:{fixed:!k,position:T,rotation:_}})},m=()=>{["X","Y","Z"].forEach(T=>{const _=`lighting.light${e}_pos${T}`;p.tracks[_]||c(_,`Light ${e+1} Pos ${T}`),d(_,f,a.position[T.toLowerCase()])})},C=(()=>{const k=["X","Y","Z"];let T=!1,_=!1,E=!1;return k.forEach(F=>{const O=`lighting.light${e}_pos${F}`,$=p.tracks[O];if($){T=!0;const R=$.keyframes.find(Y=>Math.abs(Y.frame-f)<.1);if(R&&(_=!0),!h){const Y=a.position[F.toLowerCase()];let L=0;R?L=R.value:L=ot($.keyframes,f,!1),Math.abs(L-Y)>1e-4&&(E=!0)}}}),T?_?E?"keyed-dirty":"keyed":E?"dirty":"partial":"none"})(),z=Math.max(.01,a.intensity),w=a.falloff/z,M=`lighting.light${e}`,P=k=>{if(k===0)return"0";if(Math.abs(k)<1)return k.toFixed(3);const T=k.toPrecision(5);return T.includes(".")?T.replace(/\.?0+$/,""):T};return t.jsx(vt,{width:"w-52",onClose:o,children:t.jsxs("div",{className:"relative space-y-3",children:[t.jsxs("div",{className:"flex items-center justify-between border-b border-white/10 pb-2",children:[t.jsxs("div",{className:"flex items-center gap-2",children:[a.type==="Point"&&t.jsx(Ht,{status:C,onClick:m}),t.jsxs(_e,{children:["Light ",e+1]})]}),t.jsxs("div",{className:"flex items-center gap-1",children:[t.jsx("button",{onClick:k=>{k.stopPropagation(),i("param"),s(e),l()},className:"p-1 text-gray-400 hover:text-cyan-300 hover:bg-cyan-900/20 rounded transition-colors",title:"Duplicate Light",children:t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2.5",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("rect",{x:"9",y:"9",width:"13",height:"13",rx:"2"}),t.jsx("path",{d:"M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"})]})}),t.jsx("button",{onClick:k=>{k.stopPropagation(),i("param"),n(e),l()},className:"p-1 text-red-500 hover:text-red-300 hover:bg-red-900/20 rounded transition-colors",title:"Remove Light",children:t.jsx(wt,{})}),t.jsx("button",{onClick:()=>{i("param"),b(),l()},className:`text-[9px] font-bold px-2 py-0.5 rounded border transition-colors ${a.fixed?"bg-orange-500/20 text-orange-300 border-orange-500/50":"bg-cyan-500/20 text-cyan-300 border-cyan-500/50"}`,children:a.fixed?"ANCHORED":"FLOATING"})]})]}),t.jsxs("div",{className:"space-y-1",children:[a.type==="Directional"&&t.jsx("div",{className:"mb-2",children:t.jsx(Er,{index:e,value:a.rotation,onChange:k=>r({index:e,params:{rotation:k}}),isFixed:a.fixed,width:180,height:110})}),t.jsx(me,{label:"Intensity",value:a.intensity,min:0,max:100,step:.1,onChange:k=>r({index:e,params:{intensity:k}}),customMapping:{min:0,max:100,toSlider:k=>Math.sqrt(k/100)*100,fromSlider:k=>k*k/100},mapTextInput:!1,overrideInputText:P(a.intensity),trackId:`${M}_intensity`}),a.type!=="Directional"&&t.jsx(me,{label:"Falloff",value:w,min:0,max:10,step:.01,onChange:k=>{const T=k*a.intensity;r({index:e,params:{falloff:T}})},customMapping:{min:0,max:100,toSlider:k=>Math.pow(k/10,1/1.5)*100,fromSlider:k=>Math.pow(k/100,1.5)*10},mapTextInput:!1,overrideInputText:w<1e-4?"Infinite":P(w),trackId:`${M}_falloff`}),a.type!=="Directional"&&t.jsxs("div",{className:"space-y-1",children:[t.jsxs("div",{className:"flex items-center justify-between",children:[t.jsx("label",{className:"text-xs text-gray-400 font-medium",children:"Visible Sphere"}),t.jsx("button",{onClick:()=>{const k=(a.radius??0)>.001;i("param"),r({index:e,params:{radius:k?0:.1}}),l()},className:`text-[9px] font-bold px-2 py-0.5 rounded border transition-colors ${(a.radius??0)>.001?"bg-cyan-500/20 text-cyan-300 border-cyan-500/50":"bg-white/5 text-gray-400 border-white/20 hover:border-white/40"}`,children:(a.radius??0)>.001?"ON":"OFF"})]}),(a.radius??0)>.001&&t.jsxs(t.Fragment,{children:[t.jsx(me,{label:"Sphere Radius",value:a.radius??.1,min:.01,max:5,step:.01,onChange:k=>r({index:e,params:{radius:k}}),trackId:`${M}_radius`}),t.jsx(me,{label:"Edge Softness",value:a.softness??0,min:0,max:2,step:.01,onChange:k=>r({index:e,params:{softness:k}}),trackId:`${M}_softness`})]})]})]}),t.jsxs("div",{className:"pt-2 border-t border-white/10 space-y-2",children:[t.jsxs("div",{className:"flex items-center gap-1 mb-2",children:[t.jsx("button",{onClick:()=>{g(!1),r({index:e,params:{useTemperature:!1}})},className:`text-[9px] font-bold px-2 py-0.5 rounded border transition-colors ${u?"bg-white/5 text-gray-400 border-white/20 hover:border-white/40":"bg-cyan-500/20 text-cyan-300 border-cyan-500/50"}`,children:"COLOR"}),t.jsx("button",{onClick:()=>{const k=!u;if(g(k),k){const T=Co(S);r({index:e,params:{color:T,useTemperature:!0,temperature:S}})}else r({index:e,params:{useTemperature:!1}})},className:`text-[9px] font-bold px-2 py-0.5 rounded border transition-colors ${u?"bg-amber-500/20 text-amber-300 border-amber-500/50":"bg-white/5 text-gray-400 border-white/20 hover:border-white/40"}`,children:"TEMPERATURE"})]}),u?t.jsxs("div",{className:"space-y-1",children:[t.jsxs("div",{className:"flex items-center justify-between",children:[t.jsx("label",{className:"text-[10px] text-gray-400 font-medium",children:"Temperature (K)"}),t.jsx("span",{className:"text-[10px] text-gray-300 font-mono",children:S})]}),t.jsx("input",{type:"range",min:1e3,max:1e4,step:100,value:S,onChange:k=>{const T=parseInt(k.target.value);y(T);const _=Co(T);r({index:e,params:{temperature:T,color:_}})},className:"w-full h-1.5 bg-gradient-to-r from-orange-500 via-yellow-200 to-blue-200 rounded-full appearance-none cursor-pointer",style:{background:"linear-gradient(to right, #ff6b35, #ffcc66, #ffffff, #cce5ff, #66b3ff)"}})]}):t.jsx(ga,{color:a.color,onColorChange:k=>r({index:e,params:{color:k}})}),t.jsxs("div",{className:"flex items-center justify-between pt-1",children:[t.jsx("label",{className:"text-xs text-gray-400 font-medium",children:"Cast Shadows"}),t.jsx("input",{type:"checkbox",checked:a.castShadow,onChange:k=>{i("param"),r({index:e,params:{castShadow:k.target.checked}}),l()},className:"w-3 h-3 accent-cyan-500 bg-gray-800 border-gray-600 rounded cursor-pointer"})]})]})]})})},Il=({targetRef:e,color:o,onChange:a,onClose:r,label:n})=>{const[s,i]=v.useState({x:0,y:0});return v.useLayoutEffect(()=>{if(e.current){const l=e.current.getBoundingClientRect(),c=window.innerWidth,d=window.innerHeight,f=240,p=150;let h=l.right+5,u=l.top;h+f>c&&(h=l.left-f-5),u+p>d&&(u=l.bottom-p),i({x:h,y:u})}},[e]),v.useEffect(()=>{const l=c=>{e.current&&!e.current.contains(c.target)&&(c.target.closest(".picker-popup")||r())};return window.addEventListener("mousedown",l),()=>window.removeEventListener("mousedown",l)},[r,e]),Pt.createPortal(t.jsxs("div",{className:"picker-popup fixed z-[9999] bg-black border border-white/20 p-3 rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] w-56 animate-fade-in",style:{left:s.x,top:s.y},onMouseDown:l=>l.stopPropagation(),children:[n&&t.jsx("div",{className:"text-[10px] font-bold text-gray-500 mb-2 text-center",children:n}),t.jsx(ga,{color:o,onColorChange:a})]}),document.body)},sa=({color:e,onChange:o,label:a})=>{const[r,n]=v.useState(!1),s=v.useRef(null),i=N(c=>c.openContextMenu),l=c=>{const d=Ge(c.currentTarget);d.unshift("ui.colorpicker"),d.length>0&&(c.preventDefault(),c.stopPropagation(),i(c.clientX,c.clientY,[],d))};return t.jsxs(t.Fragment,{children:[t.jsx("button",{ref:s,onClick:()=>n(!r),onContextMenu:l,className:"w-16 h-6 rounded border border-white/10 shadow-lg transition-all hover:scale-105 active:scale-95 flex items-center justify-center overflow-hidden",style:{backgroundColor:e},title:a||"Pick Color",children:t.jsx("div",{className:"text-[8px] font-mono font-bold mix-blend-difference text-white",children:e})}),r&&t.jsx(Il,{targetRef:s,color:e,onChange:o,onClose:()=>n(!1),label:a})]})};function kt({label:e,value:o,options:a,onChange:r,helpId:n,fullWidth:s,className:i="",selectClassName:l="",labelSuffix:c}){const{openContextMenu:d,handleInteractionStart:f,handleInteractionEnd:p}=N(),h=g=>{const S=Ge(g.currentTarget);S.length>0&&(g.preventDefault(),g.stopPropagation(),d(g.clientX,g.clientY,[],S))},u=g=>{var b;f("param");const S=g.target.value,y=typeof((b=a[0])==null?void 0:b.value)=="number";r(y?Number(S):S),p()};return t.jsxs("div",{className:`flex items-stretch bg-white/[0.12] rounded-t-sm h-9 md:h-[26px] overflow-hidden border-b border-white/5 ${s?"w-full":""} ${i}`,"data-help-id":n,onContextMenu:h,children:[e&&t.jsx("div",{className:"flex-1 flex items-center gap-2 px-2 min-w-0",children:t.jsxs("label",{className:"text-[10px] font-medium tracking-tight select-none truncate pointer-events-none text-gray-400",children:[e,c]})}),t.jsxs("div",{className:`${e?"w-1/2":"w-full"} relative border-l border-white/10 bg-white/[0.02] border-t border-t-white/5`,children:[t.jsx("select",{value:o,onChange:u,className:`w-full h-full bg-transparent text-[10px] font-medium text-gray-200 px-2 pr-6 outline-none cursor-pointer appearance-none text-center ${l}`,children:a.map(g=>t.jsx("option",{value:String(g.value),className:"bg-[#111] text-gray-300",children:g.label},String(g.value)))}),t.jsx("div",{className:"absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none text-gray-500",children:t.jsx("div",{className:"w-2.5 h-2.5",children:t.jsx(Tt,{})})})]})]})}const nt=({axisIndex:e,value:o,min:a,max:r,step:n,onUpdate:s,onDragStart:i,onDragEnd:l,disabled:c,highlight:d,mapping:f,mapTextInput:p,liveValue:h,defaultValue:u,hardMin:g,hardMax:S,customLabel:y})=>{const b=Sl[e],m=y||b.label;return t.jsxs("div",{className:`relative flex-1 h-9 md:h-[26px] overflow-hidden group transition-all duration-150 ${c?"opacity-50 pointer-events-none":""}`,children:[t.jsx("div",{className:`
                ...
                `,onDoubleClick:x=>{x.preventDefault(),x.stopPropagation(),u!==void 0&&(i==null||i(),s(u),l==null||l())},title:u!==void 0?`Double-click to reset to ${u}`:"No default value",children:t.jsx("span",{className:`text-[10px] font-bold ${b.text} pointer-events-none`,children:m})}),t.jsx("div",{className:"absolute inset-0 left-5",children:t.jsx(ao,{value:o,onChange:s,onDragStart:i,onDragEnd:l,step:n,min:a,max:r,hardMin:g,hardMax:S,mapping:f,mapTextInput:p,disabled:c,highlight:d,liveValue:h,defaultValue:u,variant:"compact",showTrack:!0})})]})},Do=[{label:"X",color:"bg-red-500",text:"text-red-400",border:"group-focus-within:border-red-500/50",hoverBg:"hover:bg-red-500/20",accent:"#ef4444"},{label:"Y",color:"bg-green-500",text:"text-green-400",border:"group-focus-within:border-green-500/50",hoverBg:"hover:bg-green-500/20",accent:"#22c55e"},{label:"Z",color:"bg-blue-500",text:"text-blue-400",border:"group-focus-within:border-blue-500/50",hoverBg:"hover:bg-blue-500/20",accent:"#3b82f6"}],ja=({primaryAxis:e,secondaryAxis:o,primaryIndex:a,secondaryIndex:r,primaryValue:n,secondaryValue:s,min:i,max:l,step:c,onUpdate:d,onDragStart:f,onDragEnd:p,disabled:h,onHover:u})=>{const[g,S]=v.useState(!1),y=v.useRef(!1),b=v.useRef(!1),m=v.useRef({x:0,y:0}),x=v.useRef({primary:0,secondary:0}),C=v.useRef(!1),z=v.useRef(!1),w=v.useRef(!1),M=Do[a],P=Do[r],k=()=>{S(!0),u(!0)},T=()=>{y.current||(S(!1),u(!1))},_=R=>{h||R.button!==0&&R.button!==1||(R.preventDefault(),R.stopPropagation(),R.currentTarget.setPointerCapture(R.pointerId),m.current={x:R.clientX,y:R.clientY},x.current={primary:n,secondary:s},C.current=!1,z.current=R.shiftKey,w.current=R.altKey,y.current=!0,b.current=R.button===1,f())},E=R=>{if(h||!y.current||!R.currentTarget.hasPointerCapture(R.pointerId))return;const Y=R.clientX-m.current.x,L=R.clientY-m.current.y;if((Math.abs(Y)>1||Math.abs(L)>1)&&(C.current=!0),!C.current&&Math.abs(Y)<1&&Math.abs(L)<1)return;R.preventDefault(),R.stopPropagation();const G=z.current!==R.shiftKey,X=w.current!==R.altKey;if(G||X){let D=c*.5;z.current&&(D*=10),w.current&&(D*=.1),x.current.primary=x.current.primary+Y*D,x.current.secondary=x.current.secondary-L*D,m.current={x:R.clientX,y:R.clientY},z.current=R.shiftKey,w.current=R.altKey}if(b.current){let D=.01;R.shiftKey&&(D*=3),R.altKey&&(D*=.3);const H=1+L*D;let A=x.current.primary*H,j=x.current.secondary*H;!isNaN(A)&&!isNaN(j)&&d(A,j)}else{let D=c*.5;R.shiftKey&&(D*=10),R.altKey&&(D*=.1);let H=x.current.primary+Y*D,A=x.current.secondary-L*D;!isNaN(H)&&!isNaN(A)&&d(H,A)}},F=R=>{h||(R.currentTarget.releasePointerCapture(R.pointerId),y.current=!1,b.current=!1,p(),C.current=!1,R.currentTarget.matches(":hover")||(S(!1),u(!1)))},O=R=>{h||(R.preventDefault(),R.stopPropagation(),f(),d(0,0),p())},$=g||y.current;return t.jsxs("div",{className:`
                w-6 h-9 md:h-[26px] flex-shrink-0 cursor-move overflow-hidden
                transition-all duration-150 ease-out relative
                ${$?"bg-white/10 border border-white/30":"bg-white/[0.08] border border-white/5"}
                ${h?"opacity-30 pointer-events-none":""}
            `,onPointerDown:_,onPointerMove:E,onPointerUp:F,onMouseEnter:k,onMouseLeave:T,onDoubleClick:O,title:`Drag: Horizontal=${e.toUpperCase()}, Vertical=${o.toUpperCase()} | Middle-click: Scale both proportionally | Double-click: Reset (Shift=Fast, Alt=Slow)`,children:[$&&t.jsx("div",{className:"absolute inset-0 opacity-30",style:{backgroundImage:"repeating-linear-gradient(45deg, transparent, transparent 3px, rgba(255,255,255,0.05) 3px, rgba(255,255,255,0.05) 6px)"}}),t.jsxs("div",{className:"relative w-full h-full",children:[t.jsx("div",{className:`
                        absolute bottom-0 left-0 h-[2px] ${M.color}
                        transition-all duration-150
                        ${$?"opacity-60 w-full":"opacity-0 w-0"}
                    `}),t.jsx("div",{className:`
                        absolute bottom-0 left-0 w-[2px] ${P.color}
                        transition-all duration-150
                        ${$?"opacity-60 h-full":"opacity-0 h-0"}
                    `}),t.jsxs("div",{className:`
                        absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
                        transition-opacity duration-150 pointer-events-none
                        ${$?"opacity-100":"opacity-0"}
                    `,children:[t.jsx("div",{className:`absolute w-2 h-[1px] ${M.color} -translate-x-1/2`}),t.jsx("div",{className:`absolute h-2 w-[1px] ${P.color} -translate-y-1/2`})]}),t.jsx("div",{className:`
                        absolute inset-0 pointer-events-none
                        transition-opacity duration-150
                        ${b.current?"opacity-100":"opacity-0"}
                    `,children:t.jsxs("div",{className:"absolute inset-0 flex items-center justify-center",children:[t.jsx("div",{className:"w-full h-[1px] bg-cyan-400/50 rotate-45"}),t.jsx("div",{className:"absolute w-full h-[1px] bg-cyan-400/50 -rotate-45"})]})})]})]})},Lo=({azimuth:e,pitch:o,onChange:a,onDragStart:r,onDragEnd:n,disabled:s=!1,size:i=80})=>{const l=v.useRef(null),c=v.useRef(!1),[d,f]=v.useState(!1),[p,h]=v.useState(!1),[u,g]=v.useState(!1),S=v.useRef({x:0,y:0}),y=v.useRef({azimuth:e,pitch:o}),b=v.useRef({azimuth:e,pitch:o}),m=v.useRef(null);v.useEffect(()=>{y.current={azimuth:e,pitch:o}},[e,o]);const z=p?.05:.5,w=i/2,M=i*.38,P=v.useCallback((R,Y,L)=>{const G=R/(Math.PI/2)*L,X=-(Y/(Math.PI/2))*L;return{x:G,y:X}},[]),k=v.useMemo(()=>P(e,o,M),[e,o,M,P]),T=v.useMemo(()=>{const R=Math.cos(o),Y=Math.sin(o),L=Math.cos(e),X=Math.sin(e)*R,D=Y,H=-L*R,A=X,j=-D,B=Math.sqrt(A*A+j*j),I=H>0,V=B>.001?Math.min(B,1)*M:0,W=H<=0?1+(1-Math.min(B,1))*.5:1-H*.95,q=B>.001?A/B*V:0,Q=B>.001?j/B*V:0;return{x:q,y:Q,isBack:I,length:V,headScale:W,dirX:X,dirY:D,dirZ:H}},[e,o,M]),_=v.useCallback((R,Y,L)=>{const G=R/L*(Math.PI/2),X=-(Y/L)*(Math.PI/2);return{azimuth:G,pitch:X}},[]),E=v.useCallback((R,Y)=>{let L=R,G=Y;u&&m.current&&(m.current==="x"?G=0:L=0);const X=L*z,D=G*z,H=P(y.current.azimuth,y.current.pitch,M),A=H.x+X,j=H.y+D,{azimuth:B,pitch:I}=_(A,j,M);u&&m.current?m.current==="x"?(y.current={azimuth:B,pitch:b.current.pitch},a(B,b.current.pitch)):(y.current={azimuth:b.current.azimuth,pitch:I},a(b.current.azimuth,I)):(y.current={azimuth:B,pitch:I},a(B,I))},[P,_,a,M,z,u]),F=R=>{s||R.button===0&&(R.preventDefault(),R.stopPropagation(),R.currentTarget.setPointerCapture(R.pointerId),c.current=!0,f(!0),S.current={x:R.clientX,y:R.clientY},y.current={azimuth:e,pitch:o},b.current={azimuth:e,pitch:o},m.current=null,r==null||r(),h(R.altKey),g(R.shiftKey))},O=R=>{if(s||!c.current)return;const Y=R.clientX-S.current.x,L=R.clientY-S.current.y;S.current={x:R.clientX,y:R.clientY},h(R.altKey),g(R.shiftKey),u&&!m.current&&(Math.abs(Y)>2||Math.abs(L)>2)&&(m.current=Math.abs(Y)>Math.abs(L)?"x":"y"),E(Y,L)},$=R=>{c.current&&(c.current=!1,f(!1),h(!1),g(!1),m.current=null,n==null||n())};return t.jsxs("div",{ref:l,className:`
            ...
            `,style:{width:i,height:i,touchAction:"none",boxShadow:d?"0 0 20px rgba(34, 211, 238, 0.3)":"none"},onPointerDown:F,onPointerMove:O,onPointerUp:$,onPointerLeave:$,onDoubleClick:R=>{s||(R.preventDefault(),R.stopPropagation(),r==null||r(),a(0,0),n==null||n())},onContextMenu:R=>{},title:"Drag to rotate direction, Double-click to reset",children:[t.jsx("div",{className:"absolute rounded-full border border-white/10 pointer-events-none",style:{width:M*2,height:M*2,left:w-M,top:w-M}}),t.jsx("div",{className:"absolute w-full h-px bg-white/10 pointer-events-none",style:{top:w}}),t.jsx("div",{className:"absolute h-full w-px bg-white/10 pointer-events-none",style:{left:w}}),t.jsx("div",{className:"absolute w-1.5 h-1.5 bg-white/40 rounded-full pointer-events-none",style:{left:w-3,top:w-3}}),t.jsx("div",{className:"absolute pointer-events-none rounded-full transition-transform duration-75",style:{left:w+k.x,top:w+k.y,width:8,height:8,marginLeft:-4,marginTop:-4,background:T.isBack?"#ef4444":"#22d3ee",boxShadow:`0 0 8px ${T.isBack?"#ef4444":"#22d3ee"}`,transform:d?"scale(1.2)":"scale(1)"}}),T.isBack&&t.jsx("div",{className:"absolute inset-0 rounded-full border-2 border-red-500/30 pointer-events-none animate-pulse"}),t.jsxs(t.Fragment,{children:[t.jsxs("svg",{className:"absolute inset-0 pointer-events-none",style:{width:i,height:i},children:[Math.abs(e)>.01&&t.jsxs(t.Fragment,{children:[t.jsx("ellipse",{cx:w,cy:w,rx:M*Math.abs(Math.sin(e)),ry:M,fill:"none",stroke:T.isBack?"#ef4444":"#22d3ee",strokeWidth:"1.5",opacity:T.isBack?.175:.35,clipPath:T.x>0?"url(#longitudeRight)":"url(#longitudeLeft)"}),t.jsx("ellipse",{cx:w,cy:w,rx:M*Math.abs(Math.sin(e)),ry:M,fill:"none",stroke:T.isBack?"#22d3ee":"#ef4444",strokeWidth:"1.5",opacity:T.isBack?.35:.175,clipPath:T.x>0?"url(#longitudeLeft)":"url(#longitudeRight)"})]}),Math.abs(o)>.01&&t.jsxs(t.Fragment,{children:[t.jsx("ellipse",{cx:w,cy:w,rx:M,ry:M*Math.abs(Math.sin(o)),fill:"none",stroke:T.isBack?"#ef4444":"#22d3ee",strokeWidth:"1.5",opacity:T.isBack?.15:.3,clipPath:T.y<0?"url(#latitudeTop)":"url(#latitudeBottom)"}),t.jsx("ellipse",{cx:w,cy:w,rx:M,ry:M*Math.abs(Math.sin(o)),fill:"none",stroke:T.isBack?"#22d3ee":"#ef4444",strokeWidth:"1.5",opacity:T.isBack?.3:.15,clipPath:T.y<0?"url(#latitudeBottom)":"url(#latitudeTop)"})]}),t.jsxs("defs",{children:[t.jsx("clipPath",{id:"longitudeRight",children:t.jsx("rect",{x:w,y:"0",width:w,height:i})}),t.jsx("clipPath",{id:"longitudeLeft",children:t.jsx("rect",{x:"0",y:"0",width:w,height:i})}),t.jsx("clipPath",{id:"latitudeTop",children:t.jsx("rect",{x:"0",y:"0",width:i,height:w})}),t.jsx("clipPath",{id:"latitudeBottom",children:t.jsx("rect",{x:"0",y:w,width:i,height:w})})]}),t.jsx("line",{x1:w,y1:w,x2:w+T.x,y2:w+T.y,stroke:T.isBack?"#ef4444":"#22d3ee",strokeWidth:"2",strokeDasharray:"4 2",opacity:.3+T.length/M*.5}),t.jsx("polygon",{points:"0,-8 -6,4 6,4",fill:T.isBack?"#ef4444":"#22d3ee",opacity:Math.max(.1,.6+(T.headScale-1)*.4),transform:`translate(${w+T.x}, ${w+T.y}) rotate(${Math.atan2(T.y,T.x)*180/Math.PI+90}) scale(${Math.max(.9,.9+T.headScale*.1)}, ${Math.max(.05,T.headScale)})`})]}),d&&t.jsx("div",{className:"absolute -bottom-5 left-1/2 -translate-x-1/2 whitespace-nowrap",children:t.jsxs("span",{className:"text-[9px] text-cyan-400 font-mono bg-black/60 px-1 rounded",children:[(e*180/Math.PI).toFixed(0),"° / ",(o*180/Math.PI).toFixed(0),"°"]})})]})]})},Pa=Math.PI/180,Fl=180/Math.PI,_l=e=>{const o=e.length();if(o<1e-9)return{azimuth:0,pitch:0};const a=Math.max(-1,Math.min(1,e.y/o));return{azimuth:Math.atan2(e.x/o,e.z/o),pitch:Math.asin(a)}},Dl=(e,o)=>{const a=Math.cos(o);return new U(a*Math.sin(e),Math.sin(o),a*Math.cos(e))},Ar=({label:e,value:o,onChange:a,min:r=-1e4,max:n=1e4,step:s=.01,disabled:i=!1,convertRadToDeg:l=!1,mode:c="normal",modeToggleable:d=!1,showLiveIndicator:f=!1,liveValue:p,defaultValue:h,hardMin:u,hardMax:g,axisMin:S,axisMax:y,axisStep:b,onDragStart:m,onDragEnd:x,headerRight:C,showDualAxisPads:z=!0,linkable:w=!1,scale:M})=>{const[P,k]=v.useState(o.clone()),[T,_]=v.useState(null),[E,F]=v.useState(c),[O,$]=v.useState("degrees"),[R,Y]=v.useState("degrees"),[L,G]=v.useState(w),X=v.useRef(!1),D=v.useRef(null);v.useEffect(()=>{F(c)},[c]);const H=N(ae=>ae.openContextMenu),A="z"in o,j=E==="rotation",B=E==="toggle",I=E==="mixed",V=E==="direction"&&A,W=V?_l(P):{azimuth:0,pitch:0},q=(ae,re)=>{const ue=Math.max(-Math.PI/2,Math.min(Math.PI/2,re)),Re=Dl(ae,ue);k(Re),a(Re)};v.useEffect(()=>{if(X.current)return;const ae=1e-4,re=Math.abs(o.x-P.x),ue=Math.abs(o.y-P.y),Re=A?Math.abs(o.z-P.z):0;(re>ae||ue>ae||Re>ae)&&k(o.clone())},[o,A]);const Q=()=>{X.current=!0,D.current=P.clone(),m&&m()},K=()=>{D.current=null,X.current=!1,x&&x()},ie=ae=>{if(j)return O==="degrees"?Dt:Fo;if(M==="pi")return R==="pi"?Fo:Dt;if(l)return Dt},ee=ae=>{if(j){const Ue=O==="degrees"?60*Math.PI/180:.05;return{min:-2*Math.PI,max:2*Math.PI,step:Ue,hardMin:void 0,hardMax:void 0}}const re=S||{x:r,y:r,z:r},ue=y||{x:n,y:n,z:n},Re=b||{x:s,y:s,z:s},Ye=M==="pi"&&R==="degrees"?Fl:1;return{min:re[ae],max:ue[ae],step:(Re[ae]??s)*Ye,hardMin:u,hardMax:g}},oe=(ae,re)=>{const ue=D.current||P,Re=ue.clone();if(L&&!j){const Ye=ue[ae],Ue=re-Ye;Re.x=ue.x+Ue,Re.y=ue.y+Ue,A&&(Re.z=ue.z+Ue)}else Re[ae]=re;k(Re),a(Re)},le=(ae,re,ue,Re)=>{const Ue=(D.current||P).clone();Ue[ae]=ue,Ue[re]=Re,k(Ue),a(Ue)},ce=T==="xy",pe=T==="xy"||T==="zy",he=T==="zy",Te=ae=>{if(p)return p[ae]},Ne=ae=>{if(h)return h[ae]},Se=P,te={x:0,y:1,z:2},be={x:ce,y:pe,z:he},Pe=ae=>({axisIndex:te[ae],value:P[ae],...ee(ae),onUpdate:re=>oe(ae,re),onDragStart:Q,onDragEnd:K,disabled:i,highlight:be[ae],mapping:ie(),mapTextInput:j||M==="pi",liveValue:f?Te(ae):void 0,defaultValue:Ne(ae)}),Xe=[{on:"bg-red-500/30 text-red-300 border-red-500/40",off:"bg-white/[0.04] text-gray-600 border-white/5"},{on:"bg-green-500/30 text-green-300 border-green-500/40",off:"bg-white/[0.04] text-gray-600 border-white/5"},{on:"bg-blue-500/30 text-blue-300 border-blue-500/40",off:"bg-white/[0.04] text-gray-600 border-white/5"}],ht=(ae,re,ue)=>{const Ye=P[ae]>.5,Ue=Xe[re];return t.jsxs("button",{className:`flex items-center justify-center gap-1 text-[10px] font-bold transition-all border ${Ye?Ue.on:Ue.off} ${i?"opacity-40 pointer-events-none":"cursor-pointer hover:brightness-125"} ${ue||"flex-1"}`,onClick:()=>oe(ae,Ye?0:1),disabled:i,children:[ue?null:t.jsx("span",{children:ae}),t.jsx("span",{className:`text-[8px] ${Ye?"opacity-80":"opacity-70"}`,children:Ye?"ON":"OFF"})]},ae)},St=()=>d?t.jsx("button",{onClick:()=>F(ae=>ae==="rotation"?"normal":"rotation"),className:`text-[10px] p-1 rounded transition-colors mr-2 ${E==="rotation"?"text-cyan-400 bg-cyan-500/20":"text-gray-500 hover:text-gray-300"}`,title:E==="rotation"?"Rotation mode (π units)":"Normal mode",children:"⟳"}):null,ge=()=>!w||j?null:t.jsx("button",{onClick:()=>G(ae=>!ae),className:`p-1 rounded transition-colors mr-2 ${L?"text-cyan-400 bg-cyan-500/20":"text-gray-600 hover:text-gray-400"}`,title:L?"Axes linked (uniform)":"Link axes",children:t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("path",{d:"M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"}),t.jsx("path",{d:"M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"})]})}),Be=ae=>{const re=[];j&&re.push({label:"Rotation Units",action:()=>{},isHeader:!0},{label:"Degrees (°)",checked:O==="degrees",action:()=>$("degrees")},{label:"Radians (π)",checked:O==="radians",action:()=>$("radians")}),!j&&M==="pi"&&re.push({label:"Display Units",action:()=>{},isHeader:!0},{label:"Radians (π)",checked:R==="pi",action:()=>Y("pi")},{label:"Degrees (°)",checked:R==="degrees",action:()=>Y("degrees")}),A&&(c==="rotation"||c==="axes")&&re.push({label:"Display Mode",action:()=>{},isHeader:!0},{label:"Azimuth / Pitch (A/P)",checked:E==="rotation",action:()=>F("rotation")},{label:"Per-Axis (X/Y/Z)",checked:E==="axes"||E==="normal",action:()=>F("normal")}),re.length!==0&&(ae.preventDefault(),ae.stopPropagation(),H(ae.clientX,ae.clientY,re,["ui.vector"]))};return t.jsxs("div",{className:"mb-px animate-slider-entry",children:[e&&t.jsxs("div",{className:"flex items-stretch bg-white/[0.12] rounded-t-sm h-9 md:h-[26px] overflow-hidden border-b border-white/5",children:[t.jsxs("div",{className:"flex-1 flex items-center gap-2 px-2 min-w-0",children:[d&&St(),C,t.jsx("label",{className:`text-[10px] font-medium tracking-tight select-none flex items-center gap-2 truncate pointer-events-none ${i?"text-gray-600":"text-gray-400"}`,children:e})]}),w&&!j&&t.jsx("div",{className:"flex items-center px-1 border-l border-white/5",children:ge()})]}),t.jsx("div",{className:"relative h-9 md:h-[26px] flex items-center touch-none overflow-hidden rounded-b-sm",style:{touchAction:"none"},onContextMenu:Be,"data-help-id":"ui.vector",children:t.jsx("div",{className:"flex gap-px w-full h-full",children:B?t.jsx(t.Fragment,{children:["x","y","z"].slice(0,A?3:2).map((ae,re)=>ht(ae,re))}):I?t.jsxs(t.Fragment,{children:[ht("x",0,"w-14 flex-shrink-0"),t.jsx(nt,{...Pe("y"),disabled:i||P.x<.5})]}):V?t.jsxs(t.Fragment,{children:[t.jsx("div",{className:"flex items-center justify-center px-1 flex-shrink-0",children:t.jsx(Lo,{azimuth:W.azimuth,pitch:W.pitch,onChange:(ae,re)=>{q(ae,re)},onDragStart:Q,onDragEnd:K,disabled:i,size:56})}),t.jsx(nt,{axisIndex:0,value:W.azimuth,min:-Math.PI,max:Math.PI,step:Pa,onUpdate:ae=>q(ae,W.pitch),onDragStart:Q,onDragEnd:K,disabled:i,mapping:Dt,mapTextInput:!0,customLabel:"Az"}),t.jsx(ja,{primaryAxis:"x",secondaryAxis:"y",primaryIndex:0,secondaryIndex:1,primaryValue:W.azimuth,secondaryValue:W.pitch,min:-Math.PI,max:Math.PI,step:Pa,onUpdate:(ae,re)=>q(ae,re),onDragStart:Q,onDragEnd:K,disabled:i,onHover:ae=>_(ae?"xy":null)}),t.jsx(nt,{axisIndex:1,value:W.pitch,min:-Math.PI/2,max:Math.PI/2,step:Pa,onUpdate:ae=>q(W.azimuth,ae),onDragStart:Q,onDragEnd:K,disabled:i,mapping:Dt,mapTextInput:!0,customLabel:"Pt"})]}):j?t.jsxs(t.Fragment,{children:[A&&t.jsx(nt,{...Pe("z"),customLabel:"∠"}),t.jsx("div",{className:"flex items-center justify-center px-1",children:t.jsx(Lo,{azimuth:P.x,pitch:P.y,onChange:(ae,re)=>{const ue=P.clone();ue.x=ae,ue.y=re,k(ue),a(ue)},onDragStart:Q,onDragEnd:K,disabled:i,size:56})}),t.jsx("div",{className:"contents",children:t.jsx(nt,{...Pe("x"),customLabel:"A"})}),t.jsx(nt,{...Pe("y"),customLabel:"P"})]}):t.jsxs(t.Fragment,{children:[t.jsx("div",{className:"contents",children:t.jsx(nt,{...Pe("x")})}),z&&t.jsx(ja,{primaryAxis:"x",secondaryAxis:"y",primaryIndex:0,secondaryIndex:1,primaryValue:P.x,secondaryValue:P.y,min:r,max:n,step:s,onUpdate:(ae,re)=>le("x","y",ae,re),onDragStart:Q,onDragEnd:K,disabled:i,onHover:ae=>_(ae?"xy":null)}),t.jsx(nt,{...Pe("y")}),A&&z&&t.jsx(ja,{primaryAxis:"z",secondaryAxis:"y",primaryIndex:2,secondaryIndex:1,primaryValue:Se.z,secondaryValue:Se.y,min:r,max:n,step:s,onUpdate:(ae,re)=>le("z","y",ae,re),onDragStart:Q,onDragEnd:K,disabled:i,onHover:ae=>_(ae?"zy":null)}),A&&t.jsx(nt,{...Pe("z")})]})})})]})},Nr=({interactionMode:e="param",trackKeys:o,trackLabels:a,...r})=>{var b,m;const{handleInteractionStart:n,handleInteractionEnd:s}=N(),{sequence:i,isRecording:l,addTrack:c,addKeyframe:d,snapshot:f}=fe(),p=v.useRef(r.value);v.useEffect(()=>{p.current=r.value},[(b=r.value)==null?void 0:b.x,(m=r.value)==null?void 0:m.y]);const h=()=>{n(e),l&&o&&(f(),o.forEach((x,C)=>{if(x){const z=a?a[C]:x;i.tracks[x]||c(x,z)}}))},u=()=>{if(l&&o){const x=["x","y"];o.forEach((C,z)=>{if(C){let w=p.current[x[z]];d(C,Math.round(fe.getState().currentFrame),w)}})}s()},g=x=>{p.current=new je(x.x,x.y),r.onChange(new je(x.x,x.y))},S=()=>{if(!o||o.length===0)return"none";const x=Math.round(fe.getState().currentFrame),C=["x","y"];let z=!1,w=!1;return o.forEach((P,k)=>{if(!P)return;const T=i.tracks[P];if(T){const _=T.keyframes.find(E=>Math.abs(E.frame-x)<.5);_&&(z=!0,Math.abs(_.value-p.current[C[k]])>1e-4&&(w=!0))}}),z?w?"keyed-dirty":"keyed":o.some(P=>P&&i.tracks[P])?o.some((k,T)=>{if(!k)return!1;const _=i.tracks[k];if(!_||_.keyframes.length===0)return!1;const E=ot(_.keyframes,x,!1);return Math.abs(E-p.current[C[T]])>.001})?"dirty":"partial":"none"},y=r.disabled?void 0:t.jsx(Ht,{status:S(),onClick:()=>{const x=Math.round(fe.getState().currentFrame),C=["x","y"],z=S();f(),z==="keyed"?o==null||o.forEach(w=>{if(!w)return;const M=i.tracks[w];if(M){const P=M.keyframes.find(k=>Math.abs(k.frame-x)<.5);P&&fe.getState().removeKeyframe(w,P.id)}}):(o==null||o.forEach((w,M)=>{w&&(i.tracks[w]||c(w,a?a[M]:w),d(w,x,p.current[C[M]]))}),o!=null&&o[0]&&Z.emit(we.TRACK_FOCUS,o[0]))}});return t.jsx(Ar,{...r,value:r.value,onChange:g,onDragStart:h,onDragEnd:u,headerRight:y,showDualAxisPads:!0})},$t=({interactionMode:e="param",trackKeys:o,trackLabels:a,...r})=>{var b,m,x;const{handleInteractionStart:n,handleInteractionEnd:s}=N(),{sequence:i,isRecording:l,addTrack:c,addKeyframe:d,snapshot:f}=fe(),p=v.useRef(r.value);v.useEffect(()=>{p.current=r.value},[(b=r.value)==null?void 0:b.x,(m=r.value)==null?void 0:m.y,(x=r.value)==null?void 0:x.z]);const h=()=>{n(e),l&&o&&(f(),o.forEach((C,z)=>{if(C){const w=a?a[z]:C;i.tracks[C]||c(C,w)}}))},u=()=>{if(l&&o){const C=["x","y","z"];o.forEach((z,w)=>{if(z){let M=p.current[C[w]];d(z,Math.round(fe.getState().currentFrame),M)}})}s()},g=C=>{p.current=new U(C.x,C.y,C.z??0),r.onChange(new U(C.x,C.y,C.z??0))},S=()=>{if(!o||o.length===0)return"none";const C=Math.round(fe.getState().currentFrame),z=["x","y","z"];let w=!1,M=!1;return o.forEach((k,T)=>{if(!k)return;const _=i.tracks[k];if(_){const E=_.keyframes.find(F=>Math.abs(F.frame-C)<.5);E&&(w=!0,Math.abs(E.value-p.current[z[T]])>1e-4&&(M=!0))}}),w?M?"keyed-dirty":"keyed":o.some(k=>k&&i.tracks[k])?o.some((T,_)=>{if(!T)return!1;const E=i.tracks[T];if(!E||E.keyframes.length===0)return!1;const F=ot(E.keyframes,C,!1);return Math.abs(F-p.current[z[_]])>.001})?"dirty":"partial":"none"},y=r.disabled?void 0:t.jsx(Ht,{status:S(),onClick:()=>{const C=Math.round(fe.getState().currentFrame),z=["x","y","z"],w=S();f(),w==="keyed"?o==null||o.forEach(M=>{if(!M)return;const P=i.tracks[M];if(P){const k=P.keyframes.find(T=>Math.abs(T.frame-C)<.5);k&&fe.getState().removeKeyframe(M,k.id)}}):(o==null||o.forEach((M,P)=>{M&&(i.tracks[M]||c(M,a?a[P]:M),d(M,C,p.current[z[P]]))}),o!=null&&o[0]&&Z.emit(we.TRACK_FOCUS,o[0]))}});return t.jsx(Ar,{...r,value:r.value,onChange:g,onDragStart:h,onDragEnd:u,headerRight:y,showDualAxisPads:!0})},Ll=({label:e,value:o,min:a,max:r,step:n=.01,onChange:s,size:i=40,color:l="#22d3ee",tooltip:c,unconstrained:d=!1,defaultValue:f,onDragStart:p,onDragEnd:h})=>{const[u,g]=v.useState(!1),S=v.useRef(0),y=v.useRef(0),b=r-a,m=Math.max(a,Math.min(r,o)),x=Math.max(0,Math.min(1,(m-a)/b)),C=-135+x*270,z=i/2-4,w=i/2,M=i/2,P=2*Math.PI*z,k=P,T=P*(1-x*.75),_=$=>{$.preventDefault(),$.stopPropagation(),g(!0),S.current=$.clientY,y.current=o,p&&p(),$.target.setPointerCapture($.pointerId)},E=$=>{if(!u)return;$.preventDefault();const R=S.current-$.clientY;let Y=.005;$.shiftKey&&(Y*=5),$.altKey&&(Y*=.1);const L=R*Y*b;let G=y.current+L;d||(G=Math.max(a,Math.min(r,G))),n&&(G=Math.round(G/n)*n),s(G)},F=$=>{g(!1),h&&h(),$.target.releasePointerCapture($.pointerId)},O=$=>{$.preventDefault(),$.stopPropagation(),f!==void 0&&(p&&p(),s(f),h&&h())};return t.jsxs("div",{className:"flex flex-col items-center gap-1 select-none touch-none group",title:c||`${o.toFixed(2)}`,onDoubleClick:O,children:[t.jsxs("div",{className:"relative cursor-ns-resize",style:{width:i,height:i},onPointerDown:_,onPointerMove:E,onPointerUp:F,children:[t.jsxs("svg",{width:i,height:i,className:"overflow-visible transform rotate-90",children:[t.jsx("circle",{cx:w,cy:M,r:z,fill:"none",stroke:"#333",strokeWidth:"3",strokeDasharray:P,strokeDashoffset:P*.25,strokeLinecap:"round"}),t.jsx("circle",{cx:w,cy:M,r:z,fill:"none",stroke:u?"#fff":l,strokeWidth:"3",strokeDasharray:k,strokeDashoffset:T,strokeLinecap:"round",className:"transition-colors duration-200"})]}),t.jsx("div",{className:"absolute w-1.5 h-1.5 bg-white rounded-full shadow-sm pointer-events-none",style:{top:"50%",left:"50%",marginTop:-3,marginLeft:-3,transform:`rotate(${C}deg) translate(0, -${z}px)`}})]}),t.jsx("div",{className:"h-3 min-w-[30px] flex items-center justify-center bg-black/40 rounded px-1 border border-white/5 hover:border-white/20 transition-colors",children:t.jsx(Lr,{value:o,onChange:s,min:d?void 0:a,max:d?void 0:r,step:n,onDragStart:p,onDragEnd:h})}),e&&t.jsx("span",{className:"text-[8px] text-gray-500 font-bold group-hover:text-gray-300 transition-colors -mt-0.5",children:e})]})},Mt=e=>{const{handleInteractionStart:o,handleInteractionEnd:a}=N();return t.jsx(Ll,{...e,onDragStart:()=>{o("param"),e.onDragStart&&e.onDragStart()},onDragEnd:()=>{a(),e.onDragEnd&&e.onDragEnd()}})};class El{constructor(){J(this,"components",new Map)}register(o,a){this.components.has(o)&&console.warn(`ComponentRegistry: Overwriting component '${o}'`),this.components.set(o,a)}get(o){return this.components.get(o)}}const ve=new El,Al={default:"",panel:"px-2 py-0.5 text-[9px] font-bold text-gray-500 hover:text-gray-300 bg-neutral-800 rounded-sm"},Nl=({open:e})=>t.jsx("svg",{className:`w-2 h-2 transition-transform ${e?"rotate-90":""}`,viewBox:"0 0 6 10",fill:"currentColor",children:t.jsx("path",{d:"M0 0l6 5-6 5z"})}),jt=({label:e,children:o,defaultOpen:a=!0,count:r,labelVariant:n="secondary",labelColor:s,rightContent:i,className:l="",headerClassName:c="",variant:d="default",open:f,onToggle:p})=>{const h=c||Al[d],[u,g]=v.useState(a),S=f!==void 0,y=S?f:u,b=()=>{p&&p(),S||g(m=>!m)};return t.jsxs("div",{className:l,children:[t.jsxs("button",{onClick:b,className:`flex items-center gap-1.5 w-full px-2 py-1 text-left select-none hover:bg-white/5 transition-colors rounded-sm ${h}`,children:[t.jsx(Nl,{open:y}),t.jsx(_e,{variant:n,color:s,children:e}),r!==void 0&&t.jsx("span",{className:"text-[8px] bg-white/10 text-gray-400 px-1.5 py-0.5 rounded ml-1",children:r}),i&&t.jsx("div",{className:"ml-auto flex items-center gap-1",onClick:m=>m.stopPropagation(),children:i})]}),y&&o]})},Bl={active:"bg-green-500 shadow-[0_0_4px_rgba(34,197,94,0.4)]",pending:"bg-amber-500 animate-pulse shadow-[0_0_4px_rgba(245,158,11,0.4)]",off:"bg-red-900",instant:"bg-cyan-500 shadow-[0_0_4px_rgba(6,182,212,0.4)]"},la=({status:e,className:o="",title:a})=>t.jsx("span",{className:`inline-block w-1.5 h-1.5 rounded-full shrink-0 ${Bl[e]} ${o}`,title:a}),Ua=({label:e,isActive:o,onToggle:a,numericValue:r,onNumericChange:n,options:s,onOptionChange:i,status:l,disabled:c=!1,hideCheckbox:d=!1,description:f,min:p,max:h,step:u})=>{var _;const[g,S]=v.useState(!1),y=v.useRef(null),[b,m]=v.useState({top:0,x:0,side:"right"}),x=()=>{if(f&&y.current){const E=y.current.getBoundingClientRect(),F=E.left<window.innerWidth/2;m({top:E.top+E.height/2,x:F?E.right+6:window.innerWidth-E.left+6,side:F?"left":"right"}),S(!0)}},C=()=>S(!1),z=l==="pending"?"text-amber-400":o?"text-gray-300":"text-gray-500";let w="",M="";switch(l){case"pending":w="bg-amber-500 shadow-[0_0_6px_rgba(245,158,11,0.6)] animate-pulse",M="Pending Compilation (Click Apply)";break;case"runtime":w="bg-blue-500 shadow-[0_0_4px_rgba(59,130,246,0.5)]",M="Runtime Uniform (Instant Update)";break;case"synced":default:w=o?"bg-green-500 shadow-[0_0_4px_rgba(34,197,94,0.4)]":"bg-gray-700",M="Compiled & Active";break}const P=s&&r!==void 0?(_=s.find(E=>E.value==r))==null?void 0:_.label:"",k=u??(r!==void 0&&Number.isInteger(r)?1:.01);let T=1;return p!==void 0&&h!==void 0&&h>p&&(T=(h-p)*.01/k),t.jsxs(t.Fragment,{children:[t.jsxs("div",{ref:y,className:`flex items-center justify-between px-3 py-1.5 border-b border-white/5 hover:bg-white/5 transition-colors ${c?"opacity-30 pointer-events-none":""}`,onMouseEnter:x,onMouseLeave:C,children:[t.jsxs("div",{className:"flex items-center gap-2.5 flex-1 min-w-0",children:[t.jsx("div",{className:`w-1.5 h-1.5 rounded-full flex-shrink-0 transition-all duration-300 ${w}`,title:M}),t.jsxs("span",{className:`text-[10px] font-sans font-medium tracking-tight truncate ${z}`,children:[e," ",l==="pending"&&"*"]})]}),t.jsxs("div",{className:"flex items-center gap-3",children:[s&&i?t.jsxs("div",{className:"relative w-20 h-4 bg-black/40 border border-white/10 rounded-sm hover:border-white/30 transition-colors",children:[t.jsx("select",{className:"absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10",value:r,onChange:E=>i(Number(E.target.value)),children:s.map(E=>t.jsx("option",{value:E.value,children:E.label},E.value))}),t.jsxs("div",{className:"absolute inset-0 flex items-center justify-between px-1.5 pointer-events-none",children:[t.jsx("span",{className:"text-[9px] text-cyan-400 font-mono font-medium truncate pr-1",children:P}),t.jsx("span",{className:"text-[6px] text-gray-500",children:"▼"})]})]}):n&&r!==void 0&&t.jsx("div",{className:"w-10 h-4 bg-black/40 border border-white/10 relative overflow-hidden rounded-sm",children:t.jsx(ct,{value:r,onChange:n,step:k,min:p,max:h,sensitivity:T,highlight:o})}),!d&&t.jsx("input",{type:"checkbox",checked:o,onChange:()=>a(!o),className:`w-3 h-3 appearance-none border rounded-[2px] cursor-pointer transition-colors ${o?l==="pending"?"bg-amber-600 border-amber-500":"bg-cyan-600 border-cyan-500":"bg-black/40 border-gray-600 hover:border-gray-400"}`})]})]}),g&&Pt.createPortal(t.jsx("div",{className:"fixed z-[9999] pointer-events-none flex items-center animate-fade-in",style:{top:b.top,[b.side==="left"?"left":"right"]:b.x,transform:"translateY(-50%)"},children:t.jsxs("div",{className:"bg-black text-white text-[9px] px-2 py-1 rounded border border-white/20 shadow-xl whitespace-nowrap",children:[f,b.side==="left"?t.jsxs(t.Fragment,{children:[t.jsx("div",{className:"absolute top-1/2 -left-1 -translate-y-1/2 border-t-4 border-b-4 border-r-4 border-t-transparent border-b-transparent border-r-white/20"}),t.jsx("div",{className:"absolute top-1/2 -left-[3px] -translate-y-1/2 border-t-[3px] border-b-[3px] border-r-[3px] border-t-transparent border-b-transparent border-r-black"})]}):t.jsxs(t.Fragment,{children:[t.jsx("div",{className:"absolute top-1/2 -right-1 -translate-y-1/2 border-t-4 border-b-4 border-l-4 border-t-transparent border-b-transparent border-l-white/20"}),t.jsx("div",{className:"absolute top-1/2 -right-[3px] -translate-y-1/2 border-t-[3px] border-b-[3px] border-l-[3px] border-t-transparent border-b-transparent border-l-black"})]})]})}),document.body)]})},Ol=ke.lazy(()=>pt(()=>import("./AdvancedGradientEditor-CIUH_wqD.js"),__vite__mapDeps([0,1,2,3,4]),import.meta.url)),ca=(e,o,a,r)=>{if(e.or)return e.or.some(i=>ca(i,o,a,r));if(e.and)return e.and.every(i=>ca(i,o,a,r));let n=e.param||r,s;if(n&&n.startsWith("$")){const i=n.slice(1);if(i.includes(".")){const l=i.split(".");let c=a;for(const d of l){if(c==null){c=void 0;break}c=c[d]}s=c}else s=a[i]}else if(n)s=o[n];else return!0;if(e.eq===void 0&&e.neq===void 0&&e.gt===void 0&&e.lt===void 0&&e.bool===void 0)return typeof s=="boolean"?s:typeof s=="number"?s>0:!!s;if(e.eq!==void 0||e.neq!==void 0){let i=s;if(typeof s=="object"&&s&&s.getHexString&&(i="#"+s.getHexString()),e.eq!==void 0)return i==e.eq;if(e.neq!==void 0)return i!=e.neq}return e.bool!==void 0?!!s===e.bool:e.gt!==void 0?s>e.gt:e.lt!==void 0?s<e.lt:!0},At=(e,o,a,r)=>{if(!e){if(r){const n=o[r];return typeof n=="boolean"?n:typeof n=="number"?n>0:!!n}return!0}return Array.isArray(e)?e.every(n=>ca(n,o,a,r)):ca(e,o,a,r)},$l=e=>{const o=e.min??0,a=e.max??1;if(e.scale==="pi")return{min:o/Math.PI,max:a/Math.PI,toSlider:r=>r/Math.PI,fromSlider:r=>r*Math.PI};if(!(!e.scale||e.scale==="linear")){if(e.scale==="square")return{min:0,max:100,toSlider:r=>Math.sqrt((r-o)/(a-o))*100,fromSlider:r=>o+Math.pow(r/100,2)*(a-o)};if(e.scale==="log"){const r=Math.max(1e-6,o);return{min:0,max:100,toSlider:n=>n<=o?0:(Math.log10(Math.max(r,n))-Math.log10(r))/(Math.log10(a)-Math.log10(r))*100,fromSlider:n=>n<=0?o:Math.pow(10,Math.log10(r)+n/100*(Math.log10(a)-Math.log10(r)))}}}},se=({featureId:e,groupFilter:o,className:a,isDisabled:r=!1,disabledParams:n=[],excludeParams:s=[],whitelistParams:i=[],variant:l="default",forcedState:c,onChangeOverride:d,pendingChanges:f})=>{var H;const p=ne.get(e),h=N(A=>A[e]),u=c||h,g=N(A=>A.liveModulations),S=ke.useRef(N.getState());S.current=N.getState();const y=S.current,b=y,m=N(A=>A.advancedMode),x=N(A=>A.openContextMenu),C=N(A=>A.showHints),[z,w]=v.useState(null),[M,P]=v.useState(new Set),k=v.useMemo(()=>`set${e.charAt(0).toUpperCase()+e.slice(1)}`,[e]),T=(A,j)=>{if(r||n.includes(A))return;if(d){d(A,j);return}const B=p.params[A];if((B==null?void 0:B.onUpdate)==="compile"){N.getState().movePanel("Engine","left"),setTimeout(()=>Z.emit("engine_queue",{featureId:e,param:A,value:j}),50);return}if(B!=null&&B.confirmation&&j===!0&&u[A]===!1){w({key:A,value:j,message:B.confirmation});return}const I=b[k];if(I)if(B!=null&&B.composeFrom&&j&&typeof j=="object"){const V=B.composeFrom,W="z"in j?{[V[0]]:j.x,[V[1]]:j.y,[V[2]]:j.z}:{[V[0]]:j.x,[V[1]]:j.y};I(W)}else I({[A]:j})},_=()=>{if(!z)return;if(d){d(z.key,z.value),w(null);return}const A=b[k];A&&(Z.emit("is_compiling","Optimizing Shader..."),setTimeout(()=>{A({[z.key]:z.value}),w(null)},50))},E=A=>{if(r)return;const j=Ge(A.currentTarget);j.length>0&&(A.preventDefault(),A.stopPropagation(),x(A.clientX,A.clientY,[],j))},F=(A,j)=>{if(!(r||n.includes(j))&&A.target.files&&A.target.files[0]){const B=new FileReader;B.onload=I=>{var V;(V=I.target)!=null&&V.result&&T(j,I.target.result)},B.readAsDataURL(A.target.files[0])}};if(!p||!u)return null;const O=(A,j)=>{var V,W,q,Q,K,ie;let B;if(j.composeFrom){const ee=j.composeFrom;ee.length===3?B=new U(u[ee[0]]??0,u[ee[1]]??0,u[ee[2]]??0):ee.length===2&&(B=new je(u[ee[0]]??0,u[ee[1]]??0))}else B=u[A]??j.default;const I=r||n.includes(A);if(l==="dense"){let ee="runtime";if(j.onUpdate==="compile"&&(ee=f&&f[`${e}.${A}`]!==void 0?"pending":"synced"),j.type==="boolean")return t.jsx(Ua,{label:j.label,description:j.description,isActive:!!B,onToggle:oe=>T(A,oe),status:ee,disabled:I});if(j.type==="float"||j.type==="int")return t.jsx(Ua,{label:j.label,description:j.description,isActive:!0,onToggle:()=>{},numericValue:B,onNumericChange:oe=>T(A,oe),options:j.options,onOptionChange:j.options?oe=>T(A,oe):void 0,status:ee,disabled:I,hideCheckbox:!0,step:j.step,min:j.min,max:j.max})}if(j.type==="color"){let ee=B;return typeof B=="object"&&B.getHexString&&(ee="#"+B.getHexString()),j.layout==="embedded"||j.parentId?t.jsx("div",{className:`mb-px pr-1 ${I?"opacity-30 pointer-events-none":""}`,children:t.jsx(ga,{color:ee,onColorChange:oe=>T(A,oe)})}):t.jsxs("div",{className:`flex items-center justify-between px-3 py-1 bg-gray-800/20 mb-px ${I?"opacity-30 pointer-events-none":""}`,children:[t.jsx(_e,{children:j.label}),t.jsx(sa,{color:ee,onChange:oe=>T(A,oe),label:j.label})]})}if(j.type==="boolean"){const ee=j.onUpdate==="compile"?t.jsx("span",{className:"ml-1.5",title:B?"Compiled & Active":"Compiled Off — toggle to queue change",children:t.jsx(la,{status:B?"active":"off"})}):null;return j.ui==="checkbox"?t.jsx("div",{className:I?"opacity-30 pointer-events-none":"",children:t.jsx(Ae,{label:j.label,value:B,onChange:oe=>T(A,oe),disabled:I,variant:"dense",labelSuffix:ee})}):t.jsx("div",{children:t.jsx(Ae,{label:j.label,value:B,onChange:oe=>T(A,oe),options:j.options,disabled:I,labelSuffix:ee})})}if(j.type==="float"||j.type==="int"){const ee=j.onUpdate==="compile"?t.jsx("span",{className:"ml-1.5",title:"Compile-time setting — changes queue to Engine Panel",children:t.jsx(la,{status:"active"})}):null;if(j.options)return t.jsx("div",{className:`mb-px ${I?"opacity-30 pointer-events-none":""}`,children:t.jsx(kt,{label:j.label,value:B,onChange:Se=>T(A,Se),options:j.options,fullWidth:!0,labelSuffix:ee})});if(j.ui==="knob")return t.jsx("div",{className:j.layout==="half"?"flex flex-col items-center justify-center py-2":"flex justify-center p-2",children:t.jsx(Mt,{label:j.label,value:B,min:j.min??0,max:j.max??1,step:j.step,onChange:Se=>T(A,Se),color:B>(j.min??0)?"#22d3ee":"#444",size:40})});const oe=$l(j);let le=j.format?j.format(B):void 0;j.scale==="pi"&&(le=`${(B/Math.PI).toFixed(2)}π`);let ce=j.max??1;j.dynamicMaxRef&&u[j.dynamicMaxRef]!==void 0&&(ce=u[j.dynamicMaxRef]);const pe=`${e}.${A}`,he=g[pe],Te=B!==j.default||!!j.condition,Ne=j.condition?"!animate-none !overflow-visible":"";return t.jsx("div",{children:t.jsx(me,{label:j.label,value:B,min:j.min??0,max:ce,step:j.step??.01,onChange:Se=>T(A,Se),highlight:Te,trackId:pe,liveValue:he,defaultValue:j.default,customMapping:oe,overrideInputText:le,mapTextInput:j.scale==="pi",disabled:I,labelSuffix:ee,className:Ne})})}if(j.type==="vec2"){const ee=(B==null?void 0:B.x)??((V=j.default)==null?void 0:V.x)??0,oe=(B==null?void 0:B.y)??((W=j.default)==null?void 0:W.y)??0;return t.jsx("div",{className:`mb-px ${I?"opacity-30 pointer-events-none":""}`,children:t.jsx(Nr,{label:j.label,value:new je(ee,oe),min:j.min??-1,max:j.max??1,onChange:le=>T(A,{x:le.x,y:le.y}),mode:j.mode,scale:j.scale,linkable:j.linkable})})}if(j.type==="vec3"){const ee=(B==null?void 0:B.x)??((q=j.default)==null?void 0:q.x)??0,oe=(B==null?void 0:B.y)??((Q=j.default)==null?void 0:Q.y)??0,le=(B==null?void 0:B.z)??((K=j.default)==null?void 0:K.z)??0,ce=new U(ee,oe,le),pe=j.composeFrom?j.composeFrom.map(Te=>`${e}.${Te}`):[`${e}.${A}_x`,`${e}.${A}_y`,`${e}.${A}_z`],he=j.composeFrom?void 0:[`${j.label} X`,`${j.label} Y`,`${j.label} Z`];return t.jsx("div",{className:`mb-px ${I?"opacity-30 pointer-events-none":""}`,children:t.jsx($t,{label:j.label,value:ce,min:j.min??-10,max:j.max??10,step:j.step,onChange:Te=>T(A,Te),disabled:I,trackKeys:pe,trackLabels:he,mode:j.mode,scale:j.scale,linkable:j.linkable})})}if(j.type==="image"){const ee=((ie=j.linkedParams)==null?void 0:ie.colorSpace)||"colorSpace",oe=p.params[ee],le=u[ee],ce=()=>{if(oe&&typeof le=="number"){const he=(le+1)%3;T(ee,he)}},pe=le===1?"LIN":le===2?"ACES":"sRGB";return t.jsx("div",{className:`mb-px ${I?"opacity-30 pointer-events-none":""}`,children:t.jsxs("div",{className:"bg-gray-800/30 border border-white/5 text-center overflow-hidden relative group",children:[t.jsx("input",{type:"file",accept:"image/*,.hdr,.exr",onChange:he=>F(he,A),className:"hidden",id:`file-input-${A}`}),t.jsx("label",{htmlFor:`file-input-${A}`,className:"block bg-cyan-900/40 hover:bg-cyan-800/60 text-cyan-300 w-full py-2 text-xs font-bold transition-colors cursor-pointer",children:B?"Replace Texture":j.label}),oe&&t.jsx("div",{className:"absolute right-1 top-1/2 -translate-y-1/2 text-[8px] font-bold text-gray-500 bg-black/50 px-1.5 py-0.5 rounded cursor-pointer hover:text-white hover:bg-cyan-900/80 transition-colors select-none",onClick:he=>{he.preventDefault(),ce()},title:"Input Color Profile: sRGB / Linear / ACES",children:pe})]})})}return j.type==="gradient"?t.jsx("div",{className:`pr-1 ${I?"opacity-30 pointer-events-none":""}`,children:t.jsx(v.Suspense,{fallback:null,children:t.jsx(Ol,{value:B,onChange:ee=>T(A,ee)})})}):null},$=(A,j=!1)=>{var oe,le;const B=p.params[A];if(!B||B.hidden||s.includes(A)||!At(B.condition,u,y,B.parentId)||B.isAdvanced&&!m)return null;const I=O(A,B),V=Object.keys(p.params).filter(ce=>p.params[ce].parentId===A),W=V.map(ce=>$(ce)).filter(Boolean);(oe=p.customUI)==null||oe.forEach(ce=>{if(ce.parentId!==A||o&&ce.group!==o||!At(ce.condition,u,y,ce.parentId))return;const pe=ve.get(ce.componentId);pe&&W.push(t.jsx("div",{children:t.jsx(pe,{featureId:e,sliceState:u,actions:b,...ce.props})},`custom-${ce.componentId}`))});const q=j?"flex-1 min-w-0":"flex flex-col",Q=((le=p.customUI)==null?void 0:le.some(ce=>ce.parentId===A))??!1,K=V.length>0||Q,ie=C&&B.description&&!r&&l!=="dense"&&(B.type!=="boolean"||(u==null?void 0:u[A]));ie&&K&&W.unshift(t.jsx("div",{children:t.jsx("p",{className:"px-3 py-1.5 text-[9px] text-gray-600 leading-tight bg-white/[0.06] hover:text-gray-300 transition-colors cursor-default",children:B.description})},`desc-${A}`));const ee=W.length>0;return t.jsxs("div",{className:`w-full ${q} ${K?"rounded-t-sm relative":""}`,children:[K&&t.jsx("div",{className:`absolute inset-0 bg-white/[0.06] rounded-t-sm pointer-events-none transition-opacity ${ee?"opacity-100":"opacity-0"}`}),I,ie&&!K&&t.jsx("p",{className:"px-3 py-1.5 text-[9px] text-gray-600 leading-tight bg-white/[0.06] hover:text-gray-300 transition-colors cursor-default",children:B.description}),W.length>0&&t.jsxs("div",{className:"flex flex-col",children:[W.map((ce,pe)=>{const he=pe===W.length-1;return t.jsxs("div",{className:"flex",children:[t.jsx("div",{className:`w-2 shrink-0 self-stretch border-l border-white/20 bg-white/[0.12] ${he?"border-b border-b-white/20 rounded-bl-lg":""}`}),t.jsxs("div",{className:`flex-1 min-w-0 relative ${he?"border-b border-b-white/20":""}`,children:[t.jsx("div",{className:"absolute inset-0 bg-black/20 pointer-events-none z-10"}),ce]})]},pe)}),t.jsx("div",{className:"h-2",style:{background:"linear-gradient(to bottom, transparent, rgba(255,255,255,0.08))"}})]})]},A)},R=Object.keys(p.params).filter(A=>p.params[A].parentId?!1:i&&i.length>0?i.includes(A):o?p.params[A].group===o:!0),Y=A=>{const j=[];for(let B=0;B<A.length;B++){const I=A[B],V=p.params[I];if(!(V.hidden||s.includes(I)||!At(V.condition,u,y))){if(V.layout==="half"&&l!=="dense"){let W=A[B+1],q=W?p.params[W]:null;if(q&&q.layout==="half"&&!q.hidden&&!s.includes(W)&&At(q.condition,u,y)){j.push(t.jsxs("div",{className:"flex gap-0.5 mb-px",children:[$(I,!0),$(W,!0)]},`${I}-${W}`)),B++;continue}}j.push($(I))}}return j},L=A=>{P(j=>{const B=new Set(j);return B.has(A)?B.delete(A):B.add(A),B})},G=p.groups,X=G&&!o&&!(i!=null&&i.length)&&Object.values(G).some(A=>A.collapsible),D=[];if(X&&G){const A=[],j={},B=[];for(const I of R){const V=p.params[I].group;V&&G[V]?(j[V]||(j[V]=[],A.push(V)),j[V].push(I)):B.push(I)}D.push(...Y(B));for(const I of A){const V=G[I],W=j[I];M.has(I);const q=Y(W);if(!q.every(Q=>Q===null))if(V.collapsible){const Q=q.filter(Boolean);D.push(t.jsx(jt,{label:V.label,open:!M.has(I),onToggle:()=>L(I),defaultOpen:!0,variant:"panel",children:t.jsxs("div",{className:"flex flex-col",children:[Q.map((K,ie)=>t.jsx("div",{children:K},ie)),t.jsx("div",{className:"ml-[9px] border-b border-white/10 rounded-bl mb-0.5"})]})},`group-${I}`))}else D.push(...q)}}else D.push(...Y(R));return(H=p.customUI)==null||H.forEach(A=>{if(i&&i.length>0||A.parentId||o&&A.group!==o||!At(A.condition,u,y))return;const j=ve.get(A.componentId);j&&D.push(t.jsx("div",{className:`flex flex-col mb-px ${r?"grayscale opacity-30 pointer-events-none":""}`,children:t.jsx(j,{featureId:e,sliceState:u,actions:b,...A.props})},`custom-${A.componentId}`))}),t.jsxs("div",{className:`flex flex-col relative ${a||""}`,onContextMenu:E,children:[D,z&&t.jsx("div",{className:"absolute inset-0 z-50 animate-pop-in",children:t.jsxs("div",{className:"bg-black/95 border border-white/20 rounded shadow-2xl overflow-hidden h-full flex flex-col",children:[t.jsxs("div",{className:"flex items-center gap-2 p-2 border-b border-white/10 bg-white/5",children:[t.jsx(Rt,{}),t.jsx(_e,{color:"text-gray-300",children:"Warning"})]}),t.jsxs("div",{className:"p-3 flex-1 flex flex-col justify-between",children:[t.jsx("p",{className:"text-[10px] text-gray-400 leading-relaxed whitespace-pre-wrap",children:z.message}),t.jsxs("div",{className:"flex gap-1 mt-4",children:[t.jsx("button",{onClick:()=>w(null),className:"flex-1 py-1.5 bg-gray-800 text-gray-300 text-[9px] font-bold rounded border border-white/10 hover:bg-gray-700 transition-colors",children:"Cancel"}),t.jsx("button",{onClick:_,className:"flex-1 py-1.5 bg-cyan-900/50 text-cyan-300 text-[9px] font-bold rounded border border-cyan-500/30 hover:bg-cyan-900 transition-colors",children:"Confirm"})]})]})]})})]})},Hl=()=>{const e=N(n=>{var s;return(s=n.lighting)==null?void 0:s.shadows}),o=N(n=>n.setLighting),a=N(n=>n.handleInteractionStart),r=N(n=>n.handleInteractionEnd);return t.jsx(vt,{width:"w-52",children:t.jsxs("div",{className:"relative space-y-2",children:[t.jsxs("div",{className:"flex items-center justify-between border-b border-white/10 pb-2 px-1",children:[t.jsx(_e,{children:"Shadows"}),t.jsx("button",{onClick:()=>{a("param"),o({shadows:!e}),r()},className:`text-[9px] font-bold px-2 py-0.5 rounded border transition-colors ${e?"bg-yellow-500/20 text-yellow-300 border-yellow-500/50":"bg-gray-800 text-gray-500 border-gray-600"}`,children:e?"Enabled":"Disabled"})]}),e&&t.jsx("div",{className:"space-y-1",children:t.jsx(se,{featureId:"lighting",groupFilter:"shadows"})})]})})},Gl=({isMobileMode:e,vibrate:o})=>{var E;const a=N(),r=a.lighting,{openContextMenu:n,handleInteractionStart:s,handleInteractionEnd:i}=N(),[l,c]=v.useState(null),[d,f]=v.useState(null),[p,h]=v.useState(!1),[u,g]=v.useState(!1),S=v.useRef(null),y=v.useRef(null),b=v.useRef(null),m=v.useRef(null);v.useEffect(()=>{const F=O=>{const $=O.target;y.current&&!y.current.contains($)&&!$.closest(".shadow-toggle-btn")&&h(!1),e&&d!==null&&!$.closest(".light-orb-wrapper")&&f(null),u&&m.current&&!m.current.contains($)&&!$.closest(".expand-trigger")&&g(!1)};return document.addEventListener("mousedown",F),document.addEventListener("touchstart",F),()=>{document.removeEventListener("mousedown",F),document.removeEventListener("touchstart",F)}},[d,e,u]);const x=(F,O)=>{F.preventDefault(),F.stopPropagation(),n(F.clientX,F.clientY,[],O)},C=(F,O)=>{F.preventDefault(),F.stopPropagation();const $=bt(a.lighting,O),R=[{label:`Light ${O+1}`,isHeader:!0,action:()=>{}},{label:"Enabled",checked:$.visible,action:()=>{s("param"),a.updateLight({index:O,params:{visible:!$.visible}}),i()}},{label:"Type",isHeader:!0,action:()=>{}},{label:"Point",checked:$.type==="Point",action:()=>{s("param"),a.updateLight({index:O,params:{type:"Point"}}),i()}},{label:"Directional (Sun)",checked:$.type==="Directional",action:()=>{s("param"),a.updateLight({index:O,params:{type:"Directional"}}),i()}}];n(F.clientX,F.clientY,R,["panel.light"])},z=F=>{const O=bt(a.lighting,F);if(!e){o(5),s("param"),a.updateLight({index:F,params:{visible:!O.visible}}),i();return}O.visible?d!==F?(o(5),f(F)):(o([10,30,10]),s("param"),a.updateLight({index:F,params:{visible:!1}}),i(),f(null)):(o(10),s("param"),a.updateLight({index:F,params:{visible:!0}}),i(),f(null))},w=(F,O)=>{e||O.nativeEvent.pointerType==="touch"||(S.current&&clearTimeout(S.current),c(F))},M=()=>{e||(S.current=window.setTimeout(()=>{c(null)},400))},P=F=>{o(5);const O=T[F];a.setDraggedLight((O==null?void 0:O.id)??null),e||(f(null),c(null))},k=()=>{a.addLight()},T=((E=a.lighting)==null?void 0:E.lights)||[],_=F=>{const O=T[F];return O?t.jsxs("div",{className:"relative light-orb-wrapper flex justify-center w-8 h-8",onMouseEnter:$=>w(F,$),onMouseLeave:M,onContextMenu:$=>C($,F),children:[t.jsx(Tl,{index:F,color:O.color,active:O.visible,type:O.type,rotation:O.rotation,onClick:()=>z(F),onDragStart:()=>P(F)}),a.draggedLightIndex!==O.id&&(l===F||d===F)&&t.jsx(Rl,{index:F})]},F):F<Ce?t.jsx("div",{className:"flex justify-center items-center w-8 h-8",children:t.jsx("button",{onClick:k,className:"w-8 h-8 rounded-full border border-white/10 flex items-center justify-center text-gray-500 hover:text-cyan-400 hover:border-cyan-500/50 hover:bg-white/5 transition-all",title:"Add Light",children:t.jsx(ha,{})})},F):t.jsx("div",{className:"w-8 h-8"},F)};return t.jsxs("div",{ref:b,className:"absolute left-1/2 -translate-x-1/2 flex items-center bg-white/5 pr-2 pl-6 py-1.5 rounded-full border border-white/5 shadow-inner z-[65]",children:[t.jsxs("div",{className:"relative",children:[t.jsxs("div",{className:`flex items-center gap-6 transition-opacity duration-200 ${u?"opacity-0 pointer-events-none":"opacity-100"}`,children:[[0,1,2].map(F=>_(F)),t.jsx("button",{onClick:()=>{o(5),g(!0)},className:"expand-trigger w-5 h-5 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white transition-colors ml-[-8px]",title:"Expand Light Studio",children:t.jsx(Tt,{})})]}),u&&t.jsx("div",{ref:m,className:"absolute top-[-20px] left-[-20px] bg-black/95 border border-white/20 p-5 rounded-2xl shadow-2xl animate-fade-in z-[80]",children:t.jsxs("div",{className:"grid grid-cols-3 gap-6",children:[Array.from({length:8}).map((F,O)=>_(O)),t.jsx("div",{className:"flex justify-center items-center w-8 h-8",children:t.jsx("button",{onClick:()=>g(!1),className:"w-8 h-8 rounded-full border border-white/10 flex items-center justify-center text-gray-500 hover:text-white hover:bg-white/10 transition-colors",title:"Collapse",children:t.jsx(Us,{})})})]})})]}),t.jsx("div",{className:"h-6 w-px bg-white/10 mx-4"}),t.jsxs("div",{className:"flex items-center gap-2",children:[t.jsxs("div",{className:"relative",ref:y,children:[t.jsx("button",{onClick:F=>{F.stopPropagation(),o(5),h(!p)},onContextMenu:F=>x(F,["shadows"]),className:`shadow-toggle-btn p-2 rounded-full border transition-all duration-300 ${r!=null&&r.shadows?"bg-yellow-500/10 border-yellow-500/50 text-yellow-300 shadow-[0_0_10px_rgba(234,179,8,0.1)]":"bg-transparent border-transparent text-gray-600 hover:text-gray-300 hover:bg-white/5"}`,title:"Shadow Settings",children:t.jsx(il,{})}),p&&t.jsx(Hl,{})]}),t.jsx("button",{onClick:()=>{o(5),s("param"),a.setShowLightGizmo(!a.showLightGizmo),i()},onContextMenu:F=>x(F,["ui.viewport"]),className:`p-2 rounded-full border transition-all duration-300 ${a.showLightGizmo?"bg-cyan-500/20 border-cyan-500 text-cyan-300 shadow-[0_0_10px_rgba(34,211,238,0.2)]":"bg-transparent border-transparent text-gray-600 hover:text-gray-300 hover:bg-white/5"}`,children:t.jsx(rl,{})})]})]})},Br=(e,o,a,r)=>{const n=e.replace(/[^a-zA-Z0-9 \-_]/g,"").trim().replace(/\s+/g,"_")||"Untitled",s=`v${o}`;return`${["GMT",n,s].join("_")}.${a}`},Eo=xe(),Vl=({isMobileMode:e,vibrate:o,btnBase:a,btnActive:r,btnInactive:n})=>{const s=N(),{movePanel:i}=s,[l,c]=v.useState(!1),[d,f]=v.useState(!1),p=v.useRef(null),h=v.useRef(null),u=async()=>{if(Eo.isBooted){c(!1);try{const m=await Eo.captureSnapshot();if(f(!0),!m){console.error("Snapshot generation returned null.");return}const x=s.getPreset({includeScene:!0}),C=JSON.stringify(x),z=s.prepareExport(),w=Br(s.projectSettings.name,z,"png");try{const M=await pr(m,"FractalData",C),P=URL.createObjectURL(M),k=document.createElement("a");k.download=w,k.href=P,k.click(),URL.revokeObjectURL(P),o(50)}catch(M){console.error("Metadata injection failed, saving raw image",M);const P=URL.createObjectURL(m),k=document.createElement("a");k.download=w,k.href=P,k.click(),URL.revokeObjectURL(P)}}catch(m){console.error("Snapshot failed",m)}finally{f(!1)}}},g=m=>{m.stopPropagation(),e?(o(5),c(!l)):u()},S=()=>{e||(p.current&&clearTimeout(p.current),c(!0))},y=()=>{e||(p.current=window.setTimeout(()=>{c(!1)},200))},b=()=>{o(5),i("Camera Manager","left"),c(!1)};return v.useEffect(()=>{if(!e||!l)return;const m=x=>{const C=x.target;h.current&&!h.current.contains(C)&&!C.closest(".camera-menu-trigger")&&c(!1)};return document.addEventListener("mousedown",m),document.addEventListener("touchstart",m),()=>{document.removeEventListener("mousedown",m),document.removeEventListener("touchstart",m)}},[l,e]),t.jsxs(t.Fragment,{children:[d&&t.jsx("div",{className:"fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in",children:t.jsxs("div",{className:"bg-gray-900 border border-cyan-500/50 rounded-xl px-6 py-4 shadow-2xl flex items-center gap-3",children:[t.jsx("div",{className:"w-5 h-5 border-2 border-cyan-400 border-t-transparent rounded-full animate-spin"}),t.jsx("span",{className:"text-cyan-300 font-bold text-sm",children:"Capturing..."})]})}),t.jsxs("div",{className:"relative",ref:h,onMouseEnter:S,onMouseLeave:y,children:[t.jsx("button",{onClick:g,className:`camera-menu-trigger ${a} ${l?r:n}`,title:e?"Camera Menu":"Click: Take Snapshot / Hover: Camera Menu",children:t.jsx(Ro,{})}),l&&t.jsxs(vt,{width:"w-48",align:"end",children:[t.jsx("div",{className:"px-2 py-1 text-[10px] font-bold text-gray-500 border-b border-white/10 mb-1",children:"Camera Tools"}),t.jsxs("div",{className:"space-y-1",children:[t.jsxs("button",{onClick:()=>{o(5),s.undoCamera()},disabled:s.undoStack.length===0,className:"w-full flex items-center justify-between p-2 rounded hover:bg-white/10 disabled:opacity-30 text-xs text-gray-300 text-left",children:[t.jsxs("span",{className:"flex items-center gap-2",children:[t.jsx(Zs,{})," Undo Move"]}),t.jsx("kbd",{className:"text-[8px] text-gray-500 bg-gray-800 px-1 rounded",children:"Ctrl+Shift+Z"})]}),t.jsxs("button",{onClick:()=>{o(5),s.redoCamera()},disabled:s.redoStack.length===0,className:"w-full flex items-center justify-between p-2 rounded hover:bg-white/10 disabled:opacity-30 text-xs text-gray-300 text-left",children:[t.jsxs("span",{className:"flex items-center gap-2",children:[t.jsx(Qs,{})," Redo Move"]}),t.jsx("kbd",{className:"text-[8px] text-gray-500 bg-gray-800 px-1 rounded",children:"Ctrl+Shift+Y"})]}),t.jsxs("button",{onClick:()=>{o(30),s.resetCamera(),s.setShowLightGizmo(!1)},className:"w-full flex items-center gap-2 p-2 rounded hover:bg-white/10 text-xs text-gray-300 text-left",children:[t.jsx(Ks,{})," Reset Position"]}),t.jsx("div",{className:"h-px bg-white/10 my-1"}),t.jsxs("button",{onClick:b,className:"w-full flex items-center gap-2 p-2 rounded hover:bg-white/10 text-xs text-cyan-300 text-left",children:[t.jsx(to,{})," Camera Manager"]}),t.jsx("div",{className:"h-px bg-white/10 my-1"}),t.jsxs("button",{onClick:u,className:"w-full flex items-center gap-2 p-2 rounded hover:bg-cyan-900/50 text-xs text-cyan-400 font-bold text-left",title:"Save PNG with embedded scene data",children:[t.jsx(Ro,{})," Take Snapshot"]})]})]})]})]})},Ao=xe(),Ul=({isMobileMode:e,vibrate:o,btnBase:a,btnActive:r,btnInactive:n})=>{const s=N(),i=s,[l,c]=v.useState(!1),[d,f]=v.useState(!1),[p,h]=v.useState(""),[u,g]=v.useState(null),[S,y]=v.useState(null),b=v.useRef(null),m=v.useRef(null),x=ne.getMenuFeatures(),C=ne.getExtraMenuItems(),z=vr(s),w=z.charAt(0).toUpperCase()+z.slice(1);v.useEffect(()=>{const L=Ao.gpuInfo;if(L)h(L);else{const G=setTimeout(()=>{h(Ao.gpuInfo||"Generic WebGL Device")},3e3);return()=>clearTimeout(G)}},[]),v.useEffect(()=>{const L=G=>{const X=G.target;if(b.current&&!b.current.contains(X)){if(X.closest(".portal-dropdown-content")||X.closest(".t-dropdown"))return;c(!1),f(!1)}};return l&&(document.addEventListener("mousedown",L),document.addEventListener("touchstart",L)),()=>{document.removeEventListener("mousedown",L),document.removeEventListener("touchstart",L)}},[l]);const M=L=>{L.stopPropagation(),o(5),c(!l)},P=()=>{const L=s.prepareExport(),G=s.projectSettings,X=s.getPreset(),D=new Blob([JSON.stringify(X,null,2)],{type:"application/json"}),H=URL.createObjectURL(D),A=document.createElement("a");A.href=H,A.download=Br(G.name,L,"json"),A.click(),URL.revokeObjectURL(H)},k=()=>{var L;return(L=m.current)==null?void 0:L.click()},T=async L=>{var X;const G=(X=L.target.files)==null?void 0:X[0];if(G){Z.emit("is_compiling","Processing..."),await new Promise(D=>setTimeout(D,50));try{let D="";if(G.type==="image/png"){const A=await hr(G,"FractalData");if(A)D=A;else throw new Error("No Fractal Data found in this image.")}else D=await G.text();const H=JSON.parse(D);s.loadPreset(H),o(50),c(!1)}catch(D){console.error("Load Failed:",D),Z.emit("is_compiling",!1),y("Error!"),setTimeout(()=>y(null),2e3),alert("Could not load preset. "+(D instanceof Error?D.message:String(D)))}L.target.value=""}},_=()=>{let G=s.getShareString({includeAnimations:!0}),X="";if(G.length>4096){const H=s.getShareString({includeAnimations:!1});H.length<G.length&&H.length<4096?(G=H,X=" (Anims Removed)"):X=" (Long URL)"}const D=`${window.location.origin}${window.location.pathname}#s=${G}`;navigator.clipboard.writeText(D).then(()=>{g(`Copied!${X}`),o(50),setTimeout(()=>g(null),2500)})},E=(L,G,X)=>{var j,B;const D=ne.get(L);if(((j=D==null?void 0:D.engineConfig)==null?void 0:j.mode)==="compile"&&((B=D.params[G])==null?void 0:B.onUpdate)==="compile"){s.movePanel("Engine","left"),setTimeout(()=>Z.emit("engine_queue",{featureId:L,param:G,value:X}),50);return}const H=`set${L.charAt(0).toUpperCase()+L.slice(1)}`,A=s[H];if(A&&(A({[G]:X}),D!=null&&D.tabConfig)){const I=D.tabConfig.label;L==="engineSettings"?X&&s.movePanel(I,"left"):X?s.floatTab(I):s.dockTab(I)}},F=(L,G=!1)=>{const X=i[L.id||L.featureId];if(!X)return null;const D=!!X[L.toggleParam];L.id;const H=L.id==="audio"?"text-green-400":"text-cyan-400";if(G){const j={Code:t.jsx(ia,{}),Info:t.jsx(jr,{})},B=L.icon?j[L.icon]:null;return t.jsxs("button",{onClick:I=>{I.stopPropagation(),o(5),E(L.featureId,L.toggleParam,!D),c(!1)},className:`w-full flex items-center justify-between p-2 rounded transition-colors group ${D?"bg-white/10 text-cyan-400":"hover:bg-white/5 text-gray-300"}`,children:[t.jsx("span",{className:"text-xs font-bold",children:L.label}),B]},`${L.featureId}-${L.toggleParam}`)}const A=D?L.id==="audio"?"bg-green-500/30 text-green-300 border-green-500/40":"bg-cyan-500/30 text-cyan-300 border-cyan-500/40":"bg-white/[0.04] text-gray-600 border-white/5";return t.jsxs("div",{className:"flex items-center justify-between p-2 rounded hover:bg-white/5 cursor-pointer animate-fade-in-left",onClick:()=>{o(5),E(L.id,L.toggleParam,!D)},children:[t.jsx("span",{className:`text-xs font-bold ${D?H:"text-gray-300"}`,children:L.label}),t.jsx("span",{className:`px-2 py-0.5 text-[8px] font-bold rounded-sm border transition-all ${A}`,children:D?"ON":"OFF"})]},L.id)},O=L=>{o(10),Z.emit("is_compiling","Switching Profile..."),setTimeout(()=>{s.applyPreset({mode:L.toLowerCase(),actions:s})},10)},$=x.filter(L=>!L.advancedOnly),R=x.filter(L=>L.advancedOnly),Y=C.filter(L=>L.advancedOnly);return t.jsxs(t.Fragment,{children:[!e&&t.jsxs(t.Fragment,{children:[t.jsxs("button",{onClick:_,className:`${a} ${n} relative`,title:"Copy Share Link",children:[t.jsx(Va,{}),u&&t.jsx("div",{className:"absolute top-full mt-2 left-1/2 -translate-x-1/2 px-2 py-1 bg-green-600 text-white text-[9px] font-bold rounded whitespace-nowrap animate-fade-in",children:u})]}),t.jsx("button",{onClick:P,className:`${a} ${n}`,title:"Save Preset (JSON)",children:t.jsx(Ha,{})}),t.jsxs("button",{onClick:k,className:`${a} ${n} relative`,title:"Load Preset (JSON or PNG)",children:[t.jsx(Ga,{}),S&&t.jsx("div",{className:"absolute top-full mt-2 left-1/2 -translate-x-1/2 px-2 py-1 bg-blue-600 text-white text-[9px] font-bold rounded whitespace-nowrap animate-fade-in",children:S})]})]}),t.jsx("input",{ref:m,type:"file",accept:".json,.png",className:"hidden",onChange:T}),t.jsxs("div",{className:"relative",ref:b,children:[t.jsx("button",{onClick:M,className:`${a} ${l?r:n}`,children:t.jsx(Ys,{})}),l&&t.jsx(vt,{width:"w-64",align:"end",className:"p-2 custom-scroll overflow-y-auto max-h-[85vh]",onClose:()=>M({}),children:t.jsxs("div",{className:"space-y-1",children:[t.jsxs("button",{onClick:L=>{L.stopPropagation(),_()},className:"w-full flex items-center justify-between p-2 rounded hover:bg-white/5 text-gray-300 transition-colors group",children:[t.jsx("span",{className:`text-xs font-bold ${u?"text-green-400":"group-hover:text-white"}`,children:u||"Copy Share Link"}),t.jsx(Va,{active:!!u})]}),t.jsx("div",{className:"h-px bg-white/10 my-1"}),e&&t.jsxs(t.Fragment,{children:[t.jsxs("button",{onClick:L=>{L.stopPropagation(),M(L),P()},className:"w-full flex items-center justify-between p-2 rounded hover:bg-white/5 text-gray-300 transition-colors group border-b border-white/10 mb-1",children:[t.jsx("span",{className:"text-xs font-bold group-hover:text-white",children:"Save Preset"}),t.jsx(Ha,{})]}),t.jsxs("button",{onClick:L=>{L.stopPropagation(),M(L),k()},className:"w-full flex items-center justify-between p-2 rounded hover:bg-white/5 text-gray-300 transition-colors group border-b border-white/10 mb-1",children:[t.jsx("span",{className:"text-xs font-bold group-hover:text-white",children:"Load Preset"}),t.jsx(Ga,{})]}),t.jsx("div",{className:"h-px bg-white/10 my-1"})]}),$.map(L=>F(L)),t.jsx("div",{className:"h-px bg-white/10 my-1"}),F({id:"engineSettings",toggleParam:"showEngineTab",label:"Engine Settings"}),t.jsx("div",{className:"px-2 mb-1 mt-0.5",children:t.jsx(kt,{value:w,onChange:O,selectClassName:"!text-left pl-2",options:[{label:"Fastest (Bare)",value:"Fastest"},{label:"Lite (Fast)",value:"Lite"},{label:"Balanced",value:"Balanced"},{label:"Ultra",value:"Ultra"},{label:"---",value:"Custom"}],fullWidth:!0})}),t.jsxs("button",{onClick:L=>{L.stopPropagation(),o(5),s.openWorkshop(),c(!1)},className:"w-full flex items-center justify-between p-2 rounded hover:bg-white/5 text-gray-300 transition-colors group",children:[t.jsx("span",{className:"text-xs font-bold group-hover:text-purple-400",children:"Formula Workshop"}),t.jsx(ia,{})]}),t.jsxs("button",{onClick:L=>{L.stopPropagation(),s.setIsBroadcastMode(!0)},className:"w-full flex items-center justify-between p-2 rounded hover:bg-white/5 text-gray-300 transition-colors group",children:[t.jsxs("span",{className:"text-xs font-bold group-hover:text-cyan-400",children:["Hide Interface ",t.jsx("span",{className:"text-gray-500 font-normal",children:"[B]"})]}),t.jsx(Ws,{})]}),t.jsxs("div",{className:"flex items-center justify-between p-2 rounded hover:bg-white/5 cursor-pointer",onClick:()=>{o(5),s.setInvertY(!s.invertY)},children:[t.jsx("span",{className:"text-xs text-gray-300 font-bold",children:"Invert Look Y"}),t.jsx("span",{className:`px-2 py-0.5 text-[8px] font-bold rounded-sm border transition-all ${s.invertY?"bg-cyan-500/30 text-cyan-300 border-cyan-500/40":"bg-white/[0.04] text-gray-600 border-white/5"}`,children:s.invertY?"ON":"OFF"})]}),t.jsx("div",{className:"h-px bg-white/10 my-1"}),t.jsxs("div",{className:"flex items-center justify-between p-2 rounded hover:bg-white/5 cursor-pointer",title:"Keyboard: ` (tilde)",onClick:()=>s.setAdvancedMode(!s.advancedMode),children:[t.jsxs("span",{className:"text-xs text-gray-300 font-bold",children:["Advanced Mode ",t.jsx("span",{className:"text-gray-500 font-normal",children:"[`]"})]}),t.jsx("span",{className:`px-2 py-0.5 text-[8px] font-bold rounded-sm border transition-all ${s.advancedMode?"bg-purple-500/30 text-purple-300 border-purple-500/40":"bg-white/[0.04] text-gray-600 border-white/5"}`,children:s.advancedMode?"ON":"OFF"})]}),s.advancedMode&&t.jsxs("div",{className:"mt-1 pl-2 border-l border-white/10 ml-2",children:[R.map(L=>F(L)),Y.map(L=>F(L,!0)),t.jsxs("div",{className:"flex items-center justify-between p-2 rounded hover:bg-white/5 cursor-pointer",onClick:()=>s.setDebugMobileLayout(!s.debugMobileLayout),children:[t.jsx("span",{className:"text-xs text-gray-300 font-bold",children:"Force Mobile UI"}),t.jsx("span",{className:`px-2 py-0.5 text-[8px] font-bold rounded-sm border transition-all ${s.debugMobileLayout?"bg-purple-500/30 text-purple-300 border-purple-500/40":"bg-white/[0.04] text-gray-600 border-white/5"}`,children:s.debugMobileLayout?"ON":"OFF"})]})]}),t.jsx("div",{className:"h-px bg-white/10 my-1"}),t.jsxs("div",{className:"flex items-center justify-between p-2 rounded hover:bg-white/5 cursor-pointer",title:"Keyboard: H",onClick:()=>{o(5),s.setShowHints(!s.showHints)},children:[t.jsxs("span",{className:"text-xs text-gray-300 font-bold",children:["Show Hints ",t.jsx("span",{className:"text-gray-500 font-normal",children:"[H]"})]}),t.jsx("span",{className:`px-2 py-0.5 text-[8px] font-bold rounded-sm border transition-all ${s.showHints?"bg-green-500/30 text-green-300 border-green-500/40":"bg-white/[0.04] text-gray-600 border-white/5"}`,children:s.showHints?"ON":"OFF"})]}),t.jsxs("button",{onClick:L=>{L.stopPropagation(),o(5),s.openHelp("general.shortcuts"),c(!1)},className:"w-full flex items-center justify-between p-2 rounded hover:bg-white/5 text-cyan-400 transition-colors group",children:[t.jsx("span",{className:"text-xs font-bold group-hover:text-cyan-200",children:"Help"}),t.jsx(Pr,{})]}),t.jsxs("button",{onClick:L=>{L.stopPropagation(),o(5),f(!d)},className:`w-full flex items-center justify-between p-2 rounded transition-colors ${d?"bg-white/10 text-cyan-400":"hover:bg-white/5 text-gray-300"}`,children:[t.jsx("span",{className:"text-xs font-bold",children:"About GMT"}),t.jsx(tl,{})]}),d&&t.jsx("div",{className:"p-3 bg-white/5 rounded-lg border border-white/5 animate-fade-in mt-1",children:t.jsxs("div",{className:"text-[10px] text-gray-400 leading-relaxed space-y-2",children:[p&&t.jsxs("div",{className:"mb-2 pb-2 border-b border-white/10",children:[t.jsx("div",{className:"text-[8px] text-gray-500 font-bold mb-1",children:"Active Renderer"}),t.jsx("div",{className:"text-[9px] text-green-400 font-mono break-all",children:p})]}),t.jsx("p",{className:"text-[9px] text-gray-500 font-mono mb-1",children:"v0.8.9"}),t.jsxs("p",{children:["GMT was crafted with ❤️ by ",t.jsx("span",{className:"text-white font-bold",children:"Guy Zack"})," using ",t.jsx("a",{href:"https://aistudio.google.com",target:"_blank",rel:"noopener noreferrer",className:"text-cyan-400 hover:underline",children:"Gemini"})," and ",t.jsx("a",{href:"https://claude.ai",target:"_blank",rel:"noopener noreferrer",className:"text-cyan-400 hover:underline",children:"Claude"}),"."]}),t.jsxs("div",{className:"pt-2 border-t border-white/10",children:[t.jsx("div",{className:"text-[8px] text-gray-500 font-bold mb-1",children:"Tech Stack"}),t.jsx("div",{className:"text-[9px] text-gray-500 font-mono",children:"React + TypeScript + Three.js + GLSL + Zustand + Vite"})]}),t.jsxs("div",{className:"flex flex-col gap-1 pt-2 border-t border-white/10",children:[t.jsxs("a",{href:"https://www.reddit.com/r/GMT_fractals/",target:"_blank",rel:"noopener noreferrer",className:"flex items-center gap-2 hover:text-white transition-colors",children:[t.jsx("span",{children:"Community:"}),t.jsx("span",{className:"text-cyan-400 hover:underline",children:"r/GMT_fractals"})]}),t.jsxs("a",{href:"https://github.com/gamazama/GMT-fractals",target:"_blank",rel:"noopener noreferrer",className:"flex items-center gap-2 hover:text-white transition-colors",children:[t.jsx("span",{children:"Source:"}),t.jsx("span",{className:"text-cyan-400 hover:underline",children:"GitHub (GPL-3.0)"})]})]})]})})]})})]})]})},ql=()=>{const e=N(),[o,a]=v.useState(window.innerWidth),[r,n]=v.useState(!1);v.useEffect(()=>{const f=()=>a(window.innerWidth),p=()=>n(window.matchMedia("(pointer: coarse)").matches);return window.addEventListener("resize",f),p(),()=>window.removeEventListener("resize",f)},[]);const s=e.debugMobileLayout||o<768||r,i=(f=10)=>{navigator.vibrate&&navigator.vibrate(f)},l="p-2.5 rounded-lg transition-all active:scale-95 border flex items-center justify-center",c="bg-transparent border-transparent text-gray-400 hover:text-white hover:bg-white/10 hover:border-white/10",d="bg-gray-800 border-gray-600 text-white";return t.jsxs("header",{className:"relative shrink-0 w-full h-14 z-[500] bg-black/90 border-b border-white/10 flex items-center justify-between px-6 animate-fade-in-down select-none",children:[t.jsx(kl,{isMobileMode:s,vibrate:i}),t.jsx(Gl,{isMobileMode:s,vibrate:i}),t.jsxs("div",{className:"flex gap-2 relative items-center",children:[t.jsx(Vl,{isMobileMode:s,vibrate:i,btnBase:l,btnActive:d,btnInactive:c}),t.jsx("div",{className:"h-6 w-px bg-white/10 mx-1"}),t.jsx(Ul,{isMobileMode:s,vibrate:i,btnBase:l,btnActive:d,btnInactive:c})]})]})},oo=()=>{const[e,o]=v.useState(typeof window<"u"?window.innerHeight>window.innerWidth:!1),[a,r]=v.useState(!1);return v.useEffect(()=>{const n=()=>{o(window.innerHeight>window.innerWidth),r(window.matchMedia("(pointer: coarse)").matches||window.innerWidth<768)};return window.addEventListener("resize",n),n(),()=>window.removeEventListener("resize",n)},[]),{isPortrait:e,isMobile:a}},No=({onMove:e,label:o,active:a})=>{const[r,n]=v.useState(!1),[s,i]=v.useState({x:0,y:0}),l=v.useRef(null),c=v.useRef(null),d=v.useRef({x:0,y:0}),f=v.useCallback((h,u)=>{if(!l.current)return;const g=l.current.getBoundingClientRect(),S=g.left+g.width/2,y=g.top+g.height/2,b=g.width/2;let m=(h-S)/b,x=(u-y)/b;const C=Math.sqrt(m*m+x*x);C>1&&(m/=C,x/=C);const z=30,w=Math.min(1,C),M=(C>0?m:0)*w*z,P=(C>0?x:0)*w*z;i({x:M,y:P}),d.current={x:m,y:-x}},[]),p=h=>{a&&(h.stopPropagation(),c.current=h.pointerId,n(!0),f(h.clientX,h.clientY),navigator.vibrate&&navigator.vibrate(10))};return v.useEffect(()=>{if(!r)return;const h=g=>{g.pointerId===c.current&&(g.cancelable&&g.preventDefault(),f(g.clientX,g.clientY),e(d.current.x,d.current.y))},u=g=>{g.pointerId===c.current&&(d.current={x:0,y:0},e(0,0),n(!1),i({x:0,y:0}),c.current=null,navigator.vibrate&&navigator.vibrate(5))};return window.addEventListener("pointermove",h,{passive:!1}),window.addEventListener("pointerup",u),window.addEventListener("pointercancel",u),()=>{window.removeEventListener("pointermove",h),window.removeEventListener("pointerup",u),window.removeEventListener("pointercancel",u)}},[r,f,e]),t.jsx("div",{ref:l,className:`w-36 h-36 rounded-full transition-all duration-200 relative flex items-center justify-center touch-none select-none ${r?"scale-110 shadow-[0_0_30px_rgba(34,211,238,0.1)]":"scale-100"} ${a?"pointer-events-auto":"pointer-events-none"}`,style:{touchAction:"none"},onPointerDown:p,children:t.jsxs("div",{className:`w-24 h-24 rounded-full border transition-all duration-500 flex items-center justify-center ${r?"bg-cyan-500/10 border-cyan-400":"bg-white/5 border-white/10"} ${a?"opacity-100":"opacity-0 scale-50"}`,children:[t.jsx("div",{className:`absolute -top-6 text-[8px] font-bold pointer-events-none transition-colors ${r?"text-cyan-400":"text-white/30"}`,children:o}),t.jsx("div",{className:"absolute inset-2 rounded-full border border-white/5 pointer-events-none"}),t.jsx("div",{className:`w-10 h-10 rounded-full border shadow-xl transition-transform duration-75 pointer-events-none ${r?"bg-cyan-400 border-white shadow-[0_0_20px_rgba(34,211,238,0.5)]":"bg-white/10 border-white/20"}`,style:{transform:`translate(${s.x}px, ${s.y}px)`}})]})})},Wl=()=>{const{cameraMode:e,setCameraMode:o,debugMobileLayout:a}=N(),{isMobile:r}=oo(),[n,s]=v.useState(!1);v.useEffect(()=>{s(r||a)},[a,r]);const i=v.useCallback((d,f)=>{window.dispatchEvent(new CustomEvent("joyMove",{detail:{x:d,y:f}}))},[]),l=v.useCallback((d,f)=>{window.dispatchEvent(new CustomEvent("joyLook",{detail:{x:d,y:f}}))},[]);if(!n)return null;const c=e==="Fly";return t.jsxs("div",{className:"absolute inset-0 pointer-events-none z-[100] flex flex-col justify-between p-6 pb-10",children:[t.jsx("div",{className:"flex justify-start pt-16",children:t.jsxs("button",{onClick:()=>{navigator.vibrate&&navigator.vibrate(20),o(c?"Orbit":"Fly")},className:"pointer-events-auto flex items-center gap-2 bg-black/80 border border-white/20 px-4 py-2.5 rounded-full backdrop-blur-xl shadow-2xl active:scale-90 transition-all active:border-cyan-400",children:[t.jsx("div",{className:`w-2 h-2 rounded-full ${c?"bg-cyan-400 animate-pulse shadow-[0_0_5px_cyan]":"bg-purple-400"}`}),t.jsxs("span",{className:"text-[10px] font-bold text-white",children:[e," Mode"]})]})}),t.jsxs("div",{className:"flex justify-between items-end transition-all duration-500",children:[t.jsx("div",{className:c?"pointer-events-auto":"pointer-events-none",children:t.jsx(No,{label:"Move",onMove:i,active:c})}),t.jsx("div",{className:c?"pointer-events-auto":"pointer-events-none",children:t.jsx(No,{label:"Look",onMove:l,active:c})})]})]})};class Xl{constructor(o){J(this,"ctx",null);J(this,"imageData",null);const a=o.getContext("2d",{alpha:!0});a&&(this.ctx=a)}render(o,a){if(!this.ctx)return;const r=this.ctx.canvas;(r.width!==r.clientWidth||r.height!==r.clientHeight)&&(r.width=r.clientWidth,r.height=r.clientHeight,this.imageData=null);const n=r.width,s=r.height;if(n===0||s===0)return;(!this.imageData||this.imageData.width!==n||this.imageData.height!==s)&&(this.imageData=this.ctx.createImageData(n,s));const i=this.imageData.data,l=o*.001,c=l*.5,d=Math.cos(c)*.7885,f=Math.sin(c)*.7885,p=1.5-.9*Math.pow(a,.5),h=(1-Math.pow(a,.2))*.5,u=Math.sin(h),g=Math.cos(h),S=(1-a)*.5,y=10+80*Math.pow(a,1.5),b=Math.min(Math.floor(y),90),m=1/s,x=n*.5,C=s*.5;for(let z=0;z<s;z++){const w=s-1-z,M=1/n;for(let P=0;P<n;P++){let k=(P-x)*m,T=(w-C)*m;k+=S,k*=p,T*=p;const _=g*k-u*T,E=u*k+g*T;let F=_,O=E,$=0;for(let Y=0;Y<b;Y++){const L=F*F,G=O*O;if(L+G>4)break;O=2*F*O+f,F=L-G+d,$++}const R=(z*n+P)*4;if($>=b)i[R]=0,i[R+1]=0,i[R+2]=0,i[R+3]=0;else{const Y=F*F+O*O,X=3+($-Math.log2(Math.log2(Y))+4)/64*10;let D=.5+.5*Math.cos(X),H=.5+.5*Math.cos(X+.6),A=.5+.5*Math.cos(X+1);const j=P*M,B=.1*Math.sin(j*30-l*8)*a;D+=B,H+=B,A+=B,i[R]=Math.max(0,Math.min(255,D*255|0)),i[R+1]=Math.max(0,Math.min(255,H*255|0)),i[R+2]=Math.max(0,Math.min(255,A*255|0)),i[R+3]=255}}}this.ctx.putImageData(this.imageData,0,0)}dispose(){this.ctx=null,this.imageData=null}}xe();const Bo=["Generative Math Tracer","GPU Manifold Tracer","GPU Mandelorus Tracer","Geometric Morphology Toolkit","GLSL Marching Toolkit","Generative Morphology Theater","Grand Mathematical Topography","Geometric Manifold Traversal","Gradient Mapped Topology","Generalized Mesh Tracer","Gravitational Manifold Theory","Glass Mountain Telescope","Ghost Manifold Terminal","Garden of Mathematical Terrain","Glimpse Machine Terminal","Grey Matter Telescope","Grotesque Math Theater","Geometry Mutation Terminal","Galactic Morphology Telescope","Grand Mythos Terminal","Glowing Mathematical Topologies","God's Math Toy","Gnarly Math Thing","Generally Mesmerizing Thingamajig","Give Me Tentacles","Gloriously Melted Teapots","Gaze-into Mathematical Twilight","Greenwich Mean Time","Geometrically Mangled Tesseracts","Gratuitous Mandelbulb Torture","Got More Tentacles","Groovy Morphing Thingamabob"],Yl=()=>Bo[Math.floor(Math.random()*Bo.length)],Oo=e=>`thumbnails/fractal_${e}.jpg`,Zl=({isReady:e,onFinished:o,startupMode:a,bootEngine:r})=>{var L;const n=v.useRef(null),s=v.useRef(null),i=v.useRef(!1),l=v.useRef(e),c=v.useRef(r),d=v.useRef(!1);v.useEffect(()=>{l.current=e},[e]),v.useEffect(()=>{c.current=r},[r]);const f=v.useRef(null),p=N(G=>G.formula),h=N(G=>G.setFormula),u=N(G=>G.loadPreset),g=N(G=>G.applyPreset),S=N(G=>G.quality),y=(S==null?void 0:S.precisionMode)===1,[b,m]=v.useState(0),x=v.useRef(0),[C,z]=v.useState(1),[w,M]=v.useState(!0),[P,k]=v.useState(!1);v.useEffect(()=>{i.current=P},[P]);const[T,_]=v.useState(null),[E]=v.useState(Yl),F=()=>{d.current||(d.current=!0,c.current&&c.current())},O=G=>{const X=Me.get(G);X&&X.defaultPreset&&u(X.defaultPreset),h(G),k(!1),m(0),x.current=0,d.current&&c.current&&c.current(!0)},$=async G=>{var D;const X=(D=G.target.files)==null?void 0:D[0];if(X)try{let H="";if(X.type==="image/png"){const j=await hr(X,"FractalData");if(j)H=j;else throw new Error("No Fractal Data found in PNG.")}else H=await X.text();const A=JSON.parse(H);u(A),k(!1),m(0),x.current=0,d.current&&c.current&&c.current(!0)}catch(H){alert("Load failed: "+(H instanceof Error?H.message:String(H)))}},R=()=>{const G=y?"balanced":"lite";if(Z.emit("is_compiling",`Switching to ${G} mode...`),g){const X=N.getState();g({mode:G,actions:X})}},Y=v.useMemo(()=>Me.getAll(),[]);return v.useEffect(()=>{if(!n.current)return;s.current=new Xl(n.current);let G=0,X=0,D=performance.now();const H=2500,A=j=>{const B=performance.now(),I=Math.min(B-D,60);D=B;const V=i.current;X<100&&(X+=I*(100/H)),X>100&&(X=100),Math.floor(X)>Math.floor(x.current)&&(x.current=X,m(X)),s.current&&s.current.render(j,X/100),X>=100&&!V?(F(),l.current?(s.current&&(s.current.dispose(),s.current=null),z(0),setTimeout(()=>{M(!1),o()},800)):G=requestAnimationFrame(A)):G=requestAnimationFrame(A)};return G=requestAnimationFrame(A),()=>{cancelAnimationFrame(G),s.current&&(s.current.dispose(),s.current=null)}},[]),v.useEffect(()=>{F()},[]),w?t.jsxs("div",{className:"fixed inset-0 z-[100] flex flex-col items-center justify-center bg-black transition-opacity duration-1000",style:{opacity:C},children:[t.jsxs("div",{className:"text-center mb-10 relative animate-fade-in-up z-10",children:[t.jsxs("h1",{className:"text-7xl font-black text-white tracking-tighter drop-shadow-[0_0_15px_rgba(34,211,238,0.5)] mb-2",children:["G",t.jsx("span",{className:"text-cyan-400",children:"M"}),"T"]}),t.jsx("div",{className:"text-xs text-gray-400 font-mono uppercase tracking-[0.4em]",children:E})]}),t.jsxs("div",{className:"relative z-10 w-[500px] h-16 bg-gray-900/80 rounded-full border border-gray-700/50 overflow-hidden shadow-[0_0_50px_rgba(0,255,255,0.1)] backdrop-blur-sm",children:[t.jsx("div",{className:"absolute top-0 left-0 h-full overflow-hidden will-change-[width] transition-[width] duration-75 ease-linear",style:{width:`${b}%`},children:t.jsx("canvas",{ref:n,className:"absolute top-0 left-0 w-[500px] h-16"})}),t.jsx("div",{className:"absolute inset-0 bg-gradient-to-b from-white/5 to-transparent pointer-events-none"})]}),t.jsx("div",{className:"mt-6 font-mono text-sm text-cyan-500/80 z-20 flex flex-col items-center h-10",children:a==="url"?t.jsxs("span",{className:"animate-pulse",children:["LOADING SHARED SCENE... ",Math.floor(b),"%"]}):t.jsxs("div",{className:"relative flex flex-col items-center",children:[t.jsxs("div",{className:"flex items-center gap-2",children:[t.jsx("span",{className:"text-cyan-600/80",children:"LOADING"}),t.jsxs("button",{onClick:()=>k(!P),className:"flex items-center gap-1 text-cyan-400 hover:text-white transition-colors border-b border-dashed border-cyan-500/30 hover:border-cyan-400 pb-0.5 outline-none",children:[t.jsxs("span",{className:"font-bold",children:["[",p,"]"]}),t.jsx("span",{className:`text-[10px] transform transition-transform ${P?"rotate-180":""}`,children:t.jsx(Tt,{})})]}),t.jsxs("span",{className:"text-cyan-600/80",children:[Math.floor(b),"%"]})]}),t.jsx("button",{onClick:R,className:`mt-4 px-3 py-1.5 text-[9px] font-bold rounded border transition-all ${y?"bg-orange-900/40 text-orange-200 border-orange-500/40 hover:bg-orange-800/50":"bg-white/5 text-gray-500 border-white/5 hover:text-white hover:border-white/20"}`,children:y?"Lite Render Active":"Enable Lite Render"}),P&&t.jsxs("div",{className:"absolute bottom-full mb-4 w-[340px] bg-black/95 border border-white/20 rounded-xl shadow-[0_10px_60px_rgba(0,0,0,0.9)] backdrop-blur-xl animate-fade-in text-xs z-[110]",onMouseLeave:()=>_(null),children:[T&&T!=="Modular"&&t.jsxs("div",{className:"absolute left-[350px] bottom-0 w-[256px] h-[256px] bg-black border border-cyan-500/50 rounded-lg shadow-[0_0_50px_rgba(0,0,0,1)] overflow-hidden animate-fade-in pointer-events-none",children:[t.jsx("img",{src:Oo(T),className:"w-full h-full object-cover",alt:"Preview",onError:G=>{G.currentTarget.style.display="none"}}),t.jsx("div",{className:"absolute inset-0 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.1)_50%),linear-gradient(90deg,rgba(255,0,0,0.03),rgba(0,255,0,0.01),rgba(0,0,255,0.03))] bg-[length:100%_4px,4px_100%] pointer-events-none"}),t.jsx("div",{className:"absolute bottom-0 left-0 right-0 p-3 bg-black/60 text-cyan-300 font-bold text-[9px] border-t border-white/5",children:(L=Me.get(T))==null?void 0:L.name})]}),t.jsxs("div",{className:"p-1 max-h-[400px] overflow-y-auto custom-scroll",children:[t.jsxs("button",{onClick:()=>{var G;return(G=f.current)==null?void 0:G.click()},className:"w-full flex items-center gap-2 px-3 py-2 text-left text-gray-300 hover:bg-white/10 hover:text-white rounded transition-colors mb-1 border-b border-white/10",children:[t.jsx(Tr,{})," ",t.jsx("span",{className:"font-bold text-[10px]",children:"Load From File..."})]}),t.jsx("input",{type:"file",ref:f,className:"hidden",accept:".json,.png",onChange:$}),t.jsx("div",{className:"px-3 py-1.5 text-[9px] font-bold text-gray-500 bg-white/5 mb-1",children:"Select Engine"}),Y.map(G=>t.jsxs("button",{onClick:()=>O(G.id),onMouseEnter:()=>_(G.id),className:`w-full text-left px-3 py-2.5 transition-all flex gap-3 border-b border-white/5 last:border-b-0 ${G.id===p?"bg-cyan-900/30":"hover:bg-white/5"}`,children:[t.jsxs("div",{className:"w-16 h-10 shrink-0 bg-black rounded border border-white/10 overflow-hidden relative",children:[G.id!=="Modular"?t.jsx("img",{src:Oo(G.id),alt:G.name,className:"w-full h-full object-cover",onError:X=>{X.currentTarget.style.display="none",X.currentTarget.nextElementSibling&&(X.currentTarget.nextElementSibling.style.display="flex")}}):null,t.jsx("div",{className:`w-full h-full items-center justify-center text-gray-700 bg-gray-900 ${G.id!=="Modular"?"hidden":"flex"}`,children:G.id==="Modular"?t.jsx(_r,{}):t.jsx(ma,{})})]}),t.jsx("div",{className:"flex flex-col min-w-0",children:t.jsx("span",{className:`text-[11px] font-bold tracking-tight mb-0.5 ${G.id===p?"text-cyan-400":"text-gray-200"}`,children:G.name})})]},G.id))]})]})]})})]}):null},Ql=(e,o)=>{const a=N,r=fe;v.useEffect(()=>{const n=s=>{const i=s.target,l=i.tagName==="INPUT"&&i.type==="range";if(i.tagName==="INPUT"&&!l||i.tagName==="TEXTAREA"||i.isContentEditable)return;const d=s.ctrlKey||s.metaKey,f=s.shiftKey;if(d&&!s.altKey){const p=a.getState().isTimelineHovered;if(s.code==="KeyZ"&&f){s.preventDefault(),s.stopPropagation(),a.getState().undoCamera();return}if(s.code==="KeyY"&&f){s.preventDefault(),s.stopPropagation(),a.getState().redoCamera();return}if(s.code==="KeyZ"&&!f){s.preventDefault(),s.stopPropagation(),p&&r.getState().undo()||a.getState().undoParam();return}if(s.code==="KeyY"&&!f){s.preventDefault(),s.stopPropagation(),p&&r.getState().redo()||a.getState().redoParam();return}}if(d&&!f&&!s.altKey){const p=s.code.match(/^Digit([1-9])$/);if(p){const h=parseInt(p[1])-1,u=a.getState().savedCameras;h<u.length&&(s.preventDefault(),s.stopPropagation(),a.getState().selectCamera(u[h].id));return}}switch(s.code){case"Tab":s.preventDefault(),a.getState().setCameraMode(a.getState().cameraMode==="Fly"?"Orbit":"Fly");break;case"KeyT":d||o(y=>!y);break;case"Escape":a.getState().isBroadcastMode&&a.getState().setIsBroadcastMode(!1),a.getState().interactionMode!=="none"&&a.getState().setInteractionMode("none"),r.getState().deselectAll();break;case"KeyH":a.getState().setShowHints(!a.getState().showHints);break;case"Backquote":a.getState().setAdvancedMode(!a.getState().advancedMode);break;case"KeyB":if(!d){const y=a.getState();y.setIsBroadcastMode(!y.isBroadcastMode)}break;case"Space":const{cameraMode:p,isTimelineHovered:h}=a.getState(),{sequence:u,isPlaying:g}=r.getState();let S=!1;if(e)S=p!=="Fly"||h;else{const y=Object.keys(u.tracks).length>0;p!=="Fly"&&y&&(S=!0)}S&&(s.preventDefault(),g?r.getState().pause():r.getState().play());break}};return window.addEventListener("keydown",n,{capture:!0}),()=>window.removeEventListener("keydown",n,{capture:!0})},[e,o])},Kl=xe(),$o=({onUpdate:e,autoUpdate:o,trigger:a,source:r})=>{const n=v.useRef(a);return v.useEffect(()=>{let s=0,i=0;const l=()=>{const c=a!==n.current;c&&(n.current=a),i++,(o&&i%60===0||c)&&Kl.requestHistogramReadback(r).then(f=>{f.length>0&&e(f)}),s=requestAnimationFrame(l)};return l(),()=>cancelAnimationFrame(s)},[o,a,r,e]),null},Jl=({state:e,actions:o,isMobile:a,hudRefs:r})=>{var p;const n=v.useRef(null),s=v.useRef(null),i=e.tabSwitchCount,l=o.incrementTabSwitchCount;v.useEffect(()=>{const h=fe.subscribe(u=>u.isCameraInteracting,u=>{r.container.current&&(u?(r.container.current.style.opacity="1",s.current&&(clearTimeout(s.current),s.current=null)):(s.current&&clearTimeout(s.current),s.current=window.setTimeout(()=>{r.container.current&&(r.container.current.style.opacity="0")},2e3)))});return()=>{h(),s.current&&clearTimeout(s.current)}},[]),v.useEffect(()=>N.subscribe(u=>u.cameraMode,()=>l()),[l]);const c=h=>{if(!n.current||e.cameraMode==="Orbit")return;const u=n.current.getBoundingClientRect(),g=b=>{const m=Math.max(0,Math.min(1,(b-u.left)/u.width)),x=Math.pow(10,m*3-3);o.setNavigation({flySpeed:x}),(m===0||m===1)&&navigator.vibrate&&navigator.vibrate(5)};g(h.clientX);const S=b=>g(b.clientX),y=()=>{window.removeEventListener("pointermove",S),window.removeEventListener("pointerup",y)};window.addEventListener("pointermove",S),window.addEventListener("pointerup",y)},d=((p=e.navigation)==null?void 0:p.flySpeed)??.5,f=(Math.log10(d)+3)/3;return t.jsx("div",{ref:r.container,className:"absolute inset-0 pointer-events-none z-10 transition-opacity duration-500 opacity-0",children:t.jsxs("div",{className:"absolute inset-0 flex items-center justify-center",children:[t.jsx("div",{className:"absolute pointer-events-none opacity-20",children:e.cameraMode==="Fly"?t.jsxs(t.Fragment,{children:[t.jsx("div",{className:"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[40px] h-[1px] bg-cyan-400"}),t.jsx("div",{className:"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1px] h-[40px] bg-cyan-400"})]}):t.jsxs("div",{className:"relative flex items-center justify-center",children:[t.jsx("div",{className:"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[20px] h-[1px] bg-cyan-400"}),t.jsx("div",{className:"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1px] h-[20px] bg-cyan-400"}),t.jsx("div",{className:"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[24px] h-[24px] rounded-full border border-cyan-400 opacity-60"})]})}),t.jsxs("div",{ref:r.reticle,className:"absolute w-8 h-8 pointer-events-none opacity-0 transition-opacity duration-150 ease-out will-change-transform",children:[t.jsx("div",{className:"absolute inset-0 border-2 border-cyan-400 rounded-full shadow-[0_0_15px_cyan] opacity-80"}),t.jsx("div",{className:"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-1 h-1 bg-white rounded-full"})]}),t.jsxs("div",{className:"absolute left-1/2 -translate-x-1/2 flex flex-col items-center pointer-events-none transition-all duration-300 ease-out",style:{bottom:e.showHints&&!a?"4rem":"3.5rem"},children:[t.jsx("button",{ref:r.reset,onClick:()=>{o.resetCamera(),navigator.vibrate&&navigator.vibrate(30)},className:"pointer-events-auto px-4 py-1.5 bg-black/60 hover:bg-cyan-900/80 text-cyan-400 hover:text-white text-[9px] font-bold rounded-t-lg border-x border-t border-white/10 backdrop-blur-md hidden animate-fade-in shadow-xl mb-[-1px]",children:"Reset Camera"}),t.jsxs("div",{className:"flex items-stretch gap-px bg-black/40 rounded-full border border-white/10 backdrop-blur-md overflow-hidden pointer-events-auto shadow-2xl",children:[e.cameraMode==="Fly"&&t.jsxs(t.Fragment,{children:[t.jsxs("div",{ref:n,onPointerDown:c,className:"relative flex items-center px-6 py-3 cursor-ew-resize group min-w-[120px]",children:[t.jsx("div",{className:"absolute inset-0 bg-cyan-500/10 border-r border-cyan-500/20 transition-all duration-300 ease-out",style:{width:`${f*100}%`}}),t.jsxs("span",{ref:r.speed,className:"relative z-10 font-bold text-cyan-300 font-mono text-[10px] group-hover:text-white transition-colors",children:["Spd x",d.toFixed(3)]})]}),t.jsx("div",{className:"w-px bg-white/5"})]}),t.jsx("div",{className:"px-6 py-3 bg-white/5 flex items-center min-w-[100px] justify-center",children:t.jsx("span",{ref:r.dist,className:"text-cyan-500/80 font-mono text-[10px]",children:"Dst ---"})})]}),e.showHints&&!a&&t.jsxs("div",{className:"mt-3 text-[9px] font-medium text-white/40 text-center animate-fade-in text-shadow-sm whitespace-nowrap",children:[t.jsxs("span",{className:"text-cyan-400/60 font-bold mr-2",children:["[",e.cameraMode,"]"]}),e.cameraMode==="Fly"?"WASD Move · Space/C Vert · Shift Boost":"L-Drag Rotate · R-Drag Pan · Scroll Zoom"]}),i<2&&!a&&e.showHints&&t.jsxs("div",{className:"mt-2 text-[10px] font-bold text-cyan-300 animate-pulse bg-cyan-950/40 px-3 py-1 rounded border border-cyan-500/30 shadow-lg",children:["Press ",t.jsx("span",{className:"text-white border border-white/20 rounded px-1 bg-white/10 mx-0.5",children:"Tab"})," for ",e.cameraMode==="Orbit"?"Fly":"Orbit"," navigation"]})]})]})})};class Ho{constructor(o){J(this,"element");J(this,"sourceNode",null);J(this,"gainNode",null);J(this,"fileUrl",null);J(this,"fileName",null);J(this,"isActive",!1);this.element=new Audio,this.element.loop=!0,this.element.crossOrigin="anonymous"}get isPlaying(){return!this.element.paused&&!!this.sourceNode}load(o,a,r){this.fileUrl&&URL.revokeObjectURL(this.fileUrl),this.fileUrl=URL.createObjectURL(o),this.fileName=o.name,this.element.src=this.fileUrl,this.isActive=!0,this.sourceNode||(this.sourceNode=a.createMediaElementSource(this.element),this.gainNode=a.createGain(),this.sourceNode.connect(this.gainNode),this.gainNode.connect(r))}play(){this.element.play().catch(o=>console.warn("Deck play failed",o))}pause(){this.element.pause()}stop(){this.element.pause(),this.element.currentTime=0}seek(o){this.element.currentTime=o}setVolume(o){this.gainNode&&(this.gainNode.gain.value=o)}get duration(){return this.element.duration||0}get currentTime(){return this.element.currentTime||0}}class ec{constructor(){J(this,"audioContext",null);J(this,"analyser",null);J(this,"micSource",null);J(this,"decks",[null,null]);J(this,"masterGain",null);J(this,"dataArray",null);J(this,"isMicActive",!1);J(this,"crossfade",.5)}init(){this.audioContext||(this.audioContext=new(window.AudioContext||window.webkitAudioContext),this.masterGain=this.audioContext.createGain(),this.masterGain.gain.value=.8,this.masterGain.connect(this.audioContext.destination),this.analyser=this.audioContext.createAnalyser(),this.analyser.fftSize=2048,this.analyser.smoothingTimeConstant=.8,this.dataArray=new Uint8Array(this.analyser.frequencyBinCount),this.masterGain.connect(this.analyser),this.decks[0]=new Ho(this.audioContext),this.decks[1]=new Ho(this.audioContext),this.setCrossfade(.5))}setSmoothing(o){this.analyser&&(this.analyser.smoothingTimeConstant=Math.max(0,Math.min(.99,o)))}async connectMicrophone(){if(this.init(),!(!this.audioContext||!this.masterGain)){this.decks.forEach(o=>o==null?void 0:o.pause());try{const o=await navigator.mediaDevices.getUserMedia({audio:!0});this.micSource&&this.micSource.disconnect(),this.micSource=this.audioContext.createMediaStreamSource(o),this.micSource.connect(this.analyser),this.isMicActive=!0,this.audioContext.state==="suspended"&&this.audioContext.resume()}catch(o){console.error("AudioEngine: Mic access denied",o),alert("Microphone access denied.")}}}async connectSystemAudio(){if(this.init(),!!this.audioContext)try{const o=await navigator.mediaDevices.getDisplayMedia({video:!0,audio:!0});if(o.getVideoTracks().forEach(a=>a.stop()),o.getAudioTracks().length===0)return;this.micSource&&this.micSource.disconnect(),this.micSource=this.audioContext.createMediaStreamSource(o),this.micSource.connect(this.analyser),this.micSource.connect(this.audioContext.destination),this.isMicActive=!0,this.audioContext.state==="suspended"&&this.audioContext.resume()}catch(o){console.error("AudioEngine: System audio capture failed",o),alert("System audio capture failed. Check browser permissions.")}}loadTrack(o,a){var r;this.init(),!(!this.audioContext||!this.masterGain)&&(this.micSource&&(this.micSource.disconnect(),this.micSource=null,this.isMicActive=!1),(r=this.decks[o])==null||r.load(a,this.audioContext,this.masterGain),this.setCrossfade(this.crossfade),this.audioContext.state==="suspended"&&this.audioContext.resume())}play(o){var a;(a=this.decks[o])==null||a.play()}pause(o){var a;(a=this.decks[o])==null||a.pause()}stop(o){var a;(a=this.decks[o])==null||a.stop()}deactivateDeck(o){const a=this.decks[o];a&&(a.stop(),a.isActive=!1)}seek(o,a){var r;(r=this.decks[o])==null||r.seek(a)}getTrackInfo(o){const a=this.decks[o];return{duration:(a==null?void 0:a.duration)||1,currentTime:(a==null?void 0:a.currentTime)||0,hasTrack:!!(a!=null&&a.sourceNode),fileName:(a==null?void 0:a.fileName)||null,isPlaying:(a==null?void 0:a.isPlaying)||!1,isActive:(a==null?void 0:a.isActive)||!1}}setCrossfade(o){this.crossfade=o;const a=Math.cos(o*.5*Math.PI),r=Math.cos((1-o)*.5*Math.PI);this.decks[0]&&this.decks[0].setVolume(a),this.decks[1]&&this.decks[1].setVolume(r)}setMasterGain(o){this.masterGain&&this.masterGain.gain.setTargetAtTime(o,this.audioContext.currentTime,.1)}update(){!this.analyser||!this.dataArray||this.analyser.getByteFrequencyData(this.dataArray)}getRawData(){return this.dataArray}}const Or=new ec;class tc{constructor(){J(this,"ruleValues",{});J(this,"lfoValues",{});J(this,"lfoStates",{});J(this,"outputValues",{});J(this,"lfoPrevOffsets",{});J(this,"offsets",{})}getRuleValue(o){return this.ruleValues[o]||0}updateOscillators(o,a,r){for(let n=0;n<o.length;n++){const s=o[n];if(!s.enabled)continue;const i=(a/s.period+s.phase)%1;let l=0;switch(s.shape){case"Sine":l=Math.sin(i*Math.PI*2);break;case"Triangle":l=1-Math.abs(i*2-1)*2;break;case"Sawtooth":l=i*2-1;break;case"Pulse":l=i<.5?1:-1;break;case"Noise":const h=s.id;this.lfoStates[h]||(this.lfoStates[h]=Math.random()),this.lfoStates[h]+=r*5,l=Math.sin(this.lfoStates[h])*Math.cos(this.lfoStates[h]*.73);break}const c=l*.5+.5;this.lfoValues[`lfo-${n+1}`]=c;const d=l*s.amplitude,f=this.lfoPrevOffsets[s.id]??d;let p=d;if(s.smoothing>.001){const h=50*Math.pow(1-s.smoothing,2)+.1,u=1-Math.exp(-h*r);p=f+(d-f)*u}this.lfoPrevOffsets[s.id]=p,this.offsets[s.target]=(this.offsets[s.target]||0)+p}}update(o,a){const r=Or.getRawData();for(const n of o){if(!n.enabled)continue;let s=0;n.source==="audio"?r&&(s=this.processAudioSignal(n,r)):n.source.startsWith("lfo-")&&(s=this.lfoValues[n.source]||0);const i=this.ruleValues[n.id]||0;let l=i;if(s>i){const f=1-Math.pow(n.attack,.2);l=i+(s-i)*f}else{const f=1-Math.pow(n.decay,.2);l=i+(s-i)*f}this.ruleValues[n.id]=l;let c=l;if(n.smoothing&&n.smoothing>.001){const f=this.outputValues[n.id]||0,p=1-Math.pow(n.smoothing,.5);c=f+(l-f)*p}this.outputValues[n.id]=c;const d=c*n.gain+n.offset;this.offsets[n.target]=(this.offsets[n.target]||0)+d}}resetOffsets(){this.offsets={}}processAudioSignal(o,a){const r=a.length,n=Math.floor(o.freqStart*r),s=Math.floor(o.freqEnd*r);if(n>=r||s<=n)return 0;let i=0,l=0;for(let p=n;p<s;p++)i+=a[p],l++;if(l===0)return 0;const c=i/l/255;if(c<o.thresholdMin)return 0;const d=Math.max(.001,o.thresholdMax-o.thresholdMin),f=(c-o.thresholdMin)/d;return Math.min(1,f)}}const Wt=new tc,Ve=xe(),Go={current:new Set},Vo=new U,Uo=new yt,qo=new Hi,Xt={current:new U(-1e3,-1e3,-1e3)},qa={current:-1},it={current:{}},ac=e=>{var F,O,$;const o=fe.getState(),a=N.getState(),n=Object.keys(o.sequence.tracks).length>0,s=a.animations.length>0,i=((O=(F=a.modulation)==null?void 0:F.rules)==null?void 0:O.length)>0,l=(($=a.audio)==null?void 0:$.isEnabled)??!1;if(!n&&!s&&!i&&!l)return;oa.tick(e);const c=a.modulation,d=a.audio;d&&d.isEnabled&&Or.update(),Wt.resetOffsets(),Ve.modulations={};const f=a.animations;Wt.updateOscillators(f,performance.now()/1e3,e),c&&c.rules&&Wt.update(c.rules,e);const p=Wt.offsets,h={},u=new Set(Object.keys(p)),g=Go.current,S=new Set;u.forEach(R=>S.add(R)),g.forEach(R=>S.add(R));let y=0,b=0,m=0,x=!1,C=0,z=0,w=0,M=!1;const P=Math.floor(o.currentFrame),k=fe.getState().isRecordingModulation,T=k&&P>qa.current;T&&(qa.current=P);const _=[];if(k&&S.size>0&&oa.setOverriddenTracks(S),S.forEach(R=>{var A,j,B;const Y=!u.has(R),L=Y?0:p[R]??0;Math.abs(L)>1e-4&&(M=!0);let G=0,X="",D=!1;if(R.includes(".")){const[I,V]=R.split("."),W=ne.get(I),q=a[I];if(W&&q){const Q=V.match(/^(vec[23][ABC])_(x|y|z)$/);if(Q){const K=Q[1],ie=Q[2],ee=W.params[K];if(ee){const oe=q[K];oe&&typeof oe=="object"&&(G=oe[ie]||0),ee.uniform&&(X=`${ee.uniform}_${ie}`),ee.noReset&&(D=!0)}}else{const K=W.params[V];K&&(typeof q[V]=="number"&&(G=q[V]),K.uniform&&(X=K.uniform),K.noReset&&(D=!0))}}}else R==="iterations"?(X="uIterations",G=((A=a.coreMath)==null?void 0:A.iterations)??0):R.startsWith("param")&&(X="u"+R.charAt(0).toUpperCase()+R.slice(1),G=((j=a.coreMath)==null?void 0:j[R])??0);if(T&&Math.abs(L)>1e-6){let I=G;const V=o.recordingSnapshot;if(V&&V.tracks[R]){const W=V.tracks[R],q=R.includes("rotation");I=ot(W.keyframes,o.currentFrame,q)}else it.current[R]===void 0&&(it.current[R]=G),I=it.current[R];_.push({trackId:R,value:I+L}),G=I}if(R.startsWith("coloring.")){if(R==="coloring.repeats"){const I=a.coloring;if(I&&Math.abs(I.repeats)>.001){const V=T?G:I.repeats,W=I.scale/V,q=(V+L)*W;Y||(h[R]=V+L),Ve.setUniform("uColorScale",q)}return}if(R==="coloring.phase"){const I=a.coloring,V=T?G:I.phase;Y||(h[R]=V+L),Ve.setUniform("uColorOffset",I.offset+L);return}if(R==="coloring.repeats2"){const I=a.coloring;if(I&&Math.abs(I.repeats2)>.001){const V=T?G:I.repeats2,W=I.scale2/V,q=(V+L)*W;Y||(h[R]=V+L),Ve.setUniform("uColorScale2",q)}return}if(R==="coloring.phase2"){const I=a.coloring,V=T?G:I.phase2;Y||(h[R]=V+L),Ve.setUniform("uColorOffset2",I.offset2+L);return}}if(R.startsWith("julia.")||R.startsWith("geometry.julia")){const I=a.geometry,V=(I==null?void 0:I.juliaX)??0,W=(I==null?void 0:I.juliaY)??0,q=(I==null?void 0:I.juliaZ)??0;R.endsWith("juliaX")||R.endsWith("x")?(y=V+L,h[R]=y,T&&_.push({trackId:"geometry.juliaX",value:y})):R.endsWith("juliaY")||R.endsWith("y")?(b=W+L,h[R]=b,T&&_.push({trackId:"geometry.juliaY",value:b})):(R.endsWith("juliaZ")||R.endsWith("z"))&&(m=q+L,h[R]=m,T&&_.push({trackId:"geometry.juliaZ",value:m})),x=!0;return}if(R.startsWith("camera.")){R.startsWith("camera.unified")?R.endsWith("x")?Ve.modulations["camera.unified.x"]=L:R.endsWith("y")?Ve.modulations["camera.unified.y"]=L:R.endsWith("z")&&(Ve.modulations["camera.unified.z"]=L):R.startsWith("camera.rotation")&&(R.endsWith("x")?Ve.modulations["camera.rotation.x"]=L:R.endsWith("y")?Ve.modulations["camera.rotation.y"]=L:R.endsWith("z")&&(Ve.modulations["camera.rotation.z"]=L)),h[R]=L;return}if(R.startsWith("geometry.preRot")){R.endsWith("X")?(C=L,h[R]=L):R.endsWith("Y")?(z=L,h[R]=L):R.endsWith("Z")&&(w=L,h[R]=L);return}if(R.startsWith("lighting.light")){const I=R.match(/lighting\.light(\d+)_(\w+)/);if(I){const V=parseInt(I[1]),W=I[2],q=(B=a.lighting)==null?void 0:B.lights;if(q&&q[V]){const Q=q[V];let K=0,ie=!1;if(W==="intensity"?(K=Q.intensity,ie=!0):W==="falloff"?(K=Q.falloff,ie=!0):W==="posX"?(K=Q.position.x,ie=!0):W==="posY"?(K=Q.position.y,ie=!0):W==="posZ"&&(K=Q.position.z,ie=!0),ie){if(T){let ee=K;o.recordingSnapshot&&o.recordingSnapshot.tracks[R]?ee=ot(o.recordingSnapshot.tracks[R].keyframes,o.currentFrame,!1):(it.current[R]===void 0&&(it.current[R]=K),ee=it.current[R]),_.push({trackId:R,value:ee+L}),h[R]=ee+L}else h[R]=K+L;Ve.modulations[R]=L}}}return}const H=R.match(/^(coreMath|geometry)\.(vec[23][ABC])_(x|y|z)$/);if(H){const I=H[1],V=H[2],W=H[3],q=a[I];if(q&&q[V]){const Q=q[V],K=Q[W]??0,ie=K+L;if(T){let le=K;o.recordingSnapshot&&o.recordingSnapshot.tracks[R]?le=ot(o.recordingSnapshot.tracks[R].keyframes,o.currentFrame,!1):(it.current[R]===void 0&&(it.current[R]=K),le=it.current[R]),_.push({trackId:R,value:le+L}),h[R]=le+L}else h[R]=ie;const ee="u"+V.charAt(0).toUpperCase()+V.slice(1),oe={...Q,[W]:ie};Ve.setUniform(ee,oe)}return}if(X){const I=G+L;Y||(h[R]=I),Ve.setUniform(X,I,D)}}),_.length>0&&o.batchAddKeyframes(P,_,"Linear"),x){const R=a.geometry;h["geometry.juliaX"]===void 0&&h["julia.x"]===void 0&&(y=(R==null?void 0:R.juliaX)??0),h["geometry.juliaY"]===void 0&&h["julia.y"]===void 0&&(b=(R==null?void 0:R.juliaY)??0),h["geometry.juliaZ"]===void 0&&h["julia.z"]===void 0&&(m=(R==null?void 0:R.juliaZ)??0),Vo.set(y,b,m),Ve.setUniform("uJulia",Vo)}const E=a.geometry;if(E&&E.preRotMaster){const R=E.preRotX+C,Y=E.preRotY+z,L=E.preRotZ+w;if(Math.abs(R-Xt.current.x)>1e-6||Math.abs(Y-Xt.current.y)>1e-6||Math.abs(L-Xt.current.z)>1e-6){const G=new yt().makeRotationX(R),X=new yt().makeRotationY(Y),D=new yt().makeRotationZ(L);Uo.identity().multiply(D).multiply(G).multiply(X),qo.setFromMatrix4(Uo),Ve.setUniform("uPreRotMatrix",qo),Xt.current.set(R,Y,L),M=!0}}M&&Ve.resetAccumulation(),(Object.keys(h).length>0||Object.keys(a.liveModulations).length>0)&&N.getState().setLiveModulations(h),Go.current=u},oc=()=>{const e=fe(o=>o.isRecordingModulation);return v.useEffect(()=>{e?(it.current={},qa.current=-1):oa.setOverriddenTracks(new Set)},[e]),null},dt=xe(),rc=e=>{const o=v.useRef(!1),a=v.useRef(new U),r=v.useRef(new U),n=v.useRef(!1),s=v.useRef(null),i=v.useRef({x:0,y:0}),l=fe;v.useEffect(()=>{const c=p=>{const h=N.getState(),u=h.interactionMode;if(e.current){const g=e.current.getBoundingClientRect(),S=(p.clientX-g.left)/g.width*2-1,y=-((p.clientY-g.top)/g.height)*2+1;if(i.current={x:S,y},u==="picking_focus"){n.current=!0;let b=!1,m=!1,x=-1;const C=N.getState();C.focusLock&&C.setFocusLock(!1),dt.startFocusPick(S,y).then(w=>{n.current&&(b=!0,w>0&&w!==x&&(x=w,N.getState().setOptics({dofFocus:w})))});const z=()=>{n.current&&(b&&!m&&(m=!0,dt.sampleFocusPick(i.current.x,i.current.y).then(w=>{if(m=!1,!!n.current&&w>0&&w!==x){x=w,N.getState().setOptics({dofFocus:w});const{isRecording:M,isPlaying:P,addKeyframe:k,addTrack:T,currentFrame:_,sequence:E}=l.getState();if(M){const F="optics.dofFocus";E.tracks[F]||T(F,"Focus Distance"),k(F,_,w,P?"Linear":"Bezier")}}})),s.current=requestAnimationFrame(z))};s.current=requestAnimationFrame(z)}if(u==="picking_julia"){o.current=!0;const b=h.geometry;r.current.set(b.juliaX,b.juliaY,b.juliaZ),a.current.copy(r.current);const m=(z,w,M)=>{if(w==="MandelTerrain"){const P=M.coreMath,k=Math.pow(2,P.paramB);a.current.set(z.x*(2/k)+P.paramE,z.z*(2/k)+P.paramF,0)}else w==="JuliaMorph"?a.current.set(z.x,z.y,0):a.current.copy(z)};dt.pickWorldPosition(S,y,!0).then(z=>{z&&o.current&&m(z,h.formula,h)});let x=!1;const C=()=>{if(o.current){if(x||(x=!0,dt.pickWorldPosition(i.current.x,i.current.y,!0).then(z=>{if(x=!1,!!o.current&&z){const w=N.getState();m(z,w.formula,w)}})),r.current.lerp(a.current,.1),r.current.distanceToSquared(a.current)>1e-8){N.getState().setGeometry({juliaX:r.current.x,juliaY:r.current.y,juliaZ:r.current.z});const{isRecording:w,isPlaying:M,addKeyframe:P,addTrack:k,currentFrame:T,sequence:_}=l.getState();if(w){_.tracks["geometry.juliaX"]||k("geometry.juliaX","Julia X"),_.tracks["geometry.juliaY"]||k("geometry.juliaY","Julia Y"),_.tracks["geometry.juliaZ"]||k("geometry.juliaZ","Julia Z");const E=M?"Linear":"Bezier";P("geometry.juliaX",T,r.current.x,E),P("geometry.juliaY",T,r.current.y,E),P("geometry.juliaZ",T,r.current.z,E)}}s.current=requestAnimationFrame(C)}};s.current=requestAnimationFrame(C)}}},d=p=>{var h,u;if(e.current){const g=e.current.getBoundingClientRect(),S=(p.clientX-g.left)/g.width*2-1,y=-((p.clientY-g.top)/g.height)*2+1;(o.current||n.current)&&(i.current={x:S,y});const b=N.getState();if(b.draggedLightIndex!==null&&!dt.isGizmoInteracting){const m=Ot(),x=((u=(h=b.lighting)==null?void 0:h.lights)==null?void 0:u.findIndex(C=>C.id===b.draggedLightIndex))??-1;if(m&&x>=0){const C=new Qa;C.setFromCamera(new je(S,y),m);const z=Math.max(2e-4,Math.min(20,dt.lastMeasuredDistance*.5)),w=C.ray.direction.clone().multiplyScalar(z).add(C.ray.origin),M=dt.sceneOffset,P=b.lighting.lights[x];let k;if(P.fixed&&P.visible){const _=w.clone().sub(m.position).applyQuaternion(m.quaternion.clone().invert());k={x:_.x,y:_.y,z:_.z}}else k={x:w.x+M.x+(M.xL??0),y:w.y+M.y+(M.yL??0),z:w.z+M.z+(M.zL??0)};const T={visible:!0,castShadow:!0,position:k};P.visible||(T.fixed=!1),b.updateLight({index:x,params:T}),b.lighting.shadows||b.setLighting({shadows:!0}),b.showLightGizmo||b.setShowLightGizmo(!0)}}}},f=()=>{const p=N.getState();p.draggedLightIndex!==null&&p.setDraggedLight(null),o.current&&(o.current=!1,s.current&&cancelAnimationFrame(s.current),p.setInteractionMode("none"),navigator.vibrate&&navigator.vibrate(20)),n.current&&(n.current=!1,s.current&&cancelAnimationFrame(s.current),dt.endFocusPick(),p.setInteractionMode("none"),navigator.vibrate&&navigator.vibrate(20))};return window.addEventListener("pointerdown",c),window.addEventListener("pointermove",d),window.addEventListener("pointerup",f),()=>{window.removeEventListener("pointerdown",c),window.removeEventListener("pointermove",d),window.removeEventListener("pointerup",f),s.current&&cancelAnimationFrame(s.current)}},[e])},ic=e=>{const{interactionMode:o,setInteractionMode:a,setRenderRegion:r,renderRegion:n}=N(),s=o==="selecting_region",[i,l]=v.useState(null),[c,d]=v.useState(null),[f,p]=v.useState(null),h=v.useRef(null),u=v.useRef(null),g=v.useRef(null),S=(y,b)=>({x:Math.max(0,Math.min(1,(y.clientX-b.left)/b.width)),y:Math.max(0,Math.min(1,(y.clientY-b.top)/b.height))});return v.useEffect(()=>{if(!e.current)return;const y=e.current,b=C=>{const z=C.target,w=y.getBoundingClientRect(),M=S(C,w);if(n&&!s){const P=z.dataset.handle;if(P||z.closest(".region-box")){C.stopPropagation(),h.current=P||"move",l({x:M.x,y:M.y}),u.current={...n},p({...n}),g.current={...n};return}}if(s){C.stopPropagation(),h.current="draw";const P=C.clientX-w.left,k=C.clientY-w.top;l({x:P,y:k}),d({x:P,y:k})}},m=C=>{if(!h.current)return;C.stopPropagation(),C.preventDefault();const z=y.getBoundingClientRect();if(h.current==="draw"){const w=Math.max(0,Math.min(z.width,C.clientX-z.left)),M=Math.max(0,Math.min(z.height,C.clientY-z.top));d({x:w,y:M})}else if(u.current&&i){const w=S(C,z),M=w.x-i.x,P=1-w.y-(1-i.y);let k={...u.current};const T=h.current;if(T==="move"){const E=k.maxX-k.minX,F=k.maxY-k.minY;k.minX+=M,k.maxX+=M,k.minY+=P,k.maxY+=P,k.minX<0&&(k.minX=0,k.maxX=E),k.maxX>1&&(k.maxX=1,k.minX=1-E),k.minY<0&&(k.minY=0,k.maxY=F),k.maxY>1&&(k.maxY=1,k.minY=1-F)}else T!=null&&T.includes("e")&&(k.maxX=Math.min(1,u.current.maxX+M)),T!=null&&T.includes("w")&&(k.minX=Math.max(0,u.current.minX+M)),T!=null&&T.includes("n")&&(k.maxY=Math.min(1,u.current.maxY+P)),T!=null&&T.includes("s")&&(k.minY=Math.max(0,u.current.minY+P));const _={minX:Math.min(k.minX,k.maxX),maxX:Math.max(k.minX,k.maxX),minY:Math.min(k.minY,k.maxY),maxY:Math.max(k.minY,k.maxY)};_.maxX-_.minX<.01&&(_.maxX=_.minX+.01),_.maxY-_.minY<.01&&(_.maxY=_.minY+.01),p(_),g.current=_}},x=C=>{if(h.current){if(C.stopPropagation(),h.current==="draw"&&i&&c){const z=y.getBoundingClientRect(),w=Math.min(i.x,c.x),M=Math.max(i.x,c.x),P=Math.min(i.y,c.y),k=Math.max(i.y,c.y),T=M-w,_=k-P;if(T>10&&_>10){const E=w/z.width,F=M/z.width,O=1-k/z.height,$=1-P/z.height;r({minX:E,minY:O,maxX:F,maxY:$})}a("none")}else g.current&&r(g.current);h.current=null,l(null),d(null),u.current=null,p(null),g.current=null}};return y.addEventListener("mousedown",b),window.addEventListener("mousemove",m),window.addEventListener("mouseup",x),()=>{y.removeEventListener("mousedown",b),window.removeEventListener("mousemove",m),window.removeEventListener("mouseup",x)}},[s,n,i,c,a,r,e]),{visualRegion:f,isGhostDragging:!!f,renderRegion:n,isSelectingRegion:s}},Ct=xe(),de={lowFpsBuffer:0,lastTime:performance.now(),lastFrameCount:0,setShowWarning:null,setCurrentFps:null,isPaused:!1,isScrubbing:!1,isExporting:!1,isBroadcastMode:!1,renderMode:"PathTracing",frameTimestamps:[]},nc=()=>{const e=performance.now();de.frameTimestamps.push(e);const o=e-2e3;if(de.frameTimestamps=de.frameTimestamps.filter(r=>r>o),e-de.lastTime>=500){let r=0;if(de.frameTimestamps.length>1){const l=de.frameTimestamps[0],d=de.frameTimestamps[de.frameTimestamps.length-1]-l;r=(de.frameTimestamps.length-1)/d*1e3}de.lastTime=e,de.lastFrameCount=Ct.frameCount;const n=N.getState(),s=n.sampleCap>0&&Ct.accumulationCount>=n.sampleCap;if(de.isPaused||de.isScrubbing||document.hidden||Ct.isCompiling||de.isExporting||s)de.lowFpsBuffer=0;else if(e<8e3)de.lowFpsBuffer=0;else{de.setCurrentFps&&de.setCurrentFps(r);const l=de.renderMode==="PathTracing",c=l?10:15,d=l?22:30;r<c?de.lowFpsBuffer+=r<5?2:1:r>=d&&(de.lowFpsBuffer=Math.max(0,de.lowFpsBuffer-3),de.lowFpsBuffer===0&&de.setShowWarning&&de.setShowWarning(!1)),de.lowFpsBuffer>=5&&de.setShowWarning&&de.setShowWarning(!0)}}},sc=()=>{const e=v.useRef(0);v.useRef(performance.now()),v.useRef(0);const{resolutionMode:o,setResolutionMode:a,setFixedResolution:r,fixedResolution:n,isExporting:s,isBroadcastMode:i,openContextMenu:l,aaLevel:c,setAALevel:d,renderMode:f,quality:p}=N(),h=N(O=>O.isPaused),u=fe(O=>O.isScrubbing),[g,S]=v.useState(!1),[y,b]=v.useState(60);v.useEffect(()=>(de.setShowWarning=S,de.setCurrentFps=b,de.isPaused=h,de.isScrubbing=u,de.isExporting=s,de.isBroadcastMode=i,de.renderMode=f,de.lastTime=performance.now(),de.lastFrameCount=Ct.frameCount,()=>{de.setShowWarning===S&&(de.setShowWarning=null),de.setCurrentFps===b&&(de.setCurrentFps=null)}),[h,u,s,i,f]);const m=O=>{const $=Ge(O.currentTarget);$.length>0&&(O.preventDefault(),O.stopPropagation(),l(O.clientX,O.clientY,[],$))};if(i||!g)return null;let x=window.innerWidth,C=window.innerHeight;if(Ct.renderer){const O=Ct.renderer.domElement;x=O.width,C=O.height}else o==="Fixed"&&(x=n[0],C=n[1]);const z=x>480,w=()=>{const O=Math.max(320,Math.round(x*.66/8)*8),$=Math.max(240,Math.round(C*.66/8)*8);a("Fixed"),r(O,$),F()},M=c>1,P=()=>{d(1),F()},k=p,_=!((k==null?void 0:k.precisionMode)===1),E=()=>{const O=N.getState().setQuality,$=N.getState().setLighting;O&&O({precisionMode:1,bufferPrecision:1}),$&&$({shadows:!1}),F()},F=()=>{S(!1),e.current=-10};return t.jsx("div",{className:"absolute top-2 right-4 z-[50] pointer-events-auto animate-fade-in-left origin-top-right max-w-[200px]","data-help-id":"ui.performance",onContextMenu:m,children:t.jsxs("div",{className:"flex flex-col gap-1 bg-red-950/90 border border-red-500/30 rounded-lg shadow-xl backdrop-blur-md p-2",children:[t.jsxs("div",{className:"flex items-center justify-between mb-1",children:[t.jsxs("div",{className:"flex items-center gap-2 text-red-200 text-[10px] font-bold",children:[t.jsx(Rt,{}),t.jsxs("span",{children:["Low FPS (",y.toFixed(1),")"]})]}),t.jsx("button",{onClick:()=>{S(!1),e.current=-40},className:"text-red-400 hover:text-white transition-colors p-0.5",title:"Dismiss",children:t.jsx(eo,{})})]}),t.jsxs("div",{className:"flex flex-col gap-1",children:[M&&t.jsxs("button",{onClick:P,className:"flex items-center justify-between bg-black/40 hover:bg-white/10 text-gray-300 text-[9px] px-2 py-1.5 rounded transition-colors border border-white/5",children:[t.jsxs("span",{className:"flex items-center gap-1.5",children:[t.jsx(to,{})," Reset Scale (1x)"]}),t.jsx("span",{className:"text-cyan-400 font-bold",children:"Fix"})]}),_&&t.jsxs("button",{onClick:E,className:"flex items-center justify-between bg-black/40 hover:bg-white/10 text-gray-300 text-[9px] px-2 py-1.5 rounded transition-colors border border-white/5",children:[t.jsxs("span",{className:"flex items-center gap-1.5",children:[t.jsx(ma,{})," Enable Lite Mode"]}),t.jsx("span",{className:"text-cyan-400 font-bold",children:"Fix"})]}),z&&t.jsxs("button",{onClick:w,className:"flex items-center justify-between bg-black/40 hover:bg-white/10 text-gray-300 text-[9px] px-2 py-1.5 rounded transition-colors border border-white/5",children:[t.jsxs("span",{className:"flex items-center gap-1.5",children:[t.jsx(ft,{})," Reduce Resolution"]}),t.jsx("span",{className:"text-cyan-400 font-bold",children:"-33%"})]})]})]})})},lc=[{label:"Maximum",ratio:"Max"},{label:"Square (1:1)",ratio:1},{label:"Landscape (16:9)",ratio:1.7777},{label:"Portrait (4:5)",ratio:.8},{label:"Social (9:16)",ratio:.5625},{label:"Cinematic (2.35:1)",ratio:2.35},{label:"Classic (4:3)",ratio:1.3333},{label:"Skybox (2:1)",ratio:2}],cc=({width:e,height:o,top:a,left:r,maxAvailableWidth:n,maxAvailableHeight:s,onSetResolution:i,onSetMode:l})=>{const[c,d]=v.useState(!1),f=v.useRef(null),p=v.useRef(null),h=N(m=>m.openContextMenu);v.useEffect(()=>{if(!c)return;const m=x=>{f.current&&!f.current.contains(x.target)&&d(!1)};return window.addEventListener("mousedown",m),()=>window.removeEventListener("mousedown",m)},[c]);const u=m=>{const x=Ge(m.currentTarget);x.length>0&&(m.preventDefault(),m.stopPropagation(),h(m.clientX,m.clientY,[],x))},g=m=>{m.preventDefault(),m.stopPropagation(),m.target.setPointerCapture(m.pointerId),p.current={startX:m.clientX,startY:m.clientY,startW:e,startH:o,hasMoved:!1}},S=m=>{if(!p.current)return;const x=p.current.startX-m.clientX,C=p.current.startY-m.clientY;(Math.abs(x)>3||Math.abs(C)>3)&&(p.current.hasMoved=!0);const z=Math.round(C/4),w=Math.round(x/20),P=(z+w)*8;if(P!==0){const k=p.current.startW/p.current.startH,T=Math.max(64,p.current.startW+P),_=Math.round(T/8)*8,E=_/k,F=Math.max(64,Math.round(E/8)*8);i(_,F)}},y=m=>{m.target.releasePointerCapture(m.pointerId),p.current&&!p.current.hasMoved&&d(x=>!x),p.current=null},b=m=>{const C=Math.max(100,n-40),z=Math.max(100,s-40);let w,M;m==="Max"?(w=C,M=z):C/z>m?(M=z,w=M*m):(w=C,M=w/m);const P=Math.round(w/8)*8,k=Math.round(M/8)*8;i(P,k),d(!1)};return t.jsxs("div",{className:"absolute flex items-center gap-2 z-50 transition-all duration-100 ease-out",style:{top:a,left:r},"data-help-id":"ui.resolution",onContextMenu:u,children:[t.jsxs("div",{className:"relative text-[10px] font-mono text-gray-400 bg-black/80 px-2 py-1 rounded border border-white/10 shadow-sm backdrop-blur-md cursor-ns-resize hover:text-white hover:border-cyan-500/50 transition-colors select-none flex items-center gap-2",onPointerDown:g,onPointerMove:S,onPointerUp:y,title:"Drag Up or Left to Increase Size",children:[t.jsxs("span",{children:[e," ",t.jsx("span",{className:"text-gray-600",children:"x"})," ",o]}),t.jsx("span",{className:"opacity-50",children:t.jsx(Tt,{})})]}),c&&t.jsxs("div",{ref:f,className:"absolute top-8 left-0 w-32 bg-black border border-white/20 rounded shadow-xl z-50 overflow-hidden flex flex-col py-1 animate-fade-in",children:[t.jsx("div",{className:"px-3 py-1 text-[8px] font-bold text-gray-500 border-b border-white/10 mb-1",children:"Fit to Window"}),lc.map(m=>t.jsx("button",{onClick:()=>b(m.ratio),className:"text-left px-3 py-1.5 text-[10px] text-gray-300 hover:bg-white/10 hover:text-white transition-colors flex justify-between",children:t.jsx("span",{children:m.label})},m.label))]}),t.jsxs("button",{onClick:m=>{m.stopPropagation(),l("Full")},className:"flex items-center gap-1.5 text-[9px] font-bold text-gray-300 bg-black/80 px-2 py-1 rounded border border-white/10 hover:border-cyan-500/50 hover:text-cyan-400 hover:bg-cyan-900/30 transition-all shadow-sm backdrop-blur-md group",title:"Return to Fullscreen Mode",children:[t.jsx("span",{className:"w-2 h-2 border border-current rounded-sm group-hover:scale-110 transition-transform"}),"Fill"]})]})},dc=({width:e,height:o})=>{const a=N(c=>c.compositionOverlay),r=N(c=>c.compositionOverlaySettings);if(a==="none"||!a)return null;const{opacity:n,lineThickness:s,color:i}=r;let l=i;if(i.startsWith("#")){const c=lt(i);c&&(l=`rgba(${c.r},${c.g},${c.b},${n})`)}else i.startsWith("rgba")?l=i.replace(/rgba\(([^,]+),([^,]+),([^,]+),([^)]+)\)/,`rgba($1,$2,$3,${n})`):i.startsWith("rgb(")&&(l=i.replace(/rgb\(([^,]+),([^,]+),([^)]+)\)/,`rgba($1,$2,$3,${n})`));return t.jsxs("svg",{className:"absolute inset-0 pointer-events-none z-[15]",width:e,height:o,style:{mixBlendMode:"difference"},children:[a==="grid"&&t.jsx(uc,{width:e,height:o,strokeColor:l,lineThickness:s,divisionsX:r.gridDivisionsX,divisionsY:r.gridDivisionsY}),a==="thirds"&&t.jsx(fc,{width:e,height:o,strokeColor:l,lineThickness:s}),a==="golden"&&t.jsx($r,{width:e,height:o,strokeColor:l,lineThickness:s}),a==="spiral"&&t.jsx(pc,{width:e,height:o,strokeColor:l,lineThickness:s,rotation:r.spiralRotation,positionX:r.spiralPositionX,positionY:r.spiralPositionY,scale:r.spiralScale,ratio:r.spiralRatio}),a==="center"&&t.jsx(Wo,{width:e,height:o,strokeColor:l,lineThickness:s}),a==="diagonal"&&t.jsx(hc,{width:e,height:o,strokeColor:l,lineThickness:s}),a==="safearea"&&t.jsx(Xo,{width:e,height:o,strokeColor:l,lineThickness:s}),r.showCenterMark&&a!=="center"&&t.jsx(Wo,{width:e,height:o,strokeColor:l,lineThickness:s}),r.showSafeAreas&&a!=="safearea"&&t.jsx(Xo,{width:e,height:o,strokeColor:l,lineThickness:s*.5})]})},uc=({width:e,height:o,strokeColor:a,lineThickness:r,divisionsX:n=4,divisionsY:s=4})=>{const i=[];for(let l=1;l<n;l++){const c=e/n*l;i.push(t.jsx("line",{x1:c,y1:0,x2:c,y2:o,stroke:a,strokeWidth:r*.5},`v${l}`))}for(let l=1;l<s;l++){const c=o/s*l;i.push(t.jsx("line",{x1:0,y1:c,x2:e,y2:c,stroke:a,strokeWidth:r*.5},`h${l}`))}return t.jsx(t.Fragment,{children:i})},fc=({width:e,height:o,strokeColor:a,lineThickness:r})=>{const n=e/3,s=o/3;return t.jsxs(t.Fragment,{children:[t.jsx("line",{x1:n,y1:0,x2:n,y2:o,stroke:a,strokeWidth:r}),t.jsx("line",{x1:n*2,y1:0,x2:n*2,y2:o,stroke:a,strokeWidth:r}),t.jsx("line",{x1:0,y1:s,x2:e,y2:s,stroke:a,strokeWidth:r}),t.jsx("line",{x1:0,y1:s*2,x2:e,y2:s*2,stroke:a,strokeWidth:r}),t.jsx("circle",{cx:n,cy:s,r:r*3,fill:a}),t.jsx("circle",{cx:n*2,cy:s,r:r*3,fill:a}),t.jsx("circle",{cx:n,cy:s*2,r:r*3,fill:a}),t.jsx("circle",{cx:n*2,cy:s*2,r:r*3,fill:a})]})},$r=({width:e,height:o,strokeColor:a,lineThickness:r})=>{const n=1.618033988749895,s=e/n,i=o/n;return t.jsxs(t.Fragment,{children:[t.jsx("line",{x1:s,y1:0,x2:s,y2:o,stroke:a,strokeWidth:r}),t.jsx("line",{x1:e-s,y1:0,x2:e-s,y2:o,stroke:a,strokeWidth:r}),t.jsx("line",{x1:0,y1:i,x2:e,y2:i,stroke:a,strokeWidth:r}),t.jsx("line",{x1:0,y1:o-i,x2:e,y2:o-i,stroke:a,strokeWidth:r}),t.jsx("circle",{cx:s,cy:i,r:r*3,fill:a}),t.jsx("circle",{cx:e-s,cy:i,r:r*3,fill:a}),t.jsx("circle",{cx:s,cy:o-i,r:r*3,fill:a}),t.jsx("circle",{cx:e-s,cy:o-i,r:r*3,fill:a})]})},pc=({width:e,height:o,strokeColor:a,lineThickness:r,rotation:n=0,positionX:s=.5,positionY:i=.5,scale:l=1,ratio:c=1.618033988749895})=>{const d=Math.min(e,o),f=e*s,p=o*i,h=d*.45*l,u=n*Math.PI/180,g=[],S=3,y=100;for(let b=0;b<=y;b++){const m=b/y*S*2*Math.PI,x=h*Math.pow(c,-m/(2*Math.PI)),C=f+x*Math.cos(m+u),z=p+x*Math.sin(m+u);g.push(`${b===0?"M":"L"} ${C} ${z}`)}return t.jsxs(t.Fragment,{children:[t.jsx("path",{d:g.join(" "),fill:"none",stroke:a,strokeWidth:r*1.5,strokeLinecap:"round"}),t.jsx($r,{width:e,height:o,strokeColor:a.replace(/[\d.]+\)$/,"0.2)"),lineThickness:r*.5})]})},Wo=({width:e,height:o,strokeColor:a,lineThickness:r})=>{const n=e/2,s=o/2,i=Math.min(e,o)*.05;return t.jsxs(t.Fragment,{children:[t.jsx("line",{x1:n-i,y1:s,x2:n+i,y2:s,stroke:a,strokeWidth:r}),t.jsx("line",{x1:n,y1:s-i,x2:n,y2:s+i,stroke:a,strokeWidth:r}),t.jsx("circle",{cx:n,cy:s,r:i*.3,fill:"none",stroke:a,strokeWidth:r})]})},hc=({width:e,height:o,strokeColor:a,lineThickness:r})=>t.jsxs(t.Fragment,{children:[t.jsx("line",{x1:0,y1:0,x2:e,y2:o,stroke:a,strokeWidth:r}),t.jsx("line",{x1:e,y1:0,x2:0,y2:o,stroke:a,strokeWidth:r}),t.jsx("line",{x1:e/2,y1:0,x2:e/2,y2:o,stroke:a,strokeWidth:r*.5,strokeDasharray:"4 4"}),t.jsx("line",{x1:0,y1:o/2,x2:e,y2:o/2,stroke:a,strokeWidth:r*.5,strokeDasharray:"4 4"})]}),Xo=({width:e,height:o,strokeColor:a,lineThickness:r})=>{const s=e*.05,i=o*.05,l=e*(1-.05*2),c=o*(1-.05*2),d=.1,f=e*d,p=o*d,h=e*(1-d*2),u=o*(1-d*2);return t.jsxs(t.Fragment,{children:[t.jsx("rect",{x:s,y:i,width:l,height:c,fill:"none",stroke:a,strokeWidth:r}),t.jsx("rect",{x:f,y:p,width:h,height:u,fill:"none",stroke:a,strokeWidth:r*.5,strokeDasharray:"4 4"}),[[s,i],[s+l,i],[s,i+c],[s+l,i+c]].map(([g,S],y)=>t.jsx("circle",{cx:g,cy:S,r:r*2,fill:a},y))]})},mc=(e,o,a,r)=>{const{gl:n}=ua(),s=N(m=>m.invertY),i=N(m=>m.debugMobileLayout),l=v.useRef({forward:!1,backward:!1,left:!1,right:!1,up:!1,down:!1,rollLeft:!1,rollRight:!1,boost:!1}),c=v.useRef(!1),d=v.useRef({x:0,y:0}),f=v.useRef({x:0,y:0}),p=v.useRef(0),h=v.useRef(0),u=v.useRef({x:0,y:0}),g=v.useRef({x:0,y:0}),S=v.useRef(o);v.useEffect(()=>{S.current=o},[o]);const y=()=>{p.current=performance.now()};return v.useEffect(()=>{u.current={x:0,y:0},g.current={x:0,y:0},h.current=0,y()},[e]),fa((m,x)=>{const C=l.current.rollLeft?1:l.current.rollRight?-1:0,w=1-Math.exp(-(C!==0?1:3)*x),M=Math.max(.1,S.current);h.current+=(C*M-h.current)*w,C===0&&Math.abs(h.current)<.001&&(h.current=0)}),v.useEffect(()=>{const m=P=>{if(P.target.tagName==="INPUT"||P.target.tagName==="TEXTAREA"||((P.ctrlKey||P.metaKey)&&(P.key==="w"||P.code==="KeyW")&&P.preventDefault(),P.code==="Space"&&P.preventDefault(),P.key==="Alt"&&P.preventDefault(),N.getState().isTimelineHovered))return;let T=!0;switch(P.code){case"KeyW":l.current.forward=!0;break;case"KeyS":l.current.backward=!0;break;case"KeyA":l.current.left=!0;break;case"KeyD":l.current.right=!0;break;case"KeyQ":l.current.rollLeft=!0;break;case"KeyE":l.current.rollRight=!0;break;case"Space":l.current.up=!0;break;case"KeyC":l.current.down=!0;break;case"ShiftLeft":case"ShiftRight":l.current.boost=!0;break;default:T=!1}T&&y()},x=P=>{switch(P.key==="Alt"&&P.preventDefault(),P.code){case"KeyW":l.current.forward=!1;break;case"KeyS":l.current.backward=!1;break;case"KeyA":l.current.left=!1;break;case"KeyD":l.current.right=!1;break;case"KeyQ":l.current.rollLeft=!1;break;case"KeyE":l.current.rollRight=!1;break;case"Space":l.current.up=!1;break;case"KeyC":l.current.down=!1;break;case"ShiftLeft":case"ShiftRight":l.current.boost=!1;break}},C=P=>{const k=N.getState();if($a(k))return;if(k.optics&&Math.abs(k.optics.camType-1)<.1){const E=P.deltaY>0?1:-1,O=k.optics.orthoScale*(1+E*.1);k.setOptics({orthoScale:Math.max(1e-10,Math.min(1e3,O))}),y();return}if(e==="Fly"){const E=P.deltaY>0?-1:1;let F=S.current,O=.01;F<.05&&(O=.005),F>.1&&(O=.02);let $=Math.max(.001,Math.min(1,F+E*O));S.current=$,a($),y()}else e==="Orbit"&&y()},z=()=>{l.current={forward:!1,backward:!1,left:!1,right:!1,up:!1,down:!1,rollLeft:!1,rollRight:!1,boost:!1}},w=P=>{u.current=P.detail,y()},M=P=>{g.current=P.detail,y()};return window.addEventListener("keydown",m),window.addEventListener("keyup",x),window.addEventListener("blur",z),window.addEventListener("joyMove",w),window.addEventListener("joyLook",M),n.domElement.addEventListener("wheel",C,{passive:!0}),()=>{window.removeEventListener("keydown",m),window.removeEventListener("keyup",x),window.removeEventListener("blur",z),window.removeEventListener("joyMove",w),window.removeEventListener("joyLook",M),n.domElement.removeEventListener("wheel",C)}},[e,n,a]),v.useEffect(()=>{const m=n.domElement,x=(M,P)=>{const k=m.getBoundingClientRect();return{x:(M-k.left)/k.width*2-1,y:-((P-k.top)/k.height)*2+1}},C=M=>{const P=N.getState();if($a(P))return;if(r){if(Object.values(r).some(_=>_.current&&_.current.contains(M.target)))return}else if(M.target.closest(".pointer-events-auto"))return;if(!((i||window.innerWidth<768||M.pointerType==="touch")&&e==="Fly")&&(y(),M.button===0&&e==="Fly")){c.current=!0;const{x:T,y:_}=x(M.clientX,M.clientY);d.current={x:T,y:_},f.current={x:T,y:_}}},z=M=>{if(c.current){const{x:P,y:k}=x(M.clientX,M.clientY);f.current={x:P,y:k},y()}else e==="Orbit"&&M.buttons>0&&(M.target===m||m.contains(M.target))&&y()},w=()=>c.current=!1;return m.addEventListener("mousedown",C),window.addEventListener("mousemove",z),window.addEventListener("mouseup",w),()=>{m.removeEventListener("mousedown",C),window.removeEventListener("mousemove",z),window.removeEventListener("mouseup",w)}},[e,n,i,r]),{moveState:l,isDraggingRef:c,dragStart:d,mousePos:f,speedRef:S,joystickMove:u,joystickLook:g,invertY:s,rollVelocity:h,isInteracting:()=>{const m=l.current,x=m.forward||m.backward||m.left||m.right||m.up||m.down||m.rollLeft||m.rollRight,C=Math.abs(u.current.x)>.01||Math.abs(u.current.y)>.01||Math.abs(g.current.x)>.01||Math.abs(g.current.y)>.01,z=performance.now()-p.current<200;return c.current||x||C||z}}},gt=xe(),gc=(e,o)=>{const{camera:a}=ua(),r=N(f=>f.quality),n=v.useRef(10),s=v.useRef(10),i=v.useRef(0);v.useRef(!1);const l=v.useRef(null),c=v.useRef(new Float32Array(4)),d=f=>{if(f<0||f>=1e3||!Number.isFinite(f)){n.current>0&&n.current,e.dist.current&&(e.dist.current.innerText="DST INF",e.dist.current.style.color="#888"),e.reset.current&&(e.reset.current.style.display="block");return}if(n.current=f,s.current=f,gt.lastMeasuredDistance=f,e.dist.current&&(e.dist.current.innerText=`DST ${f<.001?f.toExponential(2):f.toFixed(4)}`,e.dist.current.style.color=f<1?"#ff4444":"#00ffff"),e.reset.current){const p=f>100||f<.001;e.reset.current.style.display=p?"block":"none"}};return fa(f=>{var y,b,m,x,C;if(i.current++,!gt.hasCompiledShader||i.current<15){e.dist.current&&i.current%10===0&&(e.dist.current.innerText=`DST ${n.current.toFixed(4)}`),e.speed.current&&i.current%10===0&&(e.speed.current.innerText=`SPD ${(o.current*100).toFixed(1)}%`);return}if(!r)return;if((r.physicsProbeMode??0)===2){const z=r.manualDistance;if(n.current=z,s.current=z,gt.lastMeasuredDistance=z,e.dist.current&&(e.dist.current.innerText=`DST ${z<.001?z.toExponential(2):z.toFixed(4)}`,e.dist.current.style.color=z<1?"#ff4444":"#00ffff"),e.reset.current){const w=z>100||z<.001;e.reset.current.style.display=w?"block":"none"}e.speed.current&&i.current%10===0&&(e.speed.current.innerText=`SPD ${(o.current*100).toFixed(1)}%`);return}const h=gt.renderer;if(!h){const z=gt.lastMeasuredDistance;if(z!==n.current){d(z);const w=N.getState();if(w.focusLock&&z>0&&z<1e3){const M=((y=w.optics)==null?void 0:y.dofFocus)??0;Math.abs(z-M)/Math.max(M,1e-4)>.01&&w.setOptics({dofFocus:z})}}e.speed.current&&i.current%10===0&&(e.speed.current.innerText=`SPD ${(o.current*100).toFixed(1)}%`);return}const u=(m=(b=gt.pipeline)==null?void 0:b.getPreviousRenderTarget)==null?void 0:m.call(b);if(!u){e.dist.current&&i.current%10===0&&(e.dist.current.innerText=`DST ${n.current.toFixed(4)}`),e.speed.current&&i.current%10===0&&(e.speed.current.innerText=`SPD ${(o.current*100).toFixed(1)}%`);return}const g=u.width||1,S=u.height||1;if(g<=0||S<=0){e.dist.current&&i.current%10===0&&(e.dist.current.innerText=`DST ${n.current.toFixed(4)}`),e.speed.current&&i.current%10===0&&(e.speed.current.innerText=`SPD ${(o.current*100).toFixed(1)}%`);return}(!l.current||l.current.length!==64)&&(l.current=new Float32Array(64));try{const z=Math.floor(g/2),w=Math.floor(S/2),P=Math.floor(3/2);let k=0,T=0;for(let _=-P;_<=P;_++){for(let E=-P;E<=P;E++){const F=z+_,O=w+E;if(F>=0&&F<g&&O>=0&&O<S&&((C=(x=gt.pipeline)==null?void 0:x.readPixels)==null?void 0:C.call(x,h,F,O,1,1,c.current))){const R=c.current[3];R>0&&R<1e3&&Number.isFinite(R)&&(k+=R,T++)}}if(T>0){const E=k/T;d(E)}}if(T>0){const _=k/T;if(_>=1e3){const E=n.current>0?n.current:10;e.dist.current&&(e.dist.current.innerText="DST INF",e.dist.current.style.color="#888"),e.reset.current&&(e.reset.current.style.display="block")}else d(_)}}catch(z){console.warn("Depth readback failed:",z)}e.speed.current&&i.current%10===0&&(e.speed.current.innerText=`SPD ${(o.current*100).toFixed(1)}%`)}),{distAverageRef:n,distMinRef:s}};class xc{constructor(){J(this,"currentRotVelocity",new U);J(this,"rollVelocity",0);J(this,"smoothedDistEstimate",10);J(this,"ROTATION_SMOOTHING",20);J(this,"ROLL_SMOOTHING",3);J(this,"SENSITIVITY",2.5);J(this,"DIST_INCREASE_LERP_RATE",2)}reset(){this.currentRotVelocity.set(0,0,0),this.rollVelocity=0,this.smoothedDistEstimate=10}applyCurve(o){return Math.sign(o)*Math.pow(Math.abs(o),2)}update(o,a,r,n){let s=!1;const i=n.distEstimate,l=Math.min(a,.1);if(i>this.smoothedDistEstimate){const z=1-Math.exp(-this.DIST_INCREASE_LERP_RATE*l);this.smoothedDistEstimate+=(i-this.smoothedDistEstimate)*z}else this.smoothedDistEstimate=i;const c=this.smoothedDistEstimate,d=r.move.boost?4:1,f=n.autoSlow?Math.max(c*n.baseSpeed*d,1e-6):2*n.baseSpeed*d,p=new U(0,0,0);if(r.move.forward&&(p.z-=1),r.move.backward&&(p.z+=1),r.move.left&&(p.x-=1),r.move.right&&(p.x+=1),r.move.up&&(p.y+=1),r.move.down&&(p.y-=1),(Math.abs(r.moveJoy.x)>.01||Math.abs(r.moveJoy.y)>.01)&&(p.z-=this.applyCurve(r.moveJoy.y),p.x+=this.applyCurve(r.moveJoy.x)),p.lengthSq()>0){p.normalize().multiplyScalar(f*l);const z=p.clone().applyQuaternion(o.quaternion);Z.emit("offset_shift",{x:z.x,y:z.y,z:z.z}),s=!0}const u=r.move.rollLeft?1:r.move.rollRight?-1:0,g=u!==0?1:this.ROLL_SMOOTHING,S=1-Math.exp(-g*a),y=Math.max(.1,n.baseSpeed);this.rollVelocity+=(u*y-this.rollVelocity)*S,u===0&&Math.abs(this.rollVelocity)<.001&&(this.rollVelocity=0);const b=new U(0,0,0),m=r.invertY?-1:1,x=Math.abs(r.look.x)>.01||Math.abs(r.look.y)>.01;r.isDragging?(b.y=-r.dragDelta.x*2,b.x=r.dragDelta.y*2*m):x&&(b.y=-this.applyCurve(r.look.x)*.66,b.x=this.applyCurve(r.look.y)*.66*m),b.z=this.rollVelocity*.62;const C=1-Math.exp(-this.ROTATION_SMOOTHING*l);if(this.currentRotVelocity.lerp(b,C),this.currentRotVelocity.lengthSq()<1e-6&&this.currentRotVelocity.set(0,0,0),this.currentRotVelocity.lengthSq()>1e-8){const z=this.currentRotVelocity;o.rotateX(z.x*l*this.SENSITIVITY),o.rotateY(z.y*l*this.SENSITIVITY),o.rotateZ(z.z*l*this.SENSITIVITY),s=!0}return s}}const Ie=xe(),yc=({mode:e,onStart:o,onEnd:a,hudRefs:r,setSceneOffset:n,fitScale:s=1})=>{const{camera:i,gl:l}=ua(),c=v.useRef(null),d=N($a),f=N(te=>te.optics),p=f&&Math.abs(f.camType-1)<.1,h=N(te=>te.navigation),u=N(te=>te.setNavigation),g=fe(te=>te.setIsCameraInteracting),[S,y]=v.useState(e==="Orbit"),b=v.useRef(new U(0,0,0)),m=v.useRef(new xc),x=v.useRef(3.5),C=v.useRef({x:0,y:0,z:0,xL:0,yL:0,zL:0}),z=v.useRef(new U),w=v.useRef(new De),M=v.useRef(new U),P=v.useRef(new U);v.useRef(0);const k=v.useRef(null),{moveState:T,isDraggingRef:_,dragStart:E,mousePos:F,speedRef:O,joystickMove:$,joystickLook:R,invertY:Y,rollVelocity:L,isInteracting:G}=mc(e,(h==null?void 0:h.flySpeed)??.5,te=>u({flySpeed:te}),r),{distAverageRef:X}=gc(r,O),D=(te=!1)=>{if(i.position.lengthSq()<1e-8)return;const be=Ie.sceneOffset,Pe={x:be.x,y:be.y,z:be.z,xL:(be.xL??0)+i.position.x,yL:(be.yL??0)+i.position.y,zL:(be.zL??0)+i.position.z};if(x.current=i.position.length()||x.current,i.position.set(0,0,0),i.updateMatrixWorld(),c.current){const Xe=new U(0,0,-1).applyQuaternion(i.quaternion);c.current.target.copy(Xe.multiplyScalar(1e-4)),b.current.copy(c.current.target)}te?Ie.queueOffsetSync(Pe):n(Pe),z.current.set(0,0,0)},H=te=>{i.updateMatrixWorld(),i.up.copy(new U(0,1,0).applyQuaternion(i.quaternion));let be=te||Ie.lastMeasuredDistance;(be<=1e-7||be>=1e3)&&(be=N.getState().targetDistance||3.5),x.current=be;const Pe=new U(0,0,-1).applyQuaternion(i.quaternion),Xe=new U().copy(i.position).addScaledVector(Pe,be);b.current.copy(Xe),y(!0)};v.useEffect(()=>{const te=N.getState();i.position.set(0,0,0),i.quaternion.set(te.cameraRot.x,te.cameraRot.y,te.cameraRot.z,te.cameraRot.w),i.updateMatrixWorld(),z.current.copy(i.position),w.current.copy(i.quaternion),te.targetDistance&&(X.current=te.targetDistance,x.current=te.targetDistance),e==="Orbit"&&H(te.targetDistance)},[]),v.useEffect(()=>{const te=ge=>{if(i.position.set(ge.position.x,ge.position.y,ge.position.z),i.quaternion.set(ge.rotation.x,ge.rotation.y,ge.rotation.z,ge.rotation.w),i.updateMatrixWorld(),M.current.set(0,0,0),P.current.set(0,0,0),m.current.reset(),z.current.copy(i.position),w.current.copy(i.quaternion),ge.targetDistance&&ge.targetDistance>.001&&(X.current=ge.targetDistance,x.current=ge.targetDistance,Ie.lastMeasuredDistance=ge.targetDistance),ge.sceneOffset&&(n(ge.sceneOffset),Ie.shouldSnapCamera=!0,Ie.dirty=!0),e==="Orbit"){const Be=ge.targetDistance||X.current||3.5;x.current=Be;const ae=new U(0,0,-1).applyQuaternion(i.quaternion);b.current.copy(i.position).addScaledVector(ae,Be),c.current&&(c.current.target.copy(b.current),c.current.update())}},be=()=>{},Pe=ge=>{if(!ge.sceneOffset){te(ge);return}const Be=Ie.sceneOffset,ae=new U(Be.x+(Be.xL??0)+i.position.x,Be.y+(Be.yL??0)+i.position.y,Be.z+(Be.zL??0)+i.position.z),re=ge.sceneOffset,ue=new U(re.x+(re.xL??0)+ge.position.x,re.y+(re.yL??0)+ge.position.y,re.z+(re.zL??0)+ge.position.z);if(ae.distanceTo(ue)<1e-6){te(ge);return}k.current={active:!0,startPos:ae,startRot:i.quaternion.clone(),endState:ge,endPos:ue,endRot:new De(ge.rotation.x,ge.rotation.y,ge.rotation.z,ge.rotation.w),elapsed:0,duration:.5}},Xe=Z.on("camera_teleport",te),ht=Z.on("reset_accum",be),St=Z.on("camera_transition",Pe);return()=>{Xe(),ht(),St()}},[e,i]);const A=fe(te=>te.isPlaying),j=fe(te=>te.isScrubbing),B=fe(te=>te.isRecording),I=fe(te=>te.recordCamera),V=fe(te=>te.currentFrame),W=fe(te=>te.sequence),q=fe(te=>te.captureCameraFrame),Q=v.useRef(!1),K=v.useRef(null),ie=v.useRef(e),ee=v.useRef(!1),oe=v.useRef(!1),le=v.useRef(null),ce=v.useRef(A),pe=v.useRef(j),he=v.useRef(1),Te=v.useRef(!1),Ne=v.useRef(!1);v.useEffect(()=>{const te=l.domElement,be=()=>{oe.current=!0,le.current&&clearTimeout(le.current),le.current=setTimeout(()=>{oe.current=!1},100)};return te.addEventListener("wheel",be,{passive:!0}),()=>{te.removeEventListener("wheel",be),le.current&&clearTimeout(le.current)}},[l,e]),v.useEffect(()=>{if(e!=="Orbit")return;const te=l.domElement,be=Xe=>{if(Xe.target.closest(".pointer-events-auto")){Ne.current=!1;return}Ne.current=!0,c.current&&(c.current.enabled=!0)},Pe=()=>{Ne.current=!1};return te.addEventListener("pointerdown",be,{capture:!0}),window.addEventListener("pointerup",Pe),()=>{te.removeEventListener("pointerdown",be,{capture:!0}),window.removeEventListener("pointerup",Pe)}},[e,l]);const Se=A&&(!B||!I)&&Object.keys(W.tracks).some(te=>te.startsWith("camera."))||j;return Te.current=Se,v.useEffect(()=>{Se?y(!1):e==="Orbit"&&!S&&H()},[Se,e]),v.useLayoutEffect(()=>{if(ie.current!==e){if(Z.emit("camera_snap",void 0),D(),e==="Fly"){Z.emit("camera_snap",void 0),z.current.set(0,0,0),M.current.set(0,0,0),P.current.set(0,0,0),m.current.reset(),y(!1);const te=Ie.lastMeasuredDistance;te>.001?X.current=te:x.current>.001&&(X.current=x.current)}else e==="Orbit"&&H();ie.current=e}},[e,i]),fa((te,be)=>{var ae;const Pe=ce.current&&!A,Xe=pe.current&&!j;if((Pe||Xe)&&(Z.emit("camera_snap",void 0),e==="Orbit")){const re=X.current||Ie.lastMeasuredDistance||3.5;x.current=re;const ue=new U(0,0,-1).applyQuaternion(i.quaternion);b.current.copy(i.position).addScaledVector(ue,re),c.current&&(c.current.target.copy(b.current),c.current.update())}if(ce.current=A,pe.current=j,(ae=k.current)!=null&&ae.active){const re=k.current;re.elapsed+=be;const ue=Math.min(re.elapsed/re.duration,1),Re=ue*ue*(3-2*ue),Ye=new U().lerpVectors(re.startPos,re.endPos,Re),Ue=re.startRot.clone().slerp(re.endRot,Re),ro=ye.split(Ye.x),io=ye.split(Ye.y),no=ye.split(Ye.z),_i={x:ro.high,y:io.high,z:no.high,xL:ro.low,yL:io.low,zL:no.low};i.position.set(0,0,0),i.quaternion.copy(Ue),i.updateMatrixWorld(),n(_i),Ie.shouldSnapCamera=!0,Ie.dirty=!0,z.current.copy(i.position),w.current.copy(i.quaternion),ue>=1&&(k.current=null,Z.emit("camera_teleport",re.endState));return}if(Se){c.current&&(c.current.enabled=!1),g&&g(!1),Ie.isCameraInteracting=!1;return}e==="Orbit"&&!S&&H();const ht=i.position.distanceToSquared(z.current)>1e-12,St=i.quaternion.angleTo(w.current)>1e-11,ge=G()||ee.current||oe.current;if(ge&&k.current&&(k.current=null),g&&g(ge),Ie.isCameraInteracting=ge,ge&&(ht||St)){if(Ie.dirty=!0,!Q.current&&o){Q.current=!0;const re=Ie.virtualSpace?Ie.virtualSpace.getUnifiedCameraState(i,Ie.lastMeasuredDistance):{position:{x:i.position.x,y:i.position.y,z:i.position.z},rotation:{x:i.quaternion.x,y:i.quaternion.y,z:i.quaternion.z,w:i.quaternion.w},sceneOffset:{...Ie.sceneOffset},targetDistance:Ie.lastMeasuredDistance};o(re)}B&&I&&q(V,!0,A?"Linear":"Bezier")}if((ht||St||ge)&&(K.current&&clearTimeout(K.current),K.current=setTimeout(()=>{if(Q.current=!1,a&&a(),N.getState().isUserInteracting)return;let re=Ie.lastMeasuredDistance;(re<=0||re>=1e3)&&(re=N.getState().targetDistance||3.5);const ue=Ie.sceneOffset,Re={x:ue.x,y:ue.y,z:ue.z,xL:(ue.xL??0)+i.position.x,yL:(ue.yL??0)+i.position.y,zL:(ue.zL??0)+i.position.z};ye.normalize(Re),N.setState({cameraRot:{x:i.quaternion.x,y:i.quaternion.y,z:i.quaternion.z,w:i.quaternion.w},sceneOffset:Re,targetDistance:re})},100),z.current.copy(i.position),w.current.copy(i.quaternion)),e==="Fly"){if(d)return;const re=_.current?F.current.x-E.current.x:0,ue=_.current?F.current.y-E.current.y:0;m.current.update(i,be,{move:T.current,look:R.current,moveJoy:$.current,isDragging:_.current,dragDelta:{x:re,y:ue},invertY:Y},{baseSpeed:O.current,distEstimate:X.current,autoSlow:(h==null?void 0:h.autoSlow)??!0})}else if(e==="Orbit"&&c.current&&(c.current.enabled=!d&&!Te.current&&(Ne.current||ee.current||oe.current),c.current.zoomSpeed=he.current,c.current.rotateSpeed=1/(s||1),Math.abs(L.current)>.01)){const re=new U;i.getWorldDirection(re),i.up.applyAxisAngle(re,-L.current*2*be).normalize(),c.current.update()}const Be=Ie.sceneOffset;C.current={x:Be.x,y:Be.y,z:Be.z,xL:(Be.xL??0)+i.position.x,yL:(Be.yL??0)+i.position.y,zL:(Be.zL??0)+i.position.z}}),e!=="Orbit"||!S||Se?null:t.jsx(Ai,{ref:c,enabled:!d,makeDefault:!0,enableDamping:!1,target:b.current,enableZoom:!p,mouseButtons:{LEFT:ba.ROTATE,MIDDLE:ba.DOLLY,RIGHT:ba.PAN},touches:{ONE:co.ROTATE,TWO:co.DOLLY_PAN},onStart:()=>{if(ee.current=!0,c.current){const te=X.current>0?X.current:N.getState().targetDistance||3.5;x.current=te;const be=new U(0,0,-1).applyQuaternion(i.quaternion),Pe=new U().copy(i.position).addScaledVector(be,te);c.current.target.copy(Pe),c.current.update(),c.current.saveState();const Xe=i.position.distanceTo(Pe);Xe>1e-8&&(he.current=Math.max(.001,te/Xe*1.25)),z.current.copy(i.position)}},onEnd:()=>{ee.current=!1;const te=i.position.length();te>1e-4&&(x.current=te),D(!0)}})},bc=xe(),vc=()=>{const[e,o]=v.useState(!1),[a,r]=v.useState(0),n=v.useRef(0),s=v.useRef(0),i=v.useRef(15e3);v.useEffect(()=>{bc.isCompiling&&o(!0);const u=y=>{o(y)},g=Z.on("is_compiling",u),S=Z.on("compile_estimate",y=>{i.current=Math.max(2e3,y)});return()=>{g(),S()}},[]),v.useEffect(()=>{if(e){s.current=performance.now(),r(0);const u=i.current,g=()=>{const y=(performance.now()-s.current)/u,b=Math.min(95,95*(1-Math.exp(-3*y)));r(b),n.current=requestAnimationFrame(g)};n.current=requestAnimationFrame(g)}else a>0&&(cancelAnimationFrame(n.current),r(100));return()=>cancelAnimationFrame(n.current)},[!!e]);const l=!!e||a>=100,[c,d]=v.useState(!1);v.useEffect(()=>{if(a>=100&&!e){const u=setTimeout(()=>d(!0),800),g=setTimeout(()=>{d(!1),r(0)},1400);return()=>{clearTimeout(u),clearTimeout(g)}}},[a>=100&&!e]);const f=typeof e=="string"?e:"Compiling Shader...",p=typeof e=="string"&&e.includes("Lighting"),h=l&&!c;return t.jsx("div",{className:`fixed top-16 left-1/2 -translate-x-1/2 z-[99999] pointer-events-none transition-opacity duration-500 ${h?p?"opacity-60":"opacity-100":"opacity-0"}`,children:t.jsxs("div",{className:"bg-black/80 backdrop-blur-sm border border-white/10 px-3 py-1.5 rounded-full flex flex-col items-center gap-1.5 shadow-lg min-w-[200px]",children:[t.jsxs("div",{className:"flex items-center gap-2",children:[t.jsx(Ir,{className:`animate-spin h-3 w-3 ${p?"text-amber-400":"text-cyan-400"}`}),t.jsx("span",{className:`text-[9px] font-bold ${p?"text-amber-200":"text-cyan-200"}`,children:f}),t.jsxs("span",{className:"text-[9px] font-mono text-gray-500",children:[Math.floor(a),"%"]})]}),t.jsx("div",{className:"w-full h-1 bg-white/5 rounded-full overflow-hidden",children:t.jsx("div",{className:`h-full rounded-full transition-[width] duration-150 ease-linear ${a>=100?"bg-green-400":p?"bg-amber-400/60":"bg-cyan-400/60"}`,style:{width:`${a}%`}})})]})})},It={SNAPSHOT:0,ANIMATE:1,OVERLAY:2,UI:3},xt=[];let Wa=!1;function Ft(e,o,a){if(xt.some(n=>n.name===e))return()=>{};const r={name:e,phase:o,fn:a};return xt.push(r),Wa=!0,()=>{const n=xt.indexOf(r);n>=0&&xt.splice(n,1)}}function Yo(e){Wa&&(xt.sort((o,a)=>o.phase-a.phase),Wa=!1);for(let o=0;o<xt.length;o++)xt[o].fn(e)}const wc=.15,Sc=.4,Oe={X:"#ff4444",Y:"#44ff44",Z:"#4444ff",Hover:"#ffffff",PlaneXY:"#4444ff",PlaneXZ:"#44ff44",PlaneYZ:"#ff4444"},at=new U,Bt=new U,Zo=(e,o,a)=>(at.set(e.position.x,e.position.y,e.position.z),e.fixed?at.applyQuaternion(o.quaternion).add(o.position):at.sub({x:a.x+a.xL,y:a.y+a.yL,z:a.z+a.zL}),at.clone()),Mc=(e,o,a,r)=>{o.updateMatrixWorld();const n=o.matrixWorldInverse;if(Bt.copy(e).applyMatrix4(n),Bt.z>-5e-5)return null;at.copy(e).project(o);const s=a/2,i=r/2,l=at.x*s+s,c=-(at.y*i)+i;return Math.abs(l)>5e4||Math.abs(c)>5e4?null:{x:l,y:c,z:Bt.z,isBehindCamera:!1}},Ta=(e,o,a,r,n,s,i,l)=>{const c=e.clone().add(o.clone().multiplyScalar(a));if(Bt.copy(c).applyMatrix4(n),Bt.z>-5e-5)return null;at.copy(c).project(r),at.x*i/2,-(at.y*l/2);const d=at.x*i/2+i/2,f=-(at.y*l/2)+l/2;return{x:d-s.x,y:f-s.y}},Ra=xe(),zc=ke.forwardRef((e,o)=>{const{index:a,light:r,onDragStart:n}=e,s=v.useRef(null),[i,l]=v.useState(null),c=N(y=>y.updateLight),{handleInteractionStart:d,handleInteractionEnd:f}=N(),p=N(y=>{var b,m,x;return((x=(m=(b=y.lighting)==null?void 0:b.lights)==null?void 0:m[a])==null?void 0:x.fixed)??r.fixed}),h=y=>l({part:y}),u=()=>l(null),g=()=>{var C,z;const y=(z=(C=N.getState().lighting)==null?void 0:C.lights)==null?void 0:z[a];if(!y)return;const b=y.fixed;let m=y.position;const x=We();if(x){const w=Ra.sceneOffset;if(b){const M=new U(m.x,m.y,m.z);M.applyQuaternion(x.quaternion),M.add(x.position),m={x:M.x+w.x+(w.xL??0),y:M.y+w.y+(w.yL??0),z:M.z+w.z+(w.zL??0)}}else{const M=new U(m.x-w.x-(w.xL??0),m.y-w.y-(w.yL??0),m.z-w.z-(w.zL??0));M.sub(x.position),M.applyQuaternion(x.quaternion.clone().invert()),m={x:M.x,y:M.y,z:M.z}}}d("param"),c({index:a,params:{fixed:!b,position:m}}),f()};ke.useImperativeHandle(o,()=>({hide:()=>{s.current&&(s.current.style.display="none")},update:()=>{var X,D;const y=((D=(X=N.getState().lighting)==null?void 0:X.lights)==null?void 0:D[a])??r,b=Ot(),m=ta(),x=s.current;if(!b||!m||!x)return;const C=m.clientWidth,z=m.clientHeight,w=Ra.sceneOffset,M=Zo(y,b,w),P=Mc(M,b,C,z);if(!P){x.style.display="none";return}x.style.display="flex",x.style.transform=`translate3d(${P.x}px, ${P.y}px, 0)`;const T=M.distanceTo(b.position)*wc,_=b.matrixWorldInverse,E=new U(1,0,0),F=new U(0,1,0),O=new U(0,0,1);y.fixed&&(E.applyQuaternion(b.quaternion),F.applyQuaternion(b.quaternion),O.applyQuaternion(b.quaternion));const $=Ta(M,E,T,b,_,P,C,z),R=Ta(M,F,T,b,_,P,C,z),Y=Ta(M,O,T,b,_,P,C,z),L=(H,A)=>{x.querySelectorAll(`.${H}`).forEach(B=>{A?(B.setAttribute("x2",String(A.x)),B.setAttribute("y2",String(A.y)),B.setAttribute("visibility","visible")):B.setAttribute("visibility","hidden")})};L("axis-x-line",$),L("axis-y-line",R),L("axis-z-line",Y);const G=(H,A,j)=>{const B=x.querySelector(`.${H}`);if(B)if(A&&j){const I=Sc,V=A.x*I,W=A.y*I,q=j.x*I,Q=j.y*I,K=V+q,ie=W+Q;B.setAttribute("d",`M0,0 L${V},${W} L${K},${ie} L${q},${Q} Z`),B.setAttribute("visibility","visible")}else B.setAttribute("visibility","hidden")};G("plane-xy",$,R),G("plane-xz",$,Y),G("plane-yz",R,Y)}}));const S=(y,b)=>{var z,w;const m=Ot();if(!m)return;const x=((w=(z=N.getState().lighting)==null?void 0:z.lights)==null?void 0:w[a])??r,C=Zo(x,m,Ra.sceneOffset);n(y,a,b,C)};return r.type==="Directional"?null:t.jsxs("div",{ref:s,className:"absolute flex items-center justify-center w-0 h-0 pointer-events-auto",style:{display:"none",willChange:"transform"},children:[t.jsxs("svg",{className:"absolute overflow-visible pointer-events-none",style:{left:0,top:0},children:[t.jsxs("defs",{children:[t.jsx("marker",{id:`arrow-${a}-x`,markerWidth:"6",markerHeight:"6",refX:"5",refY:"3",orient:"auto",children:t.jsx("path",{d:"M0,0 L0,6 L6,3 z",fill:(i==null?void 0:i.part)==="axis-x"?Oe.Hover:Oe.X})}),t.jsx("marker",{id:`arrow-${a}-y`,markerWidth:"6",markerHeight:"6",refX:"5",refY:"3",orient:"auto",children:t.jsx("path",{d:"M0,0 L0,6 L6,3 z",fill:(i==null?void 0:i.part)==="axis-y"?Oe.Hover:Oe.Y})}),t.jsx("marker",{id:`arrow-${a}-z`,markerWidth:"6",markerHeight:"6",refX:"5",refY:"3",orient:"auto",children:t.jsx("path",{d:"M0,0 L0,6 L6,3 z",fill:(i==null?void 0:i.part)==="axis-z"?Oe.Hover:Oe.Z})})]}),t.jsx("path",{className:"plane-xy cursor-move pointer-events-auto transition-opacity duration-150",fill:(i==null?void 0:i.part)==="plane-xy"?Oe.Hover:Oe.PlaneXY,fillOpacity:"0.3",stroke:"none",onPointerDown:y=>S(y,"plane-xy"),onPointerEnter:()=>h("plane-xy"),onPointerLeave:u}),t.jsx("path",{className:"plane-xz cursor-move pointer-events-auto transition-opacity duration-150",fill:(i==null?void 0:i.part)==="plane-xz"?Oe.Hover:Oe.PlaneXZ,fillOpacity:"0.3",stroke:"none",onPointerDown:y=>S(y,"plane-xz"),onPointerEnter:()=>h("plane-xz"),onPointerLeave:u}),t.jsx("path",{className:"plane-yz cursor-move pointer-events-auto transition-opacity duration-150",fill:(i==null?void 0:i.part)==="plane-yz"?Oe.Hover:Oe.PlaneYZ,fillOpacity:"0.3",stroke:"none",onPointerDown:y=>S(y,"plane-yz"),onPointerEnter:()=>h("plane-yz"),onPointerLeave:u}),t.jsxs("g",{onPointerEnter:()=>h("axis-z"),onPointerLeave:u,children:[t.jsx("line",{className:"axis-z-line pointer-events-none transition-all duration-150",x1:"0",y1:"0",x2:"0",y2:"0",stroke:(i==null?void 0:i.part)==="axis-z"?Oe.Hover:Oe.Z,strokeWidth:"2",markerEnd:`url(#arrow-${a}-z)`}),t.jsx("line",{className:"axis-z-line cursor-pointer pointer-events-auto",x1:"0",y1:"0",x2:"0",y2:"0",stroke:"rgba(0,0,0,0)",strokeWidth:"12",onPointerDown:y=>S(y,"axis-z")})]}),t.jsxs("g",{onPointerEnter:()=>h("axis-y"),onPointerLeave:u,children:[t.jsx("line",{className:"axis-y-line pointer-events-none transition-all duration-150",x1:"0",y1:"0",x2:"0",y2:"0",stroke:(i==null?void 0:i.part)==="axis-y"?Oe.Hover:Oe.Y,strokeWidth:"2",markerEnd:`url(#arrow-${a}-y)`}),t.jsx("line",{className:"axis-y-line cursor-pointer pointer-events-auto",x1:"0",y1:"0",x2:"0",y2:"0",stroke:"rgba(0,0,0,0)",strokeWidth:"12",onPointerDown:y=>S(y,"axis-y")})]}),t.jsxs("g",{onPointerEnter:()=>h("axis-x"),onPointerLeave:u,children:[t.jsx("line",{className:"axis-x-line pointer-events-none transition-all duration-150",x1:"0",y1:"0",x2:"0",y2:"0",stroke:(i==null?void 0:i.part)==="axis-x"?Oe.Hover:Oe.X,strokeWidth:"2",markerEnd:`url(#arrow-${a}-x)`}),t.jsx("line",{className:"axis-x-line cursor-pointer pointer-events-auto",x1:"0",y1:"0",x2:"0",y2:"0",stroke:"rgba(0,0,0,0)",strokeWidth:"12",onPointerDown:y=>S(y,"axis-x")})]}),t.jsx("circle",{cx:"0",cy:"0",r:"6",fill:r.color,stroke:"white",strokeWidth:"2",className:`cursor-move pointer-events-auto transition-all duration-150 ${(i==null?void 0:i.part)==="free"?"stroke-cyan-400 r-[8px]":""}`,onPointerDown:y=>S(y,"free"),onPointerEnter:()=>h("free"),onPointerLeave:u})]}),t.jsxs("div",{className:"absolute top-[50px] left-0 transform -translate-x-1/2 flex items-center gap-1 bg-black/80 backdrop-blur px-1.5 py-0.5 rounded border border-white/20 select-none z-20 pointer-events-auto transition-transform hover:scale-105",children:[t.jsxs("span",{className:"text-[9px] font-bold text-white",children:["L",a+1]}),t.jsx("button",{className:"anchor-btn p-0.5 hover:text-cyan-400 transition-colors text-[9px]",onPointerDown:y=>y.stopPropagation(),onClick:y=>{y.stopPropagation(),g()},title:p?"Attached to Camera":"World Anchored",children:p?t.jsx(ul,{}):t.jsx(dl,{})})]})]})}),Ia=xe(),Hr={current:{}},Cc=()=>{var r,n,s,i;const e=(r=N.getState().lighting)==null?void 0:r.lights;if(!e)return;const o=Ea(e);o!==e&&((s=(n=N.getState()).setLighting)==null||s.call(n,{lights:o}));const a=new Map(e.map(l=>[l.id,l]));for(const[l,c]of Object.entries(Hr.current)){const d=a.get(l);if(!d||!d.visible||d.type==="Directional"){(i=c.hide)==null||i.call(c);continue}try{c.update()}catch(f){console.error("Error updating light gizmo:",f)}}},kc=()=>{const e=N(u=>u.showLightGizmo),o=N(u=>{var g;return((g=u.lighting)==null?void 0:g.lights)||[]}),a=N(u=>u.setGizmoDragging);N(u=>u.updateLight);const r=N(u=>u.setDraggedLight),{handleInteractionStart:n,handleInteractionEnd:s}=N(),i=v.useRef(null),l=v.useMemo(()=>new dr,[]),c=v.useMemo(()=>new Qa,[]),d=Hr,f=(u,g,S,y)=>{u.preventDefault(),u.stopPropagation();const b=o[g];if(!b)return;n("param"),a(!0),Ia.isGizmoInteracting=!0,r(b.id??null),u.target.setPointerCapture(u.pointerId);const m=Ot(),x=ta();if(!m||!x)return;const C=x.getBoundingClientRect(),z=new je((u.clientX-C.left)/C.width*2-1,-((u.clientY-C.top)/C.height)*2+1);if(c.setFromCamera(z,m),S==="free"||S.startsWith("plane")){const w=new U;S==="free"?m.getWorldDirection(w):S==="plane-xy"?w.set(0,0,1):S==="plane-xz"?w.set(0,1,0):S==="plane-yz"&&w.set(1,0,0),S!=="free"&&b.fixed&&w.applyQuaternion(m.quaternion),l.setFromNormalAndCoplanarPoint(w,y);const M=new U,k=c.ray.intersectPlane(l,M)?new U().subVectors(y,M):new U(0,0,0);i.current={active:!0,index:g,mode:S,startPos:y.clone(),planeNormal:w,planeOrigin:y,offsetFromIntersection:k,constraintAxes:[],startX:u.clientX,startY:u.clientY,screenAxis:new je,worldAxis:new U}}else if(S.startsWith("axis")){let w=new U;S==="axis-x"&&w.set(1,0,0),S==="axis-y"&&w.set(0,1,0),S==="axis-z"&&w.set(0,0,1),b.fixed&&w.applyQuaternion(m.quaternion);const M=y.distanceTo(m.position)*.15,P=y.clone().addScaledVector(w,M),k=y.clone().project(m),T=P.clone().project(m),_=x.getBoundingClientRect();let E=(T.x-k.x)*_.width*.5,F=-(T.y-k.y)*_.height*.5;P.applyMatrix4(m.matrixWorldInverse).z>0&&(E=-E,F=-F);let $=new je(E,F);$.lengthSq()<.5&&$.set(1,0),$.normalize(),i.current={active:!0,index:g,mode:S,startPos:y.clone(),planeNormal:new U,planeOrigin:new U,offsetFromIntersection:new U,startX:u.clientX,startY:u.clientY,screenAxis:$,worldAxis:w}}window.addEventListener("pointermove",p),window.addEventListener("pointerup",h)},p=u=>{var z;const g=i.current;if(!g||!g.active)return;u.preventDefault(),u.stopPropagation();const S=Ot();if(!S)return;const b=(((z=N.getState().lighting)==null?void 0:z.lights)||[])[g.index];if(!b)return;let m=new U;if(g.mode==="free"||g.mode.startsWith("plane")){const w=ta();if(!w)return;const M=w.getBoundingClientRect(),P=new je((u.clientX-M.left)/M.width*2-1,-((u.clientY-M.top)/M.height)*2+1);c.setFromCamera(P,S),l.setFromNormalAndCoplanarPoint(g.planeNormal,g.planeOrigin);const k=new U;if(c.ray.intersectPlane(l,k))m.copy(k).add(g.offsetFromIntersection);else return}else{const w=u.clientX-g.startX,M=u.clientY-g.startY,P=w*g.screenAxis.x+M*g.screenAxis.y,T=g.startPos.distanceTo(S.position)*.0025;m.copy(g.startPos).addScaledVector(g.worldAxis,P*T)}u.shiftKey&&(m.x=Math.round(m.x/.25)*.25,m.y=Math.round(m.y/.25)*.25,m.z=Math.round(m.z/.25)*.25);const x=Ia.sceneOffset;let C;if(b.fixed){const w=m.clone().sub(S.position).applyQuaternion(S.quaternion.clone().invert());C={x:w.x,y:w.y,z:w.z}}else C={x:m.x+x.x+(x.xL??0),y:m.y+x.y+(x.yL??0),z:m.z+x.z+(x.zL??0)};N.getState().updateLight({index:g.index,params:{position:C}})},h=u=>{i.current&&(a(!1),r(null),Ia.isGizmoInteracting=!1,s(),i.current=null,window.removeEventListener("pointermove",p),window.removeEventListener("pointerup",h))};return e?t.jsx("div",{className:"absolute inset-0 pointer-events-none",children:o.map((u,g)=>t.jsx(zc,{index:g,light:u,onDragStart:f,ref:S=>{const y=u.id;y&&(S?d.current[y]=S:delete d.current[y])}},u.id||g))}):null},Xa={displays:new Map},Ya={diamonds:new Map},jc={diamonds:new Map},Yt=(e,o)=>{o?(e.style.setProperty("background-color","#991b1b","important"),e.style.setProperty("border-color","#f87171","important")):(e.style.removeProperty("background-color"),e.style.removeProperty("border-color"))},Pc=()=>{const e=fe.getState(),{isPlaying:o,currentFrame:a,sequence:r}=e;Xa.displays.forEach((n,s)=>{if(n)try{const i=Ca(s,o,a,r);n.innerText=i.toFixed(2)}catch(i){console.error("Error updating live value display:",i)}}),o||(Ya.diamonds.forEach(n=>{const{el:s,frame:i,tid:l}=n;if(Math.abs(i-a)<.1){const d=r.tracks[l];if(d){const f=d.keyframes.find(p=>Math.abs(p.frame-i)<.1);if(f){const p=Ca(l,!1),h=Math.abs(f.value-p)>.001;Yt(s,h)}}}else Yt(s,!1)}),jc.diamonds.forEach(n=>{const{el:s,frame:i,tids:l}=n;if(Math.abs(i-a)<.1){let d=!1;for(const f of l){const p=r.tracks[f];if(p){const h=p.keyframes.find(u=>Math.abs(u.frame-i)<.1);if(h){const u=Ca(f,!1);if(Math.abs(h.value-u)>.001){d=!0;break}}}}Yt(s,d)}else Yt(s,!1)}))},Tc=({tid:e})=>{const o=v.useRef(null);return v.useEffect(()=>(o.current&&Xa.displays.set(e,o.current),()=>{Xa.displays.delete(e)}),[e]),t.jsx("span",{ref:o,className:"text-[9px] font-mono text-gray-600 w-12 text-right",children:"--"})},Rc=({tid:e,kid:o,frame:a,isSelected:r,interpolation:n})=>{const s=v.useRef(null),i=`${e}::${o}`;v.useEffect(()=>(s.current&&Ya.diamonds.set(i,{el:s.current,frame:a,tid:e}),()=>{Ya.diamonds.delete(i)}),[i,a,e]);const l=n==="Linear"?"rotate-45 rounded-sm":n==="Step"?"rounded-none":"rounded-full";return t.jsx("div",{ref:s,className:`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 border transition-transform ${r?"bg-white border-white scale-125 z-30":"bg-cyan-900 border-cyan-400 group-hover/key:scale-125 group-hover/key:bg-cyan-400"} ${l}`})},tf=v.memo(({tid:e,sequence:o,frameWidth:a,isSelected:r,selectedKeys:n,onSelect:s,onRemove:i,onAddKey:l,onKeyMouseDown:c})=>{const d=N(p=>p.openContextMenu),f=p=>{const h=Ge(p.currentTarget);h.length>0&&(p.preventDefault(),p.stopPropagation(),d(p.clientX,p.clientY,[],h))};return t.jsxs("div",{className:"flex border-b border-white/5 bg-transparent hover:bg-white/5",style:{height:32},"data-help-id":"anim.tracks",children:[t.jsxs("div",{className:`sticky left-0 z-30 w-[220px] bg-black/80 backdrop-blur-sm border-r border-white/10 shrink-0 flex items-center justify-between px-3 cursor-pointer group select-none ${r?"border-l-2 border-l-cyan-500":""}`,onClick:p=>s(p,e),onMouseDown:p=>p.stopPropagation(),onContextMenu:f,"data-help-id":"anim.tracks",children:[t.jsx("div",{className:"truncate text-[10px] font-bold text-gray-400 group-hover:text-cyan-400 pl-4",title:o.tracks[e].label,children:o.tracks[e].label}),t.jsxs("div",{className:"flex items-center gap-2",children:[t.jsx(Tc,{tid:e}),t.jsx("button",{onClick:p=>{p.stopPropagation(),i()},className:"opacity-0 group-hover:opacity-100 text-red-500 hover:text-red-400",children:t.jsx(wt,{})})]})]}),t.jsxs("div",{className:"flex-1 relative group/track z-10",onDoubleClick:p=>{p.stopPropagation();const h=p.currentTarget.getBoundingClientRect(),u=Math.max(0,Math.round((p.clientX-h.left)/a));l(u)},children:[t.jsx("div",{className:"absolute inset-0 opacity-0 group-hover/track:opacity-5 bg-white pointer-events-none"}),o.tracks[e].keyframes.map(p=>{const h=n.includes(`${e}::${p.id}`);return t.jsx("div",{className:"absolute top-1/2 -translate-y-1/2 z-20 cursor-grab group/key",style:{left:`${p.frame*a-10}px`,width:"20px",height:"20px"},onMouseDown:u=>c(u,e,p.id),"data-help-id":"anim.keyframes",children:t.jsx(Rc,{tid:e,kid:p.id,frame:p.frame,isSelected:h,interpolation:p.interpolation})},p.id)})]})]})});Ft("snapshotDisplayCamera",It.SNAPSHOT,()=>{const e=We();e&&on(e)});Ft("animationTick",It.ANIMATE,ac);Ft("lightGizmoTick",It.OVERLAY,Cc);Ft("fpsCounterTick",It.UI,xl);Ft("performanceMonitorTick",It.UI,nc);Ft("trackRowTick",It.UI,Pc);const Ic=({onLoaded:e})=>{const{camera:o,size:a,gl:r}=ua(),[n,s]=v.useState(!1),i=xe(),l=N(d=>d.dpr);v.useEffect(()=>{tn(o),an(r.domElement)},[o,r]),v.useEffect(()=>{let d=!0;return(async()=>{let p=0;for(;!i.isBooted;){if(!d)return;if(++p>=300){console.error("[WorkerTickScene] Worker boot timeout after 30s");return}await new Promise(h=>setTimeout(h,100))}for(;i.isCompiling;){if(!d)return;await new Promise(h=>setTimeout(h,100))}d&&(i.resizeWorker(a.width,a.height,l),s(!0),e&&e())})(),()=>{d=!1}},[]),v.useEffect(()=>{i.resizeWorker(a.width,a.height,l)},[a.width,a.height,l]),v.useEffect(()=>{const d=[Z.on(we.CONFIG,f=>{i.sendConfig(f)}),Z.on(we.UNIFORM,({key:f,value:p,noReset:h})=>{i.setUniform(f,p,h)}),Z.on(we.RESET_ACCUM,()=>{i.resetAccumulation()}),Z.on(we.OFFSET_SET,f=>{const p={x:f.x,y:f.y,z:f.z,xL:f.xL??0,yL:f.yL??0,zL:f.zL??0};i.setShadowOffset(p),i.post({type:"OFFSET_SET",offset:p})}),Z.on(we.OFFSET_SHIFT,({x:f,y:p,z:h})=>{i.applyOffsetShift(f,p,h),i.post({type:"OFFSET_SHIFT",x:f,y:p,z:h})}),Z.on(we.CAMERA_SNAP,()=>{i.shouldSnapCamera=!0}),Z.on(we.TEXTURE,({textureType:f,dataUrl:p})=>{i.updateTexture(f,p)}),Z.on(we.REGISTER_FORMULA,({id:f,shader:p})=>{i.registerFormula(f,p)})];return()=>{d.forEach(f=>f())}},[]);const c=ke.useRef({lastYield:0,fps:60,frames:0,lastSample:0});return fa((d,f)=>{var z;if(!n)return;const p=Math.min(f,.1),h=performance.now(),u=c.current;if(u.frames++,h-u.lastSample>=500&&(u.fps=u.frames*1e3/(h-u.lastSample),u.frames=0,u.lastSample=h),u.fps<20&&h-u.lastYield>=1e3){u.lastYield=h,Yo(p);return}Yo(p);const g=o,S=((z=N.getState().optics)==null?void 0:z.camFov)??60;g.fov!==S&&(g.fov=S,g.updateProjectionMatrix());const y={position:[g.position.x,g.position.y,g.position.z],quaternion:[g.quaternion.x,g.quaternion.y,g.quaternion.z,g.quaternion.w],fov:g.fov||60,aspect:g.aspect||a.width/a.height},b=N.getState(),m=b.sceneOffset||{x:0,y:0,z:0,xL:0,yL:0,zL:0},x={x:m.x,y:m.y,z:m.z,xL:m.xL??0,yL:m.yL??0,zL:m.zL??0},C={cameraMode:b.cameraMode,isCameraInteracting:fe.getState().isCameraInteracting,isGizmoInteracting:i.isGizmoInteracting,optics:b.optics??null,lighting:b.lighting??null,quality:b.quality??null,geometry:b.geometry??null};i.sendRenderTick(y,x,p,C)},1),null},Fc=({width:e,height:o})=>{const a=v.useRef(null),r=v.useRef(!1);return v.useEffect(()=>{const n=xe();return n.onCrash=s=>{console.error(`[WorkerDisplay] Worker crashed: ${s}.`)},()=>{n.onCrash=null}},[]),v.useEffect(()=>{var u,g;if(r.current||!a.current)return;r.current=!0;const n=window.devicePixelRatio||1,s=document.createElement("canvas");s.width=e*n,s.height=o*n,s.style.cssText="position:absolute;inset:0;width:100%;height:100%;pointer-events:none",a.current.appendChild(s);const i=N.getState(),l=kr(i),c=((u=window.matchMedia)==null?void 0:u.call(window,"(pointer: coarse)").matches)||window.innerWidth<768,d=xe(),f=i.cameraRot||{x:0,y:0,z:0,w:1},p=((g=i.optics)==null?void 0:g.camFov)??60;d.initWorkerMode(s,l,e,o,n,c,{position:[0,0,0],quaternion:[f.x,f.y,f.z,f.w],fov:p});const h=i.sceneOffset;if(h){const S={x:h.x,y:h.y,z:h.z,xL:h.xL??0,yL:h.yL??0,zL:h.zL??0};d.setShadowOffset(S),d.post({type:"OFFSET_SET",offset:S})}},[]),t.jsx("div",{ref:a,style:{position:"absolute",inset:0,width:"100%",height:"100%",pointerEvents:"none"}})},Qo=12,_c=40,Dc=()=>{const e=ne.getViewportOverlays().filter(r=>r.type==="dom"),o=N(),a=N();return t.jsx("div",{className:"absolute inset-0 pointer-events-none z-[20]",children:e.map(r=>{const n=ve.get(r.componentId),s=r.id,i=o[s];return n&&i?t.jsx(n,{featureId:s,sliceState:i,actions:a},r.id):null})})},Lc=()=>{const e=ne.getViewportOverlays().filter(r=>!r.type||r.type==="scene"),o=N(),a=N();return t.jsx(t.Fragment,{children:e.map(r=>{const n=ve.get(r.componentId),s=r.id,i=o[s];return n&&i?t.jsx(n,{featureId:s,sliceState:i,actions:a},r.id):null})})},Ec=({hudRefs:e,onSceneReady:o})=>{const a=N(),r=v.useRef(null),n=v.useRef(null),{drawing:s,interactionMode:i}=a,l=s==null?void 0:s.active,c=i==="selecting_region",{visualRegion:d,isGhostDragging:f,renderRegion:p}=ic(r);rc(r);const{isMobile:h}=oo(),[u,g]=v.useState({w:0,h:0});v.useLayoutEffect(()=>{if(!n.current)return;const R=new ResizeObserver(L=>{for(const G of L){const X=Math.max(1,G.contentRect.width),D=Math.max(1,G.contentRect.height);g({w:X,h:D})}});R.observe(n.current);const Y=n.current.getBoundingClientRect();return Y.width>0&&Y.height>0&&g({w:Y.width,h:Y.height}),()=>R.disconnect()},[]);const S=a.resolutionMode==="Fixed",[y,b]=a.fixedResolution,m=d||p,x=a.isBroadcastMode,C=40,z=Math.max(1,u.w-C),w=Math.max(1,u.h-C);let M=1;S&&(M=Math.min(1,z/y,w/b));const P=S?{width:y,height:b,transform:`scale(${M})`,transformOrigin:"center center",boxShadow:"0 0 50px rgba(0,0,0,0.5)",border:"1px solid rgba(255,255,255,0.1)",flexShrink:0}:{width:"100%",height:"100%"},k=S?y*M:u.w,T=S?b*M:u.h,_=(u.h-T)/2,E=(u.w-k)/2,F=Math.max(Qo,_-_c),O=Math.max(Qo,E),$=R=>{};return t.jsxs("div",{ref:n,className:`relative flex-1 flex items-center justify-center overflow-hidden bg-[#050505] touch-none ${c||l?"cursor-crosshair":""}`,style:{backgroundImage:S?"radial-gradient(circle at center, #111 0%, #050505 100%)":"none"},onContextMenu:R=>{R.preventDefault(),R.stopPropagation()},children:[S&&t.jsx("div",{className:"absolute inset-0 opacity-20 pointer-events-none",style:{backgroundImage:"linear-gradient(#333 1px, transparent 1px), linear-gradient(90deg, #333 1px, transparent 1px)",backgroundSize:"40px 40px"}}),!x&&t.jsx(Jl,{state:a,actions:a,isMobile:a.debugMobileLayout||h,hudRefs:e}),!x&&t.jsx(vc,{}),!x&&t.jsx(sc,{}),!x&&t.jsx(oc,{}),t.jsxs("div",{ref:r,style:P,className:"relative bg-[#111] group z-0",children:[(c||l)&&t.jsx("div",{className:"absolute inset-0 z-50 cursor-crosshair bg-transparent pointer-events-none"}),m&&!c&&!x&&t.jsx("div",{className:`absolute border-2 z-40 group/box region-box cursor-move transition-opacity duration-75 ${f?"border-cyan-400 border-dashed opacity-80":"border-cyan-500 opacity-100"}`,style:{left:`${m.minX*100}%`,bottom:`${m.minY*100}%`,right:`${(1-m.maxX)*100}%`,top:`${(1-m.maxY)*100}%`},children:t.jsxs("div",{className:"absolute top-0 right-0 bg-cyan-600 text-white text-[9px] font-bold px-1.5 py-0.5 flex items-center gap-2 pointer-events-auto shadow-md",children:[t.jsx("span",{children:f?"Moving...":"Active Region"}),t.jsx("div",{className:"w-px h-2 bg-cyan-400/50"}),t.jsx("button",{onClick:R=>{R.stopPropagation(),a.setRenderRegion(null)},className:"hover:text-black transition-colors",title:"Clear Region",children:"✕"})]})}),u.w>0&&u.h>0&&t.jsx(Fc,{width:S?y:u.w,height:S?b:u.h}),t.jsxs(Ei,{gl:{alpha:!0,depth:!1,antialias:!1,powerPreference:"high-performance",preserveDrawingBuffer:!1},camera:{position:[0,0,0],fov:60},style:{position:"absolute",inset:0,pointerEvents:"auto"},dpr:a.dpr,onPointerDown:R=>R.target.setPointerCapture(R.pointerId),onPointerMove:$,onWheel:$,children:[t.jsx(yc,{mode:a.cameraMode,hudRefs:e,onStart:R=>a.handleInteractionStart(R),onEnd:()=>a.handleInteractionEnd(),setSceneOffset:a.setSceneOffset,fitScale:M}),t.jsx(Ic,{onLoaded:o}),t.jsx(Lc,{})]}),t.jsx(Dc,{}),!x&&t.jsx(dc,{width:S?y:u.w,height:S?b:u.h}),!x&&a.histogramActiveCount>0&&t.jsx($o,{onUpdate:R=>a.setHistogramData(R),autoUpdate:a.histogramAutoUpdate,trigger:a.histogramTrigger,source:"geometry"}),!x&&a.sceneHistogramActiveCount>0&&t.jsx($o,{onUpdate:R=>a.setSceneHistogramData(R),autoUpdate:!0,trigger:a.sceneHistogramTrigger,source:"color"}),c&&!x&&t.jsx("div",{className:"absolute top-4 left-1/2 -translate-x-1/2 bg-cyan-900/80 text-cyan-100 text-[10px] font-bold px-3 py-1 rounded-full border border-cyan-500/50 shadow-lg animate-pulse pointer-events-none z-[60]",children:"Drag to select render region"})]}),S&&!x&&t.jsx(cc,{width:y,height:b,top:F,left:O,maxAvailableWidth:u.w,maxAvailableHeight:u.h,onSetResolution:a.setFixedResolution,onSetMode:a.setResolutionMode})]})},Ac=()=>{const e=N(o=>o.openContextMenu);v.useEffect(()=>{const o=a=>{if(a.defaultPrevented)return;const r=Ge(a.target);r.length>0&&(a.preventDefault(),e(a.clientX,a.clientY,[],r))};return window.addEventListener("contextmenu",o),()=>window.removeEventListener("contextmenu",o)},[e])},Nc={"general.undo":{id:"general.undo",category:"General",title:"Undo & History",content:`
...
`}},Bc={"formula.active":{id:"formula.active",category:"Formulas",title:"Active Formula",content:`
...
`}},Oc={"formula.mandelbulb":{id:"formula.mandelbulb",category:"Formulas",title:"Mandelbulb",parentId:"formula.active",content:`
## The Math
...
`}},$c={"panel.formula":{id:"panel.formula",category:"Parameters",title:"Formula Parameters",content:`
...
`}},Hc={"ui.controls":{id:"ui.controls",category:"UI",title:"Control Deck",content:`
...
`}},Gc={"ui.timeline":{id:"ui.timeline",category:"Timeline",title:"Animation Timeline",content:`
...
`}},Uc={"panel.render":{id:"panel.render",category:"Rendering",title:"Shading & Materials",content:`
...
`}},qc={"panel.gradient":{id:"panel.gradient",category:"Coloring",title:"Coloring Engine",content:`
...
`}},Wc={"ui.graph":{id:"ui.graph",category:"Graph",title:"Modular Graph Editor",content:`
...
`}},Xc={"panel.scene":{id:"panel.scene",category:"UI",title:"Scene Panel",content:`
...
**Note**: DOF requires **Temporal Accumulation** to look smooth. It uses stochastic jittering of the camera ray.
`}},Yc={"effect.droste":{id:"effect.droste",category:"Effects",title:"Escher Droste (Spiral)",content:`
...
`}},Zc={"panel.audio":{id:"panel.audio",category:"Audio",title:"Audio Engine",content:`
...
`}},Qc={...Nc,...Bc,...Oc,...$c,...Hc,...Gc,...Vc,...Uc,...qc,...Wc,...Xc,...Yc,...Zc},Gr=({x:e,y:o,items:a,targetHelpIds:r,onClose:n,onOpenHelp:s,isSubmenu:i})=>{const l=v.useRef(null),[c,d]=v.useState({x:e,y:o,opacity:0}),[f,p]=v.useState(null),h=v.useRef(null);v.useLayoutEffect(()=>{if(!l.current)return;const y=l.current.getBoundingClientRect(),b=window.innerWidth,m=window.innerHeight,x=8;let C=e,z=o;i?C+y.width>b-x&&(C=e-y.width-200,C=b-y.width-x):C+y.width>b-x&&(C=e-y.width),z+y.height>m-x&&(z=Math.max(x,m-y.height-x)),C=Math.max(x,Math.min(C,b-y.width-x)),z=Math.max(x,Math.min(z,m-y.height-x)),d({x:C,y:z,opacity:1})},[e,o,a,r,i]),v.useEffect(()=>{if(i)return;const y=m=>{m.target.closest(".fractal-context-menu")||n()},b=setTimeout(()=>window.addEventListener("mousedown",y),50);return()=>{clearTimeout(b),window.removeEventListener("mousedown",y)}},[n,i]);const u=r.map(y=>Qc[y]).filter(y=>!!y),g=(y,b)=>{if(h.current&&clearTimeout(h.current),y.children){const m=b.currentTarget.getBoundingClientRect();p({items:y.children,x:m.right,y:m.top})}else p(null)},S=t.jsxs("div",{ref:l,className:"fractal-context-menu fixed z-[9999] bg-[#1a1a1a] border border-white/20 rounded shadow-[0_4px_20px_rgba(0,0,0,0.8)] py-1 min-w-[200px] animate-fade-in [&_.animate-slider-entry]:!animate-none",style:{left:c.x,top:c.y,opacity:c.opacity},onContextMenu:y=>y.preventDefault(),children:[a.map((y,b)=>{var m;return y.element?t.jsx("div",{children:y.element},b):y.isHeader?t.jsx("div",{className:"px-4 py-1 text-[9px] text-gray-500 font-bold border-b border-white/10 mt-1 mb-1 bg-white/5",children:y.label},b):y.type==="slider"?t.jsx("div",{className:"px-3 py-1 mb-1",children:t.jsx(me,{label:y.label||"",value:y.value??0,min:y.min??0,max:y.max??1,step:y.step??.01,onChange:x=>y.onChange&&y.onChange(x),highlight:!0,overrideInputText:(m=y.value)==null?void 0:m.toFixed(2)})},b):t.jsxs("button",{onClick:()=>{!y.disabled&&!y.children&&y.action&&(y.action(),y.keepOpen||n())},onMouseEnter:x=>g(y,x),disabled:y.disabled,className:`w-full text-left px-4 py-2 text-xs flex items-center justify-between transition-colors group relative ${y.disabled?"text-gray-600 cursor-not-allowed opacity-50":y.danger?"text-red-400 hover:bg-red-900/30 hover:text-red-300":"text-gray-300 hover:bg-white/10 hover:text-white"}`,children:[t.jsxs("div",{className:"flex items-center gap-2",children:[y.icon&&t.jsx("span",{className:y.disabled?"text-gray-600":"text-gray-500",children:y.icon}),t.jsx("span",{className:y.checked?"text-cyan-400 font-bold":"",children:y.label})]}),y.checked&&t.jsx(ft,{}),y.children&&t.jsx(ra,{})]},b)}),a.length>0&&u.length>0&&t.jsx("div",{className:"h-px bg-white/10 my-1"}),u.length>0&&t.jsxs(t.Fragment,{children:[t.jsxs("div",{className:"px-4 py-1 text-[9px] text-cyan-700 font-bold mb-1 flex items-center gap-2",children:[t.jsx(Pr,{})," Context Help"]}),u.map((y,b)=>t.jsxs("button",{onClick:()=>{s(y.id),n()},className:`w-full text-left px-4 py-1.5 text-xs transition-colors flex items-center gap-2 group ${b===0?"text-cyan-400 hover:bg-cyan-900/30 hover:text-cyan-200 font-bold":"text-gray-400 hover:bg-white/5 hover:text-white"}`,style:{paddingLeft:`${16+b*8}px`},children:[b>0&&t.jsx("span",{className:"opacity-30 text-[8px]",children:"└"}),t.jsx("span",{children:y.title}),b===0&&t.jsx("span",{className:"ml-auto opacity-0 group-hover:opacity-100 transition-opacity",children:t.jsx(qs,{})})]},y.id))]}),f&&t.jsx(Gr,{x:f.x,y:f.y,items:f.items,targetHelpIds:[],onClose:n,onOpenHelp:s,isSubmenu:!0})]});return i?S:Pt.createPortal(S,document.body)},Fa=300,Ko=60,Kc=38,Jc=()=>{const[e,o]=v.useState(null),a=v.useRef({x:0,y:0}),r=N(),n=r.coreMath,{formula:s,setCoreMath:i}=r;if(!n)return null;const l={1:{key:"paramA",setter:d=>i({paramA:d}),val:n.paramA},2:{key:"paramB",setter:d=>i({paramB:d}),val:n.paramB},3:{key:"paramC",setter:d=>i({paramC:d}),val:n.paramC},4:{key:"paramD",setter:d=>i({paramD:d}),val:n.paramD},5:{key:"paramE",setter:d=>i({paramE:d}),val:n.paramE},6:{key:"paramF",setter:d=>i({paramF:d}),val:n.paramF}};if(v.useEffect(()=>{const d=h=>{a.current={x:h.clientX,y:h.clientY}},f=h=>{if(h.target.tagName==="INPUT"||h.target.tagName==="TEXTAREA")return;const u=h.key,g=l[u];if(g&&!e){const S=Me.get(s),y=parseInt(u)-1;let b=S==null?void 0:S.parameters[y];if(s==="Modular"&&(b={label:`Param ${String.fromCharCode(65+y)}`,id:g.key,min:-5,max:5,step:.01,default:0}),b){const m=b.max-b.min,x=(g.val-b.min)/m,C=Fa-24,z=12+x*C;let w=a.current.x-z,M=a.current.y-Kc;w=Math.max(10,Math.min(window.innerWidth-Fa-10,w)),M=Math.max(10,Math.min(window.innerHeight-Ko-10,M)),o({id:parseInt(u),paramKey:g.key,label:b.label,def:{min:b.min,max:b.max,step:b.step},x:w,y:M})}}},p=h=>{e&&h.key===String(e.id)&&o(null)};return window.addEventListener("mousemove",d),window.addEventListener("keydown",f),window.addEventListener("keyup",p),()=>{window.removeEventListener("mousemove",d),window.removeEventListener("keydown",f),window.removeEventListener("keyup",p)}},[e,s,n]),!e)return null;const c=l[String(e.id)];return t.jsxs("div",{className:"fixed z-[9999] bg-black/80 backdrop-blur-xl border border-cyan-500/50 rounded-xl shadow-[0_0_30px_rgba(0,0,0,0.5)] flex flex-col justify-center px-3 animate-pop-in",style:{left:e.x,top:e.y,width:Fa,height:Ko},children:[t.jsxs("div",{className:"absolute -top-3 left-1/2 -translate-x-1/2 px-2 py-0.5 bg-cyan-900 rounded text-[9px] font-bold text-cyan-200 border border-cyan-700 shadow-sm",children:["Quick Edit (",e.id,")"]}),t.jsx(me,{label:e.label,value:c.val,min:e.def.min,max:e.def.max,step:e.def.step,onChange:c.setter,highlight:!0,trackId:e.paramKey})]})},ed=()=>{const e=v.useRef(!1);return v.useEffect(()=>{e.current||(e.current=!0,xe().setWorkerModePending(),Vs())},[]),null},Zt=xe(),td=()=>typeof window>"u"?!1:window.matchMedia("(pointer: coarse)").matches||window.innerWidth<768,ad=e=>{const o=N(),[a,r]=v.useState("default"),n=v.useRef(!1),s=v.useCallback(i=>{if(!(!i&&(Zt.isBooted||n.current))){n.current=!0;try{setTimeout(()=>{var u;const l=N.getState(),c=kr(l),d=l.cameraRot||{x:0,y:0,z:0,w:1},f=((u=l.optics)==null?void 0:u.camFov)??60,p={position:[0,0,0],quaternion:[d.x,d.y,d.z,d.w],fov:f};Zt.bootWithConfig(c,p);const h=l.sceneOffset;if(h){const g={x:h.x,y:h.y,z:h.z,xL:h.xL??0,yL:h.yL??0,zL:h.zL??0};Zt.setShadowOffset(g),Zt.post({type:"OFFSET_SET",offset:g})}},50)}catch(l){console.error("Critical Engine Boot Failure:",l),n.current=!1}}},[]);return v.useEffect(()=>{const i=window.location.hash;let l=null;if(i&&i.startsWith("#s=")){const c=i.slice(3);l=zr(c),l&&r("url")}if(!l){const c=Me.get("Mandelbulb");c&&c.defaultPreset&&(l=JSON.parse(JSON.stringify(c.defaultPreset)))}if(l&&td()){l.features||(l.features={});const c=pa.lite;Object.entries(c).forEach(([d,f])=>{l.features[d]||(l.features[d]={}),Object.assign(l.features[d],f)})}l&&o.loadPreset(l)},[]),{startupMode:a,bootEngine:s}},Vr=({activeTab:e,state:o,actions:a,onSwitchTab:r})=>{if(e==="Graph"){const i=ve.get("panel-graph");if(i)return t.jsx("div",{className:"h-[600px] -m-4",children:t.jsx(i,{state:o,actions:a})})}if(e==="Camera Manager"){const i=ve.get("panel-cameramanager");if(i)return t.jsx(i,{state:o,actions:a})}if(e==="Engine"){const i=ve.get("panel-engine");if(i)return t.jsx(i,{state:o,actions:a})}const s=ne.getTabs().find(i=>i.label===e);if(s){const i=ve.get(s.componentId);if(i){const l=s.id,c=o[l];return t.jsx(i,{state:o,actions:a,onSwitchTab:r,featureId:l,sliceState:c})}}return t.jsx("div",{className:"flex h-full items-center justify-center text-gray-600 text-xs italic",children:"Select a module"})},od=()=>typeof window>"u"?!1:window.matchMedia("(pointer: coarse)").matches||window.innerWidth<768,Jo=({side:e})=>{const{panels:o,activeLeftTab:a,activeRightTab:r,togglePanel:n,movePanel:s,reorderPanel:i,startPanelDrag:l,endPanelDrag:c,draggingPanelId:d,setDockSize:f,isLeftDockCollapsed:p,isRightDockCollapsed:h,setDockCollapsed:u,openContextMenu:g,leftDockSize:S,rightDockSize:y,formula:b,advancedMode:m}=N(),x=od(),C=N.getState().audio,z=N.getState().drawing,w=e==="left"?a:r,M=e==="left"?p:h,P=e==="left"?S:y,k=Object.values(o).filter($=>{let R=$.location;return x&&($.id==="Engine"||$.id==="Camera Manager")&&(R="right"),!(R!==e||$.id==="Graph"&&b!=="Modular"||$.id==="Light"&&!m||$.id==="Audio"&&!(C!=null&&C.isEnabled)||$.id==="Drawing"&&!(z!=null&&z.enabled))}).sort(($,R)=>$.order-R.order),T=v.useRef(null),_=$=>{$.preventDefault(),T.current={startX:$.clientX,startW:P},window.addEventListener("mousemove",E),window.addEventListener("mouseup",F),document.body.style.cursor="ew-resize"},E=$=>{if(!T.current)return;const R=$.clientX-T.current.startX,Y=e==="left"?R:-R,L=Math.max(200,Math.min(800,T.current.startW+Y));f(e,L)},F=()=>{T.current=null,window.removeEventListener("mousemove",E),window.removeEventListener("mouseup",F),document.body.style.cursor=""},O=($,R)=>{$.preventDefault();const Y=Ge($.currentTarget);g($.clientX,$.clientY,[],Y)};return k.length===0?null:M?t.jsxs("div",{className:`flex flex-col w-8 bg-black border-${e==="left"?"r":"l"} border-white/10 z-40 shrink-0`,children:[t.jsx("button",{onClick:()=>u(e,!1),className:"h-10 flex items-center justify-center text-gray-500 hover:text-white",children:e==="left"?t.jsx(ra,{}):t.jsx(To,{})}),t.jsx("div",{className:"flex-1 flex flex-col items-center py-2 gap-2",children:k.map($=>t.jsx("div",{onClick:()=>n($.id,!0),className:`w-6 h-6 flex items-center justify-center rounded cursor-pointer ${$.id===w?"bg-cyan-900 text-cyan-400":"text-gray-600 hover:bg-white/10"}`,title:$.id,children:t.jsx("span",{className:"text-[10px] font-bold",children:$.id.charAt(0)})},$.id))})]}):t.jsxs("div",{className:`flex flex-col bg-[#080808] border-${e==="left"?"r":"l"} border-white/10 z-40 shrink-0 transition-all duration-75 relative`,style:{width:P},children:[t.jsx("div",{className:"flex flex-wrap gap-0.5 px-0.5 pt-1 bg-black/40 border-b border-white/10 shrink-0 relative items-end",children:k.map($=>{const R=$.id===w;return t.jsxs("button",{onClick:()=>n($.id,!0),onContextMenu:Y=>O(Y,$.id),onMouseEnter:()=>{if(d&&d!==$.id){const Y=o[d];Y&&Y.location===e&&i(d,$.id)}},onMouseUp:Y=>{d&&(Y.stopPropagation(),c())},className:`flex items-center gap-0.5 px-1 py-1 text-[9px] font-bold transition-colors group relative rounded-t-sm
                                ${R?"bg-[#080808] text-cyan-400 border-x border-t border-white/10 z-10 -mb-px pb-2":"text-gray-500 hover:bg-white/5 hover:text-gray-300 border border-transparent"}`,children:[!x&&t.jsx("div",{className:`cursor-move ${R?"text-gray-600 group-hover:text-cyan-600":"text-gray-700 group-hover:text-white"} transition-colors`,onMouseDown:Y=>{Y.stopPropagation(),l($.id)},children:t.jsx("div",{className:"transform scale-75 origin-center",children:t.jsx(Ja,{})})}),t.jsx("span",{className:"truncate max-w-[140px]",children:$.id})]},$.id)})}),t.jsx("button",{onClick:()=>u(e,!0),className:"absolute top-1 right-1 p-1 text-gray-600 hover:text-white z-20",children:e==="left"?t.jsx(To,{}):t.jsx(ra,{})}),t.jsx("div",{className:"flex-1 overflow-y-auto custom-scroll p-4 relative",children:w?t.jsx(Vr,{activeTab:w,state:N.getState(),actions:N.getState(),onSwitchTab:n}):t.jsx("div",{className:"flex h-full items-center justify-center text-gray-700 text-xs italic",children:"Select a panel"})}),t.jsx("div",{className:`absolute top-0 bottom-0 w-1 cursor-ew-resize hover:bg-cyan-500/50 transition-colors z-50 ${e==="left"?"right-[-2px]":"left-[-2px]"}`,onMouseDown:_})]})},rd=()=>{const{draggingPanelId:e,movePanel:o,endPanelDrag:a,cancelPanelDrag:r,panels:n,leftDockSize:s,rightDockSize:i,isLeftDockCollapsed:l,isRightDockCollapsed:c}=N();if(v.useEffect(()=>{if(!e)return;const g=()=>{r()};return window.addEventListener("mouseup",g),()=>window.removeEventListener("mouseup",g)},[e,r]),!e)return null;const d=n[e],f=d?d.location:null,p=(g,S)=>{g.stopPropagation(),o(e,S),a()},h=l?32:s,u=c?32:i;return t.jsxs("div",{className:"fixed inset-0 z-[1000] flex pointer-events-none",children:[t.jsx("div",{style:{width:h},className:`h-full flex items-center justify-center transition-all duration-200 border-r-2
                    ${f!=="left"?"bg-cyan-900/40 border-cyan-500/50 pointer-events-auto cursor-copy":"border-transparent pointer-events-none"}`,onMouseUp:g=>{f!=="left"&&p(g,"left")},children:f!=="left"&&t.jsx("div",{className:"bg-black/80 px-4 py-2 rounded border border-cyan-500/50 text-cyan-200 font-bold text-sm shadow-xl backdrop-blur-md",children:"Dock Left"})}),t.jsx("div",{className:`flex-1 h-full flex items-center justify-center transition-all duration-200
                    ${f!=="float"?"bg-purple-900/20 hover:bg-purple-900/30 border-x-2 border-purple-500/30 pointer-events-auto cursor-copy":"pointer-events-none"}`,onMouseUp:g=>{f!=="float"&&p(g,"float")},children:f!=="float"&&t.jsx("div",{className:"bg-black/80 px-4 py-2 rounded border border-purple-500/50 text-purple-200 font-bold text-sm shadow-xl backdrop-blur-md",children:"Float Window"})}),t.jsx("div",{style:{width:u},className:`h-full flex items-center justify-center transition-all duration-200 border-l-2
                    ${f!=="right"?"bg-cyan-900/40 border-cyan-500/50 pointer-events-auto cursor-copy":"border-transparent pointer-events-none"}`,onMouseUp:g=>{f!=="right"&&p(g,"right")},children:f!=="right"&&t.jsx("div",{className:"bg-black/80 px-4 py-2 rounded border border-cyan-500/50 text-cyan-200 font-bold text-sm shadow-xl backdrop-blur-md",children:"Dock Right"})})]})},id=({id:e,title:o,children:a,position:r,onPositionChange:n,size:s,onSizeChange:i,onClose:l,disableClose:c,zIndex:d,initialPos:f,initialSize:p})=>{const{panels:h,setFloatPosition:u,setFloatSize:g,togglePanel:S,startPanelDrag:y}=N(),b=!!e,m=e?h[e]:null,[x,C]=v.useState(f||{x:100,y:100}),[z,w]=v.useState(p||{width:300,height:200}),M=b?(m==null?void 0:m.floatPos)||{x:100,y:100}:r||x,P=b?(m==null?void 0:m.floatSize)||{width:320,height:400}:s||z,k=v.useRef(M),T=v.useRef(P),_=v.useRef(null),E=v.useRef(null);if(v.useEffect(()=>{k.current=M},[M.x,M.y]),v.useEffect(()=>{T.current=P},[P.width,P.height]),b&&(!m||!m.isOpen||m.location!=="float"))return null;const F=o||(m?m.id:"Window"),O=d||(b?100:200),$=()=>{if(l)l();else if(b&&e){const G=N.getState();e==="Audio"?G.setAudio({isEnabled:!1}):e==="Drawing"?G.setDrawing({enabled:!1}):e==="Engine"&&G.setEngineSettings({showEngineTab:!1}),S(e,!1)}},R=L=>{if(L.target.closest("button"))return;L.preventDefault(),_.current={x:L.clientX,y:L.clientY,startX:k.current.x,startY:k.current.y};const G=D=>{if(!_.current)return;const H=D.clientX-_.current.x,A=D.clientY-_.current.y,j={x:_.current.startX+H,y:_.current.startY+A};n?n(j):b&&e?u(e,j.x,j.y):C(j),k.current=j},X=()=>{_.current=null,window.removeEventListener("mousemove",G),window.removeEventListener("mouseup",X)};window.addEventListener("mousemove",G),window.addEventListener("mouseup",X)},Y=L=>{L.preventDefault(),L.stopPropagation(),E.current={x:L.clientX,y:L.clientY,startW:T.current.width,startH:T.current.height};const G=D=>{if(!E.current)return;const H=D.clientX-E.current.x,A=D.clientY-E.current.y,j={width:Math.max(200,E.current.startW+H),height:Math.max(150,E.current.startH+A)};i?i(j):b&&e?g(e,j.width,j.height):w(j),T.current=j},X=()=>{E.current=null,window.removeEventListener("mousemove",G),window.removeEventListener("mouseup",X)};window.addEventListener("mousemove",G),window.addEventListener("mouseup",X)};return Pt.createPortal(t.jsxs("div",{className:"fixed glass-panel flex flex-col overflow-hidden animate-pop-in shadow-[0_10px_40px_rgba(0,0,0,0.5)]",style:{left:M.x,top:M.y,width:P.width,height:P.height,maxHeight:"90vh",zIndex:O},children:[t.jsxs("div",{onMouseDown:R,className:"panel-header cursor-move flex items-center justify-between px-2 py-1.5 bg-gray-800/90 border-b border-white/10",children:[t.jsxs("div",{className:"flex items-center gap-2",children:[b&&t.jsx("div",{className:"cursor-grab text-gray-500 hover:text-white",onMouseDown:L=>{L.stopPropagation(),e&&y(e)},children:t.jsx(Ja,{})}),t.jsx("span",{className:"t-label text-gray-200",children:F})]}),!c&&(l||b&&!(m!=null&&m.isCore))&&t.jsx("button",{onClick:$,className:"icon-btn",title:"Close",children:t.jsx(eo,{})})]}),t.jsx("div",{className:"p-3 overflow-y-auto overflow-x-hidden custom-scroll flex-1 relative bg-black/80 backdrop-blur-md",children:a}),t.jsx("div",{onMouseDown:Y,className:"absolute bottom-0 right-0 w-4 h-4 cursor-nwse-resize z-20 touch-none text-gray-500",children:t.jsx(Xs,{})})]}),document.body)},nd=ke.lazy(()=>pt(()=>import("./Timeline-Df7BErLo.js"),__vite__mapDeps([5,1,2,6,3,4]),import.meta.url)),sd=ke.lazy(()=>pt(()=>import("./HelpBrowser-6dGz_v5c.js"),__vite__mapDeps([7,1,2,3,4]),import.meta.url)),ld=ke.lazy(()=>pt(()=>import("./FormulaWorkshop-B7iXTFgO.js"),__vite__mapDeps([8,1,2,3,4]),import.meta.url).then(e=>({default:e.FormulaWorkshop}))),cd=()=>{const e=N(),[o,a]=v.useState(!1),[r,n]=v.useState(!0),[s,i]=v.useState(!1),l=v.useRef(null),c=v.useRef(null),d=v.useRef(null),f=v.useRef(null),p=v.useRef(null),h=v.useRef(null),u=v.useMemo(()=>({container:c,speed:d,dist:f,reset:p,reticle:h}),[]),{startupMode:g,bootEngine:S}=ad(),{isMobile:y,isPortrait:b}=oo();Ql(s,i),Ac();const m=y||e.debugMobileLayout,x=e.quality,C=(x==null?void 0:x.precisionMode)===1,z=m&&e.cameraMode==="Fly",w=e.isBroadcastMode,M=e.interactionMode!=="none",P=F=>{F.preventDefault(),F.stopPropagation(),e.openContextMenu(F.clientX,F.clientY,[],["ui.timeline"])},k=()=>{const F=C?"balanced":"lite";Z.emit("is_compiling",`Switching to ${F} mode...`);const O=e.applyPreset;O&&O({mode:F,actions:e})},T=()=>{n(!1)},_=m&&!w?"min-h-[120vh] bg-black":"fixed inset-0 w-full h-full bg-black select-none overflow-hidden flex flex-col",E=Object.values(e.panels).filter(F=>F.location==="float"&&F.isOpen);return t.jsxs("div",{className:_,children:[t.jsx(ed,{}),t.jsx(rd,{}),E.map(F=>t.jsx(id,{id:F.id,title:F.id,children:t.jsx(Vr,{activeTab:F.id,state:e,actions:e,onSwitchTab:O=>e.togglePanel(O,!0)})},F.id)),m&&!w&&t.jsxs("div",{className:"w-full bg-[#080808] border-b border-white/10 p-8 pb-12 flex flex-col items-center text-center gap-3",children:[t.jsx("div",{className:"w-12 h-1 bg-gray-800 rounded-full mb-2"}),C?t.jsxs(t.Fragment,{children:[t.jsxs("div",{className:"flex items-center gap-2 text-amber-500 mb-1",children:[t.jsx(Rt,{}),t.jsx("span",{className:"text-xs font-bold",children:"Lite Render Mode"})]}),t.jsxs("p",{className:"text-[10px] text-gray-400 leading-relaxed max-w-[320px]",children:["Running lightweight engine.",t.jsx("br",{})]})]}):t.jsx(t.Fragment,{children:t.jsx("div",{className:"flex items-center gap-2 text-cyan-500 mb-1",children:t.jsx("span",{className:"text-xs font-bold",children:"High Quality Mode"})})}),t.jsx("button",{onClick:k,className:"mt-2 px-3 py-1.5 text-[9px] font-bold rounded border border-white/10 bg-white/5 text-gray-300 hover:bg-white/10",children:C?"Switch to High Quality":"Switch to Lite Mode"})]}),t.jsxs("div",{ref:l,className:`relative bg-black select-none ${M?"cursor-crosshair":""} flex flex-col ${m&&!w?"h-[100vh] sticky top-0 overflow-hidden shadow-2xl":"w-full h-full"}`,onContextMenu:F=>F.preventDefault(),children:[t.jsx(Zl,{isReady:o,onFinished:T,startupMode:g,bootEngine:S}),m&&b&&!r&&!w&&t.jsxs("div",{className:"fixed inset-0 z-[9999] bg-black flex flex-col items-center justify-center p-10 text-center text-white",children:[t.jsx("div",{className:"text-cyan-400 mb-6 animate-bounce",children:t.jsx(el,{})}),t.jsx("h2",{className:"text-2xl font-bold tracking-tight mb-2",children:"Landscape Recommended"}),t.jsx("p",{className:"text-gray-500 text-sm font-mono",children:"Rotate device to access controls."})]}),!w&&t.jsx(ql,{}),t.jsxs("div",{className:"flex-1 flex overflow-hidden relative",children:[e.workshopOpen?t.jsx(v.Suspense,{fallback:null,children:t.jsx(ld,{onClose:e.closeWorkshop,editFormula:e.workshopEditFormula})}):!w&&!m&&t.jsx(Jo,{side:"left"}),t.jsx(Ec,{hudRefs:u,onSceneReady:()=>a(!0)}),!w&&t.jsx(Jo,{side:"right"})]}),!w&&t.jsx(Wl,{}),!w&&t.jsx(Jc,{}),e.contextMenu.visible&&!w&&t.jsx(Gr,{x:e.contextMenu.x,y:e.contextMenu.y,items:e.contextMenu.items,targetHelpIds:e.contextMenu.targetHelpIds,onClose:e.closeContextMenu,onOpenHelp:e.openHelp}),e.helpWindow.visible&&t.jsx(v.Suspense,{fallback:null,children:t.jsx(sd,{activeTopicId:e.helpWindow.activeTopicId,onClose:e.closeHelp,onNavigate:e.openHelp})}),!s&&!z&&!w&&t.jsx("div",{className:"fixed bottom-4 left-4 z-50 flex gap-2 transition-all duration-500",children:t.jsx("button",{onClick:()=>i(!0),onContextMenu:P,className:"p-2 rounded-full border shadow-lg transition-all bg-gray-800 border-gray-600 text-gray-400 hover:text-white",title:"Open Timeline (T)",children:t.jsx(cl,{})})}),s&&!w&&t.jsx(v.Suspense,{fallback:null,children:t.jsx(nd,{onClose:()=>i(!1)})})]})]})},da=({label:e,active:o,variant:a="primary",size:r="default",icon:n,fullWidth:s,className:i,children:l,onClick:c,...d})=>{const f=N(g=>g.openContextMenu),p=g=>{const S=Ge(g.currentTarget);S.length>0&&(g.preventDefault(),g.stopPropagation(),f(g.clientX,g.clientY,[],S))};let h="bg-cyan-900 text-cyan-200 border-cyan-700 shadow-inner";a==="danger"&&(h="bg-red-900 text-red-200 border-red-700 shadow-inner"),a==="success"&&(h="bg-green-900 text-green-200 border-green-700 shadow-inner"),a==="warning"&&(h="bg-amber-900 text-amber-200 border-amber-700 shadow-inner");const u=r==="small"?"t-btn-sm":"t-btn";return t.jsxs("button",{className:`${u} ${o?h:"t-btn-default"} ${s?"w-full":"flex-1"} ${i||""}`,onClick:c,onContextMenu:p,...d,children:[n,e||l]})},dd=({label:e,icon:o,rightContent:a,className:r=""})=>t.jsxs("div",{className:`flex items-center justify-between px-3 py-2 bg-black/40 border-b border-white/5 ${r}`,children:[t.jsxs("div",{className:"flex items-center gap-2",children:[o,t.jsx("span",{className:"text-[10px] font-bold text-gray-300",children:e})]}),a&&t.jsx("div",{className:"flex items-center gap-2",children:a})]}),_a=xe(),ud=({className:e="-m-3"})=>{const{drawing:o,setDrawing:a,removeDrawnShape:r,clearDrawnShapes:n,updateDrawnShape:s}=N(),{active:i,activeTool:l,originMode:c,color:d,showLabels:f,showAxes:p,shapes:h,refreshTrigger:u}=o,[g,S]=v.useState(_a.lastMeasuredDistance);v.useEffect(()=>{let m;return i&&c===1&&(m=window.setInterval(()=>{const x=_a.lastMeasuredDistance;Math.abs(x-g)>1e-4&&S(x)},200)),()=>clearInterval(m)},[i,c,g]);const y=()=>{a({active:!i})},b=()=>{a({refreshTrigger:(u||0)+1}),S(_a.lastMeasuredDistance)};return t.jsxs("div",{className:`flex flex-col h-full select-none ${e}`,"data-help-id":"panel.drawing",children:[t.jsxs("div",{className:"p-3 bg-black/40 border-b border-white/5",children:[t.jsx(dd,{label:"Measurement Tools",icon:t.jsx("span",{className:"w-2 h-2 rounded-full bg-cyan-500 shadow-[0_0_5px_cyan]"})}),t.jsx("div",{className:"flex gap-2 mb-2",children:t.jsx(da,{onClick:y,active:i,variant:i?"success":"primary",className:"flex-1 py-3 text-xs shadow-lg",icon:i?t.jsx(ft,{}):t.jsx(Fr,{}),children:i?"DRAWING ACTIVE":"START DRAWING"})}),t.jsxs("div",{className:"flex bg-gray-800/50 rounded p-1 mb-3",children:[t.jsxs("button",{onClick:()=>a({activeTool:"rect"}),className:`flex-1 flex items-center justify-center gap-2 py-1.5 rounded text-[9px] font-bold transition-colors ${l==="rect"?"bg-cyan-900 text-cyan-200 shadow-sm":"text-gray-500 hover:text-white"}`,title:"Rectangle",children:[t.jsx(al,{})," RECT"]}),t.jsxs("button",{onClick:()=>a({activeTool:"circle"}),className:`flex-1 flex items-center justify-center gap-2 py-1.5 rounded text-[9px] font-bold transition-colors ${l==="circle"?"bg-cyan-900 text-cyan-200 shadow-sm":"text-gray-500 hover:text-white"}`,title:"Circle / Ellipse",children:[t.jsx(ol,{})," CIRCLE"]})]}),t.jsxs("div",{className:"flex items-center justify-between mb-1",children:[t.jsx(_e,{variant:"secondary",children:"Default Color"}),t.jsx(sa,{color:"#"+d.getHexString(),onChange:m=>a({color:new $e(m)}),label:""})]}),i&&t.jsxs("div",{className:"mt-2 px-2 py-1.5 bg-cyan-900/20 border border-cyan-500/20 rounded flex flex-col items-center gap-1 text-[9px] text-cyan-200 animate-fade-in text-center font-mono",children:[t.jsxs("div",{children:["Hold ",t.jsx("strong",{children:"X"})," to snap to World Axis"]}),t.jsxs("div",{children:["Hold ",t.jsx("strong",{children:"SHIFT"})," for 1:1 Ratio"]}),t.jsxs("div",{children:["Hold ",t.jsx("strong",{children:"ALT"})," for Center Draw"]}),t.jsxs("div",{children:["Hold ",t.jsx("strong",{children:"SPACE"})," to Move"]})]})]}),t.jsxs("div",{className:"p-3 border-b border-white/5 space-y-3 bg-white/[0.02]",children:[t.jsxs("div",{className:"space-y-1",children:[t.jsx(_e,{variant:"secondary",children:"Drawing Plane Origin"}),t.jsx(Ae,{value:c,onChange:m=>a({originMode:m}),options:[{label:"Global Zero",value:0},{label:"Surface Probe",value:1}]}),c===1&&t.jsxs("div",{className:"flex items-center justify-between bg-black/40 rounded border border-white/10 p-1.5 mt-1 animate-fade-in",children:[t.jsxs("span",{className:"text-[9px] text-gray-400 font-mono pl-1",children:["Depth: ",t.jsx("span",{className:"text-cyan-400 font-bold",children:g.toFixed(4)})]}),t.jsx("button",{onClick:b,className:"px-2 py-0.5 bg-gray-800 hover:bg-white/10 text-gray-300 text-[9px] font-bold rounded border border-white/5 hover:border-white/20 transition-all",title:"Update axis position to current probe location",children:"Refresh Axis"})]})]}),t.jsxs("div",{className:"grid grid-cols-2 gap-2 pt-1",children:[t.jsxs("label",{className:"flex items-center gap-2 cursor-pointer group",children:[t.jsx("div",{className:`w-3 h-3 border rounded transition-colors ${f?"bg-cyan-500 border-cyan-500":"border-gray-600 bg-transparent"}`}),t.jsx("input",{type:"checkbox",className:"hidden",checked:f,onChange:m=>a({showLabels:m.target.checked})}),t.jsx("span",{className:"text-[10px] font-bold text-gray-400 group-hover:text-white transition-colors",children:"Show Labels"})]}),t.jsxs("label",{className:"flex items-center gap-2 cursor-pointer group",children:[t.jsx("div",{className:`w-3 h-3 border rounded transition-colors ${p?"bg-cyan-500 border-cyan-500":"border-gray-600 bg-transparent"}`}),t.jsx("input",{type:"checkbox",className:"hidden",checked:p,onChange:m=>a({showAxes:m.target.checked})}),t.jsx("span",{className:"text-[10px] font-bold text-gray-400 group-hover:text-white transition-colors",children:"Show Axes"})]})]})]}),t.jsx("div",{className:"flex-1 overflow-y-auto custom-scroll p-3 bg-black/20",children:t.jsx(jt,{label:"Measurement List",count:(h||[]).length,defaultOpen:!0,rightContent:(h||[]).length>0?t.jsx("button",{onClick:()=>n(),className:"text-[9px] text-red-500 hover:text-red-300 font-bold transition-colors px-2 py-0.5",children:"Clear"}):void 0,children:(h||[]).length===0?t.jsx("div",{className:"text-center py-4 text-[10px] text-gray-600 italic",children:"No measurements drawn."}):t.jsx("div",{className:"space-y-1 animate-fade-in",children:(h||[]).map((m,x)=>{var z;const C=m.type==="rect"&&(m.size.z||0)>.001;return t.jsxs("div",{className:"flex flex-col bg-white/5 rounded border border-white/5 hover:border-cyan-500/30 transition-colors group",children:[t.jsxs("div",{className:"flex items-center justify-between p-2",children:[t.jsxs("div",{className:"flex items-center gap-3",children:[t.jsx("div",{className:"transform scale-75 origin-left",children:t.jsx(sa,{color:m.color,onChange:w=>s({id:m.id,updates:{color:w}}),label:""})}),t.jsxs("div",{className:"flex flex-col",children:[t.jsxs("div",{className:"flex items-center gap-2",children:[t.jsxs("span",{className:"text-[10px] text-gray-300 font-mono font-bold",children:["#",x+1]}),t.jsx("span",{className:"text-[8px] text-gray-500 font-bold bg-black/40 px-1 rounded",children:C?"CUBE":m.type})]}),t.jsxs("span",{className:"text-[9px] text-gray-500 font-mono",children:[m.size.x.toFixed(4)," x ",m.size.y.toFixed(4)," ",C?`x ${(z=m.size.z)==null?void 0:z.toFixed(4)}`:""]})]})]}),t.jsxs("div",{className:"flex items-center gap-1",children:[m.type==="rect"&&t.jsx("button",{onClick:()=>{const M=(m.size.z||0)>0?0:Math.min(m.size.x,m.size.y);s({id:m.id,updates:{size:{...m.size,z:M}}})},className:`p-1.5 rounded transition-colors ${C?"text-cyan-300 bg-cyan-900/40":"text-gray-600 hover:text-cyan-400 hover:bg-white/5"}`,title:C?"Convert to Rect":"Extrude to Cube",children:t.jsx(ma,{})}),t.jsx("button",{onClick:()=>r(m.id),className:"text-gray-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity p-1",title:"Delete",children:t.jsx(wt,{})})]})]}),C&&t.jsxs("div",{className:"px-2 pb-2 pt-0 space-y-1 animate-slider-entry bg-black/20 mt-1 rounded border border-white/5 mx-1",children:[t.jsx(me,{label:"Depth",value:m.size.z||0,onChange:w=>s({id:m.id,updates:{size:{...m.size,z:Math.max(.001,w)}}}),step:.01,min:.001,max:5,highlight:!0}),t.jsx(me,{label:"Offset",value:m.zOffset||0,onChange:w=>s({id:m.id,updates:{zOffset:w}}),step:.01,min:-2,max:2})]})]},m.id)})})})})]})},ut=xe(),Qt=new U,tt=(e,o,a,r)=>(Qt.copy(e).project(o),{x:(Qt.x*.5+.5)*a,y:(-Qt.y*.5+.5)*r,behind:Qt.z>1}),Ur=(e,o)=>{const a=e.center.x-o.x+(e.center.xL-o.xL),r=e.center.y-o.y+(e.center.yL-o.yL),n=e.center.z-o.z+(e.center.zL-o.zL);return new U(a,r,n)},fd=(e,o,a,r,n)=>{const s=Ur(e,n),i=new De(e.orientation.x,e.orientation.y,e.orientation.z,e.orientation.w),l=e.size.x/2,c=e.size.y/2,d=e.zOffset||0,f=e.size.z||0,p=e.type==="rect"&&f>.001,h=p?d-f/2:d;if(e.type==="circle"){const g=[];for(let y=0;y<=48;y++){const b=y/48*Math.PI*2,m=new U(Math.cos(b)*l,Math.sin(b)*c,h);m.applyQuaternion(i).add(s),g.push(tt(m,o,a,r))}return g}if(p){const g=[];for(const S of[-1,1])for(const y of[-1,1])for(const b of[-1,1]){const m=new U(S*l,y*c,h+b*f/2);m.applyQuaternion(i).add(s),g.push(tt(m,o,a,r))}return g}return[[-1,-1],[1,-1],[1,1],[-1,1]].map(([g,S])=>{const y=new U(g*l,S*c,h);return y.applyQuaternion(i).add(s),tt(y,o,a,r)})},pd=[[0,1],[2,3],[4,5],[6,7],[0,2],[1,3],[4,6],[5,7],[0,4],[1,5],[2,6],[3,7]],hd=()=>{const{drawing:e,setDrawing:o,addDrawnShape:a,removeDrawnShape:r}=N(),{active:n,activeTool:s,originMode:i,color:l,showLabels:c,showAxes:d,shapes:f,refreshTrigger:p}=e,h=v.useRef(null),u=v.useRef(null),[g,S]=v.useState(null),y=v.useRef(null),b=v.useRef(!1),m=v.useRef(new U),x=v.useRef(new je),C=v.useRef(new U),z=v.useRef(new dr),w=v.useRef(new U),M=v.useRef(new U),P=v.useRef({space:!1,x:!1}),k=v.useRef(0),[T,_]=v.useState(0),E=v.useCallback(()=>We(),[]),F=v.useCallback(()=>ta(),[]),O=v.useCallback(H=>{const A=E();if(!A)return new U(0,0,-1);let B=new U(0,0,-1).applyQuaternion(A.quaternion).clone().negate();if(H){const q=Math.abs(B.x),Q=Math.abs(B.y),K=Math.abs(B.z);q>Q&&q>K?B.set(Math.sign(B.x),0,0):Q>K?B.set(0,Math.sign(B.y),0):B.set(0,0,Math.sign(B.z))}let I=new U(0,1,0);Math.abs(B.dot(I))>.99&&I.set(0,0,-1);let V=I.clone().sub(B.clone().multiplyScalar(I.dot(B)));V.normalize();const W=new U().crossVectors(V,B).normalize();return w.current.copy(W),M.current.copy(V),z.current.setFromNormalAndCoplanarPoint(B,C.current),B},[E]),$=v.useCallback((H,A,j)=>{const B=E();if(!B)return null;const I=new je((H-j.left)/j.width*2-1,-((A-j.top)/j.height)*2+1),V=new Qa;V.setFromCamera(I,B);const W=new U;return V.ray.intersectPlane(z.current,W)?W:null},[E]);v.useEffect(()=>{const H=j=>{j.key==="Alt"&&j.preventDefault(),j.code==="Space"&&(P.current.space=!0,j.preventDefault()),j.key.toLowerCase()==="x"&&(P.current.x=!0)},A=j=>{j.key==="Alt"&&j.preventDefault(),j.code==="Space"&&(P.current.space=!1),j.key.toLowerCase()==="x"&&(P.current.x=!1)};return window.addEventListener("keydown",H),window.addEventListener("keyup",A),()=>{window.removeEventListener("keydown",H),window.removeEventListener("keyup",A)}},[]),v.useEffect(()=>{if(!n)return;const H=F();if(!H)return;const A=I=>{if(I.button!==0||I.target.closest(".drawing-ui"))return;const V=E();if(!V)return;const W=H.getBoundingClientRect();if(x.current.set(I.clientX,I.clientY),i===1){const K=Math.max(.1,ut.lastMeasuredDistance),ie=new U(0,0,-1).applyQuaternion(V.quaternion);C.current.copy(V.position).addScaledVector(ie,K)}else{const K=ut.sceneOffset;C.current.set(-(K.x+K.xL),-(K.y+K.yL),-(K.z+K.zL))}const q=O(P.current.x),Q=$(I.clientX,I.clientY,W);if(Q){b.current=!0,m.current.copy(Q);const K={center:void 0,size:{x:0,y:0},orientation:new De().setFromRotationMatrix(new yt().makeBasis(w.current,M.current,q)),type:s};S(K),y.current=K,H.setPointerCapture(I.pointerId)}},j=I=>{var ce;if(!b.current)return;const V=H.getBoundingClientRect(),W=O(P.current.x),q=$(I.clientX,I.clientY,V);if(!q)return;if(P.current.space){const pe=$(x.current.x,x.current.y,V);if(pe){const he=new U().subVectors(q,pe);if(m.current.add(he),(ce=y.current)!=null&&ce.center){const Te=y.current.center,Ne={...y.current,center:{...Te,xL:Te.xL+he.x,yL:Te.yL+he.y,zL:Te.zL+he.z}};S(Ne),y.current=Ne}}x.current.set(I.clientX,I.clientY);return}x.current.set(I.clientX,I.clientY);const Q=new U().subVectors(q,m.current);let K=Q.dot(w.current),ie=Q.dot(M.current),ee;if(I.altKey?(K*=2,ie*=2,ee=m.current.clone()):ee=m.current.clone().addScaledVector(w.current,K*.5).addScaledVector(M.current,ie*.5),I.shiftKey){const pe=Math.max(Math.abs(K),Math.abs(ie));K=Math.sign(K)*pe,ie=Math.sign(ie)*pe,I.altKey||(ee=m.current.clone().addScaledVector(w.current,K*.5).addScaledVector(M.current,ie*.5))}const oe=ut.sceneOffset,le={...y.current,center:{x:oe.x,y:oe.y,z:oe.z,xL:oe.xL+ee.x,yL:oe.yL+ee.y,zL:oe.zL+ee.z},size:{x:Math.abs(K),y:Math.abs(ie)},orientation:new De().setFromRotationMatrix(new yt().makeBasis(w.current,M.current,W))};S(le),y.current=le},B=I=>{if(!b.current)return;b.current=!1,H.releasePointerCapture(I.pointerId);const V=y.current,W=N.getState().drawing.color;V&&V.center&&V.size&&V.orientation&&(V.size.x>.001||V.size.y>.001)&&(a({id:Ze(),type:V.type||"rect",center:V.center,size:V.size,orientation:V.orientation,color:"#"+W.getHexString()}),o({active:!1})),S(null),y.current=null};return H.addEventListener("pointerdown",A),H.addEventListener("pointermove",j),H.addEventListener("pointerup",B),()=>{H.removeEventListener("pointerdown",A),H.removeEventListener("pointermove",j),H.removeEventListener("pointerup",B)}},[n,s,i,E,F,o,O,$,a]),v.useEffect(()=>{let H=!0;const A=()=>{H&&(_(j=>j+1),k.current=requestAnimationFrame(A))};return((f==null?void 0:f.length)>0||g||d)&&(k.current=requestAnimationFrame(A)),()=>{H=!1,cancelAnimationFrame(k.current)}},[f==null?void 0:f.length,!!g,d]);const R=E(),Y=F(),L=(Y==null?void 0:Y.clientWidth)||1,G=(Y==null?void 0:Y.clientHeight)||1,X=ut.sceneOffset,D=[];if(f)for(const H of f)D.push({shape:H,color:H.color,isTemp:!1});return g&&g.center&&g.size&&g.orientation&&D.push({shape:g,color:"#"+l.getHexString(),isTemp:!0}),t.jsxs("div",{ref:u,className:"absolute inset-0 overflow-hidden",style:{pointerEvents:"none"},children:[t.jsx("svg",{ref:h,width:L,height:G,className:"absolute inset-0",style:{pointerEvents:"none"},children:R&&D.map(({shape:H,color:A,isTemp:j})=>{const B=H.type==="rect"&&(H.size.z||0)>.001,I=fd(H,R,L,G,X);if(I.some(W=>W.behind))return null;if(B)return t.jsx("g",{children:pd.map(([W,q],Q)=>t.jsx("line",{x1:I[W].x,y1:I[W].y,x2:I[q].x,y2:I[q].y,stroke:A,strokeWidth:j?1:1.5,strokeOpacity:.9},Q))},H.id||"temp");const V=I.map((W,q)=>`${q===0?"M":"L"}${W.x},${W.y}`).join(" ")+" Z";return t.jsx("path",{d:V,fill:"none",stroke:A,strokeWidth:j?1:1.5,strokeOpacity:.9},H.id||"temp")})}),R&&c&&D.map(({shape:H,color:A,isTemp:j})=>{const B=Ur(H,X),I=new De(H.orientation.x,H.orientation.y,H.orientation.z,H.orientation.w),V=H.size.x/2,W=H.size.y/2,q=H.size.z||0,Q=H.zOffset||0,K=H.type==="rect"&&q>.001?Q-q/2:Q,ie=new U(0,W,K+q/2).applyQuaternion(I).add(B),ee=tt(ie,R,L,G),oe=new U(-V,0,K+q/2).applyQuaternion(I).add(B),le=tt(oe,R,L,G);if(ee.behind||le.behind)return null;const ce=new U(V,W,K+q/2).applyQuaternion(I).add(B),pe=tt(ce,R,L,G);return t.jsxs(ke.Fragment,{children:[t.jsx("div",{className:"absolute text-[10px] font-mono font-bold whitespace-nowrap text-cyan-300 opacity-90 px-1",style:{left:ee.x,top:ee.y,transform:"translate(-50%, -100%)",pointerEvents:"none"},children:H.size.x.toFixed(4)}),t.jsx("div",{className:"absolute text-[10px] font-mono font-bold whitespace-nowrap text-cyan-300 opacity-90 px-1",style:{left:le.x,top:le.y,transform:"translate(-100%, -50%) rotate(90deg)",transformOrigin:"right center",pointerEvents:"none"},children:H.size.y.toFixed(4)}),!j&&!pe.behind&&t.jsx("div",{className:"drawing-ui absolute cursor-pointer flex items-center justify-center w-4 h-4 bg-red-900/80 hover:bg-red-500 text-white rounded-full transition-colors shadow-sm border border-white/20",style:{left:pe.x,top:pe.y,transform:"translate(25%, -75%)",pointerEvents:"auto"},onClick:he=>{he.stopPropagation(),r(H.id)},title:"Delete Shape",children:t.jsx("span",{className:"text-[10px] font-bold leading-none mb-[1px]",children:"✕"})})]},(H.id||"temp")+"-labels")}),R&&d&&t.jsx(md,{camera:R,canvasW:L,canvasH:G,originMode:i,trigger:p})]})},md=({camera:e,canvasW:o,canvasH:a,originMode:r,trigger:n})=>{const s=v.useRef({x:0,y:0,z:0,xL:0,yL:0,zL:0});v.useEffect(()=>{let h=new U(0,0,0);if(r===1){const g=Math.max(.1,ut.lastMeasuredDistance),S=new U(0,0,-1).applyQuaternion(e.quaternion);h.copy(e.position).addScaledVector(S,g)}else{const g=ut.sceneOffset;h.set(-(g.x+g.xL),-(g.y+g.yL),-(g.z+g.zL))}const u=ut.sceneOffset;s.current={x:u.x,y:u.y,z:u.z,xL:u.xL+h.x,yL:u.yL+h.y,zL:u.zL+h.z}},[r,n,e]);const i=ut.sceneOffset,l=s.current,c=new U(l.x-i.x+(l.xL-i.xL),l.y-i.y+(l.yL-i.yL),l.z-i.z+(l.zL-i.zL)),d=2,f=[{dir:new U(d,0,0),color:"#ff4444"},{dir:new U(0,d,0),color:"#44ff44"},{dir:new U(0,0,d),color:"#4444ff"}],p=tt(c,e,o,a);return p.behind?null:t.jsxs("svg",{width:o,height:a,className:"absolute inset-0",style:{pointerEvents:"none"},children:[f.map((h,u)=>{const g=c.clone().add(h.dir),S=tt(g,e,o,a);return S.behind?null:t.jsx("line",{x1:p.x,y1:p.y,x2:S.x,y2:S.y,stroke:h.color,strokeWidth:2,strokeOpacity:.7},u)}),Array.from({length:11},(h,u)=>{const g=u-5,S=c.clone().add(new U(g,0,-5)),y=c.clone().add(new U(g,0,5)),b=c.clone().add(new U(-5,0,g)),m=c.clone().add(new U(5,0,g)),x=tt(S,e,o,a),C=tt(y,e,o,a),z=tt(b,e,o,a),w=tt(m,e,o,a),M=g===0;return t.jsxs("g",{children:[!x.behind&&!C.behind&&t.jsx("line",{x1:x.x,y1:x.y,x2:C.x,y2:C.y,stroke:M?"#ff4444":"#444444",strokeWidth:M?1.5:.5,strokeOpacity:.5}),!z.behind&&!w.behind&&t.jsx("line",{x1:z.x,y1:z.y,x2:w.x,y2:w.y,stroke:M?"#4444ff":"#444444",strokeWidth:M?1.5:.5,strokeOpacity:.5})]},u)})]})},gd=new Set(["audio","navigation","drawing","webcam","debugTools","engineSettings","quality","reflections"]),er=["coreMath","geometry","materials","coloring","atmosphere","lighting","optics"],tr=new Set(["repeats","phase","scale","offset","bias","repeats2","phase2","scale2","offset2","bias2","levelsMin","levelsMax","levelsGamma","saturation","juliaX","juliaY","juliaZ","preRotX","preRotY","preRotZ","hybridFoldLimit"]),xd=e=>{if(e==="lighting"){const o=[];for(let a=0;a<3;a++)o.push({label:`Light ${a+1} Intensity`,key:`light${a}_intensity`}),o.push({label:`Light ${a+1} Pos X`,key:`light${a}_posX`}),o.push({label:`Light ${a+1} Pos Y`,key:`light${a}_posY`}),o.push({label:`Light ${a+1} Pos Z`,key:`light${a}_posZ`});return o}return[]},yd=({x:e,y:o,onClose:a,onSelect:r})=>{const[n,s]=v.useState(null),i=v.useRef(null),[l,c]=v.useState({x:e,y:o,maxHeight:300,opacity:0,flip:!1}),d=N(u=>u.formula),p=[...ne.getAll().filter(u=>!gd.has(u.id)&&(Object.values(u.params).some(g=>g.type==="float"||g.type==="int")||u.id==="lighting")).sort((u,g)=>{const S=er.indexOf(u.id),y=er.indexOf(g.id);return S!==-1&&y!==-1?S-y:S!==-1?-1:y!==-1?1:u.name.localeCompare(g.name)}).map(u=>({id:u.id,name:u.name})),{id:"camera",name:"Camera"}];v.useLayoutEffect(()=>{const u=window.innerWidth,g=window.innerHeight,S=10,y=320,b=e+y>u-S;let m=b?Math.max(S,e-y):e;m+y>u-S&&(m=Math.max(S,u-y-S));const x=g-o-S;let C=350,z=o;x<200&&o>x?o>C+S?z=o-C:(z=S,C=Math.min(C,o-S*2)):C=Math.min(C,Math.max(150,x)),c({x:m,y:z,maxHeight:C,opacity:1,flip:b})},[e,o]),v.useEffect(()=>{const u=g=>{i.current&&!i.current.contains(g.target)&&a()};return window.addEventListener("mousedown",u,!0),()=>window.removeEventListener("mousedown",u,!0)},[a]);const h=u=>{var C;if(u==="camera")return[{label:"Camera Pos X",key:"camera.unified.x"},{label:"Camera Pos Y",key:"camera.unified.y"},{label:"Camera Pos Z",key:"camera.unified.z"},{label:"Rotation X",key:"camera.rotation.x"},{label:"Rotation Y",key:"camera.rotation.y"},{label:"Rotation Z",key:"camera.rotation.z"}].map(z=>t.jsx("button",{onClick:()=>{r(z.key),a()},className:"px-3 py-1.5 text-left text-gray-300 hover:bg-cyan-600 hover:text-white transition-colors truncate",children:z.label},z.key));const g=ne.get(u);if(!g)return null;const S=xd(u),y=[],b=u==="coreMath"&&d?Me.get(d):null,m=((C=b==null?void 0:b.parameters)==null?void 0:C.map(z=>z==null?void 0:z.id).filter(z=>!!z))||[];Object.entries(g.params).forEach(([z,w])=>{if(w.onUpdate!=="compile"&&!(w.hidden&&!tr.has(z))&&!(u==="coreMath"&&m.length>0&&!m.includes(z))){if(w.type==="vec2"||w.type==="vec3"){(w.type==="vec2"?["x","y"]:["x","y","z"]).forEach(P=>{let k=`${w.label} ${P.toUpperCase()}`;if(u==="coreMath"&&b){const T=b.parameters.find(_=>(_==null?void 0:_.id)===z);T&&(k=`${z.replace("vec","V-")}: ${T.label} ${P.toUpperCase()}`)}y.push({key:`${u}.${z}_${P}`,label:k,desc:`${w.description||w.label} - ${P.toUpperCase()} component`})});return}if(w.type==="float"||w.type==="int"){if(w.hidden&&!tr.has(z))return;let M=w.label;if(u==="coreMath"&&d){const P=Me.get(d);if(P){const k=P.parameters.find(T=>(T==null?void 0:T.id)===z);k?M=`${z.replace("param","P-")}: ${k.label}`:z.startsWith("param")&&(M=`(${w.label})`)}}y.push({key:`${u}.${z}`,label:M,desc:w.description})}}});const x=[...S.map(z=>({key:`${u}.${z.key}`,label:z.label,desc:void 0})),...y];return t.jsxs(t.Fragment,{children:[x.length===0&&t.jsx("div",{className:"px-3 py-2 text-gray-500 text-xs italic",children:"No modulatable params"}),x.map(z=>t.jsx("button",{onClick:()=>{r(z.key),a()},className:"px-3 py-1.5 text-left text-gray-300 hover:bg-cyan-600 hover:text-white transition-colors truncate flex-shrink-0",title:z.desc||z.label,children:z.label},z.key))]})};return Pt.createPortal(t.jsxs("div",{ref:i,className:"fixed z-[9999] flex text-xs font-mono",style:{left:l.x,top:l.y,opacity:l.opacity,transition:"opacity 0.05s ease-out",flexDirection:l.flip?"row-reverse":"row"},children:[t.jsx("div",{className:`w-32 bg-[#1a1a1a] border border-white/20 shadow-[0_10px_30px_rgba(0,0,0,0.5)] flex flex-col py-1 overflow-y-auto custom-scroll ${l.flip?"rounded-r -ml-px":"rounded-l"}`,style:{maxHeight:l.maxHeight},children:p.map(u=>t.jsxs("div",{onMouseEnter:()=>s(u.id),className:`px-3 py-1.5 cursor-pointer flex justify-between items-center transition-colors ${n===u.id?"bg-cyan-900/60 text-white":"text-gray-400 hover:text-white hover:bg-white/5"}`,children:[t.jsx("span",{className:`truncate ${u.id==="coreMath"?"font-bold text-cyan-300":""}`,children:u.name}),l.flip?t.jsx("span",{className:"text-gray-600",children:"‹"}):t.jsx(ra,{})]},u.id))}),n&&t.jsx("div",{className:`w-48 bg-[#222] border-y border-r border-white/20 shadow-[0_10px_30px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden ${l.flip?"rounded-l animate-fade-in-right":"rounded-r -ml-px animate-fade-in-left"}`,style:{maxHeight:l.maxHeight},children:t.jsx("div",{className:"flex-1 overflow-y-auto custom-scroll py-1",children:h(n)})})]}),document.body)},qr=({value:e,onChange:o,className:a})=>{var p,h,u;const[r,n]=v.useState(!1),s=v.useRef(null),[i,l]=v.useState({x:0,y:0}),c=N(g=>g.formula),d=()=>{if(s.current){const g=s.current.getBoundingClientRect();l({x:g.left,y:g.bottom+4}),n(!0)}};let f=e;if(e.includes(".")){const[g,S]=e.split(".");if(g==="lighting"&&S.startsWith("light")){const y=parseInt(((p=S.match(/\d+/))==null?void 0:p[0])||"0"),b=S.includes("intensity")?"Intensity":S.includes("pos")?"Pos":"Param";f=`Light ${y+1} ${b}`}else if(g==="camera")S.includes("unified")?f=`Camera Pos ${(h=S.split(".").pop())==null?void 0:h.toUpperCase()}`:S.includes("rotation")?f=`Camera Rot ${(u=S.split(".").pop())==null?void 0:u.toUpperCase()}`:f="Camera Param";else{const y=ne.get(g);if(y){const b=y.params[S];if(b)if(g==="coreMath"&&c){const m=Me.get(c),x=m==null?void 0:m.parameters.find(C=>(C==null?void 0:C.id)===S);x?f=`${S.replace("param","P-")}: ${x.label}`:f=b.label}else f=`${y.name}: ${b.label}`;else f=`${y.name}: ${S}`}}}return t.jsxs(t.Fragment,{children:[t.jsx("button",{ref:s,onClick:d,className:`text-left px-2 py-1 bg-black/40 border border-white/10 rounded text-[10px] text-cyan-400 hover:bg-white/5 truncate ${a}`,title:f,children:f}),r&&t.jsx(yd,{x:i.x,y:i.y,onClose:()=>n(!1),onSelect:o})]})},bd=()=>{const e=N(),{modulation:o,removeModulation:a,addModulation:r,openContextMenu:n}=e,s=(h,u)=>{e.updateModulation({id:h,update:u})},i=o.selectedRuleId,l=o.rules.find(h=>h.id===i),c=()=>{r({target:"coreMath.paramA",source:"audio"})},d=h=>{const u=Ge(h.currentTarget);u.length>0&&(h.preventDefault(),h.stopPropagation(),n(h.clientX,h.clientY,[],u))};if(!l)return t.jsxs("div",{className:"flex flex-col items-center justify-center py-6 text-gray-500 gap-3 border-t border-white/5",children:[t.jsx("span",{className:"text-xs italic",children:"Select a box to edit params"}),t.jsx("button",{onClick:c,className:"px-4 py-2 bg-cyan-900/50 border border-cyan-500/30 rounded text-xs font-bold text-cyan-300 hover:bg-cyan-900 transition-colors",children:"+ Add New Link"})]});const f=l.source==="audio",p=(h,u)=>{s(l.id,{freqStart:h,freqEnd:u})};return t.jsxs("div",{className:"flex flex-col gap-3 border-t border-white/5 pt-3 animate-fade-in-up","data-help-id":"audio.links",onContextMenu:d,children:[t.jsxs("div",{className:"flex justify-between items-center bg-white/5 p-2 rounded border border-white/5",children:[t.jsxs("div",{className:"flex-1 mr-2",children:[t.jsx("label",{className:"text-[9px] text-gray-500 font-bold block mb-1",children:"Target Parameter"}),t.jsx(qr,{value:l.target,onChange:h=>s(l.id,{target:h}),className:"w-full"})]}),t.jsx("div",{className:"flex flex-col items-end gap-1",children:t.jsx("button",{onClick:()=>a(l.id),className:"p-2 text-red-500 hover:text-red-300 hover:bg-red-900/20 rounded border border-transparent hover:border-red-900/50 transition-colors",title:"Remove Rule",children:t.jsx(wt,{})})})]}),t.jsxs("div",{className:"flex gap-2 items-center",children:[t.jsx("label",{className:"text-[9px] text-gray-500 font-bold",children:"Source:"}),t.jsxs("select",{value:l.source,onChange:h=>s(l.id,{source:h.target.value}),className:"t-select text-cyan-300",children:[t.jsx("option",{value:"audio",children:"Audio Spectrum"}),t.jsx("option",{value:"lfo-1",children:"LFO 1"}),t.jsx("option",{value:"lfo-2",children:"LFO 2"}),t.jsx("option",{value:"lfo-3",children:"LFO 3"})]})]}),f&&t.jsxs("div",{children:[t.jsx("label",{className:"text-[9px] text-gray-500 font-bold block mb-1",children:"Quick Frequency Bands"}),t.jsxs("div",{className:"flex gap-1",children:[t.jsx("button",{onClick:()=>p(0,.1),className:"flex-1 py-1.5 bg-white/5 hover:bg-white/10 text-[9px] font-bold text-gray-400 rounded border border-white/5",children:"Bass"}),t.jsx("button",{onClick:()=>p(.1,.5),className:"flex-1 py-1.5 bg-white/5 hover:bg-white/10 text-[9px] font-bold text-gray-400 rounded border border-white/5",children:"Mids"}),t.jsx("button",{onClick:()=>p(.5,1),className:"flex-1 py-1.5 bg-white/5 hover:bg-white/10 text-[9px] font-bold text-gray-400 rounded border border-white/5",children:"Treble"}),t.jsx("button",{onClick:()=>p(0,1),className:"flex-1 py-1.5 bg-white/5 hover:bg-white/10 text-[9px] font-bold text-gray-400 rounded border border-white/5",children:"Full"})]})]}),t.jsxs("div",{className:"bg-black/30 rounded border border-white/10 p-3",children:[t.jsxs("div",{className:"grid grid-cols-5 gap-1",children:[t.jsx("div",{className:"flex flex-col items-center",children:t.jsx(Mt,{label:"Attack",value:l.attack,min:.01,max:.99,onChange:h=>s(l.id,{attack:h}),size:40,color:"#fbbf24"})}),t.jsx("div",{className:"flex flex-col items-center",children:t.jsx(Mt,{label:"Decay",value:l.decay,min:.01,max:.99,onChange:h=>s(l.id,{decay:h}),size:40,color:"#fbbf24"})}),t.jsx("div",{className:"flex flex-col items-center",children:t.jsx(Mt,{label:"Smooth",value:l.smoothing??0,min:0,max:.99,onChange:h=>s(l.id,{smoothing:h}),size:40,color:"#a855f7"})}),t.jsx("div",{className:"flex flex-col items-center",children:t.jsx(Mt,{label:"Gain",value:l.gain,min:0,max:10,onChange:h=>s(l.id,{gain:h}),size:40,color:"#22d3ee",unconstrained:!0})}),t.jsx("div",{className:"flex flex-col items-center",children:t.jsx(Mt,{label:"Offset",value:l.offset,min:-5,max:5,onChange:h=>s(l.id,{offset:h}),size:40,color:"#22d3ee",unconstrained:!0})})]}),t.jsxs("div",{className:"grid grid-cols-5 text-[8px] text-gray-500 text-center mt-1 font-bold",children:[t.jsx("div",{children:"Rise"}),t.jsx("div",{children:"Fall"}),t.jsx("div",{children:"Lerp"}),t.jsx("div",{children:"Mult"}),t.jsx("div",{children:"Add"})]})]}),f&&t.jsxs("div",{className:"flex justify-between text-[9px] text-gray-600 px-1",children:[t.jsxs("span",{children:["Freq: ",Math.round(l.freqStart*100),"% - ",Math.round(l.freqEnd*100),"%"]}),t.jsxs("span",{children:["Threshold: ",Math.round(l.thresholdMin*100),"% - ",Math.round(l.thresholdMax*100),"%"]})]})]})},ar=({stops:e})=>{const o=v.useMemo(()=>e?Aa(e):"linear-gradient(to right, #000, #fff)",[e]);return t.jsx("div",{className:"flex-1 h-2.5 rounded-sm overflow-hidden opacity-80",style:{backgroundImage:o,backgroundSize:"100% 100%"}})},vd=({state:e,actions:o})=>{const a=e.coloring,r=e.texturing,n=o.setTexturing,s=N(u=>u.setHistogramLayer),[i,l]=v.useState("layer1"),[c,d]=v.useState(()=>((a==null?void 0:a.layer3Strength)??0)>0),f=i==="layer1",p=i==="layer2";v.useEffect(()=>{s(p?1:0)},[p,s]);const h=((a==null?void 0:a.blendOpacity)??0)>0;return a?t.jsxs("div",{className:"animate-fade-in -mx-4 -mt-4 flex flex-col","data-help-id":"panel.gradient",children:[t.jsxs("div",{className:"flex flex-col",children:[t.jsxs("div",{className:`flex items-center gap-2 px-3 py-1.5 cursor-pointer transition-colors ${f?"bg-neutral-800":"bg-neutral-800/50 hover:bg-white/5"}`,onClick:()=>l("layer1"),children:[t.jsx("span",{className:"text-[10px] font-bold text-gray-300",children:"Layer 1"}),t.jsx(ar,{stops:a.gradient}),t.jsx("svg",{className:`w-3 h-3 text-gray-500 transition-transform ${f?"rotate-180":""}`,viewBox:"0 0 20 20",fill:"currentColor",children:t.jsx("path",{fillRule:"evenodd",d:"M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z",clipRule:"evenodd"})})]}),f&&t.jsxs("div",{className:"flex flex-col animate-fade-in",children:[r&&t.jsx(Ae,{value:r.active,onChange:u=>n({active:u}),options:[{label:"Gradient",value:!1},{label:"Image Texture",value:!0}]}),r&&!r.active?t.jsxs("div",{className:"flex flex-col",children:[t.jsx(se,{featureId:"coloring",groupFilter:"layer1_top"}),t.jsx(se,{featureId:"coloring",groupFilter:"layer1_grad"}),t.jsx(se,{featureId:"coloring",groupFilter:"layer1_hist"}),t.jsx(se,{featureId:"coloring",groupFilter:"layer1_bottom"})]}):r!=null&&r.active?t.jsxs("div",{className:"flex flex-col","data-help-id":"grad.texture",children:[t.jsx(se,{featureId:"texturing",groupFilter:"main"}),t.jsx(se,{featureId:"texturing",groupFilter:"mapping"}),t.jsx(se,{featureId:"texturing",groupFilter:"transform"}),t.jsx(se,{featureId:"coloring",groupFilter:"layer1_bottom",excludeParams:["twist"]})]}):null]})]}),t.jsx(qe,{}),t.jsxs("div",{className:"flex flex-col",children:[t.jsxs("div",{className:`flex items-center gap-2 px-3 py-1.5 cursor-pointer transition-colors ${p?"bg-neutral-800":"bg-neutral-800/50 hover:bg-white/5"}`,onClick:()=>l(p?"layer1":"layer2"),children:[t.jsx("span",{className:`text-[10px] font-bold ${h?"text-gray-300":"text-gray-600"}`,children:"Layer 2"}),!p&&!h&&t.jsx("span",{className:"text-[8px] text-gray-600",children:"off"}),t.jsx(ar,{stops:a.gradient2}),t.jsx("svg",{className:`w-3 h-3 text-gray-500 transition-transform ${p?"rotate-180":""}`,viewBox:"0 0 20 20",fill:"currentColor",children:t.jsx("path",{fillRule:"evenodd",d:"M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z",clipRule:"evenodd"})})]}),p&&t.jsxs("div",{className:"flex flex-col animate-fade-in","data-help-id":"grad.layer2",children:[t.jsx(se,{featureId:"coloring",groupFilter:"layer2_top"}),t.jsx(se,{featureId:"coloring",groupFilter:"layer2_grad"}),t.jsx(se,{featureId:"coloring",groupFilter:"layer2_hist"}),(a.mode2===6||a.mode2===8)&&t.jsx(se,{featureId:"coloring",whitelistParams:["escape"]}),t.jsx(se,{featureId:"coloring",groupFilter:"layer2_bottom"})]})]}),t.jsx(qe,{}),t.jsxs("div",{className:"flex flex-col",children:[t.jsxs("div",{className:`flex items-center gap-2 px-3 py-1.5 cursor-pointer transition-colors ${c?"bg-neutral-800":"bg-neutral-800/50 hover:bg-white/5"}`,onClick:()=>d(!c),children:[t.jsx("span",{className:`text-[10px] font-bold ${(a.layer3Strength??0)>0?"text-gray-300":"text-gray-600"}`,children:"Noise"}),!c&&(a.layer3Strength??0)===0&&t.jsx("span",{className:"text-[8px] text-gray-600",children:"off"}),t.jsx("div",{className:"flex-1"}),t.jsx("svg",{className:`w-3 h-3 text-gray-500 transition-transform ${c?"rotate-180":""}`,viewBox:"0 0 20 20",fill:"currentColor",children:t.jsx("path",{fillRule:"evenodd",d:"M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z",clipRule:"evenodd"})})]}),c&&t.jsx("div",{className:"flex flex-col animate-fade-in","data-help-id":"grad.noise",children:t.jsx(se,{featureId:"coloring",groupFilter:"noise"})})]})]}):null},or=["TAB","CTRL","ALT","SHIFT","SPACE","LMB","MMB","RMB","SCROLL UP","SCROLL DOWN","Z","Y","H","T","1","2","3","4","5","6"],rr={Q:{x:0,y:0,label:"Q ↶"},W:{x:1,y:0,label:"W ▲"},E:{x:2,y:0,label:"E ↷"},A:{x:0,y:1,label:"A ◀"},S:{x:1,y:1,label:"S ▼"},D:{x:2,y:1,label:"D ▶"},C:{x:1,y:2,label:"C ⬇"}},wd={KeyW:"W",KeyA:"A",KeyS:"S",KeyD:"D",KeyQ:"Q",KeyE:"E",KeyC:"C",Space:"SPACE",ShiftLeft:"SHIFT",ShiftRight:"SHIFT",ControlLeft:"CTRL",ControlRight:"CTRL",AltLeft:"ALT",AltRight:"ALT",Tab:"TAB",KeyZ:"Z",KeyY:"Y",KeyH:"H",KeyT:"T",Digit1:"1",Digit2:"2",Digit3:"3",Digit4:"4",Digit5:"5",Digit6:"6"},Sd={0:"LMB",1:"MMB",2:"RMB"},Md=()=>t.jsxs("svg",{width:"12",height:"12",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("circle",{cx:"12",cy:"12",r:"3"}),t.jsx("path",{d:"M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"})]}),zd=["normal","screen","overlay","lighten","difference"],Cd=({sliceState:e,actions:o})=>{const r=66.66666666666667,n=.7,{isEnabled:s,opacity:i,posX:l,posY:c,width:d,height:f,cropL:p,cropR:h,cropT:u,cropB:g,blendMode:S,crtMode:y,tilt:b,fontSize:m}=e,x=o.setWebcam,C=v.useRef(null),z=v.useRef(null),[w,M]=v.useState(!1),[P,k]=v.useState(null),T=v.useRef(0),_=v.useRef(null),E=v.useRef(new Set),F=v.useRef(new Map),O=v.useRef(null),[$,R]=v.useState(!1);v.useEffect(()=>{if(!s){C.current&&C.current.srcObject&&C.current.srcObject.getTracks().forEach(Q=>Q.stop());return}k(null);const j=document.createElement("video");j.autoplay=!0,j.muted=!0,j.playsInline=!0,C.current=j,(async()=>{try{const q=await navigator.mediaDevices.getUserMedia({video:{width:640,height:480,frameRate:{ideal:24}}});C.current&&(C.current.srcObject=q,C.current.play().catch(Q=>{console.error("Webcam play error",Q),k("Video blocked. Check browser privacy settings.")}))}catch(q){console.error("Webcam access denied:",q),q instanceof DOMException&&(q.name==="NotAllowedError"||q.name==="PermissionDeniedError")?k("Camera Blocked: Check browser permissions or HTTPS."):q instanceof DOMException&&q.name==="NotFoundError"?k("No camera found."):k("Camera Error: "+(q instanceof Error?q.message:String(q)))}})();const I=q=>{const Q=wd[q.code];Q&&(q.type==="keydown"?E.current.add(Q):E.current.delete(Q))},V=q=>{const Q=Sd[q.button];Q&&(q.type==="mousedown"?E.current.add(Q):E.current.delete(Q))},W=q=>{const Q=q.deltaY<0?"SCROLL UP":"SCROLL DOWN";F.current.set(Q,1)};return window.addEventListener("keydown",I),window.addEventListener("keyup",I),window.addEventListener("mousedown",V),window.addEventListener("mouseup",V),window.addEventListener("wheel",W,{passive:!0}),()=>{C.current&&C.current.srcObject&&C.current.srcObject.getTracks().forEach(Q=>Q.stop()),window.removeEventListener("keydown",I),window.removeEventListener("keyup",I),window.removeEventListener("mousedown",V),window.removeEventListener("mouseup",V),window.removeEventListener("wheel",W)}},[s]);const Y=v.useCallback(j=>{const B=(j-(T.current||j))/1e3;if([...or,...Object.keys(rr)].forEach(I=>{let V=F.current.get(I)||0;E.current.has(I)?V=1:V-=B/n,V=Math.max(0,Math.min(1,V)),F.current.set(I,V)}),j-T.current>r){const I=z.current,V=C.current;if(I){const W=I.getContext("2d",{alpha:!1});if(W){if(T.current=j,(I.width!==d||I.height!==f)&&(I.width=d,I.height=f),W.fillStyle="#000000",W.fillRect(0,0,d,f),!P&&V&&V.readyState===V.HAVE_ENOUGH_DATA){const q=V.videoWidth,Q=V.videoHeight,K=q*p,ie=Q*u,ee=q*(1-p-h),oe=Q*(1-u-g);ee>0&&oe>0&&(W.save(),W.translate(d,0),W.scale(-1,1),W.drawImage(V,K,ie,ee,oe,0,0,d,f),W.restore())}else P&&(W.fillStyle="#330000",W.fillRect(0,0,d,f),W.fillStyle="#ff5555",W.font=`bold ${Math.max(10,m)}px monospace`,W.textAlign="center",W.textBaseline="middle",P.split(" "),W.fillText(P,d/2,f/2));L(W,d,f),G(W)}}}_.current=requestAnimationFrame(Y)},[p,h,u,g,d,f,m,s,P]);v.useEffect(()=>{if(s)return _.current=requestAnimationFrame(Y),()=>{_.current&&cancelAnimationFrame(_.current)}},[Y,s]);const L=(j,B,I)=>{j.font=`bold ${m}px monospace`,j.textAlign="left",j.textBaseline="bottom";let V=10;const W=I-10,q=m*1.6+4;or.forEach(Q=>{const K=F.current.get(Q)||0;if(K<=.01)return;const ie=j.measureText(Q),ee=m,oe=ie.width+ee;V+oe>B||(j.fillStyle=`rgba(0, 0, 0, ${.8*K})`,j.fillRect(V,W-q,oe,q),j.fillStyle=`rgba(255, 255, 255, ${K})`,j.fillText(Q,V+ee/2,W-q*.25),V+=oe+4)})},G=j=>{const B=m*2.8,I=3,V=10,W=10;j.font=`bold ${m}px monospace`,j.textAlign="center",j.textBaseline="middle",Object.entries(rr).forEach(([q,Q])=>{const K=F.current.get(q)||0;if(K<=.01)return;const ie=V+Q.x*(B+I),ee=W+Q.y*(B+I);j.fillStyle=`rgba(0, 0, 0, ${.8*K})`,j.fillRect(ie,ee,B,B),j.fillStyle=`rgba(255, 255, 255, ${K})`,j.fillText(Q.label,ie+B/2,ee+B/2+1)})},X=(j,B)=>{j.preventDefault(),j.stopPropagation(),O.current={type:B,startX:j.clientX,startY:j.clientY,startPos:{x:l,y:c},startSize:{w:d,h:f},startCrop:{l:p,r:h,t:u,b:g}},window.addEventListener("mousemove",D),window.addEventListener("mouseup",H)},D=v.useCallback(j=>{var Te,Ne;if(!O.current)return;const{type:B,startX:I,startY:V,startPos:W,startSize:q,startCrop:Q}=O.current,K=j.clientX-I,ie=j.clientY-V,ee=((Te=C.current)==null?void 0:Te.videoWidth)||640,oe=((Ne=C.current)==null?void 0:Ne.videoHeight)||480,le=ee*(1-Q.l-Q.r),ce=oe*(1-Q.t-Q.b),pe=q.w/Math.max(1,le),he=q.h/Math.max(1,ce);if(B==="move")x({posX:W.x+K,posY:W.y+ie});else if(B==="scale"){const Se=q.w/q.h,te=Math.max(100,q.w+K);x({width:te,height:te/Se})}else if(B==="crop-l"){const Se=Math.max(50,q.w-K),te=W.x+(q.w-Se),be=(q.w-Se)/pe/ee;x({posX:te,width:Se,cropR:Math.min(.9,Math.max(0,Q.r+be))})}else if(B==="crop-r"){const Se=Math.max(50,q.w+K),te=(q.w-Se)/pe/ee;x({width:Se,cropL:Math.min(.9,Math.max(0,Q.l+te))})}else if(B==="crop-t"){const Se=Math.max(50,q.h-ie),te=W.y+(q.h-Se),be=(q.h-Se)/he/oe;x({posY:te,height:Se,cropT:Math.min(.9,Math.max(0,Q.t+be))})}else if(B==="crop-b"){const Se=Math.max(50,q.h+ie),te=(q.h-Se)/he/oe;x({height:Se,cropB:Math.min(.9,Math.max(0,Q.b+te))})}},[x]),H=()=>{O.current=null,window.removeEventListener("mousemove",D),window.removeEventListener("mouseup",H)};if(!s)return null;const A=zd[Math.floor(S)]||"normal";return t.jsxs("div",{className:"absolute select-none",style:{left:l,top:c,width:d,height:f,cursor:"move",pointerEvents:"auto"},onMouseDown:j=>{j.target.closest(".settings-panel")||X(j,"move")},onMouseEnter:()=>R(!0),onMouseLeave:()=>{R(!1),M(!1)},children:[t.jsx("div",{className:"absolute inset-0 w-full h-full pointer-events-none",style:{mixBlendMode:A,perspective:"1000px"},children:t.jsxs("div",{className:"w-full h-full",style:{opacity:Math.min(1,i),filter:i>1?`brightness(${i})`:"none",transform:`rotateY(${b}deg)`,transformStyle:"preserve-3d"},children:[t.jsx("canvas",{ref:z,className:"w-full h-full block"}),y&&t.jsx("div",{className:"absolute inset-0 opacity-40 mix-blend-overlay",style:{background:"linear-gradient(rgba(18, 16, 16, 0) 50%, rgba(0, 0, 0, 0.25) 50%), linear-gradient(90deg, rgba(255, 0, 0, 0.06), rgba(0, 255, 0, 0.02), rgba(0, 0, 255, 0.06))",backgroundSize:"100% 4px, 6px 100%"}})]})}),t.jsxs("div",{className:"absolute inset-0 w-full h-full",children:[t.jsx("div",{className:`absolute top-2 right-2 transition-opacity duration-200 z-50 ${$?"opacity-100":"opacity-0 pointer-events-none"}`,children:t.jsx("button",{onClick:j=>{j.stopPropagation(),M(!w)},className:"p-1.5 rounded bg-black/60 text-gray-400 hover:text-white hover:bg-black/80 border border-white/10 shadow-lg backdrop-blur-sm",children:t.jsx(Md,{})})}),w&&t.jsx("div",{className:"settings-panel absolute top-10 right-2 w-48 bg-[#151515] border border-white/20 rounded p-2 shadow-2xl z-50 animate-fade-in",onMouseDown:j=>j.stopPropagation(),children:t.jsxs("div",{className:"space-y-2 text-[10px]",children:[t.jsxs("div",{children:[t.jsx(_e,{className:"block mb-1",children:"Blend Mode"}),t.jsxs("select",{value:Math.floor(S),onChange:j=>x({blendMode:Number(j.target.value)}),className:"t-select",children:[t.jsx("option",{value:0,children:"Normal"}),t.jsx("option",{value:1,children:"Screen"}),t.jsx("option",{value:2,children:"Overlay"}),t.jsx("option",{value:3,children:"Lighten"}),t.jsx("option",{value:4,children:"Difference"})]})]}),t.jsxs("div",{children:[t.jsxs("div",{className:"flex justify-between text-gray-500 font-bold mb-1",children:[t.jsx("span",{children:"Opacity"}),t.jsxs("span",{children:[Math.round(i*100),"%"]})]}),t.jsx("input",{type:"range",min:"0",max:"3",step:"0.05",value:i,onChange:j=>x({opacity:parseFloat(j.target.value)}),className:"w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer"})]}),t.jsxs("div",{children:[t.jsxs("div",{className:"flex justify-between text-gray-500 font-bold mb-1",children:[t.jsx("span",{children:"3D Tilt"}),t.jsxs("span",{children:[b,"°"]})]}),t.jsx("input",{type:"range",min:"-45",max:"45",step:"1",value:b,onChange:j=>x({tilt:parseInt(j.target.value)}),className:"w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer"})]}),t.jsxs("div",{children:[t.jsxs("div",{className:"flex justify-between text-gray-500 font-bold mb-1",children:[t.jsx("span",{children:"Font Size"}),t.jsxs("span",{children:[m,"px"]})]}),t.jsx("input",{type:"range",min:"8",max:"32",step:"1",value:m,onChange:j=>x({fontSize:parseInt(j.target.value)}),className:"w-full h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer"})]}),t.jsxs("label",{className:"flex items-center gap-2 cursor-pointer mt-1 pt-1 border-t border-white/10",children:[t.jsx("input",{type:"checkbox",checked:y,onChange:j=>x({crtMode:j.target.checked}),className:"cursor-pointer"}),t.jsx("span",{className:"text-gray-400 font-bold",children:"CRT Scanlines"})]})]})}),t.jsxs("div",{className:`transition-opacity duration-200 ${$?"opacity-100":"opacity-0 pointer-events-none"}`,children:[t.jsx("div",{className:"absolute bottom-[-6px] right-[-6px] w-6 h-6 bg-cyan-500/50 cursor-nwse-resize hover:bg-cyan-400 z-20 rounded-full border-2 border-black",onMouseDown:j=>X(j,"scale")}),t.jsx("div",{className:"absolute top-4 bottom-4 left-[-4px] w-3 cursor-e-resize group/l z-10 flex items-center justify-center",onMouseDown:j=>X(j,"crop-l"),children:t.jsx("div",{className:"w-1 h-8 bg-red-500/50 group-hover/l:bg-red-400 rounded-full"})}),t.jsx("div",{className:"absolute top-4 bottom-4 right-[-4px] w-3 cursor-w-resize group/r z-10 flex items-center justify-center",onMouseDown:j=>X(j,"crop-r"),children:t.jsx("div",{className:"w-1 h-8 bg-red-500/50 group-hover/r:bg-red-400 rounded-full"})}),t.jsx("div",{className:"absolute left-4 right-4 top-[-4px] h-3 cursor-s-resize group/t z-10 flex items-center justify-center",onMouseDown:j=>X(j,"crop-t"),children:t.jsx("div",{className:"h-1 w-8 bg-red-500/50 group-hover/t:bg-red-400 rounded-full"})}),t.jsx("div",{className:"absolute left-4 right-4 bottom-[-4px] h-3 cursor-n-resize group/b z-10 flex items-center justify-center",onMouseDown:j=>X(j,"crop-b"),children:t.jsx("div",{className:"h-1 w-8 bg-red-500/50 group-hover/b:bg-red-400 rounded-full"})})]})]})]})},kd=({className:e="-m-4"})=>{const o=N(),[a,r]=v.useState({}),[n,s]=v.useState(null),[i,l]=v.useState(!1),c=v.useMemo(()=>{const x={};return["lighting","ao","geometry","reflections","quality","atmosphere"].forEach(z=>{const w=o[z];w&&typeof w=="object"&&(x[z]={...w})}),Object.entries(a).forEach(([z,w])=>{const[M,P]=z.split(".");x[M]&&(x[M][P]=w)}),x},[o,a]),d=vr(c),f=d.charAt(0).toUpperCase()+d.slice(1),h=(v.useMemo(()=>ls(c),[c])/1e3).toFixed(1),u=ne.getEngineFeatures(),g=v.useRef(()=>{});v.useEffect(()=>{const x=Z.on("compile_time",w=>{s(`Compiled (${w.toFixed(2)}s)`),l(!1),setTimeout(()=>s(null),3e3)}),C=Z.on("is_compiling",w=>{l(!!w)}),z=Z.on("engine_queue",({featureId:w,param:M,value:P})=>{g.current(w,M,P)});return()=>{x(),C(),z()}},[]);const S=(x,C,z)=>{var E,F;const w=ne.get(x),M=w==null?void 0:w.params[C],P=((E=w==null?void 0:w.engineConfig)==null?void 0:E.toggleParam)===C,k=(F=w==null?void 0:w.engineConfig)==null?void 0:F.mode,T=(M==null?void 0:M.onUpdate)==="compile";if(P&&k==="compile"||T){const O=`${x}.${C}`,$=o[x];if($&&$[C]===z){const R={...a};delete R[O],r(R)}else r(R=>({...R,[O]:z}));s(null)}else{const O=`set${x.charAt(0).toUpperCase()+x.slice(1)}`,$=o[O];$&&$({[C]:z});const R=`${x}.${C}`;if(a[R]!==void 0){const Y={...a};delete Y[R],r(Y)}}};g.current=S;const y=()=>{Z.emit("is_compiling","Compiling Shaders...");const x={};Object.entries(a).forEach(([C,z])=>{const[w,M]=C.split(".");x[w]||(x[w]={}),x[w][M]=z}),setTimeout(()=>{Object.entries(x).forEach(([C,z])=>{const w=`set${C.charAt(0).toUpperCase()+C.slice(1)}`,M=o[w];M&&M(z)}),r({})},100)},b=x=>{if(x==="Custom")return;const C=pa[x];if(!C)return;const z={};Object.entries(C).forEach(([w,M])=>{Object.entries(M).forEach(([P,k])=>{var E;const T=(E=o[w])==null?void 0:E[P];let _=T!==k;typeof k=="number"&&typeof T=="number"&&(_=Math.abs(k-T)>.001),_&&(z[`${w}.${P}`]=k)})}),r(z),s(null)},m=x=>{const C=o[x];if(!C)return{};const z={...C};return Object.entries(a).forEach(([w,M])=>{const[P,k]=w.split(".");P===x&&(z[k]=M)}),z};return t.jsxs("div",{className:`flex flex-col h-full bg-[#080808] min-h-0 overflow-hidden ${e}`,"data-help-id":"panel.engine",children:[t.jsxs("div",{className:"px-3 py-2 bg-black/60 border-b border-white/10 flex items-center justify-between shrink-0",children:[t.jsx(_e,{children:"Engine Configuration"}),t.jsx("div",{className:"w-32",children:t.jsx(kt,{value:f,options:[{label:"Fastest (Bare)",value:"Fastest"},{label:"Lite (Fast)",value:"Lite"},{label:"Balanced",value:"Balanced"},{label:"Ultra",value:"Ultra"},{label:"Custom",value:"Custom"}],onChange:x=>b(x.toLowerCase())})})]}),t.jsxs("div",{className:"flex-1 overflow-y-auto custom-scroll p-0 min-h-0",children:[t.jsxs("div",{className:"flex gap-2 items-center px-3 py-2 bg-blue-900/10 border-b border-white/5 mb-1 shrink-0",children:[t.jsx("div",{className:"text-blue-400",children:t.jsx(jr,{})}),t.jsxs("p",{className:"text-[9px] text-blue-200/80 leading-tight",children:[t.jsx("span",{className:"text-green-400",children:"●"})," Compiled  ",t.jsx("span",{className:"text-amber-400",children:"●"})," Pending  ",t.jsx("span",{className:"text-blue-400",children:"●"})," Instant"]})]}),t.jsx("div",{className:"flex flex-col",children:u.map(x=>{const C=x.engineConfig,z=m(x.id),w=C.toggleParam,M=z[w],P=`${x.id}.${w}`,T=a[P]!==void 0?"pending":"synced";return t.jsxs("div",{className:"group",children:[t.jsx(Ua,{label:C.label,description:C.description,isActive:M,onToggle:_=>S(x.id,w,_),status:T}),M&&C.groupFilter&&t.jsx("div",{className:"ml-4 pl-2 border-l border-white/10 my-0.5",children:t.jsx(se,{featureId:x.id,groupFilter:C.groupFilter,excludeParams:[C.toggleParam],variant:"dense",forcedState:z,onChangeOverride:(_,E)=>S(x.id,_,E),pendingChanges:a})})]},x.id)})})]}),t.jsx("div",{className:"px-3 py-2 bg-[#1a1a1a] border-t border-white/10 flex items-center justify-between min-h-[40px] shrink-0 z-10",children:i?t.jsxs(t.Fragment,{children:[t.jsxs("div",{className:"flex items-center gap-2 text-cyan-400 text-[10px] font-bold",children:[t.jsx(Ir,{className:"animate-spin h-3 w-3"}),t.jsx("span",{children:"Compiling..."})]}),t.jsxs("div",{className:"text-[9px] text-gray-500",children:["~",h,"s"]})]}):Object.keys(a).length>0?t.jsxs(t.Fragment,{children:[t.jsxs("div",{className:"flex items-center gap-1.5",children:[t.jsxs("div",{className:"flex items-center gap-2 text-amber-500 text-[10px] font-bold animate-pulse",children:[t.jsx(Rt,{}),t.jsx("span",{children:"Pending"})]}),t.jsxs("span",{className:"text-[9px] text-gray-500 font-mono",children:["~",h,"s"]})]}),t.jsxs("button",{onClick:y,disabled:i,className:"px-4 py-1 bg-amber-600 hover:bg-amber-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-black font-bold text-[10px] rounded transition-colors flex items-center gap-1",children:[t.jsx(ft,{})," Apply"]})]}):t.jsxs(t.Fragment,{children:[t.jsxs("div",{className:"flex items-center gap-2",children:[t.jsx("span",{className:"text-[10px] text-gray-600 font-medium",children:"System Ready"}),t.jsxs("span",{className:"text-[9px] text-gray-600 font-mono",children:["~",h,"s"]})]}),n&&t.jsxs("div",{className:"text-[10px] text-green-400 font-bold animate-fade-in flex items-center gap-1",children:[t.jsx(ft,{})," ",n]})]})})]})},jd=xe(),Pd=e=>{const o=new De(e.x,e.y,e.z,e.w),a=new U(0,0,-1).applyQuaternion(o),r=.98;return a.y>r?"Bottom View":a.y<-r?"Top View":a.x>r?"Left View":a.x<-r?"Right View":a.z>r?"Back View":a.z<-r?"Front View":null},Td=(e,o)=>{let r=He.getUnifiedFromEngine().length();r<.001&&(r=3.5);let n=jd.lastMeasuredDistance;(!n||n>=1e3||n<=0)&&(n=r);const s=new U(0,0,0),i=new De;let l=!0;switch(e){case"Front":i.setFromEuler(new Fe(0,0,0));break;case"Back":i.setFromEuler(new Fe(0,Math.PI,0));break;case"Left":i.setFromEuler(new Fe(0,-Math.PI/2,0));break;case"Right":i.setFromEuler(new Fe(0,Math.PI/2,0));break;case"Top":i.setFromEuler(new Fe(-Math.PI/2,0,0));break;case"Bottom":i.setFromEuler(new Fe(Math.PI/2,0,0));break;case"Isometric":l=!1;const g=rt.degToRad(-35.264),S=rt.degToRad(45);i.setFromEuler(new Fe(g,S,0,"YXZ"));break}const c=new U(0,0,-1).applyQuaternion(i),d=s.clone().sub(c.multiplyScalar(r)),f=l?1:0;let p=o?o.orthoScale:2,h=o?o.dofStrength:0,u;return l&&(h=0,u={camType:f,orthoScale:p,dofStrength:h}),{position:d,rotation:{x:i.x,y:i.y,z:i.z,w:i.w},targetDistance:n,optics:u}},Wr=(e,o)=>{if(!e)return null;let a=1/0,r=-1/0;const n=[];for(let u=0;u<e.length;u+=4){const g=e[u];g>-.9&&(o||(g<a&&(a=g),g>r&&(r=g)),n.push(g))}let s,i;if(o)s=o.min,i=o.max;else{if(a===1/0)return null;const u=r-a;u<1e-4?(s=a-.1,i=r+.1):(s=a-u*.05,i=r+u*.05)}const l=128,c=new Array(l).fill(0),d=i-s,f=Math.max(d,1e-6);for(const u of n){const g=(u-s)/f,S=Math.floor(g*l);S>=0&&S<l&&c[S]++}const p=Math.max(...c);return{buckets:c.map(u=>u>0?Math.log(u+1)/Math.log(p+1):0),min:s,max:i}},Xr=(e,o,a)=>{if(!e||e.length<10)return null;const r=e.length,n=e.map((m,x)=>x===0||x===r-1?0:m);let s=0;if(n.forEach(m=>s+=m),s<.01)return{start:o,end:a};const i=s*.02,l=s*.98;let c=0,d=0,f=r-1,p=!1;for(let m=0;m<r;m++)if(c+=n[m],!p&&c>=i&&(d=m,p=!0),c>=l){f=m;break}const h=.05;for(;d>1&&e[d-1]>h&&!(e[d-1]>e[d]*2);)d--;for(;f<r-2&&e[f+1]>h&&!(e[f+1]>e[f]*2);)f++;const u=(a-o)/r;let g=o+d*u,S=o+f*u;const b=(S-g)*.05;return g=Math.max(o,g-b),S=Math.min(a,S+b),{start:g,end:S}},Yr=({data:e,min:o,max:a,gamma:r,repeats:n=1,phase:s=0,gradientStops:i,onChange:l,autoUpdate:c,onToggleAuto:d,onRefresh:f,isStale:p=!1,height:h=48,labelTitle:u="Levels",labelLeft:g="Black",labelMid:S="Gamma",labelRight:y="White",fixedRange:b})=>{const m=v.useRef(null),[x,C]=v.useState(b||{min:0,max:1}),z=N(I=>I.openContextMenu),w=v.useMemo(()=>{const I=Wr(e,b);return I?(C({min:I.min,max:I.max}),I.buckets):(b&&C(b),[])},[e,b]),M=Math.pow(.5,1/r)*100;v.useEffect(()=>{const I=m.current;if(!I)return;const V=I.getContext("2d");if(!V||(V.clearRect(0,0,I.width,I.height),w.length===0))return;const W=I.width,q=I.height,Q=W/w.length;V.fillStyle="#666",w.forEach((K,ie)=>{const ee=K*q;V.fillRect(ie*Q,q-ee,Q,ee)})},[w]);const P=I=>{const V=x.max-x.min;return V<1e-5?50:(I-x.min)/V*100},k=w.length>0||b,T=k?P(o):0,_=k?P(a):100,E=_-T,F=T+M/100*E,O=v.useRef(null),$=(I,V)=>{I.preventDefault(),I.stopPropagation(),O.current={type:V,startX:I.clientX,startMin:o,startMax:a,startGamma:r},window.addEventListener("mousemove",R),window.addEventListener("mouseup",Y)},R=I=>{if(!O.current||!m.current)return;const{type:V,startX:W,startMin:q,startMax:Q,startGamma:K}=O.current,ie=m.current.getBoundingClientRect(),ee=I.clientX-W,oe=x.max-x.min,le=w.length>0||b?oe:1,ce=ee/ie.width*le;let pe=q,he=Q,Te=K;if(V==="min")pe+=ce;else if(V==="max")he+=ce;else if(V==="pan")pe+=ce,he+=ce;else if(V==="gamma"){const Ne=ie.width*Math.abs(Q-q)/le,te=Math.pow(.5,1/K)*Ne,Pe=Math.max(1,Math.min(Ne-1,te+ee))/Ne;Te=Math.log(.5)/Math.log(Pe),Te=Math.max(.1,Math.min(10,Te))}pe>=he&&(V==="min"&&(pe=he-.001),V==="max"&&(he=pe+.001)),l({min:pe,max:he,gamma:Te})},Y=()=>{O.current=null,window.removeEventListener("mousemove",R),window.removeEventListener("mouseup",Y)},L=()=>{if(w.length===0)return;const I=Xr(w,x.min,x.max);I&&l({min:I.start,max:I.end,gamma:1})},G=I=>{I.preventDefault(),I.stopPropagation(),l({min:0,max:1,gamma:1})},X=I=>{const V=Ge(I.currentTarget);V.length>0&&(I.preventDefault(),I.stopPropagation(),z(I.clientX,I.clientY,[],V))},D=v.useMemo(()=>Aa(i||[{id:"b",position:0,color:"#000000"},{id:"w",position:1,color:"#ffffff"}],r),[i,r]),H=Math.max(.1,n),A={left:`${T}%`,width:`${Math.max(0,_-T)}%`},j=1+2/H,B={backgroundImage:D,backgroundSize:`${100/(H*j)}% 100%`,backgroundRepeat:"repeat-x",width:`${100*j}%`,marginLeft:`${-100/H}%`,transform:`translateX(${s*100/(H*j)}%)`};return t.jsxs("div",{className:"py-2 bg-gray-900/40","data-help-id":"ui.histogram",onContextMenu:X,children:[t.jsxs("div",{className:"flex justify-between items-center mb-2 px-3",children:[t.jsxs("div",{className:"flex items-center gap-2",children:[t.jsx("label",{className:"text-[10px] text-gray-500 font-bold",children:u}),p&&!c&&t.jsx("span",{className:"text-[8px] text-amber-400 bg-amber-900/30 px-1.5 py-0.5 rounded border border-amber-500/30",children:"Stale"}),d&&t.jsx("div",{className:"flex items-center justify-center w-4 h-4 cursor-pointer group rounded hover:bg-white/10",onClick:d,title:"Auto-update histogram (Live)",children:t.jsx("div",{className:`w-2 h-2 rounded-full transition-all duration-300 ${c?"bg-green-500 shadow-[0_0_5px_rgba(34,197,94,0.8)]":"bg-gray-600"}`})}),f&&!c&&t.jsx("button",{onClick:f,className:"text-[9px] text-cyan-500 hover:text-white ml-1",children:"Refresh"})]}),t.jsxs("div",{className:"flex items-center gap-1",children:[t.jsx("button",{onClick:()=>l({min:0,max:1,gamma:1}),className:"px-2 py-0.5 bg-gray-800/50 hover:bg-gray-700 text-gray-400 hover:text-white text-[8px] rounded border border-gray-600 transition-colors font-bold",title:"Reset to 0-1 range",children:"0-1"}),d&&t.jsx("button",{onClick:L,className:"px-2 py-0.5 bg-cyan-900/40 hover:bg-cyan-700 text-cyan-400 text-[9px] rounded border border-cyan-800 transition-colors font-bold",title:"Fit range to current data",children:"Fit"})]})]}),t.jsxs("div",{className:`relative w-full bg-black/60 overflow-hidden select-none border-y border-white/5 transition-colors group/hist ${f&&!c?"cursor-pointer hover:bg-black/40":""}`,style:{height:h},onClick:f&&!c?f:void 0,children:[t.jsxs("div",{className:"absolute inset-0 right-4 left-3 mx-2",children:[t.jsx("canvas",{ref:m,width:320,height:h,className:"w-full h-full opacity-40 absolute inset-0"}),t.jsx("div",{className:"absolute top-0 bottom-0 overflow-hidden pointer-events-none opacity-40",style:A,children:t.jsx("div",{className:"h-full",style:B})}),t.jsxs("div",{className:"absolute top-0 bottom-0 w-4 -ml-2 cursor-ew-resize z-20 group/min flex justify-center",style:{left:`${T}%`},onMouseDown:I=>$(I,"min"),children:[t.jsx("div",{className:"w-px h-full bg-white/60 group-hover/min:bg-white group-hover/min:w-0.5 transition-all shadow-[0_0_5px_rgba(0,0,0,0.8)]"}),t.jsx("div",{className:"absolute top-0 w-0 h-0 border-l-[4px] border-r-[4px] border-t-[6px] border-l-transparent border-r-transparent border-t-white"})]}),t.jsxs("div",{className:"absolute top-0 bottom-0 w-4 -ml-2 cursor-ew-resize z-20 group/max flex justify-center",style:{left:`${_}%`},onMouseDown:I=>$(I,"max"),children:[t.jsx("div",{className:"w-px h-full bg-white/60 group-hover/max:bg-white group-hover/max:w-0.5 transition-all shadow-[0_0_5px_rgba(0,0,0,0.8)]"}),t.jsx("div",{className:"absolute bottom-0 w-0 h-0 border-l-[4px] border-r-[4px] border-b-[6px] border-l-transparent border-r-transparent border-b-white"})]}),E>5&&t.jsx("div",{className:"absolute top-1/2 -translate-y-1/2 w-4 h-4 -ml-2 cursor-ew-resize z-30 group/gamma flex items-center justify-center",style:{left:`${F}%`},onMouseDown:I=>$(I,"gamma"),children:t.jsx("div",{className:"w-2 h-2 rotate-45 bg-gray-400 border border-black group-hover/gamma:bg-white group-hover/gamma:scale-125 transition-transform shadow-md"})}),t.jsx("div",{className:"absolute top-0 bottom-0 cursor-grab active:cursor-grabbing z-10",style:{left:`${T}%`,width:`${Math.max(0,_-T)}%`},onMouseDown:I=>$(I,"pan")})]}),t.jsxs("button",{onClick:G,className:"absolute top-0 bottom-0 right-0 w-4 bg-red-900/50 hover:bg-red-700/80 border-l border-white/10 z-40 opacity-0 group-hover/hist:opacity-100 transition-opacity flex items-center justify-center",title:"Reset Range",children:[t.jsx("div",{className:"w-px h-2 bg-white/80 rotate-45 transform origin-center absolute"}),t.jsx("div",{className:"w-px h-2 bg-white/80 -rotate-45 transform origin-center absolute"})]})]}),t.jsxs("div",{className:"flex justify-between items-center mt-2 px-3",children:[t.jsxs("div",{className:"flex flex-col items-start w-16",children:[t.jsx("span",{className:"text-[8px] text-gray-600",children:g}),t.jsx(ct,{value:o,onChange:I=>l({min:I,max:a,gamma:r}),step:.01,min:-1/0,max:1/0,highlight:!0})]}),t.jsxs("div",{className:"flex flex-col items-center w-16",children:[t.jsx("span",{className:"text-[8px] text-gray-600",children:S}),t.jsx(ct,{value:r,onChange:I=>l({min:o,max:a,gamma:I}),step:.01,min:.1,max:10,overrideText:r.toFixed(2)})]}),t.jsxs("div",{className:"flex flex-col items-end w-16",children:[t.jsx("span",{className:"text-[8px] text-gray-600",children:y}),t.jsx(ct,{value:a,onChange:I=>l({min:o,max:I,gamma:r}),step:.01,min:-1/0,max:1/0,highlight:!0})]})]})]})},Za=xe(),Rd=({sliceState:e,actions:o})=>{const a=N(h=>h.sceneHistogramData);N(h=>h.sceneHistogramTrigger);const r=N(h=>h.refreshSceneHistogram),n=N(h=>h.liveModulations),{levelsMin:s,levelsMax:i,levelsGamma:l}=e,c=o.setColorGrading,d=(n==null?void 0:n["colorGrading.levelsMin"])??s,f=(n==null?void 0:n["colorGrading.levelsMax"])??i,p=(n==null?void 0:n["colorGrading.levelsGamma"])??l;return t.jsx("div",{className:"mt-2 pt-2 border-t border-white/5",children:t.jsx(Yr,{data:a,min:d??0,max:f??1,gamma:p??1,onChange:({min:h,max:u,gamma:g})=>{c({levelsMin:h,levelsMax:u,levelsGamma:g})},onRefresh:r,height:40,fixedRange:{min:0,max:1}})})},Id=({sliceState:e,actions:o})=>{const{camFov:a,camType:r,dofStrength:n}=e,s=o.setOptics;N(u=>u.interactionMode),N(u=>u.setInteractionMode),N(u=>u.focusLock),N(u=>u.setFocusLock);const[i,l]=v.useState(!0),c=v.useRef(null),d=Math.abs((r??0)-0)<.1,f=()=>{const u=We();if(!u)return;const g=N.getState(),S=Za.lastMeasuredDistance;let y=S>1e-4&&S<900?S:g.targetDistance||3.5;y=Math.max(.001,y);const b=He.getUnifiedFromEngine(),m=new U(0,0,-1).applyQuaternion(u.quaternion);c.current={fov:a,dist:y,unifiedPos:{x:b.x,y:b.y,z:b.z},forward:m,quat:u.quaternion.clone()}},p=u=>{const g={camFov:u};if(i&&c.current){const{fov:w,dist:M,unifiedPos:P,forward:k,quat:T}=c.current,_=rt.degToRad(w),E=rt.degToRad(u),F=Math.tan(_/2)/Math.tan(E/2),O=M*F,$=M-O,R=k.clone().multiplyScalar($);g.dofFocus=O;const Y=new U(P.x,P.y,P.z).add(R);He.teleportPosition(Y,{x:T.x,y:T.y,z:T.z,w:T.w},O),N.setState({targetDistance:O})}s(g);const{isRecording:S,captureCameraFrame:y,addKeyframe:b,addTrack:m,currentFrame:x,sequence:C,isPlaying:z}=fe.getState();if(S){const w=z?"Linear":"Bezier";if(g.dofFocus!==void 0){const M="optics.dofFocus";C.tracks[M]||m(M,"Focus Distance"),b(M,x,g.dofFocus,w)}i&&y(x,!0,w)}},h=()=>{if(i){const{currentFrame:u,captureCameraFrame:g,isPlaying:S}=fe.getState();g(u,!0,S?"Linear":"Bezier")}};return t.jsx("div",{className:"flex flex-col",children:d&&t.jsxs("div",{children:[t.jsx(me,{label:"Field of View",value:a??60,min:10,max:150,step:1,onChange:p,onDragStart:f,overrideInputText:`${Math.round(a??60)}°`,trackId:"optics.camFov",onKeyToggle:h}),t.jsx("div",{children:t.jsx(Ae,{label:"Dolly Link",icon:t.jsx(Va,{active:i}),value:i,onChange:u=>l(u)})})]})})},Fd=({sliceState:e,actions:o})=>{const a=o.setOptics,r=N(d=>d.interactionMode),n=N(d=>d.setInteractionMode),s=N(d=>d.focusLock),i=N(d=>d.setFocusLock),l=r==="picking_focus",c=d=>{i(d),d&&Za.lastMeasuredDistance>0&&a({dofFocus:Za.lastMeasuredDistance})};return t.jsxs("div",{className:"grid grid-cols-2 gap-px p-px",children:[t.jsx(da,{active:s,onClick:()=>c(!s),label:s?"Lock On":"Focus Lock",variant:"primary"}),t.jsx(da,{active:l,onClick:()=>n(l?"none":"picking_focus"),label:l?"Picking...":"Pick Focus",variant:"success"})]})},Zr=()=>{const e=N(n=>n.sceneOffset),o=N(n=>n.cameraRot),a=He.getUnifiedPosition({x:0,y:0,z:0},e),r=He.getRotationDegrees(o);return t.jsxs(t.Fragment,{children:[t.jsx("div",{"data-help-id":"cam.position",children:t.jsx($t,{label:"Absolute Position",value:a,onChange:n=>He.teleportPosition(n),step:.1,min:-1/0,max:1/0,interactionMode:"camera",trackKeys:["camera.unified.x","camera.unified.y","camera.unified.z"],trackLabels:["Position X","Position Y","Position Z"]})}),t.jsx("div",{"data-help-id":"cam.rotation",children:t.jsx($t,{label:"Rotation (Degrees)",value:r,onChange:He.teleportRotation,step:1,min:-180,max:180,interactionMode:"camera",trackKeys:["camera.rotation.x","camera.rotation.y","camera.rotation.z"],trackLabels:["Rotation X","Rotation Y","Rotation Z"],convertRadToDeg:!0})})]})},_d=()=>{const e=N(n=>n.cameraMode),o=N(n=>n.setCameraMode),a=N(n=>n.optics),r=a&&Math.abs(a.camType-1)<.1;return t.jsxs("div",{className:"flex flex-col gap-3",children:[t.jsxs("div",{className:r?"opacity-50 pointer-events-none":"",children:[t.jsx(Ae,{value:e,onChange:n=>o(n),options:[{label:"Orbit",value:"Orbit"},{label:"Fly",value:"Fly"}]}),r&&t.jsx("p",{className:"text-[9px] text-gray-500 mt-1 text-center",children:"Fly Mode disabled in Orthographic view"})]}),e==="Fly"&&t.jsx(se,{featureId:"navigation",groupFilter:"movement"}),t.jsx(Zr,{})]})},ea=xe(),ir=[{type:"none",label:"None"},{type:"thirds",label:"Rule of Thirds"},{type:"golden",label:"Golden Ratio"},{type:"grid",label:"Grid"},{type:"center",label:"Center Mark"},{type:"diagonal",label:"Diagonal"},{type:"spiral",label:"Spiral"},{type:"safearea",label:"Safe Areas"}],nr=async()=>{try{const e=await ea.captureSnapshot();if(!e)return;const o=await createImageBitmap(e),a=128,r=document.createElement("canvas");r.width=a,r.height=a;const n=r.getContext("2d"),s=Math.min(o.width,o.height),i=(o.width-s)/2,l=(o.height-s)/2;return n.drawImage(o,i,l,s,s,0,0,a,a),r.toDataURL("image/jpeg",.7)}catch{return}},Dd=({className:e="-m-3"})=>{var X;const{savedCameras:o,activeCameraId:a,addCamera:r,deleteCamera:n,selectCamera:s,updateCamera:i,resetCamera:l,duplicateCamera:c,reorderCameras:d}=N(),f=N(D=>D.optics),p=N(D=>D.compositionOverlay),h=N(D=>D.setCompositionOverlay),u=N(D=>D.compositionOverlaySettings),g=N(D=>D.setCompositionOverlaySettings),S=N(D=>D.setOptics),y=N(D=>D.sceneOffset),b=N(D=>D.cameraRot),m=v.useCallback(D=>{const H=y,A=H.x+(H.xL??0),j=H.y+(H.yL??0),B=H.z+(H.zL??0),I=D.sceneOffset||{x:0,y:0,z:0,xL:0,yL:0,zL:0},V=I.x+(I.xL??0),W=I.y+(I.yL??0),q=I.z+(I.zL??0);return!!(Math.abs(A-V)+Math.abs(j-W)+Math.abs(B-q)>1e-4||Math.abs(b.x-D.rotation.x)+Math.abs(b.y-D.rotation.y)+Math.abs(b.z-D.rotation.z)+Math.abs(b.w-D.rotation.w)>.001||D.optics&&f&&(Math.abs((f.camType??0)-(D.optics.camType??0))>.1||Math.abs((f.orthoScale??2)-(D.optics.orthoScale??2))>.01||Math.abs((f.camFov??60)-(D.optics.camFov??60))>.1))},[y,b,f]),[x,C]=v.useState(null),[z,w]=v.useState(null),[M,P]=v.useState(""),k=D=>{C(D.id),P(D.label)},T=()=>{x&&(i(x,{label:M}),C(null))},_=D=>{D.key==="Enter"&&T(),D.key==="Escape"&&C(null)},E=D=>{const H=Td(D,f);He.teleportPosition(H.position,H.rotation,H.targetDistance),H.optics&&S&&S(H.optics),s(null)},F=v.useCallback(async()=>{const D=He.getRotationFromEngine();let H=`Camera ${o.length+1}`;const A=N.getState().optics;if(A&&Math.abs(A.camType-1)<.1){const B=Pd(D);B&&(H=B)}r(H);const j=await nr();if(j){const B=N.getState().savedCameras,I=B[B.length-1];I&&i(I.id,{thumbnail:j})}},[o.length,r,i]),O=v.useCallback(async D=>{const H=He.getUnifiedFromEngine(),A=He.getRotationFromEngine(),j=ye.split(H.x),B=ye.split(H.y),I=ye.split(H.z),V=ea.lastMeasuredDistance>0&&ea.lastMeasuredDistance<1e3?ea.lastMeasuredDistance:N.getState().targetDistance,W={...N.getState().optics},q=await nr();i(D,{position:{x:0,y:0,z:0},rotation:{x:A.x,y:A.y,z:A.z,w:A.w},sceneOffset:{x:j.high,y:B.high,z:I.high,xL:j.low,yL:B.low,zL:I.low},targetDistance:V,optics:W,...q?{thumbnail:q}:{}})},[i]),$=()=>{l(),S&&S({camType:0,camFov:60,orthoScale:2})},R=(D,H)=>{D.dataTransfer.effectAllowed="move",D.dataTransfer.setData("text/plain",String(H)),w({fromIndex:H,overIndex:H})},Y=(D,H)=>{D.preventDefault(),D.dataTransfer.dropEffect="move",z&&z.overIndex!==H&&w({...z,overIndex:H})},L=(D,H)=>{D.preventDefault(),z&&d(z.fromIndex,H),w(null)},G=()=>w(null);return t.jsxs("div",{className:`flex flex-col bg-[#080808] ${e}`,children:[t.jsxs("div",{className:"p-2 border-b border-white/10 bg-black/40 grid grid-cols-4 gap-1",children:[t.jsx("button",{onClick:()=>E("Front"),className:"bg-white/5 hover:bg-white/10 text-[9px] text-gray-400 rounded py-1",children:"FRONT"}),t.jsx("button",{onClick:()=>E("Back"),className:"bg-white/5 hover:bg-white/10 text-[9px] text-gray-400 rounded py-1",children:"BACK"}),t.jsx("button",{onClick:()=>E("Left"),className:"bg-white/5 hover:bg-white/10 text-[9px] text-gray-400 rounded py-1",children:"LEFT"}),t.jsx("button",{onClick:()=>E("Right"),className:"bg-white/5 hover:bg-white/10 text-[9px] text-gray-400 rounded py-1",children:"RIGHT"}),t.jsx("button",{onClick:()=>E("Top"),className:"bg-white/5 hover:bg-white/10 text-[9px] text-gray-400 rounded py-1",children:"TOP"}),t.jsx("button",{onClick:()=>E("Bottom"),className:"bg-white/5 hover:bg-white/10 text-[9px] text-gray-400 rounded py-1",children:"BTM"}),t.jsx("button",{onClick:()=>E("Isometric"),className:"bg-white/5 hover:bg-white/10 text-[9px] text-gray-400 rounded py-1",children:"ISO"}),t.jsx("button",{onClick:$,className:"bg-white/5 hover:bg-white/10 text-[9px] text-gray-400 rounded py-1",children:"RESET"}),t.jsxs("button",{onClick:F,className:"col-span-4 bg-cyan-900/40 hover:bg-cyan-800 text-cyan-300 border border-cyan-500/30 rounded py-1 text-[9px] font-bold flex items-center justify-center gap-1 mt-1",children:[t.jsx(ha,{})," New Camera"]})]}),t.jsxs("div",{className:"p-2 space-y-1",children:[o.length===0&&t.jsx("div",{className:"text-center text-gray-600 text-[10px] italic py-4",children:"No saved cameras"}),o.map((D,H)=>{const A=a===D.id,j=A&&m(D),B=z&&z.overIndex===H&&z.fromIndex!==H;return t.jsxs("div",{onDragOver:I=>Y(I,H),onDrop:I=>L(I,H),className:`flex items-center gap-1.5 p-1.5 rounded border transition-all group ${A?"bg-cyan-900/20 border-cyan-500/50":"bg-white/5 border-transparent hover:border-white/10"} ${B?"border-cyan-400/70 border-dashed":""}`,onClick:()=>s(D.id),children:[t.jsx("div",{draggable:!0,onDragStart:I=>{I.stopPropagation(),R(I,H)},onDragEnd:G,className:"cursor-grab opacity-0 group-hover:opacity-40 hover:!opacity-80 transition-opacity flex-shrink-0",title:"Drag to reorder",children:t.jsx(Ja,{})}),t.jsx("div",{className:"w-8 h-8 rounded overflow-hidden flex-shrink-0 bg-black/50 border border-white/5",children:D.thumbnail?t.jsx("img",{src:D.thumbnail,alt:"",className:"w-full h-full object-cover"}):t.jsx("div",{className:"w-full h-full flex items-center justify-center text-gray-700 text-[7px]",children:H+1})}),t.jsxs("div",{className:"flex-1 min-w-0",children:[x===D.id?t.jsx("input",{type:"text",value:M,onChange:I=>P(I.target.value),onBlur:T,onKeyDown:_,autoFocus:!0,className:"bg-black border border-white/20 text-xs text-white px-1 py-0.5 rounded w-full outline-none",onClick:I=>I.stopPropagation()}):t.jsx("span",{className:`text-xs font-bold truncate block cursor-text ${j?"text-amber-300 italic":A?"text-white":"text-gray-400 group-hover:text-gray-300"}`,onDoubleClick:I=>{I.stopPropagation(),k(D)},title:"Double-click to rename",children:j?`*${D.label}`:D.label}),H<9&&t.jsxs("span",{className:"text-[7px] text-gray-600",children:["Ctrl+",H+1]})]}),t.jsxs("div",{className:"flex items-center gap-0.5 flex-shrink-0",children:[A&&t.jsx("button",{onClick:I=>{I.stopPropagation(),O(D.id)},className:`p-1 transition-colors ${j?"text-amber-400 hover:text-amber-200":"text-gray-600 hover:text-gray-400 opacity-0 group-hover:opacity-100"}`,title:j?"Camera modified — click to save current view":"Update camera to current view",children:t.jsx(Ha,{})}),t.jsx("button",{onClick:I=>{I.stopPropagation(),c(D.id)},className:"p-1 text-gray-600 hover:text-cyan-400 opacity-0 group-hover:opacity-100 transition-opacity",title:"Duplicate camera",children:t.jsx(Js,{})}),t.jsx("button",{onClick:I=>{I.stopPropagation(),n(D.id)},className:"p-1 text-gray-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity",title:"Delete camera",children:t.jsx(wt,{})})]})]},D.id)})]}),t.jsxs("div",{className:"border-t border-white/10 bg-black/40 p-2 space-y-2",children:[t.jsxs("div",{className:"flex items-center justify-between",children:[t.jsx(_e,{children:a?"Active Settings":"Free Camera"}),a&&t.jsx("button",{onClick:()=>s(null),className:"text-[9px] text-gray-500 hover:text-white px-2 py-0.5 rounded border border-white/10 hover:bg-white/5 transition-colors",children:"Deselect"})]}),t.jsx(jt,{label:"Position",defaultOpen:!1,children:t.jsx("div",{className:"mt-1",children:t.jsx(Zr,{})})}),t.jsx("div",{className:"bg-white/5 rounded p-1",children:t.jsx(se,{featureId:"optics"})}),t.jsx("div",{className:"border-t border-white/10 pt-2",children:t.jsx(jt,{label:"Composition Guide",defaultOpen:!1,rightContent:p!=="none"?t.jsx("span",{className:"text-[8px] text-cyan-400",children:(X=ir.find(D=>D.type===p))==null?void 0:X.label}):null,children:t.jsxs("div",{className:"mt-2 space-y-2",children:[t.jsxs("div",{className:"flex items-center gap-2",children:[t.jsx("label",{className:"text-[9px] text-gray-500 w-16",children:"Type"}),t.jsx("select",{value:p,onChange:D=>h(D.target.value),className:"flex-1 t-select",children:ir.map(D=>t.jsx("option",{value:D.type,children:D.label},D.type))})]}),p!=="none"&&t.jsxs(t.Fragment,{children:[t.jsx(me,{label:"Opacity",value:u.opacity,min:.1,max:1,step:.1,onChange:D=>g({opacity:D})}),t.jsx(me,{label:"Line Width",value:u.lineThickness,min:.5,max:3,step:.5,onChange:D=>g({lineThickness:D})}),t.jsxs("div",{className:"flex items-center gap-2",children:[t.jsx("label",{className:"text-[9px] text-gray-500 w-16",children:"Color"}),t.jsx(sa,{color:u.color,onChange:D=>g({color:D})})]}),p==="grid"&&t.jsxs(t.Fragment,{children:[t.jsx(me,{label:"Divisions X",value:u.gridDivisionsX,min:2,max:16,step:1,onChange:D=>g({gridDivisionsX:D})}),t.jsx(me,{label:"Divisions Y",value:u.gridDivisionsY,min:2,max:16,step:1,onChange:D=>g({gridDivisionsY:D})})]}),p==="spiral"&&t.jsxs(t.Fragment,{children:[t.jsx(me,{label:"Rotation",value:u.spiralRotation,min:0,max:360,step:15,onChange:D=>g({spiralRotation:D})}),t.jsx(me,{label:"Position X",value:u.spiralPositionX,min:0,max:1,step:.05,onChange:D=>g({spiralPositionX:D})}),t.jsx(me,{label:"Position Y",value:u.spiralPositionY,min:0,max:1,step:.05,onChange:D=>g({spiralPositionY:D})}),t.jsx(me,{label:"Scale",value:u.spiralScale,min:.5,max:2,step:.1,onChange:D=>g({spiralScale:D})}),t.jsx(me,{label:"Ratio (Phi)",value:u.spiralRatio,min:1,max:2,step:.01,onChange:D=>g({spiralRatio:D})})]}),t.jsxs("div",{className:"flex items-center gap-3 pt-1",children:[t.jsxs("label",{className:"flex items-center gap-1 cursor-pointer",children:[t.jsx("input",{type:"checkbox",checked:u.showCenterMark,onChange:D=>g({showCenterMark:D.target.checked}),className:"w-3 h-3 accent-cyan-500 bg-gray-800 border-gray-600 rounded"}),t.jsx("span",{className:"text-[9px] text-gray-400",children:"Center"})]}),t.jsxs("label",{className:"flex items-center gap-1 cursor-pointer",children:[t.jsx("input",{type:"checkbox",checked:u.showSafeAreas,onChange:D=>g({showSafeAreas:D.target.checked}),className:"w-3 h-3 accent-cyan-500 bg-gray-800 border-gray-600 rounded"}),t.jsx("span",{className:"text-[9px] text-gray-400",children:"Safe Areas"})]})]})]})]})})})]})]})},Ld=`
  /* 
...
`),i},Qr=e=>{const o=d=>{const f=new RegExp(`<${d}>([\\s\\S]*?)<\\/${d}>`),p=e.match(f);return p?p[1].trim():null},a=o("Metadata");if(!a){try{const d=JSON.parse(e);if(d.id&&d.shader)return d}catch{}throw new Error("Invalid GMF: Missing Metadata tag")}const r=JSON.parse(a),n=o("Shader_Function"),s=o("Shader_Loop"),i=o("Shader_Init"),l=o("Shader_Dist");if(!n||!s)throw new Error("Invalid GMF: Missing essential shader blocks (<Shader_Function> or <Shader_Loop>)");return{...r,shader:{function:n,loopBody:s,loopInit:i||void 0,getDist:l||void 0}}},xa=.01,Kr=100,Jr=Math.log(Kr/xa),Ad=e=>Math.log(e/xa)/Jr,Nd=e=>xa*Math.exp(e*Jr),Bd=({value:e,onChange:o})=>{const a=ke.useRef(null),r=ke.useRef(!1),n=Ad(e)*100,s=d=>{const f=a.current;if(!f)return;const p=f.getBoundingClientRect(),h=Math.max(0,Math.min(1,(d-p.left)/p.width)),u=Nd(h),g=Math.round(u*100)/100;o(Math.max(xa,Math.min(Kr,g)))},i=d=>{d.preventDefault(),d.stopPropagation(),r.current=!0,d.target.setPointerCapture(d.pointerId),s(d.clientX)},l=d=>{r.current&&s(d.clientX)},c=()=>{r.current=!1};return t.jsxs("div",{ref:a,className:"relative h-[22px] cursor-pointer overflow-hidden",style:{backgroundImage:"repeating-linear-gradient(45deg, transparent, transparent 5px, rgba(255,255,255,0.03) 5px, rgba(255,255,255,0.03) 10px)"},onPointerDown:i,onPointerMove:l,onPointerUp:c,children:[t.jsx("div",{className:"absolute inset-0 bg-white/[0.12]"}),t.jsx("div",{className:"absolute top-0 bottom-0 left-0 bg-cyan-500/20 transition-[width] duration-75 ease-out",style:{width:`${n}%`}}),t.jsxs("div",{className:"absolute inset-0 flex items-center justify-between px-2 pointer-events-none",children:[t.jsx("span",{className:"text-[10px] text-gray-400 font-medium",children:"Amount"}),t.jsxs("span",{className:"text-[10px] text-gray-300 tabular-nums",children:[e>=10?Math.round(e):e.toFixed(2),"%"]})]})]})},Od=({onRandomizeParams:e,onRandomizeFull:o})=>{const[a,r]=ke.useState(100);return t.jsxs("div",{className:"py-0.5",children:[t.jsx("div",{className:"px-3 py-0.5",children:t.jsx(Bd,{value:a,onChange:r})}),t.jsxs("button",{onClick:()=>e(a/100),className:"w-full text-left px-4 py-2 text-xs flex items-center gap-2 text-gray-300 hover:bg-white/10 hover:text-white transition-colors",children:[t.jsx(pl,{})," Parameters"]}),t.jsxs("button",{onClick:()=>o(a/100),className:"w-full text-left px-4 py-2 text-xs flex items-center gap-2 text-gray-300 hover:bg-white/10 hover:text-white transition-colors",children:[t.jsx(hl,{})," Full (inc. Box/Julia)"]})]})};function ei(){const e=N.getState(),o=Me.get(e.formula),a=o==null?void 0:o.defaultPreset,r=i=>{const l=N.getState(),c=Me.get(l.formula);l.handleInteractionStart("param");const d={};if(l.formula==="Modular"){const p=l.coreMath,h=u=>u+(Math.random()*4-2)*i;d.paramA=h(p.paramA),d.paramB=h(p.paramB),d.paramC=h(p.paramC),d.paramD=h(p.paramD),d.paramE=h(p.paramE),d.paramF=h(p.paramF),l.setCoreMath(d),l.handleInteractionEnd();return}if(!c){l.handleInteractionEnd();return}const f=l.coreMath;c.parameters.forEach(p=>{if(!p)return;const h=p.max-p.min;if(p.type==="vec3"){const u=f[p.id]||{x:0,y:0,z:0};d[p.id]={x:Math.max(p.min,Math.min(p.max,u.x+(Math.random()*2-1)*h*i)),y:Math.max(p.min,Math.min(p.max,u.y+(Math.random()*2-1)*h*i)),z:Math.max(p.min,Math.min(p.max,u.z+(Math.random()*2-1)*h*i))}}else if(p.type==="vec2"){const u=f[p.id]||{x:0,y:0};d[p.id]={x:Math.max(p.min,Math.min(p.max,u.x+(Math.random()*2-1)*h*i)),y:Math.max(p.min,Math.min(p.max,u.y+(Math.random()*2-1)*h*i))}}else if(i>=1){const u=Math.random()*h+p.min;d[p.id]=p.step>0?Math.round(u/p.step)*p.step:u}else{const g=(f[p.id]??(p.min+p.max)/2)+(Math.random()*2-1)*h*i,S=Math.max(p.min,Math.min(p.max,g));d[p.id]=p.step>0?Math.round(S/p.step)*p.step:S}}),l.setCoreMath(d),l.handleInteractionEnd()},n=i=>{r(i);const l=N.getState(),c=l.geometry,d={};c.hybridMode&&(d.hybridScale=i>=1?1.5+Math.random()*1.5:Math.max(1,Math.min(3,c.hybridScale+(Math.random()*2-1)*2*i)),d.hybridMinR=i>=1?Math.random()*1:Math.max(0,Math.min(1.5,c.hybridMinR+(Math.random()*2-1)*1.5*i)),d.hybridFixedR=i>=1?.5+Math.random()*1.5:Math.max(.1,Math.min(3,c.hybridFixedR+(Math.random()*2-1)*2.9*i)),d.hybridFoldLimit=i>=1?.5+Math.random()*1.5:Math.max(.1,Math.min(2,c.hybridFoldLimit+(Math.random()*2-1)*1.9*i))),c.juliaMode&&(d.juliaX=i>=1?Math.random()*4-2:Math.max(-2,Math.min(2,c.juliaX+(Math.random()*2-1)*4*i)),d.juliaY=i>=1?Math.random()*4-2:Math.max(-2,Math.min(2,c.juliaY+(Math.random()*2-1)*4*i)),d.juliaZ=i>=1?Math.random()*4-2:Math.max(-2,Math.min(2,c.juliaZ+(Math.random()*2-1)*4*i))),c.preRotEnabled&&(d.preRotX=i>=1?(Math.random()*2-1)*Math.PI:Math.max(-Math.PI,Math.min(Math.PI,c.preRotX+(Math.random()*2-1)*Math.PI*2*i)),d.preRotY=i>=1?(Math.random()*2-1)*Math.PI:Math.max(-Math.PI,Math.min(Math.PI,c.preRotY+(Math.random()*2-1)*Math.PI*2*i)),d.preRotZ=i>=1?(Math.random()*2-1)*Math.PI:Math.max(-Math.PI,Math.min(Math.PI,c.preRotZ+(Math.random()*2-1)*Math.PI*2*i))),Object.keys(d).length>0&&l.setGeometry(d)};return[{label:"Import Options",action:()=>{},isHeader:!0},{label:"Lock Scene Settings",checked:e.lockSceneOnSwitch,action:()=>e.setLockSceneOnSwitch(!e.lockSceneOnSwitch)},{label:"Randomize",action:()=>{},isHeader:!0},{element:ke.createElement(Od,{onRandomizeParams:r,onRandomizeFull:n}),keepOpen:!0,action:()=>{}},{label:"Formula Parameters",action:()=>{},isHeader:!0},{label:"Reset to Default",action:()=>{var l,c,d,f;const i=e.getPreset();e.handleInteractionStart("param"),e.loadPreset({...i,paramA:0,paramB:0,paramC:0,paramD:0,paramE:0,paramF:0,vec2A:[0,0],vec2B:[0,0],vec2C:[0,0],vec3A:[0,0,0],vec3B:[0,0,0],vec3C:[0,0,0],features:{...i.features,coreMath:((l=a==null?void 0:a.features)==null?void 0:l.coreMath)||((c=i.features)==null?void 0:c.coreMath),geometry:((d=a==null?void 0:a.features)==null?void 0:d.geometry)||((f=i.features)==null?void 0:f.geometry)}}),e.handleInteractionEnd()}},{label:"Reset to Formula Preset",action:()=>{var l,c,d,f,p,h,u,g,S;if(!a)return;const i=e.getPreset();e.handleInteractionStart("param"),e.loadPreset({...a,cameraPos:i.cameraPos,cameraRot:i.cameraRot,sceneOffset:i.sceneOffset,targetDistance:i.targetDistance,cameraMode:i.cameraMode,lights:i.lights,features:{...a.features||{},atmosphere:(l=i.features)==null?void 0:l.atmosphere,lighting:(c=i.features)==null?void 0:c.lighting,optics:(d=i.features)==null?void 0:d.optics,materials:(f=i.features)==null?void 0:f.materials,coreMath:(p=a.features)==null?void 0:p.coreMath,geometry:(h=a.features)==null?void 0:h.geometry,coloring:(u=a.features)==null?void 0:u.coloring,texturing:(g=a.features)==null?void 0:g.texturing,quality:(S=a.features)==null?void 0:S.quality}}),e.handleInteractionEnd()},disabled:!a},{label:"Scene Parameters",action:()=>{},isHeader:!0},{label:"Reset to Default",action:()=>{var c,d,f,p,h;const i=e.getPreset();e.handleInteractionStart("camera"),e.resetCamera();const l=(c=Me.get("Mandelbulb"))==null?void 0:c.defaultPreset;l&&e.loadPreset({...i,cameraPos:l.cameraPos,cameraRot:l.cameraRot,sceneOffset:l.sceneOffset,targetDistance:l.targetDistance,features:{...i.features,atmosphere:(d=l.features)==null?void 0:d.atmosphere,lighting:(f=l.features)==null?void 0:f.lighting,optics:(p=l.features)==null?void 0:p.optics,materials:(h=l.features)==null?void 0:h.materials}}),e.handleInteractionEnd()}},{label:"Reset to Formula Preset",action:()=>{var l,c,d,f;if(!a)return;const i=e.getPreset();e.handleInteractionStart("camera"),e.loadPreset({...i,cameraPos:a.cameraPos,cameraRot:a.cameraRot,sceneOffset:a.sceneOffset,targetDistance:a.targetDistance,cameraMode:a.cameraMode,lights:a.lights,features:{...i.features,atmosphere:(l=a.features)==null?void 0:l.atmosphere,lighting:(c=a.features)==null?void 0:c.lighting,optics:(d=a.features)==null?void 0:d.optics,materials:(f=a.features)==null?void 0:f.materials}}),e.handleInteractionEnd()},disabled:!a}]}const ti={id:"Mandelbulb",name:"Mandelbulb",shortDescription:"The classic 3D extension of the Mandelbrot set. Features organic, broccoli-like recursive structures.",description:'The classic 3D extension of the Mandelbrot set. Features standard Power controls plus the "Radiolaria" mutation for skeletal/hollow effects.',shader:{function:`
    ...
    void formula_Claude(inout vec4 z, inout float dr, inout float trap, vec4 c) {
        ...
        z.xyz = p;
        trap = min(trap, length(p));
    }`,loopBody:"formula_Claude(z, dr, trap, c);",loopInit:"Claude_precalc();"},parameters:[{label:"Scale",id:"paramA",min:.5,max:3.5,step:.001,default:2},{label:"Harmonic",id:"paramB",min:-3.14,max:3.14,step:.001,default:.61},{label:"Inner R²",id:"paramC",min:.001,max:1.5,step:.001,default:.25},{label:"Fix R²",id:"paramD",min:.1,max:2.5,step:.001,default:1},{label:"Offset",id:"vec3A",type:"vec3",min:-3,max:3,step:.001,default:{x:1,y:1,z:1}},{label:"Rotation",id:"vec3B",type:"vec3",min:-6.28,max:6.28,step:.01,default:{x:0,y:0,z:0},mode:"rotation"},{label:"Twist",id:"paramF",min:-2,max:2,step:.01,default:0}],defaultPreset:{formula:"Claude",features:{coreMath:{iterations:12,paramA:2,paramB:.61,paramC:.25,paramD:1,paramF:0,vec3A:{x:1,y:1,z:1},vec3B:{x:0,y:0,z:0}},coloring:{mode:0,repeats:1,phase:.12,scale:7.8,offset:1.4,bias:1.3,twist:0,escape:2.5,mode2:4,repeats2:2,phase2:.1,blendMode:2,blendOpacity:.25,twist2:0,layer3Color:"#ffffff",layer3Scale:60,layer3Strength:0,layer3Bump:0,layer3Turbulence:0,layer3Enabled:!0,gradient:[{id:"cl_0",position:0,color:"#1B0A0F",bias:.5,interpolation:"linear"},{id:"cl_1",position:.13,color:"#4A1A0A",bias:.5,interpolation:"linear"},{id:"cl_2",position:.28,color:"#C4603A",bias:.5,interpolation:"linear"},{id:"cl_3",position:.42,color:"#E8A44A",bias:.5,interpolation:"linear"},{id:"cl_4",position:.56,color:"#F5E6D3",bias:.5,interpolation:"linear"},{id:"cl_5",position:.7,color:"#4A8B7A",bias:.5,interpolation:"linear"},{id:"cl_6",position:.85,color:"#2B3A5A",bias:.5,interpolation:"linear"},{id:"cl_7",position:1,color:"#0F0A1B",bias:.5,interpolation:"linear"}],gradient2:[{id:"cl2_0",position:0,color:"#F5E6D3",bias:.5,interpolation:"linear"},{id:"cl2_1",position:.5,color:"#C4603A",bias:.5,interpolation:"linear"},{id:"cl2_2",position:1,color:"#1B0A0F",bias:.5,interpolation:"linear"}]},ao:{aoIntensity:.38,aoSpread:.12,aoSamples:5,aoEnabled:!0,aoMode:!1},atmosphere:{fogNear:0,fogFar:12,fogColor:"#000000",fogDensity:0,glowIntensity:.004,glowSharpness:4.5,glowColor:"#E8A44A",glowMode:!1},materials:{diffuse:1.15,reflection:.12,specular:1.1,roughness:.32,rim:.18,rimExponent:4.5,envStrength:.45,envBackgroundStrength:.22,envSource:1,useEnvMap:!1,envRotation:0,emission:0,emissionMode:0,emissionColor:"#ffffff",envGradientStops:[{id:"env_0",position:0,color:"#0A0E1B",bias:.5,interpolation:"linear"},{id:"env_1",position:.3,color:"#3A2A1B",bias:.5,interpolation:"linear"},{id:"env_2",position:.5,color:"#C4A87A",bias:.5,interpolation:"linear"},{id:"env_3",position:.72,color:"#7AACCC",bias:.5,interpolation:"linear"},{id:"env_4",position:1,color:"#B4D4E8",bias:.5,interpolation:"linear"}]},geometry:{juliaMode:!1,juliaX:0,juliaY:0,juliaZ:0,hybridMode:!1},lighting:{advancedLighting:!0,ptEnabled:!0,shadows:!0,shadowSoftness:28,shadowIntensity:1,shadowBias:.001},quality:{detail:3,fudgeFactor:.7,pixelThreshold:.3,maxSteps:400,distanceMetric:1,estimator:0},optics:{camFov:40,dofStrength:0,dofFocus:2.5}},cameraPos:{x:0,y:0,z:0},cameraRot:{x:-.18,y:-.42,z:-.08,w:.89},cameraFov:40,sceneOffset:{x:1,y:1,z:1,xL:0,yL:0,zL:0},targetDistance:3.8,cameraMode:"Orbit",lights:[{type:"Point",position:{x:-.8,y:3.2,z:3.5},rotation:{x:0,y:0,z:0},color:"#FFE4CC",intensity:6,falloff:0,falloffType:"Quadratic",fixed:!1,visible:!0,castShadow:!0},{type:"Point",position:{x:3.5,y:-.5,z:1.5},rotation:{x:0,y:0,z:0},color:"#4A8BCC",intensity:2.5,falloff:0,falloffType:"Quadratic",fixed:!1,visible:!0,castShadow:!1},{type:"Point",position:{x:1,y:1,z:-2},rotation:{x:0,y:0,z:0},color:"#C4603A",intensity:1.5,falloff:0,falloffType:"Quadratic",fixed:!1,visible:!1,castShadow:!1}]}},$d=[ti,zi,di,ui,ji,ki,Ci,ri,pi,ii,Si,Ti,ni,fi,wi,Ri,ai,oi,si,li,Mi,ci,bi,hi,mi,gi,xi,yi,Pi,vi];$d.forEach(e=>Me.register(e));Me.registerAlias("UberMenger","MengerAdvanced");Me.registerAlias("FoldingBrot","BoxBulb");Me.registerAlias("HyperTorus","Mandelorus");Me.registerAlias("HyperbolicMandelbrot","MandelBolic");const Hd=[{name:"Featured Fractals",match:[ti.id,zi.id,di.id,ri.id,pi.id,ii.id,ni.id,fi.id,wi.id,Ri.id]},{name:"Geometric & Folding",match:[Si.id,Ti.id,ui.id,si.id,li.id,mi.id,hi.id]},{name:"Hybrids & Experiments",match:[ji.id,ki.id,Ci.id,Mi.id,ci.id,bi.id,gi.id,xi.id,yi.id,ai.id,oi.id,Pi.id]},{name:"Systems",match:[vi.id]}],Gd=ke.memo(({id:e,label:o})=>{const[a,r]=v.useState(!1),[n,s]=v.useState(!1),i=v.useRef(null);return v.useEffect(()=>{const l=i.current;if(!l)return;const c=new IntersectionObserver(d=>{d[0].isIntersecting&&(r(!0),c.disconnect())},{rootMargin:"50px"});return c.observe(l),()=>c.disconnect()},[]),n?null:t.jsx("div",{ref:i,className:"w-full h-full",children:a&&t.jsx("img",{src:`thumbnails/fractal_${e}.jpg`,alt:o,className:"w-full h-full object-cover",onError:()=>s(!0),loading:"lazy"})})}),Vd=({rect:e,onClose:o,onSelect:a,currentValue:r,onImport:n,showImport:s,onImportFragmentarium:i})=>{var k;const[l,c]=v.useState(null),[d,f]=v.useState({opacity:0,pointerEvents:"none"}),[p,h]=v.useState({}),[u,g]=v.useState(!1),[S,y]=v.useState([]),[b,m]=v.useState(!1),[x,C]=v.useState(!1),[z,w]=v.useState(new Set);v.useEffect(()=>{(async()=>{m(!0);try{const _=await fetch("/gmf/gallery.json");if(_.ok){const E=await _.json();y(E.categories||[])}}catch(_){console.warn("Failed to load gallery:",_)}finally{m(!1)}})()},[]);const M=async T=>{try{const _=await fetch(T.path);if(_.ok){const E=await _.text();Z.emit(we.IS_COMPILING,"Compiling Formula...");const F=Qr(E);Me.register(F),Z.emit(we.REGISTER_FORMULA,{id:F.id,shader:F.shader}),a(F.id),o()}else console.error("Failed to load formula from gallery:",T.path),alert(`Failed to load formula: ${T.name}`)}catch(_){console.error("Error loading gallery formula:",_),Z.emit(we.IS_COMPILING,!1),alert(`Error loading formula: ${T.name}`)}},P=v.useMemo(()=>{const T=Me.getAll(),_=new Set(T.map(F=>F.id)),E=[];for(const F of Hd){const O=F.match.filter($=>_.has($)?(_.delete($),!0):!1);O.length>0&&E.push({name:F.name,items:O})}return _.size>0&&E.push({name:"Custom / Imported",items:Array.from(_)}),E},[]);return v.useLayoutEffect(()=>{const T=window.innerHeight,_=window.innerWidth,E=12,F=340,O=_<768;g(O);let $=e.left;$+F>_-E&&($=_-F-E),$=Math.max(E,$);const R=T-e.bottom,Y=e.top,L=R<300&&Y>R;let G=L?Y-E:R-E;const X=Math.min(600,Math.max(150,G)),D={position:"fixed",left:`${$}px`,width:`${F}px`,maxHeight:`${X}px`,maxWidth:`calc(100vw - ${E*2}px)`,zIndex:9999,display:"flex",flexDirection:"column",opacity:1,pointerEvents:"auto"},H=L?{bottom:`${T-e.top+4}px`,top:"auto",transformOrigin:"bottom left"}:{top:`${e.bottom+4}px`,bottom:"auto",transformOrigin:"top left"};f({...D,...H}),O||(_-($+F)>260+20?h({left:"100%",marginLeft:"10px",top:L?"auto":0,bottom:L?0:"auto"}):h({right:"100%",marginRight:"10px",top:L?"auto":0,bottom:L?0:"auto"}))},[e]),v.useEffect(()=>{const T=()=>o(),_=F=>{F.target.closest(".portal-dropdown-content")||o()},E=F=>{F.target.closest(".portal-dropdown-content")||o()};return window.addEventListener("resize",T),window.addEventListener("mousedown",_,!0),window.addEventListener("wheel",E,!0),()=>{window.removeEventListener("resize",T),window.removeEventListener("mousedown",_,!0),window.removeEventListener("wheel",E,!0)}},[o]),Pt.createPortal(t.jsxs("div",{style:d,children:[t.jsxs("div",{className:"portal-dropdown-content bg-[#121212] border border-white/10 rounded-lg shadow-[0_10px_40px_rgba(0,0,0,0.8)] overflow-y-auto custom-scroll animate-fade-in-down w-full flex-1",onMouseLeave:()=>c(null),children:[s&&t.jsxs("div",{className:"p-1 border-b border-white/5 sticky top-0 bg-[#121212] z-50 space-y-1",children:[t.jsxs("button",{onClick:()=>{n(),o()},className:"w-full flex items-center justify-center gap-2 px-3 py-2 bg-cyan-900/20 hover:bg-cyan-900/40 text-cyan-400 text-[10px] font-bold rounded border border-cyan-500/20 hover:border-cyan-500/50 transition-colors",children:[t.jsx(Tr,{}),"Import Formula (.GMF)"]}),t.jsxs("button",{onClick:()=>{i(),o()},className:"w-full flex items-center justify-center gap-2 px-3 py-2 bg-purple-900/20 hover:bg-purple-900/40 text-purple-400 text-[10px] font-bold rounded border border-purple-500/20 hover:border-purple-500/50 transition-colors",children:[t.jsx(ia,{}),"Formula Workshop"]})]}),P.map(T=>t.jsxs("div",{className:"py-1",children:[t.jsx("div",{className:`px-3 py-1.5 text-[9px] font-bold text-gray-500 bg-[#121212] border-y border-white/5 sticky z-40 shadow-sm ${s?"top-[38px]":"top-0"}`,children:T.name}),T.items.map(_=>{const E=_==="Modular",F=Me.get(_),O=F?F.name:_,$=r===_;return t.jsxs("button",{onClick:()=>a(_),onMouseEnter:()=>c(_),className:`w-full text-left px-3 py-2.5 transition-all flex gap-3 group relative border-b border-white/5 last:border-b-0 ${$?"bg-cyan-900/20":"hover:bg-white/5"}`,children:[t.jsxs("div",{className:"w-16 h-10 shrink-0 bg-black rounded border border-white/10 overflow-hidden relative group-hover:border-cyan-500/50 transition-colors",children:[t.jsx("div",{className:"absolute inset-0 flex items-center justify-center text-gray-800 bg-gray-900 z-0",children:E?t.jsx(_r,{}):t.jsx(ma,{})}),!E&&t.jsx("div",{className:"relative z-10 w-full h-full",children:t.jsx(Gd,{id:_,label:O})}),$&&t.jsx("div",{className:"absolute inset-0 z-20 bg-cyan-500/20 flex items-center justify-center",children:t.jsx("div",{className:"w-4 h-4 bg-white rounded-full flex items-center justify-center text-cyan-900 shadow-lg",children:t.jsx(ft,{})})})]}),t.jsxs("div",{className:"flex flex-col min-w-0 flex-1 justify-center",children:[t.jsx("div",{className:"flex items-center gap-2 mb-0.5",children:t.jsx("span",{className:`text-[11px] font-bold tracking-tight truncate ${$?"text-cyan-400":"text-gray-200 group-hover:text-white"} ${E?"text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-purple-300 font-bold":""}`,children:O})}),(F==null?void 0:F.shortDescription)&&t.jsx("p",{className:"text-[9px] text-gray-500 line-clamp-2 leading-tight group-hover:text-gray-400",children:F.shortDescription})]})]},_)})]},T.name)),S.length>0&&t.jsxs("div",{className:"py-1 border-t border-white/10",children:[t.jsx("div",{className:"px-3 py-1.5 text-[9px] font-bold text-gray-500 bg-[#121212] sticky z-40 shadow-sm top-[38px]",children:"Add from Gallery"}),S.map(T=>t.jsxs("div",{className:"border-b border-white/5",children:[t.jsxs("button",{onClick:()=>{w(_=>{const E=new Set(_);return E.has(T.id)?E.delete(T.id):E.add(T.id),E})},className:"w-full text-left px-3 py-2 flex items-center gap-2 group hover:bg-white/5 transition-colors",children:[t.jsx("span",{className:`w-3 h-3 text-gray-500 transition-transform ${z.has(T.id)?"rotate-180":""}`,children:t.jsx(Tt,{})}),t.jsx("span",{className:"text-[11px] font-bold text-purple-400 group-hover:text-purple-300",children:T.name}),t.jsxs("span",{className:"text-[9px] text-gray-600",children:["(",T.items.length," formulas)"]})]}),z.has(T.id)&&t.jsx("div",{className:"bg-black/30",children:T.items.map(_=>t.jsxs("button",{onClick:()=>M(_),onMouseEnter:()=>c(_.id),className:"w-full text-left px-6 py-2 transition-all flex gap-3 group hover:bg-white/5",children:[t.jsx("div",{className:"w-16 h-8 shrink-0 bg-black rounded border border-white/10 overflow-hidden relative group-hover:border-purple-500/50 transition-colors",children:t.jsx("div",{className:"absolute inset-0 flex items-center justify-center text-gray-800 bg-gray-900 z-0",children:t.jsx(Ga,{})})}),t.jsx("div",{className:"flex flex-col min-w-0 flex-1 justify-center",children:t.jsx("span",{className:"text-[11px] font-bold tracking-tight truncate text-gray-200 group-hover:text-white",children:_.name})})]},_.id))})]},T.id))]}),b&&t.jsx("div",{className:"py-2 text-center text-[10px] text-gray-500",children:"Loading gallery..."})]}),l&&l!=="Modular"&&!u&&t.jsxs("div",{className:"absolute w-[256px] h-[256px] bg-black border border-cyan-500/50 rounded-lg shadow-[0_0_50px_rgba(0,0,0,1),0_0_20px_rgba(34,211,238,0.2)] overflow-hidden animate-fade-in pointer-events-none z-[10000]",style:p,children:[t.jsx("img",{src:`thumbnails/fractal_${l}.jpg`,className:"w-full h-full object-cover",alt:"Preview",onError:T=>T.currentTarget.parentElement.style.display="none"}),t.jsx("div",{className:"absolute inset-0 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.1)_50%),linear-gradient(90deg,rgba(255,0,0,0.03),rgba(0,255,0,0.01),rgba(0,0,255,0.03))] bg-[length:100%_4px,4px_100%] pointer-events-none"}),t.jsx("div",{className:"absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black to-transparent p-4",children:t.jsx("div",{className:"text-[10px] font-bold text-cyan-400 drop-shadow-md",children:((k=Me.get(l))==null?void 0:k.name)||l})})]})]}),document.body)},Ud=({value:e,onChange:o})=>{var w;const[a,r]=v.useState(!1),n=N(M=>M.openWorkshop),s=v.useRef(null),i=v.useRef(null),[l,c]=v.useState(null),d=N(M=>M.openContextMenu),f=N(M=>M.setExportIncludeScene),p=N(M=>M.exportIncludeScene),h=N(M=>M.advancedMode),u=M=>{M.preventDefault(),M.stopPropagation();const P=ei();d(M.clientX,M.clientY,P,[])},g=()=>{!a&&s.current?(c(s.current.getBoundingClientRect()),r(!0)):r(!1)},S=M=>{const P=Me.get(e);if(!P)return;const k=N.getState().getPreset({includeScene:M}),T=Ed(P,k),_=new Blob([T],{type:"text/plain"}),E=URL.createObjectURL(_),F=document.createElement("a");F.href=E,F.download=`${P.id}${M?"_Full":""}.gmf`,F.click(),URL.revokeObjectURL(E)},y=M=>{M.stopPropagation(),S(p)},b=M=>{M.preventDefault(),M.stopPropagation();const P=[{label:"Export Options",action:()=>{},isHeader:!0},{label:"Include Scene Data",checked:p,action:()=>f(!p)},{label:"Actions",action:()=>{},isHeader:!0},{label:"Export Formula Only",action:()=>S(!1)},{label:"Export Full Package",action:()=>S(!0)}];d(M.clientX,M.clientY,P,[])},m=M=>{var T;const P=(T=M.target.files)==null?void 0:T[0];if(!P)return;const k=new FileReader;k.onload=_=>{var E;try{const F=(E=_.target)==null?void 0:E.result;Z.emit(we.IS_COMPILING,"Compiling Formula...");const O=Qr(F);Me.register(O),Z.emit(we.REGISTER_FORMULA,{id:O.id,shader:O.shader}),o(O.id),i.current&&(i.current.value="")}catch(F){console.error("Failed to import formula:",F),Z.emit(we.IS_COMPILING,!1),alert("Invalid formula file. Ensure it is a valid .gmf or .json definition.")}},k.readAsText(P)},x=Me.get(e),C=x?x.name:e,z=e==="Modular";return t.jsxs("div",{className:"flex gap-2",children:[t.jsx("input",{ref:i,type:"file",accept:".json,.gmf",className:"hidden",onChange:m}),t.jsxs("button",{ref:s,onClick:g,onContextMenu:u,className:`flex-1 flex items-center justify-between border text-xs text-white rounded-lg p-2.5 outline-none transition-all group ${a?"bg-gray-900 border-cyan-500 ring-1 ring-cyan-900":z?"bg-gray-900 border-purple-500/50 shadow-[inset_0_0_10px_rgba(168,85,247,0.1)]":"bg-gradient-to-t from-white/[0.06] to-white/[0.03] border-white/10 hover:border-white/20 shadow-[inset_0_-1px_0_rgba(255,255,255,0.04)]"}`,children:[t.jsxs("div",{className:"flex items-center gap-2",children:[z&&t.jsx("span",{className:"flex w-1.5 h-1.5 rounded-full bg-purple-500 animate-pulse shadow-[0_0_5px_#a855f7]"}),t.jsx("span",{className:`font-bold ${z?"text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-purple-300":""}`,children:C})]}),t.jsx("div",{className:`w-3 h-3 text-gray-500 transition-transform ${a?"rotate-180":""}`,children:t.jsx(Tt,{})})]}),!z&&h&&t.jsx("button",{onClick:y,onContextMenu:b,className:"w-8 flex items-center justify-center bg-white/[0.04] border border-white/10 hover:border-white/20 hover:bg-white/[0.08] text-gray-400 hover:text-white rounded-lg transition-colors",title:p?"Export Full Preset (Right-click for options)":"Export Formula Only (Right-click for options)",children:t.jsx(Rr,{})}),((w=Me.get(e))==null?void 0:w.importSource)&&t.jsx("button",{onClick:()=>n(e),className:"w-8 flex items-center justify-center bg-white/[0.04] border border-white/10 hover:border-white/20 hover:bg-white/[0.08] text-gray-400 hover:text-white rounded-lg transition-colors",title:"Re-edit imported formula in Workshop",children:t.jsx(ia,{})}),a&&l&&t.jsx(Vd,{rect:l,currentValue:e,onClose:()=>r(!1),onSelect:M=>{o(M),r(!1)},onImport:()=>{var M;return(M=i.current)==null?void 0:M.click()},showImport:h,onImportFragmentarium:()=>n(void 0)})]})},qd=({shape:e,period:o,phase:a,amplitude:r,enabled:n})=>{const s=v.useRef(null);return v.useEffect(()=>{const i=s.current;if(!i)return;const l=i.getContext("2d");if(!l)return;const c=i.width,d=i.height;if(l.clearRect(0,0,c,d),l.strokeStyle="#222",l.lineWidth=1,l.beginPath(),l.moveTo(0,d/2),l.lineTo(c,d/2),l.stroke(),!n)return;l.strokeStyle="#8b5cf6",l.lineWidth=2,l.beginPath();const f=120,p=5;for(let h=0;h<=f;h++){const u=h/f,g=(u*p/o+a)%1;let S=0;e==="Sine"?S=Math.sin(g*Math.PI*2):e==="Triangle"?S=1-Math.abs(g*2-1)*2:e==="Sawtooth"?S=g*2-1:e==="Pulse"?S=g<.5?1:-1:e==="Noise"&&(S=Math.sin(g*50)*Math.cos(g*12));const y=d/2-S*Math.min(1.5,r)*(d/4);h===0?l.moveTo(u*c,y):l.lineTo(u*c,y)}l.stroke()},[e,o,a,r,n]),t.jsxs("div",{className:"relative h-12 bg-black/40 rounded border border-white/5 mb-3 overflow-hidden",children:[t.jsx("canvas",{ref:s,width:280,height:48,className:"w-full h-full"}),t.jsx("div",{className:"absolute top-1 left-2 text-[7px] font-bold text-purple-400/50 pointer-events-none",children:"Signal (5 second window)"})]})},Wd=({state:e,actions:o})=>{const a=()=>{if(e.animations.length>=3)return;const i={id:Ze(),enabled:!0,target:"coreMath.paramA",shape:"Sine",period:5,amplitude:1,baseValue:e.coreMath.paramA,phase:0,smoothing:.5};o.setAnimations([...e.animations,i])},r=i=>{o.setAnimations(e.animations.filter(l=>l.id!==i))},n=(i,l)=>{o.setAnimations(e.animations.map(c=>c.id===i?{...c,...l}:c))},s=e.animations.some(i=>i.enabled);return t.jsxs("div",{className:`flex flex-col border-t border-white/5 ${s?"bg-purple-900/10":"bg-white/[0.02]"}`,"data-help-id":"lfo.system",children:[t.jsxs("div",{className:`flex items-center justify-between px-3 py-2 border-b ${s?"border-purple-500/20":"border-white/5"}`,children:[t.jsx("label",{className:`text-[10px] font-bold ${s?"text-purple-300":"text-gray-500"}`,children:"LFO Modulators"}),t.jsx("button",{onClick:a,disabled:e.animations.length>=3,className:`w-5 h-5 flex items-center justify-center rounded border disabled:opacity-30 transition-all ${s?"bg-purple-500/20 border-purple-500/50 text-purple-300 hover:bg-purple-500 hover:text-white":"bg-white/10 border-white/10 text-gray-400 hover:bg-white/20 hover:text-white"}`,title:"Add LFO (Max 3)",children:t.jsx(ha,{})})]}),t.jsx("div",{className:"space-y-1 p-2",children:e.animations.map((i,l)=>t.jsxs("div",{className:`bg-black/40 rounded border border-purple-500/10 animate-fade-in relative transition-all ${i.enabled,"p-2"}`,children:[t.jsxs("div",{className:"flex items-center justify-between mb-2 min-h-[26px]",children:[t.jsxs("span",{className:"text-[9px] font-bold text-purple-400/50",children:["LFO ",l+1]}),t.jsxs("div",{className:"flex items-center gap-3",children:[t.jsx("button",{onClick:()=>r(i.id),className:"text-red-500 hover:text-white transition-colors opacity-50 hover:opacity-100",title:"Delete LFO",children:t.jsx(wt,{})}),t.jsx("div",{className:"w-[60px]",children:t.jsx(Ae,{value:i.enabled,onChange:c=>n(i.id,{enabled:c}),color:"bg-purple-600"})})]})]}),i.enabled&&t.jsxs("div",{className:"animate-fade-in",children:[t.jsx(qd,{...i}),t.jsxs("div",{className:"grid grid-cols-2 gap-1 mb-1",children:[t.jsxs("div",{children:[t.jsx("label",{className:"text-[9px] text-gray-500 font-bold block mb-0.5",children:"Target"}),t.jsx(qr,{value:i.target,onChange:c=>{let d=0;if(c.includes(".")){const[f,p]=c.split("."),h=e[f],u=p.match(/^(vec[23][ABC])_(x|y|z)$/);if(u&&h){const g=u[1],S=u[2],y=h[g];y&&typeof y=="object"&&(d=y[S]||0)}else h&&h[p]!==void 0&&(d=h[p])}n(i.id,{target:c,baseValue:d})},className:"w-full"})]}),t.jsxs("div",{children:[t.jsx("label",{className:"text-[9px] text-gray-500 font-bold block mb-0.5",children:"Shape"}),t.jsxs("select",{value:i.shape,onChange:c=>n(i.id,{shape:c.target.value}),className:"t-select text-white focus:border-purple-500",children:[t.jsx("option",{value:"Sine",children:"Sine"}),t.jsx("option",{value:"Triangle",children:"Triangle"}),t.jsx("option",{value:"Sawtooth",children:"Sawtooth"}),t.jsx("option",{value:"Pulse",children:"Pulse"}),t.jsx("option",{value:"Noise",children:"Noise"})]})]})]}),t.jsxs("div",{className:"space-y-0",children:[t.jsx(me,{label:"Period (Sec)",value:i.period,min:.1,max:30,step:.1,hardMin:.01,onChange:c=>n(i.id,{period:c})}),t.jsx(me,{label:"Strength",value:i.amplitude,min:.001,max:10,step:.001,onChange:c=>n(i.id,{amplitude:c}),customMapping:{min:0,max:100,toSlider:c=>(Math.log10(Math.max(.001,c))+3)/4*100,fromSlider:c=>Math.pow(10,c/100*4-3)},overrideInputText:i.amplitude<.1?i.amplitude.toFixed(3):i.amplitude.toFixed(2)}),e.advancedMode&&t.jsx(me,{label:"Phase Offset",value:i.phase,min:0,max:1,step:.01,onChange:c=>n(i.id,{phase:c}),customMapping:{min:0,max:360,toSlider:c=>c*360,fromSlider:c=>c/360},mapTextInput:!0,overrideInputText:`${(i.phase*360).toFixed(0)}°`}),t.jsx(me,{label:"Smoothing",value:i.smoothing,min:0,max:1,step:.01,onChange:c=>n(i.id,{smoothing:c})})]})]})]},i.id))})]})},Ii=({label:e,featureId:o,toggleParam:a,children:r,description:n,statusContent:s,headerClassName:i="",enabled:l,onToggle:c})=>{var y;const d=N(),f=ne.get(o),p=a||((y=f==null?void 0:f.engineConfig)==null?void 0:y.toggleParam),h=d[o],u=p?!!(h!=null&&h[p]):!0,g=l!==void 0?l:u,S=b=>{var C;if(c){c(b);return}const m=`set${o.charAt(0).toUpperCase()+o.slice(1)}`,x=d[m];x&&p&&(((C=f==null?void 0:f.engineConfig)==null?void 0:C.mode)==="compile"?(Z.emit("is_compiling","Updating Engine..."),setTimeout(()=>{x({[p]:b})},50)):x({[p]:b}))};return t.jsxs("div",{className:"flex flex-col border-t border-white/5",children:[t.jsxs("div",{className:`flex items-center justify-between px-3 py-1 ${g?"bg-neutral-800":"bg-neutral-800/50 cursor-pointer hover:bg-white/5"} ${i}`,onClick:g?void 0:()=>S(!0),children:[t.jsxs("div",{className:"flex items-center gap-1.5",children:[t.jsx("span",{className:`text-[10px] font-bold ${g?"text-gray-300":"text-gray-600"}`,children:e}),!g&&t.jsx("span",{className:"text-[8px] text-gray-600",children:"off"}),s]}),t.jsx("div",{className:"w-10",onClick:b=>b.stopPropagation(),children:t.jsx(Ae,{value:g,onChange:S})})]}),g&&t.jsxs("div",{children:[n&&t.jsx("p",{className:"px-3 py-1.5 text-[9px] text-gray-600 leading-tight bg-white/[0.06] hover:text-gray-300 transition-colors cursor-default",children:n}),r]})]})},sr=xe(),Xd=({state:e})=>{const[o,a]=v.useState({}),r=e.geometry,n=r.hybridMode,s=r.hybridCompiled,i=v.useMemo(()=>({...r,...o,hybridCompiled:!0,hybridMode:!0}),[r,o]),l=Object.keys(o).length>0,c=n&&(!s||l),d=v.useCallback(u=>{N.getState().setGeometry({hybridMode:u})},[]),f=v.useCallback((u,g)=>{a(S=>{const y={...S,[u]:g};return r[u]===g&&delete y[u],y})},[r]),p=v.useCallback(()=>{Z.emit("is_compiling","Compiling Hybrid Shader..."),setTimeout(()=>{const u={...o};s||(u.hybridCompiled=!0),N.getState().setGeometry(u),a({})},50)},[o,s]),h=t.jsxs(t.Fragment,{children:[n&&s&&!l&&t.jsx(la,{status:"active"}),n&&c&&t.jsx(la,{status:"pending"})]});return t.jsx("div",{"data-help-id":"hybrid.mode",children:t.jsx(Ii,{label:"Hybrid Box Fold",featureId:"geometry",toggleParam:"hybridMode",enabled:n,onToggle:d,statusContent:h,headerClassName:s?"":"bg-transparent",children:t.jsxs("div",{className:"bg-white/[0.02]",children:[t.jsx(jt,{label:"Compile Settings",defaultOpen:!1,variant:"panel",children:t.jsxs("div",{children:[t.jsx(se,{featureId:"geometry",whitelistParams:["hybridFoldType","hybridComplex","hybridSwap","hybridPermute"],forcedState:i,onChangeOverride:f}),c&&t.jsxs("div",{className:"flex items-center justify-between px-2 py-1 mt-1 bg-amber-900/20 border border-amber-500/20 rounded",children:[t.jsxs("div",{className:"flex items-center gap-1.5 text-amber-400",children:[t.jsx(Rt,{}),t.jsx(_e,{variant:"secondary",color:"text-amber-400",children:s?"Settings changed":"Not compiled"})]}),t.jsx("button",{onClick:p,className:"px-3 py-0.5 bg-amber-600 hover:bg-amber-500 text-black text-[9px] font-bold rounded transition-colors",children:s?"Recompile":"Compile"})]})]})}),s&&t.jsx(jt,{label:"Parameters",defaultOpen:!0,variant:"panel",children:t.jsx("div",{children:t.jsx(se,{featureId:"geometry",groupFilter:"hybrid",excludeParams:["hybridMode"]})})})]})})})},Yd=({state:e,actions:o,onSwitchTab:a})=>{var h;const r=N(u=>u.openContextMenu),[n,s]=v.useState(null);v.useEffect(()=>{const u=Z.on("compile_time",g=>{s(`Loaded in ${g.toFixed(2)}s`),setTimeout(()=>s(null),5e3)});return sr.lastCompileDuration>0&&(s(`Loaded in ${sr.lastCompileDuration.toFixed(2)}s`),setTimeout(()=>s(null),3e3)),u},[]),e.debugMobileLayout||typeof window<"u"&&window.innerWidth<768;const i=e.coreMath;if(!i||!e.formula)return null;const l=u=>{u.preventDefault(),u.stopPropagation();const g=Ge(u.target);if(e.formula){const y=`formula.${e.formula.toLowerCase()}`;g.includes(y)||g.unshift(y)}const S=ei();r(u.clientX,u.clientY,S,g)},d=(()=>{if(e.formula==="Modular"){const g=["ParamA","ParamB","ParamC","ParamD","ParamE","ParamF"],S={};return e.pipeline.forEach(y=>{if(!y.enabled||!y.bindings)return;const b=ze.get(y.type);Object.entries(y.bindings).forEach(([m,x])=>{if(x&&g.includes(x)){S[x]||(S[x]={labels:[],min:-5,max:5,step:.01});const C=b==null?void 0:b.inputs.find(z=>z.id===m);C?S[x].labels.push(`${y.type}: ${C.label}`):S[x].labels.push(`${y.type}: ${m}`)}})}),g.map(y=>{const b=S[y],m=y.charAt(0).toLowerCase()+y.slice(1);if(!b)return null;const x=b.labels.length>1?`${y} (Mixed)`:b.labels[0]||y;let C=0,z=w=>{};switch(m){case"paramA":C=i.paramA,z=w=>o.setCoreMath({paramA:w});break;case"paramB":C=i.paramB,z=w=>o.setCoreMath({paramB:w});break;case"paramC":C=i.paramC,z=w=>o.setCoreMath({paramC:w});break;case"paramD":C=i.paramD,z=w=>o.setCoreMath({paramD:w});break;case"paramE":C=i.paramE,z=w=>o.setCoreMath({paramE:w});break;case"paramF":C=i.paramF,z=w=>o.setCoreMath({paramF:w});break}return{label:x,val:C,set:z,min:-5,max:5,step:.01,def:0,id:m,trackId:`coreMath.${m}`,scale:"linear"}})}const u=Me.get(e.formula);return u?u.parameters.map(g=>{if(!g)return null;if(g.type==="vec3"){let b=i.vec3A,m=x=>o.setCoreMath({vec3A:x});switch(g.id){case"vec3A":b=i.vec3A,m=x=>o.setCoreMath({vec3A:x});break;case"vec3B":b=i.vec3B,m=x=>o.setCoreMath({vec3B:x});break;case"vec3C":b=i.vec3C,m=x=>o.setCoreMath({vec3C:x});break}return{label:g.label,val:b,set:m,min:g.min,max:g.max,step:g.step,def:g.default,id:g.id,trackId:`coreMath.${g.id}`,type:"vec3",mode:g.mode,linkable:g.linkable,scale:g.scale}}if(g.type==="vec2"){let b=i.vec2A,m=x=>o.setCoreMath({vec2A:x});switch(g.id){case"vec2A":b=i.vec2A,m=x=>o.setCoreMath({vec2A:x});break;case"vec2B":b=i.vec2B,m=x=>o.setCoreMath({vec2B:x});break;case"vec2C":b=i.vec2C,m=x=>o.setCoreMath({vec2C:x});break}return{label:g.label,val:b,set:m,min:g.min,max:g.max,step:g.step,def:g.default,id:g.id,trackId:`coreMath.${g.id}`,type:"vec2",mode:g.mode,linkable:g.linkable,scale:g.scale}}let S=0,y=b=>{};switch(g.id){case"paramA":S=i.paramA,y=b=>o.setCoreMath({paramA:b});break;case"paramB":S=i.paramB,y=b=>o.setCoreMath({paramB:b});break;case"paramC":S=i.paramC,y=b=>o.setCoreMath({paramC:b});break;case"paramD":S=i.paramD,y=b=>o.setCoreMath({paramD:b});break;case"paramE":S=i.paramE,y=b=>o.setCoreMath({paramE:b});break;case"paramF":S=i.paramF,y=b=>o.setCoreMath({paramF:b});break}return{label:g.label,val:S,set:y,min:g.min,max:g.max,step:g.step,def:g.default,id:g.id,trackId:`coreMath.${g.id}`,scale:g.scale,options:g.options}}):[{label:"Power (N)",val:i.paramA,set:g=>o.setCoreMath({paramA:g}),min:2,max:16,step:.001,def:8,id:"paramA",trackId:"coreMath.paramA"},null,null,null]})(),f=u=>{if(!u)return null;if(u.type==="vec3"){const b=u.val,m=new U(b.x,b.y,b.z),x=[`${u.trackId}_x`,`${u.trackId}_y`,`${u.trackId}_z`],C=[`${u.label} X`,`${u.label} Y`,`${u.label} Z`],z=u.mode||"normal",w=z==="rotation"||z==="direction"||z==="axes",M={rotation:["Azimuth","Pitch","Angle"],direction:["Azimuth","Pitch","Length"],axes:[`${u.label} X`,`${u.label} Y`,`${u.label} Z`]};return t.jsx("div",{className:"mb-px",children:t.jsx($t,{label:u.label,value:m,min:w?-Math.PI*2:u.min,max:w?Math.PI*2:u.max,step:u.step,onChange:u.set,trackKeys:x,trackLabels:w&&M[z]||C,mode:z==="axes"?"normal":z,defaultValue:u.def?new U(u.def.x??0,u.def.y??0,u.def.z??0):void 0,linkable:u.linkable,scale:u.scale})},u.id)}if(u.type==="vec2"){const b=u.val,m=[`${u.trackId}_x`,`${u.trackId}_y`],x=[`${u.label} X`,`${u.label} Y`];return t.jsx("div",{className:"mb-px",children:t.jsx(Nr,{label:u.label,value:new je(b.x,b.y),min:u.min,max:u.max,step:u.step,onChange:C=>u.set({x:C.x,y:C.y}),trackKeys:m,trackLabels:x,defaultValue:u.def?new je(u.def.x??0,u.def.y??0):void 0,linkable:u.linkable,mode:u.mode,scale:u.scale})},u.id)}const g=u.val;if(u.options)return t.jsx("div",{className:"mb-px",children:t.jsx(kt,{label:u.label,value:g,options:u.options,onChange:b=>u.set(b),fullWidth:!0})},u.id);const S=e.liveModulations[u.trackId]??e.liveModulations[u.id],y=e.animations.some(b=>b.enabled&&(b.target===u.trackId||b.target===u.id));return u.scale==="pi"?t.jsx(me,{label:u.label,value:g,min:u.min,max:u.max,step:.01,onChange:u.set,defaultValue:u.def,highlight:y||u.id==="paramA"&&!y,trackId:u.trackId,liveValue:S,customMapping:{min:u.min/Math.PI,max:u.max/Math.PI,toSlider:b=>b/Math.PI,fromSlider:b=>b*Math.PI},mapTextInput:!0,overrideInputText:`${(g/Math.PI).toFixed(2)}π`},u.id):t.jsx(me,{label:u.label,value:g,min:u.min,max:u.max,step:u.step,onChange:u.set,defaultValue:u.def,highlight:y||u.id==="paramA"&&!y,trackId:u.trackId,liveValue:S},u.id)},p=u=>{o.setFormula(u),u==="Modular"&&a&&a("Graph")};return t.jsxs("div",{className:"animate-fade-in -mx-4 -mt-4 min-h-full flex flex-col",onContextMenu:l,children:[t.jsxs("div",{className:"bg-gray-800/20 border-b border-white/5 p-4 pb-3","data-help-id":"formula.active",children:[t.jsxs("div",{className:"flex justify-between items-baseline mb-1",children:[t.jsx(_e,{color:"text-gray-500",children:"Active Formula"}),n&&t.jsx("span",{className:"text-[9px] text-gray-500 animate-fade-in",children:n})]}),t.jsx(Ud,{value:e.formula,onChange:p})]}),t.jsxs("div",{className:"flex flex-col","data-help-id":`panel.formula formula.${((h=e.formula)==null?void 0:h.toLowerCase())||"mandelbulb"}`,children:[t.jsx(me,{label:"Iterations",value:i.iterations,min:1,max:500,step:1,onChange:u=>o.setCoreMath({iterations:Math.round(u)}),highlight:!0,defaultValue:32,customMapping:{min:0,max:100,toSlider:u=>100*Math.pow((u-1)/499,1/3),fromSlider:u=>1+499*Math.pow(u/100,3)},mapTextInput:!1,trackId:"coreMath.iterations",liveValue:e.liveModulations["coreMath.iterations"]}),t.jsx(t.Fragment,{children:d.map(u=>f(u))})]}),t.jsx(qe,{}),t.jsx("div",{className:"border-t border-white/5","data-help-id":"formula.transform",children:t.jsx(se,{featureId:"geometry",groupFilter:"transform"})}),t.jsx("div",{className:"border-t border-white/5",children:t.jsx(se,{featureId:"geometry",groupFilter:"burning"})}),t.jsx(qe,{}),t.jsx("div",{className:"border-t border-white/5","data-help-id":"julia.mode",children:t.jsx(se,{featureId:"geometry",groupFilter:"julia"})}),t.jsx(qe,{}),t.jsx(Xd,{state:e}),t.jsx(qe,{}),t.jsx(Wd,{state:e,actions:o}),e.showHints&&t.jsx("div",{className:"text-[9px] text-gray-600 text-center mt-6 pb-2 opacity-50 font-mono",children:"PRESS 'H' TO HIDE HINTS"})]})},Zd=({state:e,actions:o})=>{const a=N(l=>l.openContextMenu),r=e.droste;e.colorGrading;const n=e.optics,s=e.waterPlane,i=l=>{const c=Ge(l.currentTarget);c.length>0&&(l.preventDefault(),l.stopPropagation(),a(l.clientX,l.clientY,[],c))};return t.jsxs("div",{className:"animate-fade-in -mx-4 -mt-4 flex flex-col",children:[t.jsx("div",{className:"flex flex-col","data-help-id":"dof.settings",children:n&&t.jsxs("div",{className:"flex flex-col",children:[t.jsx(se,{featureId:"optics",groupFilter:"dof"}),t.jsx(se,{featureId:"optics",groupFilter:"projection"})]})}),e.advancedMode&&t.jsxs("div",{className:"flex flex-col","data-help-id":"panel.scene",children:[t.jsx("div",{className:"t-section-header",onContextMenu:i,"data-help-id":"panel.scene",children:t.jsx("h3",{className:"t-section-title",children:"Camera & Navigation"})}),t.jsx(se,{featureId:"navigation",groupFilter:"controls"})]}),t.jsx("div",{className:"flex flex-col border-t border-white/5","data-help-id":"fog.settings",children:t.jsx(se,{featureId:"atmosphere",groupFilter:"fog"})}),t.jsx(qe,{}),s&&s.waterEnabled&&t.jsx("div",{className:"flex flex-col border-t border-white/5","data-help-id":"water.settings",children:t.jsxs(Ii,{label:"Water Plane",featureId:"waterPlane",description:"Infinite ocean plane at height Y.",children:[t.jsx("div",{className:"mb-2",children:t.jsx(se,{featureId:"waterPlane",groupFilter:"main"})}),t.jsxs("div",{className:"bg-black/20 p-2 rounded border border-white/5 mb-2",children:[t.jsx(se,{featureId:"waterPlane",groupFilter:"geometry"}),t.jsx(se,{featureId:"waterPlane",groupFilter:"material"})]}),t.jsxs("div",{className:"bg-black/20 p-2 rounded border border-white/5",children:[t.jsx(_e,{variant:"secondary",className:"mb-2",children:"Waves"}),t.jsx(se,{featureId:"waterPlane",groupFilter:"waves"})]})]})}),t.jsx("div",{className:"flex flex-col","data-help-id":"scene.grading",children:t.jsx(se,{featureId:"colorGrading",groupFilter:"grading"})}),t.jsx(qe,{}),t.jsxs("div",{className:"flex flex-col","data-help-id":"post.effects",children:[t.jsx(se,{featureId:"postEffects",groupFilter:"bloom"}),t.jsx(se,{featureId:"postEffects",groupFilter:"lens"})]}),t.jsx(qe,{}),r&&t.jsx(t.Fragment,{children:t.jsxs("div",{className:"flex flex-col","data-help-id":"effect.droste",children:[t.jsx(se,{featureId:"droste",groupFilter:"main"}),r.active&&t.jsxs("div",{className:"animate-fade-in flex flex-col",children:[t.jsx(se,{featureId:"droste",groupFilter:"geometry"}),t.jsx(qe,{}),t.jsx(se,{featureId:"droste",groupFilter:"structure"}),t.jsx(qe,{}),t.jsx(se,{featureId:"droste",groupFilter:"transform"})]})]})}),t.jsx(qe,{}),t.jsx("div",{className:"flex flex-col",children:t.jsx(se,{featureId:"volumetric"})})]})},lr=xe(),Qd=({state:e,actions:o})=>{const[a,r]=v.useState(0),n=e.lighting,s=e.liveModulations;v.useEffect(()=>{a>=n.lights.length&&n.lights.length>0&&r(n.lights.length-1)},[n.lights.length,a]);const i=bt(n,a),l=N(m=>m.openContextMenu),c=m=>{const x=Ge(m.currentTarget);x.length>0&&(m.preventDefault(),m.stopPropagation(),l(m.clientX,m.clientY,[],x))},d=()=>{n.lights.length<Ce&&(o.addLight(),r(n.lights.length))},f=m=>{m.stopPropagation(),n.lights.length>1&&(o.removeLight(a),r(Math.max(0,a-1)))},p=()=>{const m=bt(e.lighting,a),x=m.fixed,C=We();let z=m.position,w=m.rotation;if(C)if(m.type==="Point"){const M=lr.sceneOffset;if(x){const P=new U(z.x,z.y,z.z);P.applyQuaternion(C.quaternion),P.add(C.position),z={x:P.x+M.x+(M.xL??0),y:P.y+M.y+(M.yL??0),z:P.z+M.z+(M.zL??0)}}else{const P=new U(z.x-M.x-(M.xL??0),z.y-M.y-(M.yL??0),z.z-M.z-(M.zL??0));P.sub(C.position),P.applyQuaternion(C.quaternion.clone().invert()),z={x:P.x,y:P.y,z:P.z}}}else{const M=new U(0,0,-1).applyEuler(new Fe(w.x,w.y,w.z,"YXZ"));M.applyQuaternion(x?C.quaternion:C.quaternion.clone().invert());const P=new De().setFromUnitVectors(new U(0,0,-1),M),k=new Fe().setFromQuaternion(P,"YXZ");w={x:k.x,y:k.y,z:k.z}}o.updateLight({index:a,params:{fixed:!x,position:z,rotation:w}})},h=m=>{const x=We();if(!x){o.updateLight({index:a,params:{type:m}});return}const C=bt(e.lighting,a);let z=new U(0,0,0);if(!C.fixed){const w=new U(0,0,-1).applyQuaternion(x.quaternion);z.copy(x.position).addScaledVector(w,2);const M=lr.sceneOffset;z.add(new U(M.x+M.xL,M.y+M.yL,M.z+M.zL))}if(m==="Directional"){const w=new U(C.position.x,C.position.y,C.position.z),M=new U().subVectors(z,w).normalize();M.lengthSq()<.001&&M.set(0,-1,0);const P=new De().setFromUnitVectors(new U(0,0,-1),M),k=new Fe().setFromQuaternion(P,"YXZ");o.updateLight({index:a,params:{type:m,rotation:{x:k.x,y:k.y,z:k.z}}})}else{const w=new De().setFromEuler(new Fe(C.rotation.x,C.rotation.y,C.rotation.z,"YXZ")),M=new U(0,0,-1).applyQuaternion(w),k=z.clone().sub(M.multiplyScalar(5));o.updateLight({index:a,params:{type:m,position:{x:k.x,y:k.y,z:k.z}}})}};if(!i)return null;const u=(i.fixed,10),g=`lighting.light${a}`,S={x:s[`${g}_rotX`]??i.rotation.x,y:s[`${g}_rotY`]??i.rotation.y,z:s[`${g}_rotZ`]??i.rotation.z},y={x:s[`${g}_posX`]??i.position.x,y:s[`${g}_posY`]??i.position.y,z:s[`${g}_posZ`]??i.position.z},b=new U(y.x,y.y,y.z);return t.jsxs("div",{className:"animate-fade-in",children:[t.jsx("div",{className:"mb-4",children:t.jsxs("div",{className:"flex flex-wrap gap-1 bg-black/40 p-1 rounded border border-white/5",children:[n.lights.map((m,x)=>t.jsxs("button",{onClick:()=>r(x),className:`flex-1 min-w-[60px] py-1.5 px-2 text-[9px] font-bold rounded border transition-all relative ${a===x?"bg-cyan-900/50 border-cyan-500/50 text-cyan-200 shadow-sm":"bg-transparent border-transparent text-gray-500 hover:bg-white/5 hover:text-gray-300"}`,children:["Light ",x+1,m.visible&&t.jsx("div",{className:"absolute top-1 right-1 w-1 h-1 rounded-full bg-cyan-400"})]},x)),n.lights.length<Ce&&t.jsx("button",{onClick:d,className:"w-8 flex items-center justify-center text-gray-500 hover:text-cyan-400 hover:bg-white/5 rounded transition-colors",title:"Add Light",children:t.jsx(ha,{})})]})}),t.jsxs("div",{className:"mb-4 space-y-3","data-help-id":"panel.light",children:[t.jsxs("div",{className:"flex items-center justify-between p-3 bg-gray-800/50 rounded-lg border border-white/5",children:[t.jsx(Ae,{label:"Enabled",value:i.visible,onChange:()=>o.updateLight({index:a,params:{visible:!i.visible}}),color:"bg-green-500"}),n.lights.length>1&&t.jsx("button",{onClick:f,className:"p-1.5 text-red-500 hover:text-red-300 hover:bg-red-900/20 rounded ml-2 transition-colors",title:"Delete Light",children:t.jsx(wt,{})})]}),t.jsxs("div",{className:`transition-opacity duration-200 ${i.visible?"opacity-100":"opacity-40 pointer-events-none"}`,children:[t.jsxs("div",{className:"mb-4 p-3 bg-gray-800/50 rounded-lg","data-help-id":"light.mode",onContextMenu:c,children:[t.jsx("div",{className:"flex gap-1 mb-2","data-help-id":"light.type",children:t.jsx(Ae,{value:i.type,onChange:m=>h(m),options:[{label:"Point",value:"Point"},{label:"Directional (Sun)",value:"Directional"}]})}),t.jsx(Ae,{label:"Attachment Mode",value:i.fixed,onChange:p,options:[{label:"Headlamp",value:!0},{label:"World",value:!1}],helpId:"light.mode"})]}),i.type==="Point"?t.jsx("div",{"data-help-id":"light.pos",children:t.jsx($t,{label:i.fixed?"Offset XYZ":"World Position",value:b,onChange:m=>o.updateLight({index:a,params:{position:{x:m.x,y:m.y,z:m.z}}}),min:-u,max:u,step:.01,interactionMode:"param",trackKeys:[`lighting.light${a}_posX`,`lighting.light${a}_posY`,`lighting.light${a}_posZ`],trackLabels:[`Light ${a+1} Pos X`,`Light ${a+1} Pos Y`,`Light ${a+1} Pos Z`]})}):t.jsx("div",{"data-help-id":"light.rot",children:t.jsx(Er,{index:a,value:S,onChange:m=>o.updateLight({index:a,params:{rotation:m}}),isFixed:i.fixed,width:200,height:130})}),t.jsx(me,{label:"Intensity",value:i.intensity,min:0,max:100,step:.1,onChange:m=>o.updateLight({index:a,params:{intensity:m}}),customMapping:{min:0,max:100,toSlider:m=>Math.sqrt(m/100)*100,fromSlider:m=>m*m/100},mapTextInput:!1,overrideInputText:Kd(i.intensity),dataHelpId:"light.intensity",trackId:`${g}_intensity`,liveValue:s[`${g}_intensity`]}),i.type==="Point"&&t.jsxs(t.Fragment,{children:[t.jsx("div",{className:"mt-2 mb-1 px-3","data-help-id":"light.falloff",onContextMenu:c,children:t.jsx(Ae,{label:"Falloff Type",value:i.falloffType,onChange:m=>o.updateLight({index:a,params:{falloffType:m}}),options:[{label:"Quad",value:"Quadratic"},{label:"Linear",value:"Linear"}],helpId:"light.falloff"})}),t.jsx(me,{label:"Falloff (Decay)",value:i.falloff,min:0,max:500,step:.1,onChange:m=>o.updateLight({index:a,params:{falloff:m}}),customMapping:{min:0,max:100,toSlider:m=>Math.log10(m+1)/Math.log10(501)*100,fromSlider:m=>Math.pow(501,m/100)-1},mapTextInput:!1,overrideInputText:i.falloff<.01?"Infinite":i.falloff.toFixed(2),dataHelpId:"light.falloff",trackId:`${g}_falloff`,liveValue:s[`${g}_falloff`]}),t.jsx("p",{className:"text-[9px] text-gray-500 mb-2 -mt-2",children:"0 = No decay (Sun). Higher = shorter range."})]}),t.jsxs("div",{className:"mt-4 pt-3 border-t border-white/10 space-y-2",children:[t.jsx("label",{className:"text-xs text-gray-400 font-bold mb-2 block",children:"Color"}),t.jsx(ga,{color:i.color,onColorChange:m=>o.updateLight({index:a,params:{color:m}})}),t.jsxs("div",{className:"flex items-center justify-between pt-1",children:[t.jsx("label",{className:"text-xs text-gray-400 font-medium",children:"Cast Shadows"}),t.jsx("input",{type:"checkbox",checked:i.castShadow,onChange:m=>{o.handleInteractionStart("param"),o.updateLight({index:a,params:{castShadow:m.target.checked}}),o.handleInteractionEnd()},className:"w-3 h-3 accent-cyan-500 bg-gray-800 border-gray-600 rounded cursor-pointer"})]})]})]})]}),t.jsx("div",{className:"h-px bg-gray-800 my-4"}),t.jsx("div",{className:"flex items-center justify-between mb-4 p-3 bg-gray-800/50 rounded-lg",children:t.jsx(Ae,{label:"Show 3d helpers",value:e.showLightGizmo,onChange:o.setShowLightGizmo,color:"bg-cyan-600"})}),n&&t.jsxs("div",{className:"mt-4 p-3 bg-gray-800/50 rounded-lg","data-help-id":"shadows",children:[t.jsxs("div",{className:"flex items-center justify-between mb-2",children:[t.jsx(_e,{children:"Shadows (Global)"}),t.jsx("div",{className:"w-[60px]",children:t.jsx(Ae,{value:n.shadows,onChange:m=>o.setLighting({shadows:m}),color:"bg-yellow-500"})})]}),n.shadows&&t.jsx("div",{className:"pl-2 mt-2 border-l-2 border-yellow-500/30",children:t.jsx(se,{featureId:"lighting",groupFilter:"shadows"})})]})]})},Kd=e=>{if(e===0)return"0";if(Math.abs(e)<1)return e.toFixed(3);const o=e.toPrecision(5);return o.includes(".")?o.replace(/\.?0+$/,""):o},Jd=({state:e,actions:o})=>{v.useRef(null),N(n=>n.openContextMenu),e.debugMobileLayout||typeof window<"u"&&window.innerWidth<768;const a=e.materials,r=e.atmosphere;return e.lighting,!a||!r?null:t.jsxs("div",{className:"animate-fade-in -mx-4 -mt-4 flex flex-col","data-help-id":"panel.render",children:[t.jsx("div",{className:"flex flex-col","data-help-id":"mat.diffuse",children:t.jsx(se,{featureId:"materials",groupFilter:"surface"})}),t.jsx("div",{className:"flex flex-col","data-help-id":"mat.env",children:t.jsx(se,{featureId:"materials",groupFilter:"env"})}),t.jsx("div",{className:"flex flex-col","data-help-id":"mat.reflection",children:t.jsx(se,{featureId:"reflections",groupFilter:"shading"})}),t.jsx("div",{className:"flex flex-col","data-help-id":"mat.glow",children:t.jsx(se,{featureId:"atmosphere",groupFilter:"glow"})}),t.jsx("div",{className:"flex flex-col","data-help-id":"mat.emission",children:t.jsx(se,{featureId:"materials",groupFilter:"emission"})}),t.jsx("div",{className:"flex flex-col","data-help-id":"mat.ao",children:t.jsx(se,{featureId:"ao",groupFilter:"shading"})})]})},eu=({state:e,actions:o})=>{const a=N(b=>b.openContextMenu),r=e.quality,n=e.lighting,[s,i]=e.fixedResolution,[l,c]=v.useState("Free"),d=(n==null?void 0:n.ptEnabled)!==!1,f=o.setLighting,p=v.useMemo(()=>{const b=`${s}x${i}`;return["800x600","1280x720","1920x1080","2560x1440","3840x2160","1080x1080","1080x1350","1080x1920","2048x1024","4096x2048"].includes(b)?b:"Custom"},[s,i]),h=b=>{const m=Ge(b.currentTarget);m.length>0&&(b.preventDefault(),b.stopPropagation(),a(b.clientX,b.clientY,[],m))},u=async b=>{e.renderMode!==b&&(Z.emit("is_compiling","Switching Engine..."),await new Promise(m=>setTimeout(m,50)),o.setRenderMode(b))},g=(b,m)=>{const x=Math.max(64,Math.round(b/8)*8),C=Math.max(64,Math.round(m/8)*8);o.setFixedResolution(x,C)},S=(b,m)=>{l==="Free"?g(b==="w"?m:s,b==="h"?m:i):b==="w"?g(m,m/l):g(m*l,m)},y=[.25,.5,1,1.5,2];return t.jsxs("div",{className:"animate-fade-in -mx-4 -mt-4 flex flex-col",children:[t.jsxs("div",{className:"flex flex-col","data-help-id":"render.engine",children:[t.jsxs("div",{className:"px-3 py-2",children:[t.jsx(_e,{className:"block mb-1",children:"Render Engine"}),t.jsxs("div",{className:"flex bg-black/40 rounded p-0.5 border border-white/10",children:[t.jsx("button",{onClick:()=>u("Direct"),className:`flex-1 py-1.5 text-[10px] font-bold rounded transition-colors ${e.renderMode==="Direct"?"bg-cyan-700 text-white":"text-gray-500 hover:text-gray-300"}`,children:"Direct (Fast)"}),t.jsx("button",{onClick:()=>d&&u("PathTracing"),disabled:!d,className:`flex-1 py-1.5 text-[10px] font-bold rounded transition-colors ${d?e.renderMode==="PathTracing"?"bg-purple-700 text-white":"text-gray-500 hover:text-gray-300":"text-gray-700 cursor-not-allowed opacity-50 bg-transparent"}`,title:d?"Switch to Path Tracer (GI)":"Path Tracer Disabled in Engine Panel",children:"Path Tracer (GI)"})]})]}),e.renderMode==="PathTracing"&&n&&t.jsxs("div",{className:"animate-fade-in","data-help-id":"pt.global",children:[t.jsx(me,{label:"Max Bounces",value:n.ptBounces,min:1,max:8,step:1,onChange:b=>f({ptBounces:Math.round(b)})}),t.jsx(me,{label:"GI Brightness",value:n.ptGIStrength,min:0,max:5,step:.01,onChange:b=>f({ptGIStrength:b}),trackId:"lighting.ptGIStrength"})]})]}),t.jsx(qe,{}),t.jsxs("div",{className:"w-full flex flex-col rounded-t-sm relative",onContextMenu:h,"data-help-id":"panel.quality",children:[t.jsx("div",{className:"absolute inset-0 bg-white/[0.06] rounded-t-sm pointer-events-none"}),t.jsx("div",{className:"flex items-center bg-white/[0.12] rounded-t-sm h-9 md:h-[26px] overflow-hidden border-b border-white/5 px-2",children:t.jsx("span",{className:"text-[10px] text-gray-400 font-medium tracking-tight select-none",children:"Resolution"})}),t.jsxs("div",{className:"flex flex-col",children:[(()=>{const b=[t.jsx("div",{children:t.jsx(Ae,{value:e.resolutionMode,onChange:o.setResolutionMode,options:[{label:"Fill Screen",value:"Full"},{label:"Fixed",value:"Fixed"}]})},"mode")];return e.resolutionMode==="Fixed"&&b.push(t.jsx("div",{className:"animate-fade-in",children:t.jsxs("div",{className:"flex flex-col gap-2 px-3 py-2 bg-neutral-800/50",children:[t.jsx(kt,{label:"Preset",value:p,options:[{label:"SVGA (800 x 600)",value:"800x600"},{label:"HD (1280 x 720)",value:"1280x720"},{label:"FHD (1920 x 1080)",value:"1920x1080"},{label:"QHD (2560 x 1440)",value:"2560x1440"},{label:"4K (3840 x 2160)",value:"3840x2160"},{label:"Square 1:1 (1080p)",value:"1080x1080"},{label:"Portrait 4:5 (1080p)",value:"1080x1350"},{label:"Vertical 9:16 (1080p)",value:"1080x1920"},{label:"Skybox Low (2048 x 1024)",value:"2048x1024"},{label:"Skybox High (4096 x 2048)",value:"4096x2048"},{label:"Custom",value:"Custom"}],onChange:m=>{if(m!=="Custom"){const[x,C]=m.split("x").map(Number);g(x,C)}},fullWidth:!0}),t.jsxs("div",{className:"flex gap-2",children:[t.jsxs("div",{className:"flex-1",children:[t.jsx(_e,{variant:"secondary",className:"block mb-0.5",children:"Width"}),t.jsx("div",{className:"h-6 bg-black/40 rounded border border-white/10 relative",children:t.jsx(ct,{value:s,onChange:m=>S("w",m),step:8,min:64,max:8192,overrideText:`${s}`})})]}),t.jsxs("div",{className:"flex-1",children:[t.jsx(_e,{variant:"secondary",className:"block mb-0.5",children:"Height"}),t.jsx("div",{className:"h-6 bg-black/40 rounded border border-white/10 relative",children:t.jsx(ct,{value:i,onChange:m=>S("h",m),step:8,min:64,max:8192,overrideText:`${i}`})})]}),t.jsxs("div",{className:"w-[35%]",children:[t.jsx(_e,{variant:"secondary",className:"block mb-0.5",children:"Ratio"}),t.jsx("div",{className:"h-6",children:t.jsx(kt,{value:l,options:[{label:"Free",value:"Free"},{label:"16:9",value:1.7777},{label:"4:3",value:1.3333},{label:"1:1",value:1},{label:"4:5 (Portrait)",value:.8},{label:"9:16 (Vertical)",value:.5625},{label:"2:1 (Sky)",value:2}],onChange:m=>{c(m),m!=="Free"&&g(s,s/m)},fullWidth:!0,className:"!px-1"})})]})]})]})},"fixed")),b.push(t.jsx("div",{"data-help-id":"quality.scale",children:t.jsxs("div",{className:"px-3 py-2",children:[t.jsxs("div",{className:"flex items-center justify-between mb-1.5",children:[t.jsx(_e,{variant:"secondary",children:"Internal Scale"}),t.jsx("span",{className:"text-[10px] font-mono text-cyan-400 font-bold",children:`${e.aaLevel.toFixed(2)}x`})]}),t.jsx("div",{className:"grid grid-cols-5 gap-px bg-white/5 border border-white/5 rounded overflow-hidden",children:y.map(m=>t.jsx("button",{onClick:()=>o.setAALevel(m),className:`py-1.5 text-[9px] font-bold transition-all ${e.aaLevel===m?"bg-cyan-600/40 text-cyan-300 shadow-[inset_0_0_10px_rgba(34,211,238,0.1)]":"bg-transparent text-gray-500 hover:text-gray-300 hover:bg-white/5"}`,children:m},m))})]})},"scale"),t.jsx("div",{children:t.jsx(se,{featureId:"quality",groupFilter:"performance"})},"performance")),b.map((m,x)=>{const C=x===b.length-1;return t.jsxs("div",{className:"flex",children:[t.jsx("div",{className:`w-2 shrink-0 self-stretch border-l border-white/20 bg-white/[0.12] ${C?"border-b border-b-white/20 rounded-bl-lg":""}`}),t.jsxs("div",{className:`flex-1 min-w-0 relative ${C?"border-b border-b-white/20":""}`,children:[t.jsx("div",{className:"absolute inset-0 bg-black/20 pointer-events-none z-10"}),m]})]},x)})})(),t.jsx("div",{className:"h-2",style:{background:"linear-gradient(to bottom, transparent, rgba(255,255,255,0.08))"}})]})]}),t.jsx(qe,{}),t.jsxs("div",{className:"flex flex-col","data-help-id":"quality.tss",children:[t.jsx(Ae,{label:"Temporal AA (Remove Noise)",value:e.accumulation,onChange:o.setAccumulation,color:"bg-green-500",helpId:"quality.tss"}),d&&t.jsx(Ae,{label:"Area Lights (Soft Shadows)",value:(n==null?void 0:n.ptStochasticShadows)??!1,onChange:b=>f&&f({ptStochasticShadows:b}),helpId:"pt.global"})]}),(n==null?void 0:n.shadowsCompile)&&(n==null?void 0:n.shadows)&&t.jsx("div",{className:"flex flex-col","data-help-id":"shadows",children:t.jsx(se,{featureId:"lighting",groupFilter:"shadow_quality"})}),t.jsx(qe,{}),t.jsx("div",{className:"flex flex-col",children:r&&t.jsx(se,{featureId:"quality",groupFilter:"kernel"})})]})},tu=({layer:e,state:o,histogramData:a,onChange:r,onRefresh:n,autoUpdate:s,onToggleAuto:i,liveModulations:l})=>{const c=v.useRef(!1),d=e===1?o.repeats:o.repeats2,f=e===1?o.phase:o.phase2,p=e===1?o.scale:o.scale2,h=e===1?o.offset:o.offset2,u=e===1?o.bias:o.bias2,g=e===1?o.gradient:o.gradient2,S=e===1?o.mode:o.mode2,y=e===1?"scale":"scale2",b=e===1?"offset":"offset2",m=e===1?"repeats":"repeats2",x=e===1?"phase":"phase2",C=e===1?"bias":"bias2",z=v.useRef(d),w=v.useRef(f),M=v.useRef(p),P=v.useRef(h),k=v.useRef(S);v.useEffect(()=>{if(S!==k.current){const E=Math.abs(p-M.current)>.001,F=Math.abs(h-P.current)>.001;!E&&!F&&(c.current=!0,s||n()),k.current=S}},[S,p,h,s,n]),v.useEffect(()=>{if(c.current&&a){const E=Wr(a);if(E){const F=Xr(E.buckets,E.min,E.max);if(F){const O=F.end-F.start,$=Math.abs(O)<1e-4?1e-4:O,R=d/$,Y=f-F.start*R;r({[y]:R,[b]:Y}),c.current=!1}}}},[a,d,f,y,b,r]),v.useEffect(()=>{const E=Math.abs(d-z.current)>.001,F=Math.abs(f-w.current)>.001,O=Math.abs(p-M.current)>.001,$=Math.abs(h-P.current)>.001;if((E||F)&&!O&&!$){const R=Math.max(1e-4,p),L=Math.max(1e-4,z.current)/R,G=(w.current-h)/R,X=d/L,D=f-G*X;r({[y]:X,[b]:D})}z.current=d,w.current=f,M.current=p,P.current=h},[d,f,p,h,y,b,r]);const T=(f-h)/p,_=T+d/p;return t.jsxs("div",{className:"flex flex-col",children:[t.jsx(Yr,{data:a,min:T,max:_,gamma:u,repeats:d,phase:f,gradientStops:g,labelTitle:"Range",labelLeft:"Min",labelMid:"Bias",labelRight:"Max",onChange:({min:E,max:F,gamma:O})=>{const $=F-E,R=Math.abs($)<1e-4?1e-4:$,Y=d/R,L=f-E*Y,G={[y]:Y,[b]:L,[C]:O};r(G)},autoUpdate:s,onToggleAuto:i,onRefresh:n}),t.jsx(me,{label:"Repeats",value:d,min:.1,max:100,step:.1,onChange:E=>r({[m]:E}),trackId:`coloring.${m}`,liveValue:l==null?void 0:l[`coloring.${m}`]}),t.jsx(me,{label:"Phase",value:f,min:-1,max:1,step:.01,onChange:E=>r({[x]:E}),trackId:`coloring.${x}`,liveValue:l==null?void 0:l[`coloring.${x}`]})]})},au=({sliceState:e,actions:o})=>{const a=e,r=a.hybridComplex,[n,s]=v.useState(!1),i=o.setGeometry,l=d=>{d.stopPropagation(),Z.emit("is_compiling","Optimizing Shader..."),setTimeout(()=>{i({hybridComplex:!0}),s(!1)},50)},c=d=>{d.preventDefault(),d.stopPropagation(),s(!0)};return t.jsxs("div",{className:"relative mt-2 pt-2 border-t border-white/5",children:[t.jsxs("div",{className:`transition-all duration-300 ${r?"":"opacity-30 blur-[0.5px] pointer-events-none grayscale"}`,children:[t.jsx("div",{className:"flex items-center gap-1 mb-1",children:t.jsx(_e,{variant:"secondary",children:"Advanced Mixing"})}),t.jsx(me,{label:"Box Skip (Mod)",value:a.hybridSkip,min:1,max:8,step:1,onChange:d=>i({hybridSkip:d}),overrideInputText:Math.round(a.hybridSkip)<=1?"Consecutive":Math.round(a.hybridSkip)===2?"Every 2nd":`Every ${Math.round(a.hybridSkip)}th`,trackId:"geometry.hybridSkip"}),t.jsx("div",{className:"mt-1",children:t.jsx(Ae,{label:"Swap Order",value:a.hybridSwap,onChange:d=>i({hybridSwap:d})})})]}),!r&&!n&&t.jsx("div",{className:"absolute inset-0 cursor-pointer z-10 bg-gray-900/50 hover:bg-gray-800/40 transition-colors flex items-center justify-center group rounded",onClick:c,title:"Click to enable Advanced Hybrid Mode",children:t.jsx("div",{className:"text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity transform scale-75",children:t.jsxs("svg",{width:"16",height:"16",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",children:[t.jsx("rect",{x:"3",y:"11",width:"18",height:"11",rx:"2",ry:"2"}),t.jsx("path",{d:"M7 11V7a5 5 0 0 1 10 0v4"})]})})}),n&&t.jsxs("div",{className:"absolute top-[-20px] left-0 right-0 z-50 animate-pop-in",children:[t.jsxs("div",{className:"bg-black/95 border border-white/20 rounded-lg shadow-[0_10px_40px_rgba(0,0,0,0.9)] backdrop-blur-md overflow-hidden cursor-pointer hover:border-cyan-500/50 transition-colors",onClick:l,children:[t.jsxs("div",{className:"flex items-start justify-between p-2 border-b border-white/10 bg-white/5",children:[t.jsxs("div",{className:"flex items-center gap-2 text-gray-300",children:[t.jsx(Rt,{}),t.jsx(_e,{children:"Advanced Shader"})]}),t.jsx("button",{onClick:d=>{d.stopPropagation(),s(!1)},className:"text-gray-500 hover:text-white -mt-0.5 -mr-0.5 p-1",children:t.jsx(eo,{})})]}),t.jsxs("div",{className:"p-3",children:[t.jsxs("p",{className:"text-[10px] text-gray-400 leading-relaxed mb-3",children:["Enable Advanced Hybrid Integration?",t.jsx("br",{}),"This allows ",t.jsx("strong",{children:"alternating formulas"})," between Box Folds and the Main Fractal.",t.jsx("br",{}),t.jsx("br",{}),t.jsx("span",{className:"text-orange-300",children:"Compilation may take 30-60 seconds."})]}),t.jsx("div",{className:"flex items-center justify-center p-1.5 bg-white/5 rounded border border-white/10 text-cyan-400 text-[9px] font-bold group-hover:bg-cyan-900/30 group-hover:text-cyan-200 group-hover:border-cyan-500/30 transition-all",children:"Click to Load"})]})]}),t.jsx("div",{className:"fixed inset-0 z-[-1]",onClick:d=>{d.stopPropagation(),s(!1)}})]})]})},ou=({actions:e,targetMode:o,label:a,activeLabel:r,helpText:n,variant:s="primary"})=>{const i=N(f=>f.interactionMode),{setInteractionMode:l}=e,c=i===o,d=()=>{l(c?"none":o)};return t.jsxs("div",{className:"flex flex-col animate-fade-in",children:[c&&n&&t.jsx("div",{className:"mb-px p-2 bg-green-900/30 border border-green-500/30 rounded text-[9px] text-green-200 animate-pulse text-center leading-tight",children:n}),t.jsx(da,{onClick:d,label:c?r||"Cancel":a,variant:c?"success":s,fullWidth:!0})]})},ru=()=>t.jsxs("svg",{width:"13",height:"13",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"1.8",strokeLinecap:"round",strokeLinejoin:"round",children:[t.jsx("rect",{x:"3",y:"3",width:"18",height:"18",rx:"3"}),t.jsx("circle",{cx:"8.5",cy:"8.5",r:"1.2",fill:"currentColor",stroke:"none"}),t.jsx("circle",{cx:"15.5",cy:"8.5",r:"1.2",fill:"currentColor",stroke:"none"}),t.jsx("circle",{cx:"8.5",cy:"15.5",r:"1.2",fill:"currentColor",stroke:"none"}),t.jsx("circle",{cx:"15.5",cy:"15.5",r:"1.2",fill:"currentColor",stroke:"none"}),t.jsx("circle",{cx:"12",cy:"12",r:"1.2",fill:"currentColor",stroke:"none"})]}),iu=()=>{const e=v.useRef(null),o=v.useRef(0),a=v.useRef({x:0,y:0,z:0}),r=v.useRef(0),n=v.useRef({shift:!1,alt:!1}),s=v.useCallback(()=>{const c=(Date.now()-o.current)/1e3,d=.5*c*c+.1*c,f=Math.min(8,d),p=f-r.current;r.current=f;let h=1;n.current.shift?h=5:n.current.alt&&(h=.2);const u=p*h,g=a.current,S=N.getState().geometry;N.getState().setGeometry({juliaX:S.juliaX+g.x*u,juliaY:S.juliaY+g.y*u,juliaZ:S.juliaZ+g.z*u})},[]),i=v.useCallback(c=>{n.current={shift:c.shiftKey,alt:c.altKey},N.getState().handleInteractionStart("param");const f=Math.random()*2-1,p=Math.random()*2-1,h=Math.random()*2-1,u=Math.sqrt(f*f+p*p+h*h)||1;a.current={x:f/u,y:p/u,z:h/u},o.current=Date.now(),r.current=0,s(),e.current=setInterval(s,30)},[s]),l=v.useCallback(()=>{e.current&&(clearInterval(e.current),e.current=null),N.getState().handleInteractionEnd()},[]);return v.useEffect(()=>{const c=d=>{n.current={shift:d.shiftKey,alt:d.altKey}};return window.addEventListener("keydown",c),window.addEventListener("keyup",c),()=>{window.removeEventListener("keydown",c),window.removeEventListener("keyup",c),e.current&&clearInterval(e.current)}},[]),t.jsxs("button",{onPointerDown:i,onPointerUp:l,onPointerLeave:l,className:"w-full h-[26px] flex items-center justify-center gap-1.5 bg-white/[0.06] border-b border-white/5 hover:bg-cyan-500/10 text-gray-500 hover:text-cyan-300 transition-colors cursor-pointer select-none",title:"Hold to randomize — Shift: faster, Alt: slower",children:[t.jsx(ru,{}),t.jsx("span",{className:"text-[9px] font-medium",children:"Randomize"})]})};function ya(e){const o=ke.lazy(e);return a=>t.jsx(v.Suspense,{fallback:null,children:t.jsx(o,{...a})})}const nu=ya(()=>pt(()=>import("./FlowEditor-DC7xKxyS.js"),__vite__mapDeps([9,1,2,10,3,4]),import.meta.url)),su=ya(()=>pt(()=>import("./AudioPanel-DkvtaZqK.js"),__vite__mapDeps([11,1,2,12,3,4]),import.meta.url).then(e=>({default:e.AudioPanel}))),lu=ya(()=>pt(()=>import("./AudioSpectrum-CP3g0i07.js"),__vite__mapDeps([12,1,2,3,4]),import.meta.url).then(e=>({default:e.AudioSpectrum}))),cu=ya(()=>pt(()=>import("./DebugToolsOverlay-7cTdqBWa.js"),__vite__mapDeps([13,3,1,2]),import.meta.url).then(e=>({default:e.DebugToolsOverlay}))),du=e=>{const o=N(d=>d.histogramData),a=N(d=>d.histogramAutoUpdate),r=N(d=>d.setHistogramAutoUpdate),n=N(d=>d.refreshHistogram),s=N(d=>d.liveModulations),i=N(d=>d.registerHistogram),l=N(d=>d.unregisterHistogram);v.useEffect(()=>(i(),()=>l()),[i,l]);const c=d=>{const f=N.getState().setColoring;f&&f(d)};return t.jsx(tu,{layer:e.layer,state:e.sliceState,histogramData:o,onChange:c,onRefresh:n,autoUpdate:a,onToggleAuto:()=>r(!a),liveModulations:s})},uu=e=>{const o=N(r=>r.registerSceneHistogram),a=N(r=>r.unregisterSceneHistogram);return v.useEffect(()=>(o(),()=>a()),[o,a]),t.jsx(Rd,{...e})},fu=()=>{ve.register("panel-drawing",ud),ve.register("overlay-drawing",hd),ve.register("panel-audio",su),ve.register("overlay-lighting",kc),ve.register("overlay-webcam",Cd),ve.register("overlay-debug-tools",cu),ve.register("panel-engine",kd),ve.register("panel-cameramanager",Dd),ve.register("panel-formula",Yd),ve.register("panel-scene",Zd),ve.register("panel-light",Qd),ve.register("panel-shading",Jd),ve.register("panel-gradients",vd),ve.register("panel-quality",eu),ve.register("panel-graph",nu),ve.register("coloring-histogram",du),ve.register("hybrid-advanced-lock",au),ve.register("interaction-picker",ou),ve.register("julia-randomize",iu),ve.register("audio-spectrum",lu),ve.register("audio-link-controls",bd),ve.register("scene-histogram",uu),ve.register("optics-controls",Id),ve.register("optics-dof-controls",Fd),ve.register("navigation-controls",_d)};if("serviceWorker"in navigator)try{navigator.serviceWorker.getRegistrations().then(e=>{for(let o of e)o.unregister().then(()=>console.log("SW Unregistered"))}).catch(()=>{})}catch{console.debug("SW cleanup skipped")}fu();const Fi=document.getElementById("root");if(!Fi)throw new Error("Could not find root element to mount to");const pu=Ni.createRoot(Fi);pu.render(t.jsx(ke.StrictMode,{children:t.jsx(cd,{})}));export{Ht as $,kt as A,Au as B,ft as C,id as D,qu as E,Iu as F,Mu as G,Rt as H,ct as I,Ha as J,We as K,gl as L,Vu as M,Hu as N,Bu as O,ml as P,Nu as Q,_u as R,Pu as S,wt as T,Lu as U,Cu as V,Uu as W,Du as X,Eu as Y,Ys as Z,eo as _,oa as a,Wu as a0,Xu as a1,Tu as a2,Va as a3,vu as a4,bu as a5,tf as a6,jc as a7,Su as a8,wu as a9,Qc as aa,Tt as ab,ra as ac,Me as ad,ze as ae,Ru as af,ju as ag,Ze as ah,Or as ai,ha as aj,bd as ak,Tr as al,jt as am,Aa as an,ga as ao,lt as ap,Nt as aq,ws as ar,zr as as,Yu as b,Zu as c,ef as d,Rs as e,Ku as f,ot as g,Ju as h,zu as i,N as j,Ca as k,Ou as l,$u as m,Gu as n,Ge as o,me as p,Wt as q,ne as r,Qu as s,xe as t,fe as u,ku as v,Z as w,we as x,da as y,Fu as z};
