var xe=Object.defineProperty;var ze=(e,t,o)=>t in e?xe(e,t,{enumerable:!0,configurable:!0,writable:!0,value:o}):e[t]=o;var q=(e,t,o)=>ze(e,typeof t!="symbol"?t+"":t,o);import{aa as x,ak as D,bU as I,a2 as y,az as m,f,bV as Se,bW as we,bX as _e,bY as Fe,bZ as Me,b_ as Ie}from"./toastStore-D9E9kUoO.js";import{n as w,d as B,p as Ce,q as Pe,r as $,s as Z,c as g,l as O}from"./three-DQWx7qFd.js";import{b as Ee,d as Te,r as N,aw as oe,ax as De,ay as Ae,az as Re}from"./gradientSeam-Moku5dqu.js";import{a as Be}from"./three-drei-BQSiXBFe.js";function Le(e,t,o){var a,i;return!!((a=e.shader.capabilities)!=null&&a.has(o)||(i=t==null?void 0:t.shader.capabilities)!=null&&i.has(o))}const X=`
...
`});u.register({id:"Custom",label:"Custom (Legacy)",category:"Utils",description:"Legacy node.",inputs:[],glsl:e=>""});const Mt=e=>{const t=new Map;return e.forEach(o=>{t.has(o.target)||t.set(o.target,[]),t.get(o.target).push(o)}),t},It=e=>{const t=new Set,o=["root-end"],a=new Set;for(;o.length>0;){const i=o.pop();a.has(i)||(a.add(i),i!=="root-end"&&i!=="root-start"&&t.add(i),(e.get(i)??[]).forEach(r=>o.push(r.source)))}return t},Ct=(e,t)=>{const o=Mt(t),a=It(o),i=e.filter(s=>a.has(s.id));if(!i||i.length===0)return`
    ...
    `;const l=new Map;l.set("root-start","v_start");let n=0;i.forEach((s,v)=>{const d=`v_${s.id.replace(/[^a-zA-Z0-9]/g,"")}`;l.set(s.id,d);const b=o.get(s.id)??[],M=b.find(F=>!F.targetHandle||F.targetHandle==="a"),P=b.find(F=>F.targetHandle==="b"),E=M&&l.get(M.source)||"v_start",ye=P&&l.get(P.source)||"v_start";if(r+=`    // Node: ${s.type} (${s.id})
`,r+=`    vec3 ${d}_p = ${E}_p;
`,r+=`    float ${d}_d = ${E}_d;
`,r+=`    float ${d}_dr = ${E}_dr;
`,s.enabled){const F=u.get(s.type);if(F){const H=s.condition&&s.condition.active;let G="    ";if(H){const T=n<I?`uModularParams[${n++}]`:"2.0",ve=n<I?`uModularParams[${n++}]`:"0.0";r+=`    { int ${d}_cmod = max(1, int(${T})); int ${d}_crem = int(${ve});
`,r+=`    if ( (i - (i/${d}_cmod)*${d}_cmod) == ${d}_crem ) {
`,G="        "}const be=T=>s.bindings&&s.bindings[T]?`u${s.bindings[T]}`:n<I?`uModularParams[${n++}]`:"0.0";r+=F.glsl({varName:d,in1:E,in2:ye,getParam:be,indent:G}),H&&(r+=`    }}
`)}}r+=`
