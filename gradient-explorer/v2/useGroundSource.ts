...
import { ALL_SET_ID, favientsToEntries, listGroundSets, membersOf, parseSetId, type GroundSetDesc } from '../../palette/core/groundSets';
...
export const useGroundSets = (): GroundSetDesc[] => {
  ...
  const snapshotCount = useVariantsStore((s) => s.variants.length);
...
export const useGroundSource = (setId: string, sets: GroundSetDesc[]): GroundSource | null => {
  ...
  return useMemo<GroundSource | null>(() => {
    ...
    if (parseSetId(setId).kind === 'snapshots') return null; // Phase D.3 wires Snapshots
