...
export const AuthOverlay: React.FC<Props> = ({ open, onClose }) => {
    ...
    return (
        <Modal onClose={onClose} z={Z.overlayTop} dismissOnBackdrop={false} backdropClassName="bg-black/70 backdrop-blur-sm" className="">
            <div
                className="bg-surface-sunken border border-line/10 rounded-lg shadow-[0_10px_40px_rgba(0,0,0,0.8)] w-[400px] max-h-[90vh] overflow-y-auto"
            ...
            >
                <header className="flex items-center justify-between px-4 py-3 border-b border-line/10">
                    ...
                    <button onClick={onClose} className="text-fg-dim hover:text-fg text-lg leading-none">&times;</button>
                </header>
                ...
                {mode === 'check-email' && (
                    <div className="p-6 text-center space-y-3">
                        <div className="text-xs text-fg-tertiary leading-relaxed">
                            We sent a confirmation link to <span className="font-mono text-accent-300">{email}</span>.
                        </div>
                        ...
                        <button
                            ...
                            className="mt-2 text-[10px] text-accent-300 hover:text-cyan-200"
                        >
                            Back to sign in
                        </button>
                    </div>
                ...
                {mode === 'reset-sent' && (
                    <div className="p-6 text-center space-y-3">
                        <div className="text-xs text-fg-tertiary leading-relaxed">
                            If <span className="font-mono text-accent-300">{email}</span> has an account, a password reset link is on its way.
                        </div>
                        <button onClick={() => setMode('signin')} className="mt-2 text-[10px] text-accent-300 hover:text-cyan-200">
                            Back to sign in
                        </button>
                    </div>
                )}
                ...
                {mode === 'signin' && (
                    <form onSubmit={signIn} className="p-4 space-y-3">
                        ...
                        <div className="flex items-center justify-between text-[10px] pt-1">
                            <button type="button" onClick={() => setMode('forgot')} className="text-accent-300 hover:text-cyan-200">
                                Forgot password?
                            </button>
                            <button type="button" onClick={() => setMode('signup')} className="text-fg-muted hover:text-fg">
                                Create account
                            </button>
                        </div>
                ...
                {mode === 'signup' && (
                    <form onSubmit={signUp} className="p-4 space-y-3">
                        ...
                        <button type="button" onClick={() => setMode('signin')} className="w-full text-center text-[10px] text-fg-muted hover:text-fg">
...
const LabelledInput: React.FC<{
    ...
    <div>
        <label className="text-[9px] text-fg-dim font-bold uppercase tracking-wider block mb-1">
            {label}
        </label>
        <input
            type={type}
        ...
        />
        {hint && <div className="text-[9px] text-fg-faint mt-1">{hint}</div>}
    </div>
);

const Divider: React.FC = () => (
    <div className="flex items-center gap-2 text-[9px] text-fg-ghost my-1">
        <div className="flex-1 border-t border-line/5" />
        <span>or</span>
        <div className="flex-1 border-t border-line/5" />
    </div>
);
...
const GoogleButton: React.FC<{ onClick: () => void; busy: boolean }> = ({ onClick, busy }) => (
    <GhostButton
        ...
        onClick={onClick}
        disabled={busy}
        className="w-full py-2 rounded text-[11px] font-bold text-fg-secondary flex items-center justify-center gap-2 disabled:opacity-30"
    >
