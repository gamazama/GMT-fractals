...
export interface FormulaCompat {
    ...
    v3FailGate?: string;
    v4FailGate?: string;
    /** True when a V3 import of this formula can be a weave slot: v3 passes the
     *  honest harness AND the V3 transform lands per-iteration (full-de / V4
     *  imports register `shape:self-contained` → greyed "can't weave" in the
     *  weave picker). Stamped by debug/annotate-catalog-weavable.mts. */
    weavable?: boolean;
}
 ...
 *
 * @assumption The `v3-v4-catalog.json` file was generated by the (since
 * stripped, 77f6d66a) `catalog:build` harness from honest GPU snapshots and is
 * now a frozen artifact; the `weavable` flag on each entry is stamped by
 * `debug/annotate-catalog-weavable.mts` (idempotent, re-run after V3 pipeline
...
/** True when this library formula, imported through the Workshop (V3 pipeline),
 *  can be a weave slot. False for unknown ids / absent catalog — the picker
 *  filter should fail closed rather than advertise unweavable entries. */
export function isFormulaWeavable(id: string): boolean {
    return _compat?.[id]?.weavable === true;
}
