...
uniform bool UpLock; checkbox[false]
...
void main(void)
{
	...
	Upn = UpLock ? vec3(0.0,0.0,1.0) : normalize(Up);
...
vec2 rand2(vec2 co){
return vec2(fract(sin(dot(co.xy ,vec2(rand(co.x)*12.9898,rand(co.y)*78.233))) * 43758.5453), fract(cos(dot(co.xy ,vec2(rand(co.x)*4.898,rand(co.y)*7.23))) * 23421.631));
}
//for iq-clouds
float rand(vec3 pos){
	uint ix = floatBitsToUint(pos.x);
    ...
    // I use three hash calls to untangle pos.xyz
...
uniform int   BloomTaps; slider[1,4,50]
