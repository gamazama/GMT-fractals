...
uniform int RefineSteps; slider[1,4,16];
...
uniform float NormalBackStep; slider[0,1,31]
...
uniform int maxIterAO; slider[0,20,100];
...
uniform float ShadowSoft; slider[0.0,0.0,20]
...
float shadow(vec3 ro, vec3 lightPos, float eps) {
		...
		rd/=lightDist;
		float coneGrad=ShadowSoft;
		coneGrad=lightDist/(ShadowBlur+0.001); 
