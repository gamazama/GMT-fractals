...
export function handleCameraSlotShortcut(e: KeyboardEvent): boolean {
    ...
    if (isCtrl) {
        e.preventDefault();
        e.stopPropagation();
        useEngineStore.getState().saveToSlot(slotIndex);
        return true;
    }
    ...
    const cameras = useEngineStore.getState().savedCameras;
    if (slotIndex < cameras.length) {
        e.preventDefault();
        e.stopPropagation();
        useEngineStore.getState().selectCamera(cameras[slotIndex].id);
    }
    return true;
}
