...
export const CameraManagerPanel: React.FC<CameraManagerPanelProps> = ({ className = "-m-3" }) => {
    const { savedCameras, activeCameraId, addCamera, deleteCamera, selectCamera, updateCamera, resetCamera, duplicateCamera, reorderCameras } = useEngineStore();
    ...
    const liveOffset = useEngineStore(s => s.sceneOffset);
    const liveRot = useEngineStore(s => s.cameraRot);
    ...
    const handleAddCamera = useCallback(async () => {
        ...
        const opticsNow = useEngineStore.getState().optics;
        ...
        if (thumb) {
            const latest = useEngineStore.getState().savedCameras;
    ...
    const handleUpdateCamera = useCallback(async (id: string) => {
        ...
        const dist = engine.lastMeasuredDistance > 0 && engine.lastMeasuredDistance < 1000
            ? engine.lastMeasuredDistance : useEngineStore.getState().targetDistance;
        const currentOptics = { ...(useEngineStore.getState().optics) };
        const thumb = await captureThumbnail();
