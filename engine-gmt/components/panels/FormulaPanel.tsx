...
import type { FormulaType, LfoTarget, PanelId } from '../../../types';
...
import { CompilableFeatureSection } from '../../../components/CompilableFeatureSection';
...
const FormulaPanel = ({ state, actions, onSwitchTab }: { state: FractalState, actions: FractalActions, onSwitchTab?: (tab: PanelId) => void }) => {
  ...
  const getParams = (): (FormulaParam | null)[] => {
    if (state.formula === 'Modular') {
        ...
        ((state as any).pipeline ?? []).forEach((node: any) => {
            ...
            Object.entries(node.bindings).forEach(([paramKey, targetKey]) => {
                ...
                if (tk && boundParams.includes(tk)) {
                    if (!mappings[tk]) mappings[tk] = { labels: [], min: -5, max: 5, step: 0.01 };
