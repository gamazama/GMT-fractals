...
export const usePreviewTarget = (containerRef: React.RefObject<HTMLElement>) => {
    ...
    const outputWidth = useEngineStore(s => s.outputWidth);
    const outputHeight = useEngineStore(s => s.outputHeight);
    ...
    const setPreviewRegion = useEngineStore(s => s.setPreviewRegion);
