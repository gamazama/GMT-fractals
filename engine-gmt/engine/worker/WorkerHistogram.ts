...
/**
 * @assumption Color source path silently no-ops (posts `Float32Array(0)`)
 *   when `pipeline.getOutputTexture()` is null. Main-thread consumer
 *   must handle this.
 * @assumption Per-call `histogramPass.mesh.material` swap — the module-
 ...
 * @assumption Geometry source path MUST zero `uCameraPosition`.
