...
## 13. Queue (owner + orchestrator, 2026-09-06) — what is still lined up

Built and committed on `ge-v2` so far: Phase 1 · S1 Browse · S2 hero · S3 Mix/Image (reworked: session bin, Mix, Image, the strip, the split hero, two-click pick) · S4 Wallpaper. In order:

0. **Room to evaluate (owner, 2026-09-06: "we are not quite there yet").** Before S5 freezes the vocabulary, the owner walks the whole flow — pick, keep, edit, mix, image, bin, share, export, wallpaper — on the light-grey shell and critiques it surface by surface. Nothing in S5/S6 starts until that walk has produced its list. The queue below is the order after it.
1. **Hook up what is still a placeholder.** DONE 2026-09-06: Share copies a link that opens the working gradient (`gradient-explorer/v2/shareUrl.ts`, `?g=`, stops only, harness `test:gx-share`); Export is `ExportMenu.tsx` — every format in `palette/core/exportFormats.ts`, grouped by use (web · design apps · fractal + 3D · code + data), Copy + Download, plus a PNG strip. The S5 formats (.ase, Tailwind, design tokens) join that registry.
2. **Light grey colour scheme** — DONE 2026-09-06 as the seeded default: `v2/main.tsx` applies the engine's `light-grey` preset ONCE per browser (`gmt.ge.themeSeeded`); the switch stays in Settings ▸ Colour. The theme axes are shared across the GMT apps, so app-gmt follows whatever the user last chose, as before. The shell's hard-coded `bg-black` / `bg-white/…` classes became `bg-surface` / `bg-line/10` tokens. Originally: The engine's runtime UI theming (ADR-0080) is the mechanism: the v2 entry picks a light-grey scheme as its default rather than app-gmt's dark one; the hero, strip, wall chrome and picker popovers are re-checked on it (contrast of the dashed armed outline, the white knot strokes, the palette row borders).
3. **S5 Export · Share · labels** — the export dialog grouped by use (+ .ase, Tailwind, design-tokens JSON), the share URL (stops only, or stops + palette rule/N — still open), the label sweep across the three hosts.
4. **Curves mode** — deferred at the owner's word; now a split-hero mode like the others, still to be walked.
5. **S6 phone layout** — sheet, bottom tabs, safe area; the strip becomes the sheet's peek row.
6. **Phase 3** — parity checklist vs the old shell, polish pass, swap the entry point, retire the old shell, push `ge-v2` to `dev` for the preview, `npm run context:map`.
