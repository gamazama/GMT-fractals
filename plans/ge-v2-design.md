...
1. **Hook up what is still a placeholder.** Share and Export in the top bar are toasts ("lands in Phase 2"). Anything else found unwired in the walk-through joins this item.
2. **Light grey colour scheme** for the v2 shell (owner, 2026-09-06). The engine's runtime UI theming (ADR-0080) is the mechanism: the v2 entry picks a light-grey scheme as its default rather than app-gmt's dark one; the hero, strip, wall chrome and picker popovers are re-checked on it (contrast of the dashed armed outline, the white knot strokes, the palette row borders).
