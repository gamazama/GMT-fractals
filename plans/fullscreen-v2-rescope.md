...
- **(b) render** — exactly one `kind`:
  - `'cpuField'` → `field(ctx) => { pos: Float32Array, cov: Float32Array }` (e.g. `sampleGeometry`). The harness samples the LUT at the **float** position in GL (LINEAR-filtered → no 256-step banding) and blends toward the background by coverage, dithering before the 8-bit write. **This is the right kind for any "1D gradient mapped across a 2D field" mode** (all six geometry modes). Dithering does real work here. MUST stay pure + deterministic.
