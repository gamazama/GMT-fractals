...
### P2 — Inert primitive + bridge  · Status: IN PROGRESS (core landed 2026-05-30, `e87498f`)
**Done:** the pure `engine/InteractionSessionMachine.ts` (gesture-activity core, `now`-injected) + `debug/test-interaction-session.mts` (`npm run test:interaction`, 37 assertions green, tsc clean). API as specced: `begin/end/poke/isInteracting(filter)/isIdle(ms)/tickWatchdog/interactionSources`; `hardActive` edge-only, debounce tail on the poll, watchdog backstop.
**Remaining (next P2 session):** `createInteractionSlice` wrapper (owns the machine in a module ref, throttles poke ~50ms, writes the reactive boolean only on `edge`) · `useInteractionDrag` hook (dispatch-only, `onPointerDown/Up/LostPointerCapture/Cancel` + unmount cleanup) · worker bridge (`interacting` in `renderState`, UNUSED) + read-path test · per-consumer kill-switch flags · dev overlay (`debug_tools` toggle) · latency+outcome harness → `debug/interaction-latency-baseline.json` · prove sibling (fluid/fractal-toy) inertness · add `test:interaction` to a pre-push gate.
