...
### Architecture Rules
- **Features are isolated.** A feature's state lives at `store[featureId]`. Reading another feature's state requires declaring `dependsOn: ['otherId']` in the feature def. **This is a convention, not a runtime guarantee** — `dependsOn` drives registration ordering and a register-time existence check, and nothing intercepts an undeclared read. (Corrected 2026-09-01: this line previously claimed "undeclared access throws in dev, warns in prod". Grep `dependsOn` in `engine/FeatureSystem.ts` — there is no read-time interception of any kind.) See `docs/history/engine/02_Feature_Registry.md`.
