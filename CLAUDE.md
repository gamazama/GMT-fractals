...
| **Audio modulation (VJ rig)** — WebAudio graph + live-input constraints, FFT→rule pipeline, Hz band mapping, the live-session hold across scene loads, the THREE durable routing-string stores | JSDoc on `engine/features/audioMod/AudioAnalysisEngine.ts` + `freqScale.ts` + `engine/features/modulation/ModulationEngine.ts` + `engine-gmt/animation/retargetTracks.ts` | ADR-0103 |
