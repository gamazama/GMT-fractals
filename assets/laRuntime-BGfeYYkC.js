var E=Object.defineProperty;var P=(r,t,i)=>t in r?E(r,t,{enumerable:!0,configurable:!0,writable:!0,value:i}):r[t]=i;var s=(r,t,i)=>P(r,typeof t!="symbol"?t+"":t,i);const X=256;class J{constructor(t){s(this,"mainTex",null);s(this,"collisionTex",null);s(this,"interiorTex",null);s(this,"version",0);this.gl=t}getTexture(t){return t==="main"?this.mainTex:t==="collision"?this.collisionTex:this.interiorTex}setBuffer(t,i){const e=this.gl,a=256*4;i.length!==a&&console.warn(`[GradientLut] ${t} buffer length ${i.length} (want ${a})`);let o=this.getTexture(t);o||(o=e.createTexture(),t==="main"?this.mainTex=o:t==="collision"?this.collisionTex=o:this.interiorTex=o),e.activeTexture(e.TEXTURE0),e.bindTexture(e.TEXTURE_2D,o),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MIN_FILTER,e.LINEAR),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MAG_FILTER,e.LINEAR),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_S,e.REPEAT),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_T,e.CLAMP_TO_EDGE),e.texImage2D(e.TEXTURE_2D,0,e.RGBA,256,1,0,e.RGBA,e.UNSIGNED_BYTE,i),this.version++}ensure(t){if(this.getTexture(t))return;const i=256,e=new Uint8Array(i*4);if(t==="collision")for(let a=0;a<i;++a)e[a*4+3]=255;else for(let a=0;a<i;++a)e[a*4+0]=a,e[a*4+1]=a,e[a*4+2]=a,e[a*4+3]=255;this.setBuffer(t,e)}dispose(){const t=this.gl;this.mainTex&&(t.deleteTexture(this.mainTex),this.mainTex=null),this.collisionTex&&(t.deleteTexture(this.collisionTex),this.collisionTex=null),this.interiorTex&&(t.deleteTexture(this.interiorTex),this.interiorTex=null)}}const y=(r,t)=>{const i=r+t,e=i-r,a=r-(i-e)+(t-e);return[i,a]},q=(r,t,i)=>{const[e,a]=y(r,i),[o,n]=y(e,t+a);return[o,n]},R=(r,t,i,e)=>{const[a,o]=y(r,-i),[n,l]=y(a,o+(t-e));return[n,l]},V=`
...
}`,L=1.4,F=1e-4,N=1e-100,U=2e5,ee=(r,t=1)=>{const i=Math.log10(L/Math.max(r,F)),e=Math.max(200,Math.min(2e3,200+220*Math.max(0,i)));return Math.round(e*Math.max(0,t))},j=r=>{const t=Math.log10(L/Math.max(r,N));return Math.round(Math.min(2e4,Math.max(2e3,1500+900*Math.max(0,t))))},Z=(r,t=1)=>Math.round(Math.min(U,j(r)*Math.max(0,t))),te=(r,t,i,e=1)=>i>0?Z(r,e):Math.max(Math.round(200*Math.min(1,Math.max(0,e))),t),G=r=>{const t=(r&32768)>>15,i=(r&31744)>>10,e=r&1023;return i===0?(t?-1:1)*Math.pow(2,-14)*(e/1024):i===31?e?NaN:t?-1/0:1/0:(t?-1:1)*Math.pow(2,i-15)*(1+e/1024)},I=(r,t,i,e,a)=>{const o=t*i;let n=0;for(let l=0;l<r.length;l++)if(n+=r[l],n>=o)return e+(l+.5)*(a-e)/r.length;return a};function re(r,t,i,e,a){const o=i*e;if(o<=0)return null;r.bindFramebuffer(r.FRAMEBUFFER,t),r.readBuffer(r.COLOR_ATTACHMENT0);const n=r.getParameter(r.IMPLEMENTATION_COLOR_READ_TYPE);let l;if(n===r.FLOAT){const c=new Float32Array(o*4);r.readPixels(0,0,i,e,r.RGBA,r.FLOAT,c),l=u=>c[u*4+1]}else{const c=new Uint16Array(o*4);r.readPixels(0,0,i,e,r.RGBA,r.HALF_FLOAT,c),l=u=>G(c[u*4+1])}const d=r.getError();if(r.bindFramebuffer(r.FRAMEBUFFER,null),d!==r.NO_ERROR)return console.warn("[fitIterationRange] readPixels failed",d,"readType",n),null;const m=Math.max(a,1);let f=1/0,h=-1/0,v=0;for(let c=0;c<o;c++){const u=l(c);!Number.isFinite(u)||u<=1e-4||u>=.999||(u<f&&(f=u),u>h&&(h=u),v++)}if(v===0||h<=f)return console.warn("[fitIterationRange] no escaped pixels in view to fit"),null;const x=256,b=new Int32Array(x),T=(x-1)/(h-f);for(let c=0;c<o;c++){const u=l(c);!Number.isFinite(u)||u<=1e-4||u>=.999||b[Math.round((u-f)*T)]++}const g=I(b,v,.02,f,h),p=I(b,v,.98,f,h),z=Math.log(1+Math.max(g*m,0)),M=Math.log(1+Math.max(p*m,0)),k=Math.max(M-z,.001);return{offset:z,scale:1/k}}const B=r=>{if(!Number.isFinite(r)||r===0)return{mantissa:0,exp:0};const t=Math.floor(Math.log2(Math.abs(r)));return{mantissa:r/Math.pow(2,t),exp:t}},H=r=>[r.mantissa,r.exp],w=r=>H(B(r)),C=(r,t,i,e)=>{i&&(r.activeTexture(r.TEXTURE0+t),r.bindTexture(r.TEXTURE_2D,i),r.uniform1i(e,t))};class ae{constructor(t){s(this,"refOrbitTex",null);s(this,"refOrbitTexW",2048);s(this,"refOrbitTexH",0);s(this,"refOrbitLen",0);s(this,"refOrbitCenter",[0,0]);s(this,"refOrbitCenterLow",[0,0]);s(this,"refOrbitPeriod",0);s(this,"laTableTex",null);s(this,"laTableTexW",1024);s(this,"laTableTexH",0);s(this,"laTotalCount",0);s(this,"laStages",new Float32Array(0));s(this,"laStageCount",0);s(this,"laEnabled",!1);s(this,"atPayload",null);s(this,"version",0);this.gl=t}hasOrbit(){return this.refOrbitTex!==null&&this.refOrbitLen>1}setReferenceOrbit(t,i,e,a=[0,0],o=0){this.refOrbitCenter=[e[0],e[1]],this.refOrbitCenterLow=[a[0],a[1]],this.refOrbitPeriod=o,this.uploadOrbitTexture(t,i),this.refOrbitLen=i,this.version++}clearReferenceOrbit(){this.refOrbitLen=0,this.refOrbitPeriod=0,this.version++}setLATable(t,i,e){this.uploadLaTexture(t,i),this.laTotalCount=i,this.laStages=e,this.laStageCount=e.length/2,this.version++}setLAEnabled(t){this.laEnabled=t}clearLATable(){this.laTotalCount=0,this.laStages=new Float32Array(0),this.laStageCount=0,this.version++}setAT(t){this.atPayload=t,this.version++}clearAT(){this.atPayload!==null&&(this.atPayload=null,this.version++)}bindUniforms(t,i,e){const a=this.gl,o=i.deepZoomEnabled&&this.hasOrbit();a.uniform1i(t.uniforms.uDeepZoomEnabled,o?1:0),a.uniform1i(t.uniforms.uRefOrbitTexW,this.refOrbitTexW),a.uniform1i(t.uniforms.uRefOrbitLen,this.refOrbitLen),a.uniform1i(t.uniforms.uRefPeriod,o?this.refOrbitPeriod:0);const n=R(i.center[0],i.centerLow[0],this.refOrbitCenter[0],this.refOrbitCenterLow[0]),l=R(i.center[1],i.centerLow[1],this.refOrbitCenter[1],this.refOrbitCenterLow[1]),d=n[0]+n[1],m=l[0]+l[1],f=w(d),h=w(m);a.uniform4f(t.uniforms.uDeepCenterOffset,f[0],f[1],h[0],h[1]);const v=w(i.zoom);a.uniform2f(t.uniforms.uDeepScale,v[0],v[1]),C(a,6,this.refOrbitTex??e,t.uniforms.uRefOrbit);const x=o&&this.laEnabled&&this.laTableTex!==null&&this.laTotalCount>1;if(a.uniform1i(t.uniforms.uLAEnabled,x?1:0),a.uniform1i(t.uniforms.uLATexW,this.laTableTexW),a.uniform1i(t.uniforms.uLATotalCount,this.laTotalCount),a.uniform1i(t.uniforms.uLAStageCount,this.laStageCount),this.laStageCount>0){const T=Math.min(this.laStageCount,64),g=new Float32Array(T*4);for(let p=0;p<T;p++)g[p*4+0]=this.laStages[p*2+0],g[p*4+1]=this.laStages[p*2+1];a.uniform4fv(t.uniforms["uLAStages[0]"],g)}C(a,7,this.laTableTex??e,t.uniforms.uLATable);const b=o&&this.atPayload!==null;a.uniform1i(t.uniforms.uATEnabled,b?1:0),this.atPayload?(a.uniform1i(t.uniforms.uATStepLength,this.atPayload.stepLength),a.uniform1f(t.uniforms.uATThresholdC,this.atPayload.thresholdC),a.uniform1f(t.uniforms.uATSqrEscapeRadius,this.atPayload.sqrEscapeRadius),a.uniform2f(t.uniforms.uATRefC,this.atPayload.refC[0],this.atPayload.refC[1]),a.uniform2f(t.uniforms.uATCCoeff,this.atPayload.ccoeff[0],this.atPayload.ccoeff[1]),a.uniform2f(t.uniforms.uATInvZCoeff,this.atPayload.invZCoeff[0],this.atPayload.invZCoeff[1])):(a.uniform1i(t.uniforms.uATStepLength,1),a.uniform1f(t.uniforms.uATThresholdC,0),a.uniform1f(t.uniforms.uATSqrEscapeRadius,4),a.uniform2f(t.uniforms.uATRefC,0,0),a.uniform2f(t.uniforms.uATCCoeff,1,0),a.uniform2f(t.uniforms.uATInvZCoeff,1,0))}dispose(){const t=this.gl;this.refOrbitTex&&(t.deleteTexture(this.refOrbitTex),this.refOrbitTex=null),this.laTableTex&&(t.deleteTexture(this.laTableTex),this.laTableTex=null)}uploadOrbitTexture(t,i){const e=this.gl,a=this.refOrbitTexW,o=Math.max(1,Math.ceil(i/a)),n=a*o*4,l=t.length>=n?t.subarray(0,n):(()=>{const d=new Float32Array(n);return d.set(t),d})();this.refOrbitTex||(this.refOrbitTex=S(e),this.refOrbitTexH=0),e.bindTexture(e.TEXTURE_2D,this.refOrbitTex),o!==this.refOrbitTexH?(e.texImage2D(e.TEXTURE_2D,0,e.RGBA32F,a,o,0,e.RGBA,e.FLOAT,l),this.refOrbitTexH=o):e.texSubImage2D(e.TEXTURE_2D,0,0,0,a,o,e.RGBA,e.FLOAT,l)}uploadLaTexture(t,i){const e=this.gl,a=i*3,o=this.laTableTexW,n=Math.max(1,Math.ceil(a/o)),l=o*n*4,d=t.length>=l?t.subarray(0,l):(()=>{const m=new Float32Array(l);return m.set(t),m})();this.laTableTex||(this.laTableTex=S(e),this.laTableTexH=0),e.bindTexture(e.TEXTURE_2D,this.laTableTex),n!==this.laTableTexH?(e.texImage2D(e.TEXTURE_2D,0,e.RGBA32F,o,n,0,e.RGBA,e.FLOAT,d),this.laTableTexH=n):e.texSubImage2D(e.TEXTURE_2D,0,0,0,o,n,e.RGBA,e.FLOAT,d)}}const S=r=>{const t=r.createTexture();return r.bindTexture(r.TEXTURE_2D,t),r.texParameteri(r.TEXTURE_2D,r.TEXTURE_MIN_FILTER,r.NEAREST),r.texParameteri(r.TEXTURE_2D,r.TEXTURE_MAG_FILTER,r.NEAREST),r.texParameteri(r.TEXTURE_2D,r.TEXTURE_WRAP_S,r.CLAMP_TO_EDGE),r.texParameteri(r.TEXTURE_2D,r.TEXTURE_WRAP_T,r.CLAMP_TO_EDGE),t};class W{constructor(){s(this,"worker",null);s(this,"nextId",1);s(this,"pending",new Map)}ensureWorker(){if(this.worker)return this.worker;const t=new Worker(new URL(""+new URL("deepZoomWorker-9GnfDDzm.js",import.meta.url).href,import.meta.url),{type:"module"});return t.onmessage=i=>{const e=i.data,a=this.pending.get(e.id);a&&(this.pending.delete(e.id),e.type==="orbit"?a.resolve({orbit:new Float32Array(e.orbit),length:e.length,escaped:e.escaped,precisionBits:e.precisionBits,buildMs:e.buildMs,laBuildMs:e.laBuildMs??0,refCenterX:e.refCenterX,refCenterY:e.refCenterY,refCenterLowX:e.refCenterLowX,refCenterLowY:e.refCenterLowY,relocated:e.relocated??!1,laUnsafe:e.laUnsafe??!1,period:e.period??0,laEpsilonLog2:e.laEpsilonLog2,laTable:e.laTable?new Float32Array(e.laTable):void 0,laStages:e.laStages?new Float32Array(e.laStages):void 0,laCount:e.laCount??0,laStageCount:e.laStageCount??0,at:e.at}):a.reject(new Error(e.message)))},t.onerror=i=>{var a;const e=new Error(`deep-zoom worker crashed: ${i.message}`);for(const o of this.pending.values())o.reject(e);this.pending.clear(),(a=this.worker)==null||a.terminate(),this.worker=null},this.worker=t,t}computeReferenceOrbit(t){const i=this.ensureWorker(),e=this.nextId++;return new Promise((a,o)=>{this.pending.set(e,{resolve:a,reject:o});const n={type:"computeOrbit",id:e,...t};i.postMessage(n)})}cancel(t){if(!this.worker)return;const i={type:"cancel",id:t};this.worker.postMessage(i),this.pending.delete(t)}dispose(){this.worker&&(this.worker.terminate(),this.worker=null),this.pending.clear()}}let A=null;const ie=()=>(A||(A=new W),A);export{ae as D,$ as F,D as G,V as O,Y as V,J as a,Q as b,ee as c,q as d,Z as e,re as f,te as g,ie as h,j as i,X as j};
