...
This rotation is bound to 'ParamC' (Slider below). Try dragging it!`},{id:"rot-1",type:"Rotate",enabled:!0,params:{x:0,y:0,z:0},bindings:{z:"ParamC"}},{id:"bulb-1",type:"Mandelbulb",enabled:!0,params:{power:8}},{id:"add-c",type:"AddConstant",enabled:!0,params:{scale:1}}],Vo=(e,o)=>({pipeline:xt,pipelineRevision:1,graph:gt(xt),setGraph:t=>{const a=mt(t.nodes,t.edges),i=o(),r=!Vt(i.pipeline,a),n=r||!No(i.pipeline,a);if(r&&i.autoCompile){const l=i.pipelineRevision+1;e({graph:t,pipeline:a,pipelineRevision:l}),w.emit($.CONFIG,{pipeline:a,graph:t,pipelineRevision:l})}else r?e({graph:t}):n?(e({graph:t,pipeline:a}),w.emit($.CONFIG,{pipeline:a})):e({graph:t})},setPipeline:t=>{const a=o().pipelineRevision+1,i=gt(t);e({pipeline:t,graph:i,pipelineRevision:a}),w.emit($.CONFIG,{pipeline:t,graph:i,pipelineRevision:a})},refreshPipeline:()=>{const t=o(),a=mt(t.graph.nodes,t.graph.edges),i=t.pipelineRevision+1;e({pipeline:a,pipelineRevision:i}),w.emit($.CONFIG,{pipeline:a,graph:t.graph,pipelineRevision:i})}}),St=`
...
`,defaults:{hybridScale:2,hybridFoldLimitVec:{x:1,y:1,z:1},hybridMinR:.3,hybridFixedR:1,hybridIter:4}},pa={id:"menger",label:"Menger (Cubic)",glsl:`
void foldOperation(inout vec3 z, inout float dr, vec3 foldLimit) {
...
`,selfContained:!0,extraParams:{hybridMengerOffset:{type:"vec3",default:new E(1,1,1),label:"Offset",shortId:"hmo",uniform:"uHybridMengerOffset",min:0,max:2,step:.01,group:"hybrid",linkable:!0},hybridMengerCenterZ:{type:"boolean",default:!0,label:"Center Z",shortId:"hmz",uniform:"uHybridMengerCenterZ",group:"hybrid"}},defaults:{hybridScale:3,hybridFoldLimitVec:{x:1,y:1,z:1},hybridMinR:.5,hybridFixedR:1,hybridIter:3,hybridMengerOffset:{x:1,y:1,z:1},hybridMengerCenterZ:!0}},Be=[ra,na,sa,la,ca,da,ua,fa,pa],ha=Be.map((e,o)=>({label:e.label,value:o}));function ma(e){return Be[e]??Be[0]}const ga=`
...
`,ya=["xyz","xzy","yxz","yzx","zxy","zyx"];function ba(e){const o=ya[e]??"xyz";return o==="xyz"?"vec3 c_perm = c.xyz;":`vec3 c_perm = c.${o};`}function va(e,o,t=!1){return`
mat3 hybridRotMat;
bool hybridHasRot;
...
`,l+=`
                ...
                `}e.addHybridFold("",n,l)}},wt=["uParamA","uParamB","uParamC","uParamD","uParamE","uParamF"],It=["uVec2A","uVec2B","uVec2C"],Ct=["uVec3A","uVec3B","uVec3C"],Mt=["uVec4A","uVec4B","uVec4C"],qt=["uInterlaceParamA","uInterlaceParamB","uInterlaceParamC","uInterlaceParamD","uInterlaceParamE","uInterlaceParamF"],Ut=["uInterlaceVec2A","uInterlaceVec2B","uInterlaceVec2C"],Wt=["uInterlaceVec3A","uInterlaceVec3B","uInterlaceVec3C"],Xt=["uInterlaceVec4A","uInterlaceVec4B","uInterlaceVec4C"];function _a(){const e=[];for(let o=0;o<wt.length;o++)e.push([new RegExp(`\\b${wt[o]}\\b`,"g"),qt[o]]);for(let o=0;o<It.length;o++)e.push([new RegExp(`\\b${It[o]}\\b`,"g"),Ut[o]]);for(let o=0;o<Ct.length;o++)e.push([new RegExp(`\\b${Ct[o]}\\b`,"g"),Wt[o]]);for(let o=0;o<Mt.length;o++)e.push([new RegExp(`\\b${Mt[o]}\\b`,"g"),Xt[o]]);return e}const wa=_a();function nt(e){let o=e;for(const[t,a]of wa)o=o.replace(t,a);return o}function Ge(e,o){const t=[...o].sort((i,r)=>r.length-i.length);let a=e;for(const i of t)a=a.replace(new RegExp(`\\b${i}\\b`,"g"),`interlace_${i}`);return a}function Ne(e){const o=[],t=/\b(?:void|vec[234]|float|int|mat[234]|bool)\s+(\w+)\s*\(/g,a=/^\s*(?:const\s+)?(?:vec[234]|float|int|mat[234]|bool)\s+([^;]+);/,i=/^\s*(\w+)/;let r=0;for(const n of e.split(`
`)){if(r===0){t.lastIndex=0;let l;for(;(l=t.exec(n))!==null;){const c=l[1];!c.startsWith("formula_")&&!o.includes(c)&&o.push(c)}const s=a.exec(n);if(s){const c=[];let d=0,h="";for(const p of s[1]){if(p==="(")d++;else if(p===")")d--;else if(p===","&&d===0){c.push(h),h="";continue}h+=p}h&&c.push(h);for(const p of c){const f=i.exec(p);f&&!o.includes(f[1])&&o.push(f[1])}}}for(const l of n)l==="{"?r++:l==="}"&&r--}return o}function Ia(e,o,t){let a=e;a=a.replace(new RegExp(`\\b${o}_\\w+\\b`,"g"),r=>`interlace_${r}`);const i=Ne(e);for(const r of i)a=a.replace(new RegExp(`\\b${r}\\b`,"g"),`interlace_${r}`);return t&&t.length>0&&(a=Ge(a,t)),a=nt(a),a}function Ca(e,o,t,a){let i=e;if(i=i.replace(new RegExp(`\\bformula_${o}\\b`,"g"),"formula_Interlace"),i=nt(i),t&&t.length>0&&(i=Ge(i,t)),a&&a.length>0)for(const r of a)i=i.replace(new RegExp(`\\b${r}\\b`,"g"),`interlace_${r}`);return i}function Ma(e,o,t){let a=e.replace(new RegExp(`\\bformula_${o}\\b`,"g"),"formula_Interlace");return t&&t.length>0&&(a=Ge(a,t)),a}function Ra(e,o,t,a){let i=e;if(i=i.replace(new RegExp(`\\b${o}_\\w+\\b`,"g"),r=>`interlace_${r}`),i=nt(i),t&&t.length>0&&(i=Ge(i,t)),a&&a.length>0)for(const r of a)i=i.replace(new RegExp(`\\b${r}\\b`,"g"),`interlace_${r}`);return i}function Ea(e,o,t){let a="";o&&(a=`
...
`});F.register({id:"Custom",label:"Custom (Legacy)",category:"Utils",description:"Legacy node.",inputs:[],glsl:e=>""});const Da=e=>{const o=new Map;return e.forEach(t=>{o.has(t.target)||o.set(t.target,[]),o.get(t.target).push(t)}),o},Fa=e=>{const o=new Set,t=["root-end"],a=new Set;for(;t.length>0;){const i=t.pop();a.has(i)||(a.add(i),i!=="root-end"&&i!=="root-start"&&o.add(i),(e.get(i)??[]).forEach(r=>t.push(r.source)))}return o},Oa=(e,o)=>{const t=Da(o),a=Fa(t),i=e.filter(d=>a.has(d.id));if(!i||i.length===0)return`
...
`},Na=e=>{let o="d = 0.5 * log(max(r, 1.0e-5)) * r / dr_safe;";return e<.5?o=`
        float logR2 = log2(m2);
...
`,n=i.shader.loopBody,l=i.shader.loopInit||"",i.shader.preamble&&e.addPreamble(i.shader.preamble),i.shader.getDist&&(c=`vec2 getDist(float r, float dr, float iter, vec4 z) { ${i.shader.getDist} }`));e.addFunction(r),e.setFormula(n,l,c)}};let Ha=0;function Me(){return`l${++Ha}`}const $a=(e,o)=>{if(!e)return`
