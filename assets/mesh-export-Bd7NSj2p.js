...
`),c=hn(this.useRotation),m=oo(this.formulaLoopBody,this.formulaInit,this.formulaDist,this.hybridInit.join(`
`),this.hybridPreLoop.join(`
`),this.hybridInLoop.join(`
`),this.distOverrideInit,this.distOverrideInLoopFull,this.distOverrideInLoopGeom,this.distOverridePostFull,this.distOverridePostGeom,this.postMapCode.join(`
`),this.postDistCode.join(`
...
`)),p=no(this.renderMode),F=En(this.postProcessLogic.join(`
`)),h=[["Defines",t],["Uniforms",o],["Headers",n],["Math",c],["BlueNoise",ut],["Coloring",lt],["Preambles",s],["Formulas",i],["DE",m],["PostDE",a],["MatEval",x],["MissHandler",d],["Ray",p],["Trace",v],["TraceLean",g],["Integrators",r],["Post",F],["Main",M]];return h.filter(([,y])=>y.length>0).map(([y,w])=>`${y}: ${(w.length/1024).toFixed(1)}kb`).join(" | "),h.reduce((y,[,w])=>y+w.length,0),`
${t}
${o}
${n}
${c}
...
`)[0])}return s}function rt(e,t,o,n){const s=e.createProgram();if(!s)throw new Error("Failed to create program");if(e.attachShader(s,co(e,e.VERTEX_SHADER,t,n)),e.attachShader(s,co(e,e.FRAGMENT_SHADER,o,n)),e.linkProgram(s),!e.getProgramParameter(s,e.LINK_STATUS)){const i=e.getProgramInfoLog(s)||"";throw n("Program link error: "+i,"error"),new Error("Program link: "+i)}return s}function vt(){const e=document.createElement("canvas");e.width=2048,e.height=2048;const t=e.getContext("webgl2",{antialias:!1});if(!t)throw new Error("WebGL2 not supported");return t.getExtension("EXT_color_buffer_float"),t.getExtension("OES_texture_float_linear"),t}function Wt(e,t){const o={};for(let n=0;n<He.length;n++)o[He[n]]=e.getUniformLocation(t,He[n]);return o}function Ze(e,t,o,n,s,i,a){ke.register(o),i&&ke.register(i.definition);const r=Pn.generateMeshSDFLibrary(Wn(o,i,a)),c=n,m=`#version 300 es
precision highp float;
uniform float uZ;
uniform float uPower;
uniform int   uIters;
