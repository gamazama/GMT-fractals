...
This rotation is bound to 'ParamC' (Slider below). Try dragging it!`},{id:"rot-1",type:"Rotate",enabled:!0,params:{x:0,y:0,z:0},bindings:{z:"ParamC"}},{id:"bulb-1",type:"Mandelbulb",enabled:!0,params:{power:8}},{id:"add-c",type:"AddConstant",enabled:!0,params:{scale:1}}],Go=(e,o)=>({pipeline:xt,pipelineRevision:1,graph:gt(xt),setGraph:t=>{const a=mt(t.nodes,t.edges),i=o(),r=!Vt(i.pipeline,a),n=r||!jo(i.pipeline,a);if(r&&i.autoCompile){const l=i.pipelineRevision+1;e({graph:t,pipeline:a,pipelineRevision:l}),I.emit($.CONFIG,{pipeline:a,graph:t,pipelineRevision:l})}else r?e({graph:t}):n?(e({graph:t,pipeline:a}),I.emit($.CONFIG,{pipeline:a})):e({graph:t})},setPipeline:t=>{const a=o().pipelineRevision+1,i=gt(t);e({pipeline:t,graph:i,pipelineRevision:a}),I.emit($.CONFIG,{pipeline:t,graph:i,pipelineRevision:a})},refreshPipeline:()=>{const t=o(),a=mt(t.graph.nodes,t.graph.edges),i=t.pipelineRevision+1;e({pipeline:a,pipelineRevision:i}),I.emit($.CONFIG,{pipeline:a,graph:t.graph,pipelineRevision:i})}}),St=`
...
`});F.register({id:"Custom",label:"Custom (Legacy)",category:"Utils",description:"Legacy node.",inputs:[],glsl:e=>""});const Fa=e=>{const o=new Map;return e.forEach(t=>{o.has(t.target)||o.set(t.target,[]),o.get(t.target).push(t)}),o},Oa=e=>{const o=new Set,t=["root-end"],a=new Set;for(;t.length>0;){const i=t.pop();a.has(i)||(a.add(i),i!=="root-end"&&i!=="root-start"&&o.add(i),(e.get(i)??[]).forEach(r=>t.push(r.source)))}return o},Na=(e,o)=>{const t=Fa(o),a=Oa(t),i=e.filter(d=>a.has(d.id));if(!i||i.length===0)return`
    ...
    `;const n=new Map;n.set("root-start","v_start");let l=0;i.forEach((d,h)=>{const f=`v_${d.id.replace(/[^a-zA-Z0-9]/g,"")}`;n.set(d.id,f);const m=t.get(d.id)??[],b=m.find(_=>!_.targetHandle||_.targetHandle==="a"),y=m.find(_=>_.targetHandle==="b"),g=b&&n.get(b.source)||"v_start",v=y&&n.get(y.source)||"v_start";if(r+=`    // Node: ${d.type} (${d.id})
`,r+=`    vec3 ${f}_p = ${g}_p;
`,r+=`    float ${f}_d = ${g}_d;
`,r+=`    float ${f}_dr = ${g}_dr;
`,d.enabled){const _=F.get(d.type);if(_){const M=d.condition&&d.condition.active;let x="    ";if(M){const P=l<Ce?`uModularParams[${l++}]`:"2.0",A=l<Ce?`uModularParams[${l++}]`:"0.0";r+=`    { int ${f}_cmod = max(1, int(${P})); int ${f}_crem = int(${A});
`,r+=`    if ( (i - (i/${f}_cmod)*${f}_cmod) == ${f}_crem ) {
`,x="        "}const L=P=>d.bindings&&d.bindings[P]?`u${d.bindings[P]}`:l<Ce?`uModularParams[${l++}]`:"0.0";r+=_.glsl({varName:f,in1:g,in2:v,getParam:L,indent:x}),M&&(r+=`    }}
`)}}r+=`
