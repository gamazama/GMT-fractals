...
| `core/importGradientFiles.ts`, `store/favientFiling.ts`, `store/groundSet.ts`, `store/favientsStore.ts` `removeGroup`, `core/groundSets.ts` `membersOfMany` + the empty-group chip, `store/favientsPanelPersist.ts` `setV2FavientsPanelKey` (the shelf's MANAGE surface, 2026-09-09 — §8b item 4) | `debug/test-palette-shelf-manage.mts` (`npm run test:palette-shelf`; falsified eleven ways the day it was written, three of them assertions that passed under mutation first and were rewritten — see its header) |
