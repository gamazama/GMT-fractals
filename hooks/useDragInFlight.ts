 ...
 * tracker for the drag/drop affordances (P0e interface (b)).
 *
 * Wires `dragFlightReducer` (store/dragFlight.ts) to window drag events:
 ...
 * listener (see the coexistence note in ImageStage).
 *
 ...
 *   current drag, suitable for a send target's `acceptsTypes` drag-visibility filter.
 */
 ...
 *   reports a permanent idle state — lets a consumer (e.g. `DropTargetLayer`)
 *   stay free until at least one target is registered. Defaults true.
 */
