...
export const FluidToggleButton: React.FC = () => {
    ...
    return (
        <button
            ...
            className={`flex items-center gap-1.5 px-2 h-7 rounded text-[12px] border transition-colors ${
                on
                    ? 'text-cyan-300 bg-cyan-900/30 border-cyan-500/40'
                    : 'text-gray-400 border-white/10 hover:text-white hover:border-cyan-500/40'
            }`}
        >
            {/* water-drop glyph */}
            ...
            <span className="hidden md:inline">Fluid {on ? 'On' : 'Off'}</span>
        </button>
    );
};
