...
Docs read first: [`engine/08_Animation.md`](../docs/history/engine/08_Animation.md), [`gmt/04_Animation_Engine.md`](../docs/history/gmt/04_Animation_Engine.md), [`gmt/24_Formula_Interlace_System.md`](../docs/history/gmt/24_Formula_Interlace_System.md). Selection state is already shared across views (per [§7.2 SelectionSlice](../docs/history/gmt/04_Animation_Engine.md) — `setTrackSelection` / `selectKeyframe` etc. live in `store/animation/selectionSlice.ts`); group-collapse state is the missing piece. Track grouping for both views is centralised in [`utils/groupTracks.ts`](../engine-gmt/utils/groupTracks.ts) per the docs — that's the single file behind #13 and #14.
...
**Constraint:** changing labels does NOT change track IDs (per [doc 04 §2.2](../docs/history/gmt/04_Animation_Engine.md)) — saved presets keep working. Labels are display-only.
...
**Where:**
- DopeSheet has it: [components/timeline/SelectionTransformBar.tsx:10-34](engine-gmt/components/timeline/SelectionTransformBar.tsx#L10) — orange move + left/right scale handles. Per [doc 04 §8](../docs/history/gmt/04_Animation_Engine.md), TrackGroup renders one bar per group when ≥2 keys selected.
...
**Out of scope:** soft-selection falloff visualisation around the bbox (per [doc 04 §7.2](../docs/history/gmt/04_Animation_Engine.md), soft selection is settings-based — leave the existing Linear/Dome/Pinpoint/S-Curve UI alone).
