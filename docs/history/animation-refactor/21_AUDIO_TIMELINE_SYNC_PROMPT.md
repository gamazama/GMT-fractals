...
4. **[`docs/history/animation-refactor/00_INDEX.md`](./00_INDEX.md)** §"Audio + multi-key bbox" — the original audio integration landed in the `audio_timeline_phase` work (memory pointer: `memory/project_audio_timeline_phase.md`). The README of that landed work captures the original seconds↔frames mapping decisions.
