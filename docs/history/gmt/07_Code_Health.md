...
**Last Updated:** 2026-05-26 (FormulaPicker landing + deferred consolidation entries)
...
| **Portal-anchored popover layout helper** | `engine-gmt/components/panels/formula/FormulaGallery.tsx` (PortalDropdown), `components/CategoryPickerMenu.tsx`, `engine-gmt/components/FormulaPicker/FormulaPicker.tsx` (PopoverShell) | Three independent implementations of "anchor a portal'd box to a trigger DOMRect with viewport-clamped horizontal + flip-up vertical positioning." Extract `computeAnchoredBox(anchorRect, { width, minHeight, maxHeight, padding })` → `{ style, flipped }` into `engine-gmt/hooks/` or `utils/`. Hover-preview placement in FormulaPicker would also collapse into this. |
| **`useDismissOnOutside` hook** | Same three call sites as above, plus `components/Popover.tsx` (Escape-only variant) | Outside-click + close-on-resize + close-on-wheel handlers re-implemented per site. Extract a single hook taking `(ref, onClose, { closeOnResize?, closeOnWheel?, closeOnEscape? })`. Four call sites today; will be five once the New Scene wizard lands. |
| **AutoFeaturePanel custom-widget extension point** | `components/AutoFeaturePanel.tsx:340-359` special-cases `key === 'interlaceFormula'` to route to `componentRegistry`'s `'interlace-secondary-picker'` instead of the flat `<Dropdown>`. | Each new per-param custom widget will grow this string-match list. Generalize via an opt-in field — either `customUI` with a `replace: true` semantic (currently customUI only *appends* children), or a new `customWidget?: string` field on `ParamConfig` looked up in `componentRegistry`. Wait until at least one more feature wants a per-param widget before promoting. |
| **`menuConfig.panelId` field for system-menu panel surfacing** | `engine-gmt/topbar.tsx:474-481` hardcodes `feat.id === 'audio' / 'drawing'` → panel id when activating the system-menu toggle. Used to call `togglePanel(panelId, true)` so the dock surfaces. | Two call sites currently; will grow if more features get menu toggles + dedicated panels. Add `panelId?: string` to the feature's `menuConfig` and read it from the generic loop — same shape as the existing `togglePanel` reveal of the Engine Config panel. |
| **FormulaPicker `PickerBody` props grouping** | `engine-gmt/components/FormulaPicker/FormulaPicker.tsx` — `PickerBody` takes ~35 props in clusters (search, view, sidebar, grid). | Single call site, so the prop sprawl is contained; flagged only because the clusters are obvious. Either inline `PickerBody` into `FormulaPicker` (only consumer) or group into `search` / `view` / `sidebar` / `grid` prop objects. Wait for a second consumer before deciding. |
| **FormulaPicker grid/list + scene/formula card duplication** | `engine-gmt/components/FormulaPicker/FormulaPicker.tsx` — `GridItems` vs `ListItems` are near-identical maps; `FormulaCard` vs `SceneCard` and `FormulaRow` vs `SceneRow` repeat the disabled/selected/focused class-mux + thumbnail + label scaffold. | Acceptable as-is — the components are short (~25 lines) and the variation between "formula id with hardcoded thumb path" and "scene with caller-supplied thumbnailUrl + badge" is real. Promote to a shared `<PickerItem variant='card'|'row' thumb label sublabel badge ...>` only if a third item type appears. |
