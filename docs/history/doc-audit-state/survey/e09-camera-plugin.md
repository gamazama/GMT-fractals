...
## Drift from existing doc (`docs/history/engine/15_Camera_Manager_Extraction.md`)
...
`docs/history/engine/04_Core_Plugins.md` and `docs/history/engine/16_Type_Augmentation.md` already cover slices of this; an integration doc would consolidate.
...
- **`docs/history/engine/16_Type_Augmentation.md`** documents how apps declare-merge their `installStateLibrary` keys into `engine/store/types.d.ts`. Not in this audit's file list, but tightly coupled to the StateLibrary surface — recommend cross-citing from any new doc.
