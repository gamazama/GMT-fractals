...
- **Single manifest, no per-feature dock declarations.** `GmtPanels` is one literal array of `PanelDefinition`s; no panel state lives on individual features. The engine's `Dock` and `PanelRouter` read it via `applyPanelManifest` (see `docs/history/engine/14_Panel_Manifest.md:1`). One feature can appear in multiple panels — Coloring shows in both Gradient and Scene, Geometry shows four times in Formula as separate compilables (`engine-gmt/panels.ts:51-109`).
