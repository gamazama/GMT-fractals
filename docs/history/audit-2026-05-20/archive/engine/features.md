...
From `docs/history/doc-audit-state/phase-2-carry-in.json` `by_subsystem`
`e10-engine-features`:
...
- **q-022 (drift)** — `runTicks(delta)` is seconds, not ms. Two stale
  doc sites at `docs/history/engine/01_Architecture.md:83` and
  `docs/history/engine/04_Core_Plugins.md:206` still use ms-flavoured wording.
...
- **q-077 (`(store as any).updateModulation`)** — acknowledged tech
  ...
  `docs/history/gmt/07_Code_Health.md:316-326`'s Cat-3 table; a future
