...
|---|---|---|
| review-#2 | high | Existing `docs/history/engine/04_Core_Plugins.md` has 13 break-severity drift rows; specifically calls out a CLOSED architectural decision (2026-04-22 nine-plugin lineup) that has been silently reversed by shipping `@engine/menu`, `@engine/help`, `@engine/hud`, `@engine/pwa-update`, `@engine/render-dialog` plus topbar widgets. See `docs/history/doc-audit-state/reviews/review-2026-05-19-2.md`. |
...
This module doc supersedes `docs/history/engine/04_Core_Plugins.md` for the current API and invariants. The legacy doc was written for the original 8-plugin lineup (2026-04-23) and was never updated as five additional plugins (`@engine/menu`, `@engine/help`, `@engine/hud`, `@engine/pwa-update`, `@engine/render-dialog`) and two topbar widgets (`PauseControls`, `BucketRender`) shipped. Sub-A's drift table flagged 13 rows; many describe APIs that never existed in code (e.g. `installTopBar({ slots? })`, `sceneIO.registerField`, `<SaveMenu />`/`<LoadDropZone />` components, `Ctrl+S` / `Ctrl+O` hotkeys, "HelpMenu in right slot:0" as a topbar default).
