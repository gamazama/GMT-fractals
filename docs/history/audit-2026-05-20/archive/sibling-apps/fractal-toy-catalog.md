...
No entries for `t02-fractal-toy` exist in `docs/history/doc-audit-state/phase-2-carry-in.json` `by_subsystem`. The 14 files in the secondary survey were all flagged as orphan-sweep candidates with no break-severity drift rows. Open observations carried forward from the survey:
