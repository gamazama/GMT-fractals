...
- `plans/context-protocol/subsystems.json` — 37 entries. Each has `status: pending|in_progress|written|verified|failed`.
- `docs/history/doc-audit-state/progress.json` — append-only log.
- `docs/history/doc-audit-state/coverage.yaml` — file coverage manifest.
- `docs/history/doc-audit-state/survey/` — Phase 1 survey outputs.
...
> Read every file in `<files>` top-to-bottom. To record current blob SHAs, run `node docs/history/doc-audit-state/scripts/blob-sha.mjs <path1> <path2> …` — the script accepts one or more paths and prints `<path><TAB><sha>` per line (single-arg form still works and prints the bare sha). Produce a survey doc at `docs/history/doc-audit-state/survey/<id>.md` with the exact shape below. The `files:` block MUST use block YAML (one `- path:` entry per file with two-space indented `blob_sha:` and `lines_read:` keys) — flow syntax (`{ path: ..., blob_sha: ... }`) is rejected by the orchestrator's frontmatter parser and will record zero files:
>
> ```
> ---
> subsystem_id: <id>
...
- path: <relative path>
  ...
  summary_ref: docs/history/doc-audit-state/survey/<id>.md
```
...
>
> Output to `docs/history/doc-audit-state/survey/_docs/<id>.md` as:
>
> ```
> ---
> batch_id: <id>
...
Track this via `docs/history/doc-audit-state/review-state.json`:
```json
...
>
> Inputs:
> - `docs/history/doc-audit-state/protocol-feedback.json` (filter to `status: "open"`)
> - Last 10 entries of `docs/history/doc-audit-state/progress.json`
> - The current `docs/history/doc-audit-state/LOOP_PROMPT.md`
>
...
>
> Output a review doc at `docs/history/doc-audit-state/reviews/review-<ISO date>.md`:
>
> ```
> ---
...
6. Append to `docs/history/doc-audit-state/review-log.md` (create if missing): one-line summary of what changed.
...
State for Phase 1.5 lives in `docs/history/doc-audit-state/open-questions.json` (created on first iteration of this phase). Entry shape:
...
1. Read every file in `docs/history/doc-audit-state/survey/*.md` (NOT `_docs/` — those don't have open questions).
...
> Read `docs/history/doc-audit-state/open-questions.json`. For each question:
...
5. After agent returns:
   - PASS → `status: "answered"`, `answer_ref: "docs/history/doc-audit-state/survey/_followups/<id>.md"`, `completed_at: <iso>`.
...
Final iteration after Phase 1.5 completes: print summary (files covered, questions answered/unresolvable, review ticks fired, LOOP_PROMPT edits applied, gaps remaining). If Phase 2 brief at `docs/history/doc-audit-state/phase-2-brief-draft.md` is in scope (orchestrator authority), continue; otherwise exit.
