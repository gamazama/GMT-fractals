...
- Historical-context section recounting `docs/history/engine/10_Viewport.md` aspirations — preserved by pointer in that doc itself; not load-bearing for code edits today.
...
- **ADR-00XX — `installStateLibrary` shipped as two keys (arrayKey + activeIdKey), not one**
  - **Context:** The original proposal (`docs/history/engine/15_Camera_Manager_Extraction.md:131`) imagined a single `storeKey` option with derived field names. As the factory shipped, GMT and fluid-toy both wanted to control the active-id field name independently (`activeCameraId` vs `activeViewId`) for declaration merging on `EngineStoreState`.
...
- Historical-context section comparing shipped surface to `docs/history/engine/15_Camera_Manager_Extraction.md` proposal — preserved as a pointer; the proposal is the design rationale and stays read-only.
