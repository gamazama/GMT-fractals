...
- **ADR-0048 — Camera-lock and ignoreCamera are duals, not duplicates**
  ...
  - **Consequences:** When changing playback semantics, all three readers must be re-audited together. See followup q-111 in `docs/history/doc-audit-state/survey/_followups/`.
...
- **ADR-0049 — FormulaType is a literal union; registry is the truth (hand-maintained drift)**
  ...
  - **Decision:** Keep `FormulaType` as the hand-maintained literal union for the engine-gmt fork (parent project's stable branch has separately adopted opaque-tag `FormulaType = string` — the fork has not). Drift is acknowledged and contained by the `as FormulaType[]` cast. Three remediation options are recorded in followup `docs/history/doc-audit-state/survey/_followups/q-102.md`.
