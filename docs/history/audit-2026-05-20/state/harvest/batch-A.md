...
- On the boot-poll loop (line 90): add
  ...
  @bug PRODUCTION: 300 × 100 ms poll silently fails after 30 s.
    ...
    docs/history/doc-audit-state/survey/_followups/q-002.md.
  ```
...
### Notes
- The module doc explicitly supersedes `docs/history/engine/02_Feature_Registry.md`
...
- **0014-shared-ui-mid-migration-store-context-vs-direct-store**
  - Context: Primitives in `components/` initially read `useEngineStore`
    ...
    context") was sketched in `docs/history/engine/05_Shared_UI.md` with five
  ...
  - Consequences: The "primitives must not import the store" rule
    from the original `docs/history/engine/05_Shared_UI.md` is aspirational,
