...
_Generated: 2026-05-21. Walks all 119 followups under `docs/history/doc-audit-state/survey/_followups/q-*.md` and classifies every "## Related findings" bullet against `docs/modules/bugs.md`, `docs/modules/backlog.md`, the 58 ADRs in `docs/adr/`, and source-file annotations cited by the bullet._
...
- **From q-077:** Three audioMod cast sites at `AudioPanel.tsx:134` are still `(store as any)` casts — neither `@ts-ignore` nor `@ts-expect-error`. Either `docs/history/gmt/07_Code_Health.md:316-326` is stale, or the conversion was reverted.
...
- **From q-096:** `docs/history/gmt/08_File_Structure.md` as a whole does not surface `engine-gmt/` paths — likely applies to `ShaderFactory.ts`, `ConfigManager.ts`, `RenderPipeline.ts`. Whole-file audit warranted, not just ShaderBuilder line fix.
...
- **From q-080:** `engine/worker/ViewportRefs.ts:120-122` `mouseOverCanvas` already flagged in `docs/history/engine/10_Viewport.md:310` for migration to viewport slice — confirmed by q-080 but not lifted into `backlog.md`.
...
- **From q-084:** `'system'` menu registration gated behind `advancedMode` at `engine-gmt/topbar.tsx:575` — `docs/history/engine/17_Mobile_Layout.md:31-33` does not mention this gate.
...
- q-063: `docs/history/engine/16_Type_Augmentation.md` cross-link missing — backlog row exists.
...
- q-080: `docs/history/engine/10_Viewport.md:161` already labels `mouseOverCanvas` "GENERIC (misplaced in worker folder)".
...
- q-024: `docs/history/engine/01_Architecture.md:185` self-references the warning — cite both directions OK.
...
- q-076: `docs/history/doc-audit-state/survey/e01-feature-system.md:102` claim is stale — minor.
...
- q-112: Doc claim at `docs/history/gmt/02_Rendering_Internals.md:664` is already aligned; only code drifts.
