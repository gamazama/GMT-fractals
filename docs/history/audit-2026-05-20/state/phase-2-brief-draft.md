...
>
> 1. **Disposition lookup** — read `docs/history/doc-audit-state/phase-2-disposition.json` and find the entry for `<existing_doc_ref>`. The `disposition` field tells you how to treat the old doc:
...
>
> 2. **Docs-existing summary** — read `docs/history/doc-audit-state/survey/_docs/d0X-*.md` entries that mention the docs in `<existing_doc_ref>`. These are 3-6 sentence summaries with `Preservable:` blocks. Pull rationale, research, aspirations — never code claims.
>
> 3. **Survey** — read `docs/history/doc-audit-state/survey/<id>.md`. This is the code-truth snapshot from Phase 1: Public API surface, Architecture (10-25 bullets each with file:line), Invariants and gotchas, Drift table. Treat survey as truth-as-of-audit; verify against current code in step 5.
>
> 4. **Followups** — read every `docs/history/doc-audit-state/survey/_followups/q-NNN.md` where `source_subsystem == "<id>"`. Followups OVERRULE surveys on facts they investigated (review #12 contract). Also read `docs/history/doc-audit-state/phase-2-carry-in.json` under `by_subsystem["<id>"]` for cross-cutting items (production bugs, doc-rewrite targets, cleanups, drifts) — surface these in the module doc's "Known issues" or "Phase 2 carry-in" section as appropriate.
>
> 5. **Code** — re-Read every file in `<files_claimed>` to capture any drift since the audit. Use `node docs/history/doc-audit-state/scripts/blob-sha.mjs <path>` to get current blob SHAs (these go in the frontmatter as `last_verified_sha`). If a file is >800 lines, read in chunks but read every line — do NOT skim.
>
...
> - All `path:N` and `path:N-M` line anchors must reference real files with in-range lines. **All citation paths are REPO_ROOT-relative — do NOT prepend `dev/` even though the working directory IS `dev/`.** REPO_ROOT for verify-doc.mjs is `h:/GMT/workspace-gmt/dev/`; `dev/docs/foo.md` resolves to `dev/dev/docs/foo.md` and fails. Write `engine/TickRegistry.ts:42` and `docs/history/engine/01_Architecture.md:90`, not `dev/engine/...` or `dev/docs/...`.
> - `depends_on` should name OTHER subsystem ids (e.g. `e01-feature-system`), not file paths. `depends_on: []` is valid for leaf substrates — do not invent phantom dependencies. Look at `plans/context-protocol/subsystems.json` to find valid ids.
...
```bash
node docs/history/doc-audit-state/scripts/verify-doc.mjs <target_doc>
```
