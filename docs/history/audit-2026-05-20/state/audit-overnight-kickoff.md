...
**Goal:** bring `dev/docs/` from "last refreshed 2026-04-23" to current. Scope: `engine/` core, `engine-gmt/` plugin layer, `app-gmt/` app. Output: updated/added docs under `docs/history/engine/` and `docs/history/gmt/`, refreshed `DOCS_INDEX.md`, refreshed `CODEBASE_MAP.md` deltas.
...
- **No destructive edits.** Existing docs get patched in place or replaced wholesale only when fully superseded; old versions move to `dev/docs/history/archive/` with a "why archived" header.
...
2. Create `dev/docs/history/doc-audit-state/` with:
...
4. If replacing an existing doc wholesale, move the old to `dev/docs/history/archive/` with a header.
...
4. Write a new `docs/history/engine/22_Audit_2026-05-XX.md` summarising what the audit found (carrying forward the style of `21_Code_Review_2026-04-25.md`).
...
2. Should the loop also touch `docs/history/gmt/` (pre-extraction GMT docs)? Default: leave alone, port content as needed into `engine/3x_GMT_*.md` docs.
3. Where should survey files live? Proposal: `dev/docs/history/doc-audit-state/survey/` — kept out of `dev/docs/` since they're working drafts, not finished docs.
...
1. Loop prompt reads `dev/docs/history/doc-audit-state/progress.json` first.
...
- **Module docs live at `dev/docs/modules/<area>/<file-or-feature>.md`** — a NEW tree, parallel to `docs/history/engine/` (which stays read-only). Cross-link old → new where applicable.

### Existing docs are immutable

Per user direction: do not edit anything currently in `docs/history/engine/` or `docs/history/gmt/`. New artifacts only, in `dev/docs/modules/`. DOCS_INDEX may be updated to add a "Module Docs (2026-05-19 audit)" section pointing at the new tree.

### Coverage manifest

```yaml
# dev/docs/history/doc-audit-state/coverage.yaml
schema_version: 1
...
1. **Pre-flight pass** (Phase 0.5): an agent runs `git ls-files dev/` filtered against an exclude list (debug/, reference/, frag/, public/, node_modules, dist/, *.snap, *.lock, .png/.jpg/.gif binary assets). Output: `dev/docs/history/doc-audit-state/file-inventory.yaml` — the canonical list of files that MUST be covered.
...
*Next step: user approves the refined plan; I scaffold `dev/docs/history/doc-audit-state/` (including the file-inventory pre-flight); user reviews the inventory + subsystem assignment; we kick off /loop.*
