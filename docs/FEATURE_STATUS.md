...
| LFO modulation via `animations` array | ✅ | `fluid-toy/orbitTick.ts` installs two sine LFOs on `julia.juliaC_x/_y` when orbit is enabled. `liveModulations` populated; `FluidToyApp.tsx` reads and pushes to `FluidEngine.setParams`. Verified via `smoke-anim-orbit.mts` (F13 landed 2026-04-23). |
