...
> 2026-05-18 — Modulation off-switch prompt drafted (`23_`) as the last pre-push ship blocker: per-LFO / per-audio-rule UI toggles + global LFO toggle + verify global audio toggle gates rule evaluation. ~0.5-1d. After ships → smoke → push. Earlier today: audio fps-sync (`22_`, four compounding bugs incl. 2× tick-rate from double-mounted drivers), lighting normalize (`942e2b5`).
...
| [`23_MODULATION_OFF_SWITCH_PROMPT.md`](./23_MODULATION_OFF_SWITCH_PROMPT.md) | **Pre-push ship blocker.** Audio modulation + LFO modulation need off-switches at two granularities — per-individual (each LFO, each audio rule) and per-global (LFO panel, audio panel). Three of four switches exist or partly wired; one entirely missing (global LFO toggle). Three gaps: (1) per-audio-rule UI in `AudioLinkControls` + `AudioModulationList`; (2) `lfosEnabled` boolean + `LfoList` header toggle + runtime gate; (3) verify `audio.isEnabled` gates rule evaluation (currently only gates analyser update). ~0.5-1d. | drafted, blocks push |
| `24_MODULATION_OFF_SWITCH_REPORT.md` | Output of `23`. Includes a 2×2 toggle-combination matrix (audio×LFO) to verify each silences exactly what it should. | pending fresh session |
