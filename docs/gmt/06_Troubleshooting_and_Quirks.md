...
## 16. Escape Radius vs DE Bailout — two separate uniforms

Two distinct radii that are easy to conflate; they were a single coupled value before this split.

*   **`uEscapeThresh` (Escape Radius) — coloring only.** Owned by the **coloring** feature (`coloring.escape`, group `layer1_bottom`, shown for the Potential / Decomposition / Green's-Flow mapping modes). Drives those coloring modes' escape capture (`shaders/chunks/de.ts:74/134`) and the smooth-iteration normalization (`features/core_math.ts`). It does **not** touch geometry for per-iteration formulas.
*   **`uDeBailout` (DE Bailout) — geometry only.** Owned by the **quality** feature (`quality.deBailout`, group `metric`, beside Distance Metric and Estimator). Absolute `|z|²` at which the raymarch DE stops iterating (`shaders/chunks/de.ts` — `bailout = max(uDeBailout, 1.0)`). Default 100. Low values bail early and slice the fractal into shells (overstep artifacts by design); fast-escaping formulas only respond once it nears their structure scale.
*   **History — they used to be one.** Originally `bailout = max(100.0, uEscapeThresh + 100.0)`, so the escape radius nudged the geometry bailout. That caused a bug: copying just the "Gradients" group across formulas dragged the escape radius along and retuned the target's DE bailout. Splitting out `uDeBailout` fixed the coupling, leaving `uEscapeThresh` as a pure coloring control — so it lives on `coloring` and correctly travels with gradient copies.
*   **Self-contained formulas:** MandelTerrain runs its own loop; it now bails on `uDeBailout` (matching per-iteration formulas) and keeps `uEscapeThresh` only for its smooth-iteration coloring. Its `defaultPreset` carries `coloring.escape: 20` + `quality.deBailout: 20` to preserve its original look.
*   **Backward compat:** Legacy files store the value at `features.coloring.escape` (its current home) → loaded unchanged. A brief earlier dev build housed it at `features.quality.escape`; the load-time migration (`app-gmt/migrations.ts`, v1) routes that back to `coloring.escape`. For **MandelTerrain**, the migration also seeds `quality.deBailout` from the legacy escape value so old scenes bail where they always did. Idempotent; never overwrites an existing `deBailout`. Covered by `npm run test:migrations` (real `public/gmf/fragmentarium/BioCube.gmf` fixture + MandelTerrain bail-preservation cases).
