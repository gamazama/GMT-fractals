...
| Brush / pointer interactions | `fluid-toy/pointer/gestures/*` (+ `fluid-toy/brush/*`) |
| Deep-zoom precision | `engine/fractal/deepZoom/*` — the pipeline is shared with gradient-explorer; `fluid-toy/deepZoom/` is two re-export shims plus diagnostics + benchmark |
...
`main.tsx` registers fluid-toy features + cross-tree handles → `FluidToyApp.tsx` mounts the engine shell + fluid-specific panels → `useFluidEngine.ts` owns its **own** `requestAnimationFrame` loop and calls `engine.frame()` on it.

Note the sim is not driven off `TickRegistry`: `fluid-toy/` contains zero references to it. `<RenderLoopDriver />` is mounted (`FluidToyApp.tsx`) so engine-side per-frame consumers such as animation still tick, but the fluid step itself rides `useFluidEngine`'s RAF. `features/fluidSim.ts` only declares params and a `syncFluidSimToEngine` push — it registers no tick.

## Historical context

The full file catalog (80 files across feature/shader/store/UI/preset/component subtrees) is archived at [`docs/history/audit-2026-05-20/archive/sibling-apps/fluid-toy-catalog.md`](../../history/audit-2026-05-20/archive/sibling-apps/fluid-toy-catalog.md) — useful when navigating the codebase but heavy. The README + `CODE_MAP.md` (in the sibling-app root) are the canonical onboarding entry points per the audit's `keep-as-is` disposition for `fluid-toy/README.md`.
