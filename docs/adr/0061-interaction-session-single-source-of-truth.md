...
- **Ref-counted per source**; an unbalanced `end` can't strand the set (dev-warns). **Safe only when each `begin` gets a reliable matching `end`** (a single balanced path — e.g. a slider). A token fed by MULTIPLE independently-ending paths (camera: custom drag + drei `OrbitControls` + middle-drag, where drei suppresses its own `onEnd` mid-gesture) must present to the session as ONE open/close pair, not N begins/ends — close on "pointer fully released". (P3a learning — see the camera note under Touch.)
...
Export `useInteractionDrag(source)` returning `{ onPointerDown, onPointerUp, onLostPointerCapture, onPointerCancel }` (and a `useEffect` cleanup). `onPointerCancel` is not optional — touch interruption fires `pointercancel`, not `pointerup` (see Touch / multi-pointer below). **JSX** drag components (the ~90 P3b sliders/knobs/vectors) spread these instead of hand-calling store actions. The hook is **dispatch-only** — it reads nothing from the store, so mounting it in ~90 components costs zero renders (see Performance). **The hook fits JSX-handler components only.** Imperative producers wired with `addEventListener` loops rather than JSX props (P3a camera/gizmo/picker — confirmed by P3a) cannot spread it; they **replicate its exact safety inline** (cancel + capture-loss + unmount cleanup). State this split when instructing producer wiring so it isn't taken as the universal mechanism. Dev-mode warns if `beginInteraction` is called for a source never seen before. (Review #1/#2 + #3 silent-miss risk.)
...
- **Camera (touch):** one-finger rotate, two-finger pinch-zoom, and two-finger pan all flow through drei's `<OrbitControls>` (`Navigation.tsx:1353`, `touches={{ ONE: TOUCH.ROTATE, TWO: TOUCH.DOLLY_PAN }}` at `:1372`), which exposes a single `onStart`/`onEnd` pair (`:1373`/`:1395`). Wiring `beginInteraction('camera')`/`endInteraction('camera')` into those two callbacks — a **P3a site already in scope** — covers every touch camera gesture. drei collapses the 1→2→1-finger transition into ONE `onStart…onEnd`, so there is **no multi-pointer double-begin**: the `camera` token is one session per gesture. (Naive per-finger `pointerdown` ref-counting *would* misbehave on multi-touch, but the OrbitControls wiring sidesteps it.) This is already the de-facto signal today — touch camera activity reaches adaptive via `isOrbitDragging` (set in the same `onStart`/`onEnd`, `:1382`/`:1396`), folded into `isCurrentlyActive` at `Navigation.tsx:1096`. The `camera` token additionally has the desktop-only custom left-drag orbit, middle-drag, and wheel `poke`. **P3a learning — do NOT ref-count across these paths** (the original framing here said "ref-counting under one token makes the overlaps safe"; that was wrong and drove three rounds of rework). They don't reliably balance: a custom cursor-anchor handler sets `orbitRef.enabled=false` to take over, and drei only fires `onEnd` while enabled — so a `begin('camera')` from drei never gets its matching `end`, the count drifts, and the session strands. The camera producer instead presents as ONE **release-driven** session: open on the first real pointer movement (not pointerdown — avoids a click-pop), close on a window `pointerup` with `e.buttons === 0` (or `pointercancel` / unmount). Ref-counting is sound only WITHIN a single balanced path (a slider via the transaction lifecycle), never ACROSS independently-ending paths that share a token.
