...
- **Open next:** L2811 (`lastCompileFailed` signal — a failed compile renders empty frames with no in-app sign), then Tier 2 tooling (smoke:all driver, mobile-layout guard, frag catalog drift) and the `@invariant → @assumption` sweep.
