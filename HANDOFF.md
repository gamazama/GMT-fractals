...
- **L2811 closed the same day** (`lastCompileFailed` + a sticky `failed` phase on the compile store + a dismissable red pill; `smoke:compile-failed`). A failed compile is now visible in-app. **Open next:** Tier 2 tooling (smoke:all driver, mobile-layout guard, frag catalog drift) and the `@invariant → @assumption` sweep.
