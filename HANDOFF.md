...
- **Green:** `typecheck`, `test:palette` (incl. stopfit + blendspaces + working + groundsets + shelf), `smoke:boot`, `smoke:ge-tray` / `-hero` / `-ground` / `-next` / `-pagepick` / `-livedrag` / `-setsave` / `-cursors` / `-phone` / `-walltouch`, `check:rule-guards`, `check:text-bytes`. **Not pushed** — the owner's call, and nothing here has been walked on a real phone yet.
