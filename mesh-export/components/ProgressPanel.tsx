...
export const ProgressPanel: React.FC = () => {
  ...
  return (
    <div className="font-mono flex flex-col gap-2">
      {/* Status */}
      {status && (
        <div className="text-[13px] text-warn font-bold">{status}</div>
      )}

      {/* Main progress */}
      <div className="h-1 bg-surface-header rounded overflow-hidden">
        <div
      ...
      {/* Phase */}
      {phaseName && <div className="text-[11px] text-fg-dim">{phaseName}</div>}
      <div className="h-[3px] bg-surface-header rounded overflow-hidden">
        <div
      ...
      {lastMesh && lastTimings && !isRunning && (
        <div className="text-[11px] leading-relaxed bg-surface-section border border-line/10 rounded px-2 py-1.5">
          ...
          {newton && <span className="text-fg-muted"> · Newton projected</span>}
          {smoothingSkipped && <span className="text-warn"> · smoothing skipped (&gt;5M verts)</span>}
          <br />
      ...
      {memoryBlocks.length > 0 && (
        <div>
          ...
          <div className="text-[10px] text-fg-faint mt-0.5">Memory: {totalMB} MB active</div>
        </div>
      )}
      ...
      {logEntries.length > 0 && (
        <div>
          <div className="max-h-[200px] overflow-y-auto bg-surface border border-line/10 rounded p-1.5 text-[11px] leading-relaxed">
            {logEntries.map((entry, i) => (
              <div key={i} className={LOG_COLORS[entry.type] || LOG_COLORS.info}>
                <span className="text-fg-faint">{entry.time}</span> {entry.msg}
              </div>
            ))}
            <div ref={logEndRef} />
          </div>
          <div className="flex gap-1.5 mt-1">
            <button onClick={handleCopyLog} className="text-[10px] px-2 py-0.5 bg-line/10 text-fg-muted border border-line/10 rounded-sm hover:bg-line/15 cursor-pointer">
              Copy
            </button>
            <button onClick={clearLog} className="text-[10px] px-2 py-0.5 bg-line/10 text-fg-muted border border-line/10 rounded-sm hover:bg-line/15 cursor-pointer">
              Clear
            </button>
          </div>
        </div>
